				 ...
				 struct fib_config *cfg)
{
	...
	if (rt->rt_dev) {
		...
		pr_debug(KERN_INFO "[mtk_net][RTlog info]  ip_rt_ioctl   rt.dev =%s \n",devname);	
		#endif  	
...
int ip_rt_ioctl(struct net *net, unsigned int cmd, void __user *arg)
{
	...
	case SIOCDELRT:		/* Delete a route */
		...
		if (copy_from_user(&rt, arg, sizeof(rt)))
			return -EFAULT;
		#ifdef CONFIG_MTK_NET_LOGGING  
		pr_debug(KERN_INFO "[mtk_net][RTlog info]ip_rt_ioctl rt.flags =%08x, rt.rt_dst =%08x rt.rt_gateway =%08x \n",rt.rt_flags,sk_extract_addr(&(rt.rt_dst)),sk_extract_addr(&(rt.rt_gateway)));
		#endif
		...
		if (err == 0) {
			...
			if (cmd == SIOCDELRT) {
				#ifdef CONFIG_MTK_NET_LOGGING 
				pr_debug(KERN_INFO "[mtk_net][RTlog delete]  p_rt_ioctl: cmd == SIOCDELRT! >>\n");
				#endif
				...
				#ifdef CONFIG_MTK_NET_LOGGING 
				pr_debug(KERN_INFO "[mtk_net][RTlog insert]  p_rt_ioctl: cmd == SIOCADDRT! >>\n");
				#endif
...
static int inet_rtm_delroute(struct sk_buff *skb, struct nlmsghdr *nlh)
{
	...
	int err;
 //   #ifdef CONFIG_MTK_NET_LOGGING 
//	pr_debug(KERN_INFO "[mtk_net][RTlog delete] inet_rtm_delroute !\n");
//	#endif
...
static int fib_inetaddr_event(struct notifier_block *this, unsigned long event, void *ptr)
{
	...
	case NETDEV_DOWN:
	//	#ifdef CONFIG_MTK_NET_LOGGING 
	//	pr_debug(KERN_INFO "[mtk_net][RTlog delete]   fib_inetaddr_event()  %s NETDEV_DOWN!\n", ifa->ifa_dev->dev->name);
	//	#endif
