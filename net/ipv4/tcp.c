        ...
        case SIOCKILLSOCK:
         {
             ...
             if (copy_from_user(&uid_e, (char __user *)arg, sizeof(uid_e)))
                 return -EFAULT;
             pr_debug(KERN_WARNING "SIOCKILLSOCK uid = %d , err = %d",
...
int tcp_nuke_addr(struct net *net, struct sockaddr *addr)
{
  ...
  int count = 0; 
	...
	if (family == AF_INET) {
		...
		/*mtk_net:debug log*/
  pr_debug(KERN_INFO "[mtk_net][tcp]tcp_nuke_addr: tcp_hashinfo.ehash_mask = %d\n",tcp_hashinfo.ehash_mask);
...
restart:
		...
		sk_nulls_for_each(sk, node, &tcp_hashinfo.ehash[bucket].chain) {
			...
			if(sk->sk_state != TCP_CLOSE && sk->sk_shutdown != SHUTDOWN_MASK)
				{
					pr_debug(KERN_INFO "[mtk_net][tcp]skip ALPS01866438 Google Issue!\n");			 
				}
			tcp_done(sk);
			bh_unlock_sock(sk);
			local_bh_enable();
	...
	pr_debug(KERN_INFO "[mtk_net][tcp]tcp_nuke_addr : count = %d\n",count);
	return 0;
}
