	...
	if (((card->ext_csd.card_type & EXT_CSD_CARD_TYPE_HS400_1_2V) &&
	 ...
	 || ((card->ext_csd.card_type & EXT_CSD_CARD_TYPE_HS400_1_8V) &&
		...
		clock = MMC_HS200_MAX_DTR;
		    ...
		    (card->ext_csd.card_type & EXT_CSD_CARD_TYPE_HS200_1_8V))
		|| ((host->caps2 & MMC_CAP2_HS200_1_2V_SDR) &&
		    (card->ext_csd.card_type & EXT_CSD_CARD_TYPE_HS200_1_2V))) {
...
int mmc_ffu_install(struct mmc_card *card, u8 *ext_csd)
{
	...
	if ((card->host->caps2 & MMC_CAP2_CACHE_CTRL)
	 && !(card->quirks & MMC_QUIRK_DISABLE_CACHE)) {
		mmc_cache_ctrl(card->host, 1);
	}
#endif
