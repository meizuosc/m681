...
#endif
extern char emmc_info[20];// Req_135100 /dev/dev_info add emmc_info, lizheng, Add extern code, 2016/01/05
...
static int mmc_decode_cid(struct mmc_card *card)
{
    ...
    {
    case 0x15:
        strcpy(emmc_info, "SAMSUNG-");
        strncat(emmc_info, card->cid.prod_name, strlen(card->cid.prod_name));//Req_135100 /dev/dev_info emmc_info optimize prod_name length, lizheng, modify code, 2016/01/15
        break;
    case 0x20:
        strcpy(emmc_info, "SANDISK-");
        strncat(emmc_info, card->cid.prod_name, strlen(card->cid.prod_name));//Req_135100 /dev/dev_info emmc_info optimize prod_name length, lizheng, modify code, 2016/01/15
        break;
    case 0x13:
        strcpy(emmc_info, "MICRON-");
        strncat(emmc_info, card->cid.prod_name, strlen(card->cid.prod_name));//Req_135100 /dev/dev_info emmc_info optimize prod_name length, lizheng, modify code, 2016/01/15
        break;
    case 0x90:
        strcpy(emmc_info, "HYNIX-");
        strncat(emmc_info, card->cid.prod_name, strlen(card->cid.prod_name));//Req_135100 /dev/dev_info emmc_info optimize prod_name length, lizheng, modify code, 2016/01/15
        break;
    default:
	...
	struct mmc_card *oldcard)
{
	...
	#endif
	if (oldcard) {
		if (memcmp((void *)cid, (void *)oldcard->raw_cid,
			sizeof(cid)) != 0) {
			err = -ENOENT;
			goto err;
		}
