			...
			bool input)
{
	...
	if(pctl->devdata->mt_set_gpio_dir){
		...
		pr_debug("fwq pinctrl set_direction pin=%d,dirinput=%d base not ready\n",offset,input);
	}
...
static void mtk_gpio_set(struct gpio_chip *chip, unsigned offset, int value)
{
	...
	if(pctl->devdata->mt_set_gpio_out){
		mt_set_gpio_out(offset|0x80000000,value);
	}
	else{
		pr_debug("fwq pinctrl set gpio out base not ready pin=%d,value=%d\n",offset,value);
	}
		...
		int value, enum pin_config_param param)
{
	int ret;
    //only set ies
	pr_debug("fwq pinctrl set ies &smt pin=%d,value=%d,para=%d\n",pin,value,param);
	
	if (pctl->devdata->mt_set_gpio_ies) {
		ret = pctl->devdata->mt_set_gpio_ies(pin|0x80000000,value);
	}
	/*
...

{
   if(pctl->devdata->mt_set_gpio_dir){
		...
		pr_debug("fwq pinctrl set_direction pin=%d,dir=%d base not ready\n",pin,value);
   }
}
		...
		unsigned int pin, unsigned char driving)
{
	...
	pr_debug("fwq pinctrl set driving pin=%d,driving=%x\n",pin,driving);
		...
		unsigned int pin, bool enable, bool isup, unsigned int arg)
{
	...
	if (pctl->devdata->mt_set_gpio_pull_select) {
		...
		pr_debug("pinctrl gpin pull select base not ready\n" );
	}
		...
		enum pin_config_param arg)
{
	...
	case PIN_CONFIG_INPUT_SCHMITT_ENABLE:
		mtk_pconf_set_smt(pctl, pin, arg, param);
		break;
		...
		unsigned long pin, unsigned long mode)
{
	...
	//mt_set_gpio_mode(pin|0x80000000,mode);
    ...
    pr_debug("fwq reg_addr=%x\n",reg_addr);
	bit = pin % MAX_GPIO_MODE_PER_REG;
	pr_debug("fwq bit=%x\n",bit);
	mask <<= (GPIO_MODE_BITS * bit);
	val = (mode << (GPIO_MODE_BITS * bit));

	pr_debug("fwq pinctrl set mode done\n");
	return regmap_update_bits(mtk_get_regmap(pctl, pin),
			reg_addr, mask, val);
...
static int mtk_gpio_get_direction(struct gpio_chip *chip, unsigned offset)
{
	...
	if(pctl->devdata->mt_get_gpio_dir){
		...
		pr_debug("fwq pinctrl get dir pin=%d,base not ready\n",offset);
	}
...
static int mtk_gpio_get(struct gpio_chip *chip, unsigned offset)
{
	...
	return value;
	/*
	pr_debug("fwq pinctrl get gpio out pin=%d\n",offset);
	if(pctl->devdata->mt_get_gpio_out){
		dout = mt_get_gpio_out(offset|0x80000000);
	}
	else{
		pr_debug("fwq pinctrl set gpio out base not ready pin=%d\n",offset);
	}
	return dout;
	*/
