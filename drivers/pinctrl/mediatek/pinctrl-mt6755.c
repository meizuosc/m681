...
#if 0
static const struct mtk_pin_spec_pupd_set mt6755_spec_pupd[] = {
			 ...
			 unsigned char align, bool isup, unsigned int r1r0)
{
	...
	for (i = 0; i < ARRAY_SIZE(mt6755_spec_pupd); i++) {
		if (pin == mt6755_spec_pupd[i].pin) {
			find = true;
			break;
		}
	}
	...
	spec_pupd_pin = mt6755_spec_pupd + i;
...
#if 0
static const struct mtk_pin_ies_smt_set mt6755_ies_smt_set[] = {
...
#if 0
int spec_ies_smt_set(struct regmap *regmap, unsigned int pin, unsigned char align, int value)
{
	unsigned int i, reg_addr, bit;
	bool find = false;

	for (i = 0; i < ARRAY_SIZE(mt6755_ies_smt_set); i++) {
		if (pin >= mt6755_ies_smt_set[i].start && pin <= mt6755_ies_smt_set[i].end) {
			find = true;
			break;
		}
	}
	...
	if (value)
		reg_addr = mt6755_ies_smt_set[i].offset + align;
	else
		reg_addr = mt6755_ies_smt_set[i].offset + (align << 1);

	bit = BIT(mt6755_ies_smt_set[i].bit);
...
static int mt6755_pinctrl_probe(struct platform_device *pdev)
{
	...
	return mtk_pctrl_init(pdev, &mt6755_pinctrl_data);
}
