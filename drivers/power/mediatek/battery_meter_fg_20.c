...
extern char battery_info[40];
void battery_info_set(int id)
{

     switch(id)
	 {
	 	case 0:
			strcpy(battery_info,"DEERNENG-ATL");
			break;
		case 1:
			strcpy(battery_info,"JIADE-ATL");
			break;
		case 2:
			strcpy(battery_info,"DEERNENG-GUANGYU");
			break;
		default:
			strcpy(battery_info,"NO DEFINED");
			break;
	}
	printk("zmlin battery_id:%d,battery_info:%s \n",id,battery_info);
}
...
void fgauge_get_profile_id(void)
{
	...
	battery_info_set(g_fg_battery_id);
...
int __batt_meter_init_cust_data_from_cust_header(struct platform_device *dev)
{
	...
	for(i=0;i<batt_meter_table_cust_data.r_profile_t0_size;i++){
		batt_meter_table_cust_data.r_profile_t0[i].resistance+=r_profile_offset[g_fg_battery_id][0];
		}
	...
	for(i=0;i<batt_meter_table_cust_data.r_profile_t1_size;i++){
		batt_meter_table_cust_data.r_profile_t1[i].resistance+=r_profile_offset[g_fg_battery_id][1];
		}
	...
	for(i=0;i<batt_meter_table_cust_data.r_profile_t2_size;i++){
		batt_meter_table_cust_data.r_profile_t2[i].resistance+=r_profile_offset[g_fg_battery_id][2];
		}
	...
	for(i=0;i<batt_meter_table_cust_data.r_profile_t3_size;i++){
		batt_meter_table_cust_data.r_profile_t3[i].resistance+=r_profile_offset[g_fg_battery_id][3];
		}	
	...
	if (Res >= batt_temperature_table[0].TemperatureR) {
		...
		for (i = 0; i <= 18; i++) {
			if (Res >= batt_temperature_table[i].TemperatureR) {
				...
				printk("zmlin TMP2:[%d],RES:[%d]\n",batt_temperature_table[i].BatteryTemp,batt_temperature_table[i].TemperatureR);
				break;
			}
			{	/* hidden else */
				RES1 = batt_temperature_table[i].TemperatureR;
				TMP1 = batt_temperature_table[i].BatteryTemp;
				printk("zmlin1 TMP2:[%d],RES:[%d]\n",batt_temperature_table[i].BatteryTemp,batt_temperature_table[i].TemperatureR);
			}
		}
...
int BattVoltToTemp(int dwVolt)
{
	...
	do_div(TRes_temp, (pmic_get_vbif28_volt() - dwVolt));
	 printk("[RBAT_PULL_UP_VOLT_BY_BIF] vbif28:%d\n",pmic_get_vbif28_volt()); 
#else
	...
	bm_debug("[BattVoltToTemp] %d %d %d\n", RBAT_PULL_UP_R, pmic_get_vbif28_volt(),(int)TRes);
	return sBaTTMP;
}
