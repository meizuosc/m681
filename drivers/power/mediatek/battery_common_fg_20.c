...
/* ////////////////////////////////////////////////////////////////////////////// */
int Enable_BATDRV_LOG = BAT_LOG_DEBG;
...
int g_charging_current_by_temp = 0;
...
INT32 batterypseudo1=BATTERYPSEUDO1;
INT32 batterypseudo100=BATTERYPSEUDO100;
...
kal_uint8 fg_ipoh_reset;

extern void charging_enable_ctrl(int enable);
...
#endif
	if (!g_bat_init_flag) {
		battery_log(BAT_LOG_CRTI, "[upmu_is_chr_det] battery thread not ready, will do after bettery init.\n");
		return KAL_FALSE;
	}
#if defined(CONFIG_POWER_EXT)
	/* return KAL_TRUE; */
				       ...
				       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_batt_temp:
		...
		battery_cmd_thermal_test_mode = 1;
		battery_cmd_thermal_test_mode_value = (val->intval/10);
		break;
	default:
		ret = -EINVAL;
		break;
	}
	printk("zmlin battery_set_property:%d %d %d\n",g_battery_thermal_throttling_flag,battery_cmd_thermal_test_mode,battery_cmd_thermal_test_mode_value);
	return ret;
}
				...
				enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW :
		...
		va=battery_meter_get_battery_current();
...
/**charging enable sys/devices/platform/battery/Charging_Current_Level  **/
static int enable_arg = 0;
static ssize_t show_Charging_Enable(struct device *dev, struct device_attribute *attr, char *buf)
{
	battery_log(BAT_LOG_CRTI, "[EM] Charging_enable : %d\n", enable_arg);
	return sprintf(buf, "%u\n", enable_arg);
}
...
/* ///////////////////////////////////////////////////////////////////////////////////////// */
static ssize_t show_Charging_Current_Level(struct device *dev, struct device_attribute *attr,
						  char *buf)
{
	...
	if(BMT_status.charger_exist == KAL_TRUE)
		...
		charging_current = 0;
	battery_log(BAT_LOG_CRTI, "[EM] Charging_Current : %d/10 mA\n",
			    charging_current);
	return sprintf(buf, "%u\n", charging_current);
}

static ssize_t store_Charging_Current_Level(struct device *dev,
						   struct device_attribute *attr, const char *buf,
						   size_t size)
{
	CHR_CURRENT_ENUM charging_current;

	sscanf(buf, "%u", &charging_current);
	//battery_charging_control(CHARGING_CMD_SET_INPUT_CURRENT,
	//					 &charging_current);
	//battery_charging_control(CHARGING_CMD_SET_CURRENT, &charging_current);
	battery_log(BAT_LOG_CRTI, "[EM] Charging_Current : %d\n",
			    charging_current);
	return size;
}
static DEVICE_ATTR(Charging_Current_Level, 0664, show_Charging_Current_Level,
		   store_Charging_Current_Level);
//end
...
/*0C <=temp< 10C :900ma 10C <=temp<= 45C :normal*/
	if((BMT_status.temperature >= 0)&&(BMT_status.temperature < 10))
		g_charging_current_by_temp = 1;
	else
		g_charging_current_by_temp = 0;
...
static void mt_battery_notify_VBatTemp_check(void)
{
	...
	if (BMT_status.temperature >= MAX_CHARGE_TEMPERATURE_HIGH) {
...
#else
#ifdef BAT_LOW_TEMP_PROTECT_ENABLE
	else if (BMT_status.temperature < MIN_CHARGE_TEMPERATURE_LOW) {
		g_BatteryNotifyCode |= 0x0020;
		battery_log(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) out of range(too low)\n",
				    BMT_status.temperature);
	}
...
static int battery_probe(struct platform_device *dev)
{
	...
	/* For EM */
	{
		...
		//charging current level
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_Charging_Current_Level);
		//end
#if defined(CONFIG_MTK_PUMP_EXPRESS_SUPPORT) || defined(CONFIG_MTK_PUMP_EXPRESS_PLUS_SUPPORT)
...
#else
	battery_log(BAT_LOG_CRTI, "******** battery driver suspend-1 ********\n");
#ifdef CONFIG_MTK_POWER_EXT_DETECT
	if (KAL_TRUE == bat_is_ext_power())
		return 0;
#endif
	mutex_lock(&bat_mutex);
	battery_log(BAT_LOG_CRTI, "******** battery driver suspend-2 ********\n");
	//cancel timer
	hrtimer_cancel(&battery_kthread_timer);
	battery_log(BAT_LOG_CRTI, "******** battery driver suspend-3 ********\n");
	hrtimer_cancel(&charger_hv_detect_timer);
	battery_log(BAT_LOG_CRTI, "******** battery driver suspend-4 ********\n");

	battery_suspended = KAL_TRUE;
	battery_log(BAT_LOG_CRTI, "******** battery driver suspend-5 ********\n");
	mutex_unlock(&bat_mutex);
	battery_log(BAT_LOG_CRTI, "******** battery driver suspend-6 ********\n");

	battery_log(BAT_LOG_CRTI, "@bs=1@\n" );
#endif

    get_xtime_and_monotonic_and_sleep_offset(&xts, &tom, &g_bat_time_before_sleep);
	battery_log(BAT_LOG_CRTI, "******** battery driver suspend-7 ********\n");
}
...
static void battery_timer_resume(void)
{
	...
	battery_log(BAT_LOG_CRTI, "[battery_timer_resume] is_pcm_timer_trigger %d bat_spm_timeout %d\n", is_pcm_timer_trigger, bat_spm_timeout);
