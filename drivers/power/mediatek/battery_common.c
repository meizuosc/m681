...
static kal_bool mt_battery_is_kpoc(void)
{
...
static kal_bool mt_battery_0Percent_tracking_check(void)
{
	...
	static kal_uint16 reboot_cnt = 1;
	...
	if (BMT_status.UI_SOC <= 0) {
		...
		if (BMT_status.bat_vol <= SYSTEM_OFF_VOLTAGE) {
			reboot_cnt++;
			if (reboot_cnt >= 3 && mt_battery_is_kpoc() == KAL_FALSE) {
				battery_log(BAT_LOG_CRTI,
					    "0Percent, shutdown ,VBAT < %d UI_SOC=%d reboot_cnt:%d \r\n",
...
#endif				/* #ifdef FG_BAT_INT */
#ifdef FG_SELF_DISCHARGE_CHECK
			else if (KAL_TRUE == g_refresh_ui_soc) {
				BMT_status.UI_SOC--;
				timer_counter = 0;
				g_refresh_ui_soc = KAL_FALSE;
			}
#endif				/* #ifdef FG_SELF_DISCHARGE_CHECK */
			else
				timer_counter++;
...
#ifndef DISABLE_DLPT_FEATURE
	if (bat_data->BAT_CAPACITY <= DLPT_POWER_OFF_THD && mt_battery_is_kpoc() == KAL_FALSE) {
		static kal_uint8 cnt = 1;
...
static void mt_battery_notify_TotalChargingTime_check(void)
{
	...
	if ((g_battery_thermal_throttling_flag == 2) || (g_battery_thermal_throttling_flag == 3)) {
		battery_log(BAT_LOG_FULL, "[TestMode] Disable Safty Timer : no UI display\n");
	} else {
		if (BMT_status.total_charging_time >= MAX_CHARGING_TIME)
