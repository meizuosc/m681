...
static ssize_t hotknot_open(struct inode *node, struct file *flip);
static ssize_t hotknot_release(struct inode *node, struct file *flip);
...
static ssize_t _hotknot_read(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
    ...
    else if (!cmd_head2.wr)
    {
        ...
        while (data_len > 0)
        {
            ...
            memcpy(&page[loc], &cmd_head2.data[GTP_ADDR_LENGTH], len);
            loc += len;
