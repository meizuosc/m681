...
static unsigned long gt9xx_finger_status = 0;
static unsigned long gt9xx_finger_status_pre = 0;
...
#if GTP_COVER_CONTROL
static RAW_NOTIFIER_HEAD(cover_chain);
#endif

#if GTP_P_GESTURE_CONTROL
static RAW_NOTIFIER_HEAD(gesture_chain);
#endif

static struct device *mx_tsp;   //add by chen for MX gesture
...
#if GTP_COVER_CONTROL
s32 gtp_send_cfg_cover(struct i2c_client *client);
#endif
...
#if GTP_COVER_CONTROL
static u8 config_cover[GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH]
    = {GTP_REG_CONFIG_DATA >> 8, GTP_REG_CONFIG_DATA & 0xff};
#if GTP_CHARGER_SWITCH
static u8 gtp_charger_config_cover[GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH]
	= {GTP_REG_CONFIG_DATA >> 8, GTP_REG_CONFIG_DATA & 0xff};
#endif
u8 cfg_cover_len = 0;
u8 pnl_cover_init_error = 0;
#endif
...
#if GTP_COVER_CONTROL
u8 driver_num_cover = 0;
u8 sensor_num_cover = 0;
#endif
...
//extern int ltr559_read_ps(void);//add by chen
static int ctp_read_ps(void); 
...
#if GTP_COVER_CONTROL
int register_cover_notifier(struct notifier_block *nb)
{
  return raw_notifier_chain_register(&cover_chain, nb);
}

int unregister_cover_notifier(struct notifier_block *nb)
{
  return raw_notifier_chain_unregister(&cover_chain,nb);
}


int cover_notifier_call_chain(unsigned long val, void *v)
{
  return raw_notifier_call_chain(&cover_chain, val, v);
}
EXPORT_SYMBOL(cover_notifier_call_chain);

static int cover_event(struct notifier_block *this, unsigned long event, void *ptr)
{
  GTP_DEBUG("cover_event event=%ld\n",event);
  if(event==0)
  {
  ...
  {
  	gtp_send_cfg_cover(i2c_client_point);
  }
  return 0;
}


static struct notifier_block cover_notifier1 =
{
   .notifier_call = cover_event,
};
#endif


#if GTP_P_GESTURE_CONTROL
int register_gesture_notifier(struct notifier_block *nb)
{
  return raw_notifier_chain_register(&gesture_chain, nb);
}

int unregister_gesture_notifier(struct notifier_block *nb)
{
  return raw_notifier_chain_unregister(&gesture_chain,nb);
}


int gesture_notifier_call_chain(unsigned long val, void *v)
{
  return raw_notifier_call_chain(&gesture_chain, val, v);
}
EXPORT_SYMBOL(gesture_notifier_call_chain);

static int gesture_event(struct notifier_block *this, unsigned long event, void *ptr)
{
  GTP_DEBUG("gesture_event event=%ld\n",event);
  if(event==0)
  {
  	p_gesture_control = 0;
  }
...
static struct notifier_block gesture_notifier1 =
{
   .notifier_call = gesture_event,
};
#endif



//add by chen for MX gesture
...
*/
	GTP_DEBUG("gesture_data_show gesture_data=%x\n",gesture_data);
	return snprintf(buf, PAGE_SIZE, "%d\n", gesture_data);
}
...
{  
    ...
    GTP_DEBUG("gesture_three_byte_one=%x,gesture_three_byte_two=%x,gesture_three_byte_three=%x,gesture_three_byte_four=%x\n",gesture_three_byte_one,gesture_three_byte_two,gesture_three_byte_three,gesture_three_byte_four);
    return count;
}
/*----------------------------------------------------------------------------*/
...
static ssize_t cover_control_store(struct device* dev, struct device_attribute *attr,
                                  const char *buf, size_t count)
{  
	u32 value = 0;
	u8 buf_value = buf[0];
    	sscanf(buf, "%d", &value);
	GTP_DEBUG("cover_control_store buf_value=%d,value=%d\n",buf_value,value);
	if(value==0)
       {
  	    gtp_send_cfg(i2c_client_point);
       }
...
static DEVICE_ATTR(cover_control,      		S_IWUSR | S_IRUGO, cover_control_show,  cover_control_store);
...
s32 gtp_send_cfg(struct i2c_client *client)
{
	...
	if (pnl_init_error)
    {
        GTP_ERROR("Error occurred in init_panel, no config sent!");
        return 0;
    }
...
s32 gtp_send_cfg_cover(struct i2c_client *client)
{
    ...
    for (retry = 0; retry < 5; retry++)
    {
        ret = gtp_i2c_write(client, config_cover, GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH);
...
/*******************************************************
Function:
    Read goodix touchscreen lcm color info function.

Input:
    client: i2c client struct.
    lcm_color_info:address to store lcm color info

Output:
    Executive outcomes.0---succeed.

add by xunanbin 20151109
*******************************************************/

s32 gtp_read_lcm_color_info(struct i2c_client *client, u8 *lcm_color_info)
{
    ...
    ret = i2c_read_bytes(client, GTP_REG_LCM_COLOR_INFO, lcm_color_info, sizeof(*lcm_color_info));

    if (ret < 0)
    {
        GTP_ERROR("GTP read lcm color info failed");
        return ret;
    }


    if (*lcm_color_info == 0x55)
    {
        tp_type_flag = 1;
    }
    else if(*lcm_color_info == 0xaa)
    {
        tp_type_flag = 0;
    }
...
static s32 gtp_init_panel(struct i2c_client *client)
{
    ...
    else
    {
        ...
        if(sensor_id==0)
	 {
	 	info += sprintf(info,"GT915L,");
		info += sprintf(info,"OFILM,");
	 	info += sprintf(info, "%04x", tpd_info.vid);
	 }
        else if(sensor_id==1)
	 {
	 	info += sprintf(info,"GT915L,");
		info += sprintf(info,"YASSY,");
	 	info += sprintf(info, "%04x", tpd_info.vid);
	 }		
    }
...
#endif
	{
	    ...
	    if (ret == SUCCESS)
	    {
	        GTP_INFO("CFG_CONFIG_GROUP%d Config Version: %d, 0x%02X; IC Config Version: %d, 0x%02X", sensor_id, 
...
Output:
    Executive outcomes.0---succeed.
*******************************************************/
static s32 gtp_cover_panel(struct i2c_client *client)
{
    s32 ret = 0;
    ...
    u8 cfg_cover_info_group0[] = CTP_CFG_COVER_GROUP0;
    u8 cfg_cover_info_group1[] = CTP_CFG_COVER_GROUP1;
    u8 cfg_cover_info_group2[] = CTP_CFG_COVER_GROUP2;
    u8 cfg_cover_info_group3[] = CTP_CFG_COVER_GROUP3;
    u8 cfg_cover_info_group4[] = CTP_CFG_COVER_GROUP4;
    u8 cfg_cover_info_group5[] = CTP_CFG_COVER_GROUP5;
    u8 *send_cover_cfg_buf[] = {cfg_cover_info_group0, cfg_cover_info_group1, cfg_cover_info_group2,
                        cfg_cover_info_group3, cfg_cover_info_group4, cfg_cover_info_group5};
    u8 cfg_cover_info_len[] = { CFG_GROUP_LEN(cfg_cover_info_group0), 
                          CFG_GROUP_LEN(cfg_cover_info_group1),
                          CFG_GROUP_LEN(cfg_cover_info_group2),
                          CFG_GROUP_LEN(cfg_cover_info_group3), 
                          CFG_GROUP_LEN(cfg_cover_info_group4),
                          CFG_GROUP_LEN(cfg_cover_info_group5)};
#if GTP_CHARGER_SWITCH
	const u8 cfg_cover_grp0_charger[] = GTP_CFG_COVER_GROUP0_CHARGER;
	const u8 cfg_cover_grp1_charger[] = GTP_CFG_COVER_GROUP1_CHARGER;
	const u8 cfg_cover_grp2_charger[] = GTP_CFG_COVER_GROUP2_CHARGER;
	const u8 cfg_cover_grp3_charger[] = GTP_CFG_COVER_GROUP3_CHARGER;
	const u8 cfg_cover_grp4_charger[] = GTP_CFG_COVER_GROUP4_CHARGER;
	const u8 cfg_cover_grp5_charger[] = GTP_CFG_COVER_GROUP5_CHARGER;
	const u8 *cfgs_cover_charger[] = {
		cfg_cover_grp0_charger, cfg_cover_grp1_charger, cfg_cover_grp2_charger,
		cfg_cover_grp3_charger, cfg_cover_grp4_charger, cfg_cover_grp5_charger
	};
	u8 cfg_cover_lens_charger[] = {
						CFG_GROUP_LEN(cfg_cover_grp0_charger),
						CFG_GROUP_LEN(cfg_cover_grp1_charger),
						CFG_GROUP_LEN(cfg_cover_grp2_charger),
						CFG_GROUP_LEN(cfg_cover_grp3_charger),
						CFG_GROUP_LEN(cfg_cover_grp4_charger),
						CFG_GROUP_LEN(cfg_cover_grp5_charger)};
#endif

    GTP_DEBUG("Config Groups\' Lengths: %d, %d, %d, %d, %d, %d", 
        cfg_cover_info_len[0], cfg_cover_info_len[1], cfg_cover_info_len[2], cfg_cover_info_len[3],
        cfg_cover_info_len[4], cfg_cover_info_len[5]);

    if ((!cfg_cover_info_len[1]) && (!cfg_cover_info_len[2]) && 
        (!cfg_cover_info_len[3]) && (!cfg_cover_info_len[4]) && 
        (!cfg_cover_info_len[5]))
    {
        sensor_id = 0; 
    }
    else
    ...
    cfg_cover_len = cfg_cover_info_len[sensor_id];
    
    GTP_INFO("CTP_CONFIG_GROUP%d used, config length: %d", sensor_id, cfg_cover_len);
    
    if (cfg_cover_len < GTP_CONFIG_MIN_LENGTH)
    {
        GTP_ERROR("CTP_CONFIG_GROUP%d is INVALID CONFIG GROUP! NO Config Sent! You need to check you header file CFG_GROUP section!", sensor_id);
        pnl_cover_init_error = 1;
        return -1;
...
#endif
	{
	    ...
	    if (ret == SUCCESS)
	    {
	        GTP_DEBUG("CFG_CONFIG_GROUP%d Config Version: %d, 0x%02X; IC Config Version: %d, 0x%02X", sensor_id, 
	                    send_cover_cfg_buf[sensor_id][0], send_cover_cfg_buf[sensor_id][0], opr_buf[0], opr_buf[0]);
	
			flash_cfg_version = opr_buf[0];
			drv_cfg_version = send_cover_cfg_buf[sensor_id][0];       // backup  config version
			
	        if (flash_cfg_version < 90 && flash_cfg_version > drv_cfg_version) {
	            send_cover_cfg_buf[sensor_id][0] = 0x00;
	        }
	    }
	    else
	    {
    ...
    memset(&config_cover[GTP_ADDR_LENGTH], 0, GTP_CONFIG_MAX_LENGTH);
    memcpy(&config_cover[GTP_ADDR_LENGTH], send_cover_cfg_buf[sensor_id], cfg_cover_len);

#if GTP_CUSTOM_CFG
    config_cover[RESOLUTION_LOC]     = (u8)GTP_MAX_WIDTH;
    config_cover[RESOLUTION_LOC + 1] = (u8)(GTP_MAX_WIDTH>>8);
    config_cover[RESOLUTION_LOC + 2] = (u8)GTP_MAX_HEIGHT;
    config_cover[RESOLUTION_LOC + 3] = (u8)(GTP_MAX_HEIGHT>>8);
    
    if (GTP_INT_TRIGGER == 0)  //RISING
    {
        config_cover[TRIGGER_LOC] &= 0xfe; 
    }
    else if (GTP_INT_TRIGGER == 1)  //FALLING
    {
        config_cover[TRIGGER_LOC] |= 0x01;
    }
#endif  // GTP_CUSTOM_CFG

	check_sum = 0;
	for (i = GTP_ADDR_LENGTH; i < cfg_cover_len; i++)
	{
		check_sum += config_cover[i];
	}
	config_cover[cfg_cover_len] = (~check_sum) + 1;

#if GTP_CHARGER_SWITCH
	GTP_DEBUG("Charger Config Groups Length: %d, %d, %d, %d, %d, %d", cfg_cover_lens_charger[0],
		  cfg_cover_lens_charger[1], cfg_cover_lens_charger[2], cfg_cover_lens_charger[3], cfg_cover_lens_charger[4], cfg_cover_lens_charger[5]);

	memset(&gtp_charger_config_cover[GTP_ADDR_LENGTH], 0, GTP_CONFIG_MAX_LENGTH);
	if (cfg_cover_lens_charger[sensor_id] == cfg_cover_len) 
		memcpy(&gtp_charger_config_cover[GTP_ADDR_LENGTH], cfgs_cover_charger[sensor_id], cfg_cover_len);

#if GTP_CUSTOM_CFG
	gtp_charger_config_cover[RESOLUTION_LOC] = (u8) GTP_MAX_WIDTH;
	gtp_charger_config_cover[RESOLUTION_LOC + 1] = (u8) (GTP_MAX_WIDTH >> 8);
	gtp_charger_config_cover[RESOLUTION_LOC + 2] = (u8) GTP_MAX_HEIGHT;
	gtp_charger_config_cover[RESOLUTION_LOC + 3] = (u8) (GTP_MAX_HEIGHT >> 8);

	if (GTP_INT_TRIGGER == 0) 	/* RISING  */
		gtp_charger_config_cover[TRIGGER_LOC] &= 0xfe;
	else if (GTP_INT_TRIGGER == 1) /* FALLING */
		gtp_charger_config_cover[TRIGGER_LOC] |= 0x01;
#endif /* END GTP_CUSTOM_CFG */
	if (cfg_cover_lens_charger[sensor_id] != cfg_cover_len) 
		memset(&gtp_charger_config_cover[GTP_ADDR_LENGTH], 0, GTP_CONFIG_MAX_LENGTH);	
	...
	for (i = GTP_ADDR_LENGTH; i < cfg_cover_len; i++)
	{
		check_sum += gtp_charger_config_cover[i];
	}
	gtp_charger_config_cover[cfg_cover_len] = (~check_sum) + 1;

#endif /* END GTP_CHARGER_SWITCH */    
  
#else // DRIVER NOT SEND CONFIG
    cfg_cover_len = GTP_CONFIG_MAX_LENGTH;
    ret = gtp_i2c_read(client, config_cover, cfg_cover_len + GTP_ADDR_LENGTH);
    if (ret < 0)
    {
        GTP_ERROR("Read Config Failed, Using DEFAULT Resolution & INT Trigger!");
        abs_x_max = GTP_MAX_WIDTH;
    ...
    if ((abs_x_max == 0) && (abs_y_max == 0))
    {
        abs_x_max = (config_cover[RESOLUTION_LOC + 1] << 8) + config_cover[RESOLUTION_LOC];
        abs_y_max = (config_cover[RESOLUTION_LOC + 3] << 8) + config_cover[RESOLUTION_LOC + 2];
        int_type = (config_cover[TRIGGER_LOC]) & 0x03; 
    }
    ...
    if (CHIP_TYPE_GT9F == gtp_chip_type)
    {
        ...
        if (is_950)
        {
            driver_num_cover = config_cover[GTP_REG_MATRIX_DRVNUM - GTP_REG_CONFIG_DATA + 2];
            sensor_num_cover = config_cover[GTP_REG_MATRIX_SENNUM - GTP_REG_CONFIG_DATA + 2];
        }
        else
        {
            driver_num_cover = (config_cover[CFG_LOC_DRVA_NUM]&0x1F) + (config_cover[CFG_LOC_DRVB_NUM]&0x1F);
            sensor_num_cover = (config_cover[CFG_LOC_SENS_NUM]&0x0F) + ((config_cover[CFG_LOC_SENS_NUM]>>4)&0x0F);
        }
        
        have_key = config_cover[GTP_REG_HAVE_KEY - GTP_REG_CONFIG_DATA + 2] & 0x01;  // have key or not
        if (1 == have_key)
        {
            driver_num_cover--;
        }
        ...
        GTP_INFO("Driver * Sensor: %d * %d(Key: %d), X_MAX = %d, Y_MAX = %d, TRIGGER = 0x%02x",
            driver_num_cover, sensor_num_cover, have_key, abs_x_max,abs_y_max,int_type);
    }
    else
#endif
    {
...
#endif
	  {
        /* for resume to send config */
		if (flash_cfg_version < 90 && flash_cfg_version > drv_cfg_version) {		
	        config_cover[GTP_ADDR_LENGTH] = drv_cfg_version;
	        check_sum = 0;
	        for (i = GTP_ADDR_LENGTH; i < cfg_cover_len; i++)
	        {
	            check_sum += config_cover[i];
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    u8 lcm_color_info;
    u16 version_info;
GTP_INFO("MediaTek gt91xx touch panel driver init  tpd_i2c_probe enter\n");
#if GTP_HAVE_TOUCH_KEY
    s32 idx = 0;
#endif
#ifdef TPD_PROXIMITY
...
//#ifdef VELOCITY_CUSTOM
    ...
    ret = gtp_read_lcm_color_info(client, &lcm_color_info);

    if (ret < 0)
    {
        GTP_ERROR("Read lcm color info failed.");
    }	
...
#if GTP_COVER_CONTROL
    ret = gtp_cover_panel(client);

    if (ret < 0)
    {
        GTP_ERROR("GTP cover panel failed.");
    }
#endif
    
    // Create proc file system
    gt91xx_config_proc = proc_create(GT91XX_CONFIG_PROC_FILE, 0644, NULL, &config_proc_ops);
	...
	if (err < 0)
	{
	   GTP_ERROR("unable to create gesture attribute file\n");
	}

	//add end

#if GTP_COVER_CONTROL
	err =register_cover_notifier(&cover_notifier1);
  	if (err)
  	{
   	    GTP_ERROR("register cover_notifier1 error\n");
  	}
#endif

#if GTP_P_GESTURE_CONTROL
	err =register_gesture_notifier(&gesture_notifier1);
  	if (err)
  	{
   	    GTP_ERROR("register gesture_notifier1 error\n");
  	}
#endif
...
//    input_set_capability(tpd->dev, EV_KEY, KEY_POWER);
...
#endif
#ifdef TYPE_B_PROTOCOL
    input_mt_init_slots(tpd->dev, GTP_MAX_TOUCH, INPUT_MT_DIRECT);
#endif
    ...
    tpd_load_status = 1;
GTP_INFO("MediaTek gt91xx touch panel driver init  tpd_i2c_probe end\n");
    return 0;
}
...
static void tpd_down(s32 x, s32 y, s32 size, s32 id)
{
    ...
    set_bit(id, &gt9xx_finger_status);
...
static void tpd_up(s32 x, s32 y, s32 id)
{
   ...
   // input_report_key(tpd->dev, BTN_TOUCH, 0);
    clear_bit(id, &gt9xx_finger_status);
#ifndef TYPE_B_PROTOCOL
    input_mt_sync(tpd->dev);
#endif
...
static void gt9xx_clear_all_touch(void)
{
	int i = 0;
	gt9xx_finger_status = gt9xx_finger_status_pre;
	while(gt9xx_finger_status != 0) {
		tpd_up(0, 0, i);
		i++;
		if(i > GTP_MAX_TOUCH) break;
	}
	input_sync(tpd->dev);
	gt9xx_finger_status_pre = 0;
}
#if GTP_CHARGER_SWITCH
static void gtp_charger_switch(s32 dir_update)
{
...
static int ctp_read_ps(void)
{
    ...
    GTP_INFO("JerryXie Note: ps value = %d .\n", value);
...
static int touch_event_handler(void *unused)
{
    ...
    do
    {
	   ...
	   //   ps = ltr559_read_ps();
	     ...
	     if(ps == 0)
	     {
                ...
                */
                if ((doze_buf[2] == 'c') || (doze_buf[2] == 'e') || (doze_buf[2] == 'm') || 
			(doze_buf[2] == 'o') || (doze_buf[2] == 's') || (doze_buf[2] == 'v') || 
                    (doze_buf[2] == 'w') || (doze_buf[2] == 'z') 
                    )
                {
                	if (((doze_buf[2] == 'v') && ((gesture_three_byte_three & 0x01) == 0x01)) || 
			     ((doze_buf[2] == 'c') && ((gesture_three_byte_three & 0x02) == 0x02)) || 
			     ((doze_buf[2] == 'e') && ((gesture_three_byte_three & 0x04) == 0x04)) || 
			     ((doze_buf[2] == 'w') && ((gesture_three_byte_three & 0x08) == 0x08)) || 
			     ((doze_buf[2] == 'm') && ((gesture_three_byte_three & 0x10) == 0x10)) || 
			     ((doze_buf[2] == 's') && ((gesture_three_byte_three & 0x20) == 0x20)) || 
			     ((doze_buf[2] == 'z') && ((gesture_three_byte_three & 0x40) == 0x40)) ||
                    	     ((doze_buf[2] == 'o') && ((gesture_three_byte_three & 0x80) == 0x80)) 
                    )
                	{
                	    GTP_INFO("Wakeup by gesture(%c), light up the screen!", doze_buf[2]);
			    doze_status = DOZE_WAKEUP;
			    ...
			    else if(doze_buf[2] == 'c')
			    {
			        gesture_data = 0xC1;
			    }
			    else if(doze_buf[2] == 'e')
			    {
			        gesture_data = 0xC0;
			    }
			    ...
			    else if(doze_buf[2] == 's')
			    {
			        gesture_data = 0xC5;
			    }
			    else if(doze_buf[2] == 'z')
			    {
			        gesture_data = 0xCA;
			    }
			    else  //doze_buf[2] == 'o'
			...
			if(((type == 0) && ((gesture_three_byte_four & 0x01) == 0x01)) ||
				((type == 1) && ((gesture_three_byte_four & 0x04) == 0x04)) ||
				((type == 2) && ((gesture_three_byte_four & 0x08) == 0x08)) ||
				((type == 3) && ((gesture_three_byte_four & 0x02) ==0x02))
			)
			{
			...
			else
			{
                ...
                {
                	if((gesture_three_byte_two & 0x01)== 0x01)
                	{
	...
	else
	{
        ...
        if (touch_num)
        {
            ...
            if(gt9xx_finger_status != gt9xx_finger_status_pre) 
            { // up non-contact finger
	            for(i = 0; i < GTP_MAX_TOUCH; i++)
		            if(!test_bit(i, &gt9xx_finger_status) && test_bit(i, &gt9xx_finger_status_pre))
		            tpd_up(0, 0, i);
            }
        }
        else
        ...
        pre_touch = touch_num;
        gt9xx_finger_status_pre = gt9xx_finger_status;
        gt9xx_finger_status = 0;
...
static void tpd_suspend(struct early_suspend *h)
{
  ...
  //  printk("xunanbin tp_type_flag = %d",tp_type_flag);
...
static void tpd_resume(struct early_suspend *h)
{
 ...
 //   printk("xunanbin tp_type_flag = %d\n",tp_type_flag);
#ifdef TPD_PROXIMITY
...
#if GTP_COVER_CONTROL
    unregister_cover_notifier(&cover_notifier1);
#endif
#if GTP_P_GESTURE_CONTROL
    unregister_gesture_notifier(&gesture_notifier1);
#endif
