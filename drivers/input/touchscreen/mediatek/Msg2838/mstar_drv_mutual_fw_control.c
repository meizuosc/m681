...
#include "msg28xx_yeji_white_update_bin.h"
#include "msg28xx_yeji_black_update_bin.h"
...
#define WHITE_COVERLENS		1
#define BLACK_COVERLENS		1
...
static s32 _DrvFwCtrlParsePacket(u8 *pPacket, u16 nLength, TouchInfo_t *pInfo)
{
    ...
    if (g_GestureWakeupFlag == 1)
    {
        ...
        if (bIsCorrectFormat)
        {
            ...
            switch (nWakeupMode)
            {
                case 0x58:
                #ifdef ENABLE_MX_GESTURE
                	if((ges_three_byte_two & 0x01)== 0x01)
                #endif
                	{
                        ...
                        input_report_key(g_InputDevice, KEY_GESTURE, 1);
                        input_sync(g_InputDevice);
                        input_report_key(g_InputDevice, KEY_GESTURE, 0);
                        input_sync(g_InputDevice);
			    ges_data = DOUBLE_TAP;	
                	}
                    break;	
                case 0x60:
                #ifdef ENABLE_MX_GESTURE
                	if((ges_three_byte_four & 0x08) == 0x08)
                #endif
                	{
    ...
    //                    input_report_key(g_InputDevice, KEY_UP, 0);
                ...
                case 0x61:
                #ifdef ENABLE_MX_GESTURE
                	if((ges_three_byte_four & 0x04) == 0x04)
                #endif
                	{
    ...
    //                    input_report_key(g_InputDevice, KEY_DOWN, 0);
                        input_report_key(g_InputDevice, KEY_GESTURE, 0);
                        input_sync(g_InputDevice);
			    ges_data = SWIPE_Y_DOWN;	
                    }
                    break;	
                case 0x62:
                #ifdef ENABLE_MX_GESTURE
                	if((ges_three_byte_four & 0x02) ==0x02)
                #endif
                	{
    ...
    //                    input_report_key(g_InputDevice, KEY_LEFT, 0);
                        input_report_key(g_InputDevice, KEY_GESTURE, 0);
                        input_sync(g_InputDevice);
			    ges_data = SWIPE_X_LEFT;
                    }
                    break;		
                case 0x63:
                #ifdef ENABLE_MX_GESTURE
                	if((ges_three_byte_four & 0x01) == 0x01)
                #endif
                	{
    ...
    //                    input_report_key(g_InputDevice, KEY_RIGHT, 0);
                        input_report_key(g_InputDevice, KEY_GESTURE, 0);
                        input_sync(g_InputDevice);
			    ges_data = SWIPE_X_RIGHT;
                    }
                    break;		
                case 0x64:
                #ifdef ENABLE_MX_GESTURE
                	if((ges_three_byte_three & 0x10) == 0x10)
                #endif
                	{
    ...
    //                    input_report_key(g_InputDevice, KEY_M, 0);
                ...
                case 0x65:
                #ifdef ENABLE_MX_GESTURE
                	if((ges_three_byte_three & 0x08) == 0x08)
                #endif
                	{
    ...
    //                    input_report_key(g_InputDevice, KEY_W, 0);
                ...
                case 0x66:
                #ifdef ENABLE_MX_GESTURE
                	if((ges_three_byte_three & 0x02) == 0x02)
                #endif
                	{
    ...
    //                    input_report_key(g_InputDevice, KEY_C, 0);
                ...
                case 0x67:
                #ifdef ENABLE_MX_GESTURE
                	if((ges_three_byte_three & 0x04) == 0x04)
                #endif
                	{
    ...
    //                    input_report_key(g_InputDevice, KEY_E, 0);
                ...
                case 0x68:
                #ifdef ENABLE_MX_GESTURE
                	if((ges_three_byte_three & 0x01) == 0x01)
                #endif
                	{
    ...
    //                    input_report_key(g_InputDevice, KEY_V, 0);
                        input_report_key(g_InputDevice, KEY_GESTURE, 0);
                        input_sync(g_InputDevice);
    			    ges_data = UNICODE_V_DOWN;
                    }
                    break;
                case 0x69:
                #ifdef ENABLE_MX_GESTURE
                	if((ges_three_byte_three & 0x80) == 0x80)
                #endif
                	{
    ...
    //                    input_report_key(g_InputDevice, KEY_O, 0);
                ...
                case 0x6A:
                #ifdef ENABLE_MX_GESTURE
                	if((ges_three_byte_three & 0x20) == 0x20)
                #endif
                	{
    ...
    //                    input_report_key(g_InputDevice, KEY_S, 0);
                ...
                case 0x6B:
                #ifdef ENABLE_MX_GESTURE
                	if((ges_three_byte_three & 0x40) == 0x40)
                #endif
                	{
...
void _DrvFwCtrlMsg28xxCheckFirmwareUpdateBySwId(void) 
{
    ...
    if (nCrcMainA == nCrcMainB) 
    {
        ...
        else if (eSwId == MSG28XX_SW_ID_YEJI)
        {
#ifdef CONFIG_UPDATE_FIRMWARE_BY_TWO_DIMENSIONAL_ARRAY // By two dimensional array
            if (tp_type_flag == WHITE_COVERLENS)
            {
                nUpdateBinMajor = msg28xx_yeji_white_update_bin[127][1013]<<8 | msg28xx_yeji_white_update_bin[127][1012]; 
                nUpdateBinMinor = msg28xx_yeji_white_update_bin[127][1015]<<8 | msg28xx_yeji_white_update_bin[127][1014];
            }
            else
            {
                nUpdateBinMajor = msg28xx_yeji_black_update_bin[127][1013]<<8 | msg28xx_yeji_black_update_bin[127][1012]; 
                nUpdateBinMinor = msg28xx_yeji_black_update_bin[127][1015]<<8 | msg28xx_yeji_black_update_bin[127][1014];
            }

#else // By one dimensional array
                nUpdateBinMajor = msg28xx_yeji_white_update_bin[0x1FFF5]<<8 | msg28xx_yeji_white_update_bin[0x1FFF4];
                nUpdateBinMinor = msg28xx_yeji_white_update_bin[0x1FFF7]<<8 | msg28xx_yeji_white_update_bin[0x1FFF6];
#endif
        }
        ...
        if (nUpdateBinMinor > nMinor)
        {
            ...
            else if (eSwId == MSG28XX_SW_ID_YEJI)
            {
                for (i = 0; i < MSG28XX_FIRMWARE_WHOLE_SIZE; i ++)
                {
                    ...
                    if (tp_type_flag == WHITE_COVERLENS)
                    {
                        _DrvFwCtrlStoreFirmwareData(msg28xx_yeji_white_update_bin[i], 1024);
                    }
                    else
                    {
                        _DrvFwCtrlStoreFirmwareData(msg28xx_yeji_black_update_bin[i], 1024);
                    }
#else // By one dimensional array
                    _DrvFwCtrlStoreFirmwareData(&(msg28xx_yeji_white_update_bin[i*1024]), 1024);
#endif
                }
            }
            else
