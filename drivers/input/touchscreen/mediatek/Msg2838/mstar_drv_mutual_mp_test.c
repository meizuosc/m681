...
#include "msg28xx_mp_test_yeji.h"
...
static u16 SENSE_NUM = 24;
static u16 DRIVE_NUM = 14;
...
static s16  GOLDEN_CH_0[MAX_MUTUAL_NUM]  = {0};
...
static s32 _DrvMpTestItoOpenTestMsg28xxOpenJudge(u16 nItemID, s8 *pNormalTestResult, u16 *pNormalTestResultCheck)
{
    s32 nRetVal = 0;
    u16 nCSub = CSUB_REF;
...
//    u32 nSum=0/*, nAvg=0, nDelta=0, nPrev=0*/;
    ...
    u16 passCount = 0;
    u16 passCount1 = 0;    
    u16 ratioAvg_border = 0;
//    u16 ratioAvg_border_max = 0;
//    u16 ratioAvg_border_min = 0; 
    u16 ratioAvg_move = 0;
    u16 ratioAvg_border_move = 0;    
    s16 * ratio;
    s16 * ratio_border;
    s16 * ratio_move;        
    s16 * ratio_border_move;
    s16 * normalTestFail_check_Deltac;
    s16 * normalTestFail_check_Ratio;
    s16 * GOLDENSAMPLE_CH_0;    
    s16 * GOLDENSAMPLE_CH_0_Max;
    s16 * GOLDENSAMPLE_CH_0_Min;    

    //calculate golden range
    u16 value = 0, value_up = 0;
    ...
    ratio = kzalloc(sizeof(s16) * MAX_MUTUAL_NUM, GFP_KERNEL);
    ratio_border = kzalloc(sizeof(s16) * MAX_MUTUAL_NUM, GFP_KERNEL);
    ratio_move = kzalloc(sizeof(s16) * MAX_MUTUAL_NUM, GFP_KERNEL);
    ratio_border_move = kzalloc(sizeof(s16) * MAX_MUTUAL_NUM, GFP_KERNEL);    
    normalTestFail_check_Deltac = kzalloc(sizeof(s16) * MAX_MUTUAL_NUM, GFP_KERNEL);
    normalTestFail_check_Ratio = kzalloc(sizeof(s16) * MAX_MUTUAL_NUM, GFP_KERNEL);
    GOLDENSAMPLE_CH_0 = kzalloc(sizeof(s16) * MAX_MUTUAL_NUM, GFP_KERNEL);    
    GOLDENSAMPLE_CH_0_Max = kzalloc(sizeof(s16) * MAX_MUTUAL_NUM, GFP_KERNEL);
    GOLDENSAMPLE_CH_0_Min = kzalloc(sizeof(s16) * MAX_MUTUAL_NUM, GFP_KERNEL);

    memset(ratio, 0, MAX_MUTUAL_NUM);
    memset(ratio_border, 0, MAX_MUTUAL_NUM);
    memset(ratio_move, 0, MAX_MUTUAL_NUM);
    memset(ratio_border_move, 0, MAX_MUTUAL_NUM);
    memset(normalTestFail_check_Deltac, 0xFFFF, sizeof(MAX_MUTUAL_NUM));
    memset(normalTestFail_check_Ratio, 0xFFFF, sizeof(MAX_MUTUAL_NUM));
    memset(GOLDENSAMPLE_CH_0, 0, MAX_MUTUAL_NUM);
    
    //calculate golden range
    for (j = 0; j < (sizeof(GOLDEN_CH_0) / sizeof(GOLDEN_CH_0[0])); j++)
    {
        GOLDENSAMPLE_CH_0[j] = GOLDEN_CH_0[j];
    }
	
	DBG("*** Msg28xx Golden# Show  ***\n"); 
	for (j = 0; j < _gDriveLineNum; j ++)
	{
		for (i = 0; i < _gSenseLineNum; i ++)
		{
			DBG("%d  ", GOLDENSAMPLE_CH_0[i * _gDriveLineNum + j]);
		}
		DBG("\n");                
	}     
    ...
    for (i = 0; i < MAX_MUTUAL_NUM; i++)
    {
    	value = DC_RANGE * abs(GOLDENSAMPLE_CH_0[i]) / 100;
        value_up = DC_RANGE_UP * abs(GOLDENSAMPLE_CH_0[i]) / 100;
        GOLDENSAMPLE_CH_0_Max[i] = GOLDENSAMPLE_CH_0[i] + value + value_up;
        GOLDENSAMPLE_CH_0_Min[i] = GOLDENSAMPLE_CH_0[i] - value;
    }

    for (k = 0; k < (sizeof(_gDeltaC) / sizeof(_gDeltaC[0])); k++)
    {
    	if (0 == GOLDENSAMPLE_CH_0[k]) {
    		if (k == 0){
    	...
    	if (_gResult[k] != -32000)
    	{
    		ratio[k] = _gResult[k]*100 / GOLDENSAMPLE_CH_0[k];

    		if (0 == _DrvMpTestItoTestCheckValueInRange(_gResult[k], GOLDENSAMPLE_CH_0_Max[k], GOLDENSAMPLE_CH_0_Min[k]))
    		{
    			pNormalTestResult[0] = 1;
    			pNormalTestResultCheck[k] = (u16)(((k*100 / _gDriveLineNum) + 100) + ((k % _gDriveLineNum) + 1));
    			DBG("Golden issue: _gResult[%d]=%d\n", k, _gResult[k]);                 
    		}
    		else
    		{
    			pNormalTestResultCheck[k] = PIN_NO_ERROR;
    			if ((PATTERN_TYPE == 3) && (KEY_NUM == 0) && ((k % _gDriveLineNum == 0 )||((k + 1) % _gDriveLineNum == 0)))
    			{
    				ratioAvg_border += ratio[k];
    				passCount1 += 1;
    			}
    			else if ((PATTERN_TYPE == 3) && (KEY_NUM != 0) && ((k % _gDriveLineNum == 0) || ((k + 2) % _gDriveLineNum == 0)))
    			{
    				ratioAvg_border += ratio[k];
    				passCount1 += 1;
    			}
    ...
    //DBG("*** Msg28xx Open Test# normalTestFail_check_Deltac Channel ***\n");

    ratioAvg_max = (s16)(100 + DC_RATIO + DC_RATIO_UP);
    ratioAvg_min = (s16)(100 - DC_RATIO);
    //ratioAvg_border_max=(s16)(100 + DC_BORDER_RATIO + DC_RATIO_UP);
    //ratioAvg_border_min = (s16)(100 - DC_BORDER_RATIO);
    ...
    if (passCount != 0)
    {
        if (passCount1 != 0)
        {
            ...
            for (i = 0; i < MAX_MUTUAL_NUM; i++)
            {
                if ((KEY_NUM == 0) && ((i % _gDriveLineNum == 0) || ((i + 1) % _gDriveLineNum == 0)))
                {
                    ratio_move[i] = ratio[i] - ratioAvg_border_move + 100;                    
                }
                else if ((KEY_NUM != 0) && ((i % _gDriveLineNum == 0) || ((i + 2) % _gDriveLineNum == 0)))
                {
                    ratio_move[i] = ratio[i] - ratioAvg_border_move + 100;
                }
                else
                {
                    ratio_move[i] = ratio[i] - ratioAvg_move + 100;                    
                }
    ...
    {
        memcpy(ratio, ratio_move, MAX_MUTUAL_NUM * sizeof(s16));
    }

    for (j = 0; j < (sizeof(_gDeltaC) / sizeof(_gDeltaC[0])); j++)
    {
        if (0 == GOLDENSAMPLE_CH_0[j]) {
        	if (j == 0){
        ...
        if (PIN_NO_ERROR == pNormalTestResultCheck[j])
        {
            if (_gResult[j] != -32000)
            {
                if ((PATTERN_TYPE == 3) && (KEY_NUM == 0) && ((j % _gDriveLineNum == 0) || ((j + 1) % _gDriveLineNum == 0)))
                {
                  /*
                    if (0 == _DrvMpTestItoTestCheckValueInRange(ratio_move[j], ratioAvg_border_max, ratioAvg_border_min))
                    {
                    	pNormalTestResult[1] = 1;
                    	pNormalTestResultCheck[j] = (u16)(((j*100 / _gDriveLineNum) + 100) + ((j % _gDriveLineNum) + 1));
                    }
                    else
                    {
                ...
                else if ((PATTERN_TYPE == 3) && (KEY_NUM != 0) && ((j % _gDriveLineNum == 0) || ((j + 2) % _gDriveLineNum == 0)))
                {
                  /* 
                    if (0 == _DrvMpTestItoTestCheckValueInRange(ratio_move[j], ratioAvg_border_max, ratioAvg_border_min))
                    {
                ...
                {
//                	DBG("ratiomove[%d] = %f, ratioAvg_max = %f, ratioAvg_min = %f",j,ratio_move[j], ratioAvg_max, ratioAvg_min);
                    if (0 == _DrvMpTestItoTestCheckValueInRange(ratio_move[j], ratioAvg_max, ratioAvg_min))
                    {
                    	pNormalTestResult[1] = 1;
                    	pNormalTestResultCheck[j] = (u16)(((j*100 / _gDriveLineNum) + 100) + ((j % _gDriveLineNum) + 1));
                        DBG("Ratio issue: ratio_move[%d]= %d\n", j, ratio_move[j]); 
                    }
                    else
                    {
                    	pNormalTestResultCheck[j] = PIN_NO_ERROR;
    ...
    //DBG("*** Msg28xx Open Test# normalTestFail_check_Ratio Channel ***\n");
   ...
   */ 
	DBG("*** Msg28xx Open Ratio# Show  ***\n"); 
	for (j = 0; j < _gDriveLineNum; j ++)
	{
		for (i = 0; i < _gSenseLineNum; i ++)
		{
			DBG("%d  ", ratio_move[i * _gDriveLineNum + j]);
		}
		DBG("\n");                
	}     
    ...
    if ((pNormalTestResult[0] != 0) || (pNormalTestResult[1] != 0)){
        DBG("Open test golden result: pNormalTestResult[0]=%d\n", pNormalTestResult[0]);
        DBG("Open test ratio result: pNormalTestResult[1]=%d\n", pNormalTestResult[1]);
    	nRetVal = -1;
    }    
    ...
    kfree(ratio_border);
    kfree(ratio_move);
    kfree(ratio_border_move);
    kfree(normalTestFail_check_Deltac);
    kfree(normalTestFail_check_Ratio);
    kfree(GOLDENSAMPLE_CH_0);    
    kfree(GOLDENSAMPLE_CH_0_Max);
    kfree(GOLDENSAMPLE_CH_0_Min);    
...
static u16 _DrvMpTestMsg28xxItoTestChooseTpType(void)
{
    ...
    //GOLDEN_CH_0[MAX_MUTUAL_NUM]  = {0};
    //g_MapVaMutual = NULL;

    memset(GOLDEN_CH_0, 0, sizeof(GOLDEN_CH_0));
    ...
    if (TP_TYPE_X == _gTpType) // Modify. 
    {
	 ...
	 for (i= 0; i < (sizeof(GOLDEN_CH_X) / sizeof(GOLDEN_CH_X[0])); i++)
        {
        	GOLDEN_CH_0[i] = GOLDEN_CH_X[i];
         }
	 
        //GOLDEN_CH_0 = GOLDEN_CH_X;                                                            
    ...
    else if (TP_TYPE_Y == _gTpType) // Modify. 
    {
	 ...
	 for (i= 0; i < (sizeof(GOLDEN_CH_Y) / sizeof(GOLDEN_CH_Y[0])); i++)
        {
        	GOLDEN_CH_0[i] = GOLDEN_CH_Y[i];
         }
	 
       // GOLDEN_CH_0 = GOLDEN_CH_Y;                                                               
