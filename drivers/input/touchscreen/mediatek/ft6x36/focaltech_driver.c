...
#ifdef TPD_PROXIMITY
#define APS_ERR(fmt,arg...)           	pr_debug("<<proximity>> "fmt"\n",##arg)
...
static int tpd_touchinfo(struct touch_info *cinfo, struct touch_info *pinfo)
{
	...
	//pr_debug(" tpd cinfo->x[0] = %d, cinfo->y[0] = %d, cinfo->p[0] = %d\n", cinfo->x[0], cinfo->y[0], cinfo->p[0]);
	return true;
};
...
static int fts_report_value(struct ts_event *data)
{
	...
	data->touch_point_num=buf[FTS_TOUCH_POINT_NUM] & 0x0F;
	data->touch_point = 0;

	FTS_STEP = FTS_TOUCH_STEP;

	//TPD_DMESG("Tommy 1-2 ... \n");
	//pr_debug("tpd  fts_updateinfo_curr.TPD_MAX_POINTS=%d fts_updateinfo_curr.chihID=%d \n", fts_updateinfo_curr.TPD_MAX_POINTS,fts_updateinfo_curr.CHIP_ID);
	for(i = 0; i < MT_MAX_TOUCH_POINTS; i++)
	{
	...
	for(i = 0; i < data->touch_point; i++)
	{
		...
		if(data->au8_touch_event[i]== 0 || data->au8_touch_event[i] == 2)
		{
			...
			//TPD_DMESG("Tommy %s tpd D x[%d] =%d,y[%d]= %d",__func__,i,data->au16_x[i],i,data->au16_y[i]);
		}
		else
		{
			up_point++;
	...
	input_sync(tpd->dev);
	return 0;
	//pr_debug("tpd D x =%d,y= %d",event->au16_x[0],event->au16_y[0]);
}
...
static int tpd_enable_ps(int enable)
{
	...
	pr_debug("[proxi_fts]read: 999 0xb0's value is 0x%02X\n", state);
	...
	if(ret<0)
	{
		pr_debug("[Focal][Touch] write value fail");
		//return ret;
	}
...
static int fts_read_Gestruedata(void)
{
	...
	//pr_debug( "tpd read FTS_GESTRUE_POINTS_HEADER.\n");
	if(ret < 0)
	{
		pr_debug("%s read touchdata failed.\n", __func__);
		return ret;
	}
	...
	if(0x24 == buf[0])
	{
		gestrue_id = 0x24;
		check_gesture(gestrue_id);
		pr_debug("tpd %d check_gesture gestrue_id.\n", gestrue_id);
		return -1;
	}
	...
	check_gesture(gestrue_id);
	//pr_debug( "tpd %d read gestrue_id.\n", gestrue_id);
...
static int touch_event_handler(void *unused)
{
	...
	do
	{
		...
		//pr_debug("tpd touch_event_handler\n");
#ifdef FTS_GESTRUE
		...
		//pr_debug("tpd fts_read_Gestruedata state=%d\n",state);
		if(state ==1)
		{
...
*/
	...
	pr_debug("mtk_tpd[FTS] report rate is %dHz.\n",uc_reg_value * 10);
	uc_reg_addr = FTS_REG_FW_VER;
	fts_i2c_Write(i2c_client, &uc_reg_addr, 1);
	fts_i2c_Read(i2c_client, &uc_reg_addr, 0, &uc_reg_value, 1);
	pr_debug("mtk_tpd[FTS] Firmware version = 0x%x\n", uc_reg_value);
	uc_reg_addr = FTS_REG_CHIP_ID;
	fts_i2c_Write(i2c_client, &uc_reg_addr, 1);
	retval=fts_i2c_Read(i2c_client, &uc_reg_addr, 0, &uc_reg_value, 1);
	pr_debug("mtk_tpd[FTS] chip id is %d.\n",uc_reg_value);

	if(retval<0)
	{
		pr_debug("mtk_tpd[FTS] Read I2C error! driver NOt load!! CTP chip id is %d.\n",uc_reg_value);
		return 0;
	}
	...
	*/
	...
	pr_debug("********************Enter CTP Auto Upgrade********************\n");
...
#endif
	pr_debug("fts Touch Panel Device Probe %s\n", (retval < TPD_OK) ? "FAIL" : "PASS");
	return 0;
}
/************************************************************************
* Name: tpd_remove
...
static void gtp_esd_check_func(struct work_struct *work)
{
	...
	if(1 == flag_error)
	{
		pr_debug("focal--tpd reset.1 == flag_error...data=%d	count_irq\n ", data, count_irq);
		force_reset_guitar();
		reset_flag = 1;
	...
	else
	{
		if(g_old_91_Reg_Value == data)
		{
			...
			pr_debug("focal 91 value ==============, g_91value_same_count=%d\n", g_91value_same_count);

			if(RESET_91_REGVALUE_SAMECOUNT == g_91value_same_count)
			{
				force_reset_guitar();
				pr_debug("focal--tpd reset. g_91value_same_count = 5\n");
...
{
	//pr_debug(KERN_ERR "MediaTek fts touch panel driver init\n");
