			...
			switch ( value )
			{
				...
				case 1:  
					...
					else
					{
						...
						TPD_LOG("[SM][Synaptics_ic_ctrl] after masking MULTITAP_ENABLE_REG = %d ",r_mem);
						...
						{
							TPD_LOG("[SM][Synaptics_ic_ctrl]after masking write MULTITAP_ENABLE_REG1 = %d ",r_mem);
						}
					}
...
static int synaptics_touch_event_handler ( void *unused )
{
	...
	do
	{
		...
		*/
		...
		TPD_LOG("[SM]suspend_status = %d , knock_on_enable = %d",suspend_status,knock_on_enable);
		...
		if ( ( int_status & ABS0_MASK ) && ( int_enable & ABS0_MASK ) )
		{
			for ( finger_count = 0 ; finger_count < MAX_NUM_OF_FINGER ; finger_count++ )
			{
				...
				if ( /*( ( sensor_data.finger_state[reg_num] >> ( finger_order * 2 ) ) & FINGER_STATE_MASK ) == 1*/
					sensor_data.finger_data[finger_count][0] == F12_FINGER_STATUS
