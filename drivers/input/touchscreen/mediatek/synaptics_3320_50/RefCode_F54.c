...
int AbsRawRef[16] = {77, 11919, 14023, 15163, 16192, 18319, \
19337, 21491, 22633, 24692, 26853, 27993, 30147, 32253, 34411, 37605};
...
/* unsigned char data; */
	...
	for (i = 0; i < (int)TxChannelCount; i++) {
		...
		for (j = 0; j < (int)RxChannelCount; j++) {
			...
			TPD_LOG("Data[k] = %d, Data[k+1] =%d, Data[k]|Data[k+1]<<8 = %d ", \
			Data[k], Data[k+1], ((short)Data[k] | ((short)Data[k+1] << 8)));
			...
			case 0x54:
				if (!bHaveF54) {
				 ...
				 ? (int)RxChannelCount : (int)TxChannelCount;
					...
					if (bHavePerFrequencyNoiseControl) offset += \
					...
					if (bHaveSlewMetric || bHaveHBlank || bHaveVBlank \
					|| bHaveLongHBlank || bHaveNoiseMitigation2 \
     || bHaveSlewOption) offset += 1; /* Ctrl 63 */
					...
					bHaveLongHBlank) offset += 8; /* Ctrl 68/69/70/71/72/73 */
					if (bHaveSlewMetric) offset += 2; /* Ctrl 74 */
					bHaveEnhancedStretch = ((Data[9] & 0x80) == 0x80);
					if (bHaveEnhancedStretch) offset += \
					(int)NumberOfSensingFrequencies; /* Ctrl 75 */
