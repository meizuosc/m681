...
void tpd_get_dts_info(void)
{
	...
	if(node1) {
		...
		if(tpd_dts_data.use_tpd_button){
			...
			for(i = 0;i<4; i++){
				pr_info("[tpd]tpd_dts_data.tpd_key_dim_local[%d].key_x = %d\n", i,tpd_dts_data.tpd_key_dim_local[i].key_x);
				pr_info("[tpd]tpd_dts_data.tpd_key_dim_local[%d].key_y = %d\n", i,tpd_dts_data.tpd_key_dim_local[i].key_y);
				pr_info("[tpd]tpd_dts_data.tpd_key_dim_local[%d].key_W = %d\n", i,tpd_dts_data.tpd_key_dim_local[i].key_width);
				pr_info("[tpd]tpd_dts_data.tpd_key_dim_local[%d].key_H = %d\n", i,tpd_dts_data.tpd_key_dim_local[i].key_height);
			}
		}
...
static int tpd_probe(struct platform_device *pdev)
{
	...
	pr_info("mtk_tpd: TPD_RES_X = %d, TPD_RES_Y = %d\n", TPD_RES_X, TPD_RES_Y);
