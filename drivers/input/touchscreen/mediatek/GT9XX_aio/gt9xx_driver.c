...
static bool tpd_scp_doze_en = FALSE; //change for test by xuwen1
#endif
#endif
...
reset_proc:
...
#endif
    {
        ...
        if (ret < 0)
        {
            GTP_ERROR("after_gtp_i2c_test I2C communication ERROR!");
...
#if GTP_SCP_GESTURE_WAKEUP 
    printk("[TSP-xw]enter in SCP mode\n");
	tpd_enter_doze();
#elif GTP_SLIDE_WAKEUP
...
#if GTP_SCP_GESTURE_WAKEUP
     printk("[TSP-xw]tpd_resume  SCP_GESTURE\n");
	if (tpd_scp_doze_en)
	{
	    ret = get_md32_semaphore(SEMAPHORE_TOUCH);
	    if (ret < 0)
