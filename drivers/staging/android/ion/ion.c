			     ...
			     unsigned int flags)
{
	...
	if (end-start > 100000000ULL) { /* unit is ns */
		trace_printk("warn: ion alloc buffer size: %zu time: %lld ns\n", buffer->size, end-start);
...
static int ion_debug_client_show(struct seq_file *s, void *unused)
{
	...
	sizes = kzalloc(ION_NUM_HEAP_IDS * sizeof(size_t), GFP_ATOMIC);

	if (!sizes) {
		pr_err("kmalloc for sizes fail\n");
		return -ENOMEM;
	}

	names = kzalloc(ION_NUM_HEAP_IDS * sizeof(char *), GFP_ATOMIC);

	if (!names) {
		pr_err("kmalloc for names fail\n");
		return -ENOMEM;
	}
