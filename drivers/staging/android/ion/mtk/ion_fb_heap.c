				      ...
				      unsigned long size, unsigned long align)
{
	...
	if (!offset) {
		IONMSG("[ion_fb_alloc]:fail! size = 0x%lx, align = 0x%lx, fb_total_size=0x%lx\n", size, align, (unsigned long)gen_pool_size(fb_heap->pool));
				      ...
				      unsigned long flags)
{
	...
	if(paddr==ION_CARVEOUT_ALLOCATE_FAIL){
		IONMSG("[ion_fb_heap_alloc]:dump fb heap info==========>\n");
		ion_fb_heap_debug_show(heap, NULL, NULL);
		return -ENOMEM;
	}
    ...
    {
        IONMSG("[ion_fb_heap_allocate]: Error. Allocate ion_buffer_info failed.\n");
        ret = -EFAULT;
		goto free_buffer;
    }
				 ...
				 struct ion_buffer *buffer)
{
	...
	if(pBufferInfo)
	{
		...
		{
			m4u_dealloc_mva_sg(pBufferInfo->eModuleID, buffer->sg_table, buffer->size, pBufferInfo->MVA);
		}
		mutex_unlock(&(pBufferInfo->lock));
	}
							...
							void *data)
{
	...
	ION_PRINT_LOG_OR_SEQ(s, "phys_addr=0x%x bits=", (unsigned int)chunk->phys_addr);
			
	for(i=0; i<nlongs; i++)
	{
		ION_PRINT_LOG_OR_SEQ(s, "0x%x ", (unsigned int)chunk->bits[i]);
	}
...
static int ion_fb_heap_debug_show(struct ion_heap *heap, struct seq_file *s, void *unused)
{
	...
	ION_PRINT_LOG_OR_SEQ(s, "************************************************************\n");
	ION_PRINT_LOG_OR_SEQ(s, "total_size=0x%x, free_size=0x%x, fb_base_addr=0x%lx\n",
			(unsigned int)total_size, (unsigned int)size_avail, fb_heap->base);
	ION_PRINT_LOG_OR_SEQ(s, "************************************************************\n");
