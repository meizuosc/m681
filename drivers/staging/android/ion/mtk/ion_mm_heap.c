...
static unsigned int order_enable[] = { 1, 1 };
...
static size_t mm_heap_total_memory = 0;
...
static struct timer_list ion_enable_order_timer;

static void ion_order_enable(unsigned int order)
{
	IONMSG("enable ion order = %d.\n", order);
	order_enable[order] = 1;
}

static int ion_enable_order_timer_init(void)
{
	init_timer(&ion_enable_order_timer);
	ion_enable_order_timer.function = ion_order_enable;
	return 0;
}

static int ion_order_disable(unsigned int order, int delay)
{
	order_enable[order] = 0;
	ion_enable_order_timer.data = order;
	/* ion_enable_order_timer.expires = jiffies + delay * HZ; */
	mod_timer(&ion_enable_order_timer, jiffies + delay * HZ);
	IONMSG("warning: disable order %d for %ds\n", order, delay);
	return 0;
}
				...
				unsigned long size, unsigned long align, unsigned long flags)
{
	...
	if (end - start > 100000000ULL)	{ /* unit is ns */
		trace_printk("warn: ion mm heap allocate buffer size: %zu time: %lld ns\n",
			     buffer->size, end - start);
		IONMSG("warn: ion mm heap allocate buffer size: %zu time: %lld ns\n", buffer->size,
		       end - start);
	}
...
static int ion_mm_heap_debug_show(struct ion_heap *heap, struct seq_file *s, void *unused)
{
	...
	for (i = 0; i < num_orders; i++) {
		...
		ION_PRINT_LOG_OR_SEQ(s, "%d order %u highmem pages in cached_pool = %lu total\n",
				     pool->high_count, pool->order,
				     (1 << pool->order) * PAGE_SIZE * pool->high_count);
		ION_PRINT_LOG_OR_SEQ(s, "%d order %u lowmem pages in cached_pool = %lu total\n",
				     pool->low_count, pool->order,
				     (1 << pool->order) * PAGE_SIZE * pool->low_count);
	}
...

	{
		...
		ION_PRINT_LOG_OR_SEQ(s,
				...
				"flag", "pid", "comm(client)", "v1", "v2", "v3", "v4",
				"dbg_name");
	...
	for (n = rb_first(&dev->clients); n; n = rb_next(n)) {
		...
		if (client->task) {
			...
			ION_PRINT_LOG_OR_SEQ(s, "client(0x%p) %s (%s) pid(%u) ================>\n",
...
struct ion_heap *ion_mm_heap_create(struct ion_platform_heap *unused)
{
	...
	heap->cached_pools = kzalloc(sizeof(struct ion_page_pool *) * num_orders, GFP_KERNEL);
	...
	case ION_MM_CONFIG_BUFFER:
		if (Param.config_buffer_param.handle) {
			...
			if ((int)buffer->heap->type == ION_HEAP_TYPE_MULTIMEDIA) {
				...
				IONMSG
				    ("[ion_heap]: Error. Cannot configure buffer that is not from %c heap.\n",
				     buffer->heap->type);
				ret = 0;
			}
	...
	case ION_MM_SET_DEBUG_INFO:
		{
			...
			if (Param.buf_debug_info_param.handle) {
				...
				if ((int)buffer->heap->type == ION_HEAP_TYPE_MULTIMEDIA) {
					...
					IONMSG
					    ("[ion_heap]: Error. Cannot set dbg buffer that is not from %c heap.\n",
					     buffer->heap->type);
					ret = -EFAULT;
				}
				ion_drv_put_kernel_handle(kernel_handle);
	...
	case ION_MM_GET_DEBUG_INFO:
		{
			...
			if (Param.buf_debug_info_param.handle) {
				...
				if ((int)buffer->heap->type == ION_HEAP_TYPE_MULTIMEDIA) {
					...
					IONMSG
					    ("[ion_heap]: Error. Cannot get dbg buffer that is not from %c heap.\n",
					     buffer->heap->type);
					ret = -EFAULT;
				}
				ion_drv_put_kernel_handle(kernel_handle);
	...
	case ION_MM_SET_SF_BUF_INFO:
		{
			...
			if (Param.sf_buf_info_param.handle) {
				struct ion_handle *kernel_handle;
				kernel_handle =
				    ion_drv_get_handle(client, Param.sf_buf_info_param.handle,
				...
				if ((int)buffer->heap->type == ION_HEAP_TYPE_MULTIMEDIA) {
					...
					mutex_lock(&(pBufferInfo->lock));
					ion_mm_copy_sf_buf_info(&(Param.sf_buf_info_param),
								&(pBufferInfo->sf_buf_info));
					mutex_unlock(&(pBufferInfo->lock));
				} else if ((int)buffer->heap->type == ION_HEAP_TYPE_FB) {
					ion_fb_buffer_info *pBufferInfo = buffer->priv_virt;
					...
					IONMSG
					    ("[ion_heap]: Error. Cannot set sf_buf_info buffer that is not from %c heap.\n",
					     buffer->heap->type);
					ret = -EFAULT;
				}
				ion_drv_put_kernel_handle(kernel_handle);
				...
				IONMSG
				    ("[ion_heap]: Error. set sf_buf_info buffer with invalid handle.\n");
				ret = -EFAULT;
			}
		}
		break;
	...
	case ION_MM_GET_SF_BUF_INFO:
		{
			...
			if (Param.sf_buf_info_param.handle) {
				...
				if ((int)buffer->heap->type == ION_HEAP_TYPE_MULTIMEDIA) {
					...
					mutex_lock(&(pBufferInfo->lock));
					ion_mm_copy_sf_buf_info(&(pBufferInfo->sf_buf_info),
					...
					IONMSG
					    ("[ion_heap]: Error. Cannot get sf_buf_info buffer that is not from %c heap.\n",
					     buffer->heap->type);
					ret = -EFAULT;
				}
				ion_drv_put_kernel_handle(kernel_handle);
				...
				IONMSG
				    ("[ion_heap]: Error. get sf_buf_info buffer with invalid handle.\n");
				ret = -EFAULT;
			}
		}
		break;
