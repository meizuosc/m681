...
#include <linux/wakelock.h>
#include <mach/eint.h>
//#include <mach/mt_gpio_gyro.h>
static unsigned int gyro_irq_num;
#define GYRO_EINT_NUM     17
#define GPIO_GYRO_EINT_PIN     (GPIO17|0x80000000)
extern int fact_calib[3];
							...
							u64 t, s16 *d)
{
	...
	if ((hdr & (~STEP_INDICATOR_MASK)) == ACCEL_HDR) {
		if (st->accel_wakeup_enabled) {
			st->wake_sensor_received = true;
			pr_debug("ACCEL_HDR wake_sensor_received=%d\n",
					st->wake_sensor_received);
		}
	}

	if ((hdr & (~STEP_INDICATOR_MASK)) == GYRO_HDR) {
		if (st->gyro_wakeup_enabled) {
			st->wake_sensor_received = true;
			pr_debug("GYRO_HDR wake_sensor_received=%d\n",
					st->wake_sensor_received);
		}
	}
									...
									int *q)
{
	...
	if ((hdr & (~STEP_INDICATOR_MASK)) == GYRO_HDR) {
		if (st->gyro_wakeup_enabled) {
			st->wake_sensor_received = true;
			pr_debug("IWA GYRO_HDR wake_sensor_received=%d\n",
					st->wake_sensor_received);
		}
	}

	if ((hdr & (~STEP_INDICATOR_MASK)) == SIXQUAT_HDR) {
		if (st->quat_wakeup_enabled) {
			st->wake_sensor_received = true;
			pr_debug("SIXQUAT_HDR wake_sensor_received=%d\n",
					st->wake_sensor_received);
		}
	}
...
#if defined(CONFIG_INV_AUX_COMPASS) || defined(CONFIG_INV_AUX_PRESSURE)
...
{
	u8 rn[] = {0xa3, 0xa3, 0xa3};
	u8 rf[] = {0xf4, 0x08, 0xa4}; //PREV_PQUAT, 2212, 0x08a4	
	int result;
	u8 *r;
...
{
	u8 rn[] = {0xa3, 0xa3, 0xa3};
	u8 rf[] = {0xf4, 0x08, 0x81}; //GYRO_FIFO_RATE, 2177, 0x881
	int result;
	u8 *r;
...
{
	u8 rn[] = {0xa3, 0xa3, 0xa3};
	u8 rf[] = {0xf4, 0x08, 0x17}; //QUAT6_FIFO_RATE, 2071, 0x0817
	int result;
	u8 *r;
...
{
	u8 rn[] = {0xa3, 0xa3, 0xa3};
	u8 rf[] = {0xf4, 0x08, 0x3b}; //PQUAT_FIFO_RATE, 2107, 0x083B
	int result;
	u8 *r;
...
{
	u8 rn[] = {0x03, 0xa3, 0xa3};
	u8 rf[] = {0xf4, 0x08, 0x5e}; //ACCEL_FIFO_RATE, 2142, 0x085E
	u8 *r;
	int result;
...
{
	u8 rn[] = {0xf3, 0xf3, 0xf3};
	u8 rf[] = {0xf4, 0x0c, 0x4d}; //PED_STEP_COUNT2_DETECTED, 3149, 0x0C4D
	int result;
	u8 *r;
...
#if defined(CONFIG_INV_AUX_PRESSURE) || defined(CONFIG_INV_AUX_COMPASS)
static int inv_send_compass_dmp_data(struct inv_mpu_state *st, bool on)
{
	u8 rn[] = {0xa3, 0xa3, 0xa3};
	u8 rf[] = {0xf4, 0x08, 0xcb};
	u8 *r;
	int result;
...
{
	u8 rn[] = {0xa3, 0xa3, 0xa3};
	u8 rf[] = {0xf4, 0x07, 0xf3}; //STEPDET_END, 2035, 0x07F3
	u8 *r;
	int result;
		...
		if (bpm) {
			while (fifo_count >= bpm) {
				...
				inv_report_gyro_accel(indio_dev, data,
								st->last_ts);
				fifo_count -= bpm;
#if defined(CONFIG_INV_AUX_COMPASS)
				inv_send_compass_data(st);
#endif
#if defined(CONFIG_INV_AUX_PRESSURE)
...
int inv_mpu_configure_ring(struct iio_dev *indio_dev)
{
	...
	mt_eint_set_hw_debounce(GYRO_EINT_NUM, 0);
       gyro_irq_num = mt_gpio_to_irq(GYRO_EINT_NUM);
	st->client->irq = gyro_irq_num;
	if (INV_MPU3050 == st->chip_type)
