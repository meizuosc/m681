...
int modem_dtr_set(int on, int low_latency){return 0;}
int modem_dcd_state(void){return 0;}
int modem_buffer_push(int port_num, void *buf, int count){return 0;}
int sdio_rawbulk_intercept(int port_num, unsigned int inception){return 0;}
#endif
...
static ssize_t rawbulk_attr_show(struct device *dev, struct device_attribute *attr,
    ...
    switch (idx) {
        ...
        case ATTR_NUPS:
			return sprintf(buf, "nups<%d>,udata<%d>,ucnt<%d>,drop<%d>,alloc_fail<%d>,tran<%d>\n", 
					enab? fn->nups: -1,
