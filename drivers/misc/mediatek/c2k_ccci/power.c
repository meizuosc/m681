...
#define CMDM_IOCTL_GET_C2K_POST_FIX		_IOR('c', 0x12, int)
#define CMDM_IOCTL_GET_C2K_EX_TYPE		_IOR('c', 0x13, int)
...
static int modem_userspace_notifier(int msg, void *data)
{
	...
	if (!hd) {
		pr_debug(KERN_ERR "%s:error sync user\n", __func__);
		return -ENODEV;
	}
			     ...
			     cmd, unsigned long arg)
{
	...
	case CMDM_IOCTL_GET_MD_STATUS:
		if (cmdata->modem)
			...
			return -EFAULT;
		break;
	case CMDM_IOCTL_GET_C2K_POST_FIX:
		snprintf(post_fix, IMG_POSTFIX_LEN, "%s", "3_3g_n");
		if (copy_to_user((void __user *)arg, post_fix, IMG_POSTFIX_LEN)) {
			pr_debug("[C2K SDIO]get post fix fail.\n");
			return -EFAULT;
		}	else
			pr_debug("[C2K SDIO]get post fix done(%s).\n", post_fix);
		break;
	case CMDM_IOCTL_GET_C2K_EX_TYPE:
		if (cmdata->modem)
			ret = put_user((unsigned int)cmdata->modem->ex_info.header.ex_type, (unsigned int __user *)arg);
		else
			return -EFAULT;
		pr_debug("get modem exception type=%d ret=%d\n", cmdata->modem->ex_info.header.ex_type, ret);
		break;
