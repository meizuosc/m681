...
static void modem_heart_beat_poll_work(struct work_struct *work)
{
	...
	if (!modem || (modem->status != MD_READY)) {
		LOGPRT(LOG_ERR, "%s: md not availiable now\n", __func__);
		goto down_out;
	}
...
#if ENABLE_CCMNI
/*When send data to ccmni port, we should analyze msg header first, so we seperate ccmni port work from others*/
	...
	while (fifo_total_count > 0) {
		...
		if (ccmni_port->tx_state == CCMNI_TX_STOP) {
			if (kfifo_len(&ccmni_port->transmit_fifo) <= FIFO_SIZE/2) {
				...
				LOGPRT(LOG_INFO,
				       "ccmni(ch:%d) is stoped, fifo_total_count(%d).\n",
...
*/
	...
	if (pure_int & SDIO_CHISR_RX_RDY) {
		...
		if (!keep_recv) {
			...
			LOGPRT(LOG_DEBUG, "%s %d as_packet availiable now\n",
			       __func__, __LINE__);
...
static void modem_port_remove(struct sdio_modem *modem)
{
	...
	for (index = 0; index < SDIO_TTY_NR; index++) {
		...
		if (tty) {
			LOGPRT(LOG_NOTICE,
			       "%s %d destory tty,index=%d port->index=%d\n",
...
int sdio_modem_ccmni_send_pkt(int md_id, int tx_ch, void *data)
{
	...
	if (data_len > (FIFO_SIZE - chars_in_fifo)) {
		...
		/* stop tx queue */
		LOGPRT(LOG_INFO, "ccmni port %d is stoped.\n",
			       ccmni_port->index);
