...
// ANDROID_LOG_DEBUG
// ANDROID_LOG_VERBOSE
#define TAG_NAME "leds_LT_strobe_allenyao_part1"
...
static int constant_flashlight_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	PK_DBG("SGM3784_LED2_constant_flashlight_ioctl() line=%d ior_shift=%d, iow_shift=%d iowr_shift=%d arg=%d\n",__LINE__, ior_shift, iow_shift, iowr_shift, (int)arg);
    switch(cmd)
    {
    	...
    	case FLASH_IOC_SET_ONOFF :
			...
			LED2Closeflag = 0;
			FlashIc_Enable();
			//FL_dim_duty(m_duty1);	//add by lijin 2015.5.13
			FL_dim_duty(m_duty2);
    			FL_Enable();
    		}
    		else
