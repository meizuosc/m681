...
extern int getFlashPartId(void);
...
static long flashlight_ioctl_core(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case FLASH_IOC_IS_LOW_POWER:
		...
		{
			int isLow = 0;
			/*remove lowpower dete,for the same with m81&ma01*/
...
/* ======================================================================== */
//change   by allenyao  for torch control
extern int hight_cct_led_storbe_enable_part1(void);
extern int hight_cct_led_storbe_setduty_part1(int duty);
extern int hight_cct_led_storbe_on_part1(int onoff);
extern int low_cct_led_storbe_enable_part1(void);
extern int low_cct_led_storbe_setduty_part1(int duty);
extern int low_cct_led_storbe_on_part1(int onoff);
//-------------------------------------------------
extern int hight_cct_led_storbe_enable_part2(void);
extern int hight_cct_led_storbe_setduty_part2(int duty);
extern int hight_cct_led_storbe_on_part2(int onoff);
extern int low_cct_led_storbe_enable_part2(void);
extern int low_cct_led_storbe_setduty_part2(int duty);
extern int low_cct_led_storbe_on_part2(int onoff);
//-------------------------------------------------
static int open_flag1=0;
int flashduty1;
int flashduty[]={1,3,4,5,7,8};
int flashduty_part2[]={16,35,53,70,89,106};

int hight_cct_led_storbe_enable(void){
	if(getFlashPartId() == 2)
		hight_cct_led_storbe_enable_part2();
	else
		hight_cct_led_storbe_enable_part1();
}

int hight_cct_led_storbe_setduty(int duty){
	if(getFlashPartId() == 2)
		hight_cct_led_storbe_setduty_part2(duty);
	else
		hight_cct_led_storbe_setduty_part1(duty);
}

int hight_cct_led_storbe_on(int onoff){
	if(getFlashPartId() == 2)
		hight_cct_led_storbe_on_part2(onoff);
	else
		hight_cct_led_storbe_on_part1(onoff);
}

int low_cct_led_storbe_enable(void){
	if(getFlashPartId() == 2)
		low_cct_led_storbe_enable_part2();
	else
		low_cct_led_storbe_enable_part1();
}

int low_cct_led_storbe_setduty(int duty){
	if(getFlashPartId() == 2)
		low_cct_led_storbe_setduty_part2(duty);
	else
		low_cct_led_storbe_setduty_part1(duty);
}

int low_cct_led_storbe_on(int onoff){
	if(getFlashPartId() == 2)
		low_cct_led_storbe_on_part2(onoff);
	else
		low_cct_led_storbe_on_part1(onoff);
}
...
static ssize_t store_flashduty1(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	hight_cct_led_storbe_enable();
work1:
	if(flashduty1>=0){
		if(getFlashPartId() == 2)
		  hight_cct_led_storbe_setduty(flashduty_part2[flashduty1]);
		else
		  hight_cct_led_storbe_setduty(flashduty[flashduty1]);
		if(pF->flashlight_ioctl(FLASH_IOC_SET_TIME_OUT_TIME_MS, 0)){
			logE("set timeout fail!\n");
			goto out;
		}
		hight_cct_led_storbe_on(1);
	}
...
release_ledssource1:
	if(flashduty1<0){//off
		open_flag1=0;
		hight_cct_led_storbe_setduty(0);
		hight_cct_led_storbe_on(0);
...
static DEVICE_ATTR(flash1, S_IWUGO | S_IRUGO, show_flashduty1, store_flashduty1);
//add end

//add begin by yaoguizhen for torch control
static int flashduty2=0;
static int open_flag2=0;
...
static ssize_t store_flashduty2(struct device *dev, struct device_attribute *attr, const char *buf,
			  size_t size)
{
	FLASHLIGHT_FUNCTION_STRUCT* pF1=0;
	unsigned char arg;
	char *pvalue=NULL;
	...
	if(getFlashPartId() == 2)
		strobeInit_main_sid2_part2(&pF1);
	else
	  strobeInit_main_sid2_part1(&pF1);

	//open leds resource
	if(pF1->flashlight_open(NULL)){
			logE("open fail!\n");
			if(flashduty2<0)
				goto release_ledssource2;
			else if(open_flag2>0)
				goto work2;
			else
				return -EBUSY;
	}else{
	...
	low_cct_led_storbe_enable();
	
work2:
	if(flashduty2>=0){
		if(getFlashPartId() == 2)
		  low_cct_led_storbe_setduty(flashduty_part2[flashduty2]);
		else
		  low_cct_led_storbe_setduty(flashduty[flashduty2]);
		if(pF1->flashlight_ioctl(FLASH_IOC_SET_TIME_OUT_TIME_MS, 0)){
			logE("set timeout fail!\n");
			goto out;
		}
		low_cct_led_storbe_on(1);
	}

release_ledssource2:	
	if(flashduty2<0){//off
		open_flag2=0;
		low_cct_led_storbe_setduty(0);
		low_cct_led_storbe_on(0);
		pF1->flashlight_release(NULL);
	}
...
static DEVICE_ATTR(flash2, 0666, show_flashduty2, store_flashduty2);
//add end
...
static ssize_t store_flashduty_test(struct device *dev, struct device_attribute *attr, const char *buf,
			  size_t size)
{
	FLASHLIGHT_FUNCTION_STRUCT* pF1=0;
	FLASHLIGHT_FUNCTION_STRUCT* pF2=0;
	...
	if (buf != NULL && size != 0) {
		...
		PK_DBG_FUNC("flashduty_test1: %d\n", flashduty_test1);

		if (*pvalue ) {
			flashduty_test2 = simple_strtol((pvalue + 1), NULL, 0);
			PK_DBG_FUNC("flashduty_test2: %d\n", flashduty_test2);
		}
	}
	...
	if(getFlashPartId() == 2) {
		strobeInit_main_sid1_part2(&pF1);
		strobeInit_main_sid2_part2(&pF2);
	} else {
	  constantFlashlightInit(&pF1);
	  strobeInit_main_sid2_part1(&pF2);
	}


	//open leds resource
	if(pF1->flashlight_open(NULL)||pF2->flashlight_open(NULL)){
			logE("open fail!\n");
			if((flashduty_test1<0)&&(flashduty_test2<0))
				goto out;
	...
	if(flashduty_test1>=26)
		flashduty_test1=26;
	if(flashduty_test2>=26)
		flashduty_test2=26;
work3:
	if((flashduty_test1<0)&&(flashduty_test2<0)){
		PK_DBG_FUNC("flashduty_test1: %d,flashduty_test2:%d,%d\n", flashduty_test1,flashduty_test2,__LINE__);
		//pF1->flashlight_release(NULL);
		//pF2->flashlight_release(NULL);
		//open_flag4test=0;
		//return size;
	}else if(((flashduty_test1>=0)&&(flashduty_test1<=1))&&(flashduty_test2<0)){
		PK_DBG_FUNC("flashduty_test1: %d,flashduty_test2:%d,%d\n", flashduty_test1,flashduty_test2,__LINE__);
		pF1->flashlight_ioctl(FLASH_IOC_SET_DUTY, flashduty_test1);
		pF1->flashlight_ioctl(FLASH_IOC_SET_TIME_OUT_TIME_MS, 20000);
		pF1->flashlight_ioctl(FLASH_IOC_SET_ONOFF, 1);
		pF2->flashlight_ioctl(FLASH_IOC_SET_DUTY, flashduty_test2);
		pF2->flashlight_ioctl(FLASH_IOC_SET_TIME_OUT_TIME_MS, 20000);
		pF2->flashlight_ioctl(FLASH_IOC_SET_ONOFF, 0);
		...
		pF1->flashlight_ioctl(FLASH_IOC_SET_DUTY, flashduty_test1);
		pF1->flashlight_ioctl(FLASH_IOC_SET_TIME_OUT_TIME_MS, 20000);
		pF1->flashlight_ioctl(FLASH_IOC_SET_ONOFF, 0);
		pF2->flashlight_ioctl(FLASH_IOC_SET_DUTY, flashduty_test2);
		pF2->flashlight_ioctl(FLASH_IOC_SET_TIME_OUT_TIME_MS, 20000);
		pF2->flashlight_ioctl(FLASH_IOC_SET_ONOFF, 1);
	...
	open_flag4test=0;
	pF1->flashlight_ioctl(FLASH_IOC_SET_DUTY, -1);
	pF2->flashlight_ioctl(FLASH_IOC_SET_DUTY, -1);
	pF1->flashlight_ioctl(FLASH_IOC_SET_ONOFF, 0);
	pF2->flashlight_ioctl(FLASH_IOC_SET_ONOFF, 0);
	pF1->flashlight_release(NULL);
	pF2->flashlight_release(NULL);
...
//**************************************************************************
//start by jerryxie
//node: /sys/class/flashlightdrv/kd_camera_flashlight/flashledid
static ssize_t show_flashledid(struct device *dev, struct device_attribute *attr, char *buf)
{
  if(getFlashPartId() == 2) {
    PK_DBG_FUNC("get flashledid -- value is:%d\n", 2);
    return sprintf(buf, "%d\n", 2);
  }
	
  PK_DBG_FUNC("get flashledid default!!!\n");
  return -1;
}

static ssize_t store_flashledid(struct device *dev, struct device_attribute *attr, const char *buf,
			  size_t size)
{
  PK_DBG_FUNC("store_flashledid NULL!!!\n");
  return -1;
}

static DEVICE_ATTR(flashledid, 0666, show_flashledid, store_flashledid);
//end by jerryxie
//**************************************************************************
...
#define FLASHLIGHT_MAJOR 242
  ...
  //add begin by jerryxie for costdown flashlight control
  ret = device_create_file(flashlight_device,&dev_attr_flashledid);
  if (ret) {
    logE("[flashlight_probe]device_create_file flashledid fail!\n");
  }

	//add begin by yaoguizhen for torch control
	ret = device_create_file(flashlight_device,&dev_attr_flash1);
	if (ret) {
		logE("[flashlight_probe]device_create_file flash2 fail!\n");
	}

	ret = device_create_file(flashlight_device, &dev_attr_flash2);
	if (ret) {
		logE("[flashlight_probe]device_create_file flash2 fail!\n");
	}

	ret = device_create_file(flashlight_device, &dev_attr_flash3);
	if (ret) {
		logE("[flashlight_probe]device_create_file flash3 fail!\n");
	}
	//add end
