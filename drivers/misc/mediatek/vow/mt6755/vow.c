...
#if DEBUG_VOWDRV
#define PRINTK_VOWDRV(format, args...) pr_debug(format, ##args)
#else
...
static uint32 VowDrv_Wait_Queue_flag;
DECLARE_WAIT_QUEUE_HEAD(VowDrv_Wait_Queue);
static uint32 VoiceData_Wait_Queue_flag0;
...
bool vow_ipi_sendmsg(vow_ipi_msgid_t id, void *buf, unsigned int size, unsigned int type,
		     uint32 wait)
{
#if !AP_ONLY_UT
	ipi_status status;
...
static void vow_service_Init(void)
{
	...
	if (init_flag == 1) {
		...
		vowserv.voicedata_scp_addr =
		    dma_map_single(&dev, (void *)vowserv.voicddata_scp_ptr,
				   VOW_VOICE_DATA_LENGTH_BYTES, DMA_BIDIRECTIONAL);
		...
		while (!vow_ipi_sendmsg(AP_IPIMSG_VOW_APREGDATA_ADDR,
					(void *)&vowserv.vow_info_dsp[0], 4, 0, 0))
					;
...
static int vow_service_CopyModel(int model, int slot)
{
	...
	case VOW_MODEL_SPEAKER:
		if (copy_from_user
		    ((void *)(vowserv.vow_speaker_model[slot].model_ptr),
		     (const void __user *)(vowserv.vow_info_apuser[1]),
		     vowserv.vow_info_apuser[2])) {
...
static bool vow_service_ReleaseSpeakerModel(int id)
{
	...
	vowserv.ipimsgwait = true;
	while (!vow_ipi_sendmsg
	       (AP_IPIMSG_SET_VOW_MODEL, (void *)&vowserv.vow_info_dsp[0], 16, 0, 0))
		;
...
static bool vow_service_SetSpeakerModel(unsigned long arg)
{
	...
	PRINTK_VOWDRV("SetSPKModel:CheckValue:%x %x %x %x %x %x\n", *(char *)&ptr8[0],
		      *(char *)&ptr8[1], *(char *)&ptr8[2], *(char *)&ptr8[3], *(short *)&ptr8[160],
		      *(int *)&ptr8[7960]);

	vowserv.vow_info_dsp[0] = VOW_MODEL_SPEAKER;
	vowserv.vow_info_dsp[1] = vowserv.vow_info_apuser[0];
	vowserv.vow_info_dsp[2] =
	    dma_map_single(&dev, vowserv.vow_speaker_model[I].model_ptr, vowserv.vow_info_apuser[2],
			   DMA_TO_DEVICE);
	vowserv.vow_info_dsp[3] = vowserv.vow_info_apuser[2];
	dma_sync_single_for_device(&dev, vowserv.vow_info_dsp[2], vowserv.vow_info_apuser[2],
				   DMA_TO_DEVICE);

	PRINTK_VOWDRV("SetSpeakerModel:model_%x,addr_%x, id_%x, size_%x\n", vowserv.vow_info_dsp[0],
		      vowserv.vow_info_dsp[2], vowserv.vow_info_dsp[1], vowserv.vow_info_dsp[3]);
...
static bool vow_service_SetInitModel(unsigned long arg)
{
	...
	PRINTK_VOWDRV("SetInitModel:CheckValue:%x %x %x %x %x\n", *(int *)&ptr8[0],
		      *(int *)&ptr8[1600], *(int *)&ptr8[2240], *(short *)&ptr8[2880],
		      *(short *)&ptr8[15360]);

	vowserv.vow_info_dsp[0] = VOW_MODEL_INIT;
	vowserv.vow_info_dsp[1] = 0;
	vowserv.vow_info_dsp[2] =
	    dma_map_single(&dev, vowserv.vow_init_model, vowserv.vow_info_apuser[2], DMA_TO_DEVICE);
	vowserv.vow_info_dsp[3] = vowserv.vow_info_apuser[2];
	dma_sync_single_for_device(&dev, vowserv.vow_info_dsp[2], vowserv.vow_info_apuser[2],
				   DMA_TO_DEVICE);
...
static bool vow_service_SetNoiseModel(unsigned long arg)
{
	...
	vowserv.vow_info_dsp[1] = 0;
	vowserv.vow_info_dsp[2] =
	    dma_map_single(&dev, vowserv.vow_noise_model, vowserv.vow_info_apuser[2],
			   DMA_TO_DEVICE);
	vowserv.vow_info_dsp[3] = vowserv.vow_info_apuser[2];
	dma_sync_single_for_device(&dev, vowserv.vow_info_dsp[2], vowserv.vow_info_apuser[2],
				   DMA_TO_DEVICE);
...
static bool vow_service_SetFirModel(unsigned long arg)
{
	...
	vowserv.vow_info_dsp[1] = 0;
	vowserv.vow_info_dsp[2] =
	    dma_map_single(&dev, vowserv.vow_fir_model, vowserv.vow_info_apuser[2], DMA_TO_DEVICE);
	vowserv.vow_info_dsp[3] = vowserv.vow_info_apuser[2];
	dma_sync_single_for_device(&dev, vowserv.vow_info_dsp[2], vowserv.vow_info_apuser[2],
				   DMA_TO_DEVICE);
	...
	vowserv.ipimsgwait = true;
	while (!vow_ipi_sendmsg
	       (AP_IPIMSG_SET_VOW_MODEL, (void *)&vowserv.vow_info_dsp[0], 4, 0, 0))
		;
	ret = vow_ipimsg_wait(AP_IPIMSG_SET_VOW_MODEL);
	return ret;
}
	...
	while (1) {
		...
		if (VoiceData_Wait_Queue_flag == 1) {
			...
			if ((VowDrv_GetHWStatus() == VOW_PWR_OFF)
				...
				dma_sync_single_for_cpu(&dev, vowserv.voicedata_scp_addr,
							VOW_VOICE_DATA_LENGTH_BYTES,
							DMA_FROM_DEVICE);
#if 0
				if (firstbuffer == 1) {
					memcpy_from_md32(&vowserv.voicedata_kernel_ptr
					...
					memcpy_from_md32(&vowserv.voicedata_kernel_ptr
							 [vowserv.voicedata_idx],
							 MD32_DTCM + vowserv.voicedata_tcm_addr +
							 640, VOW_VOICE_DATA_LENGTH_BYTES);
					firstbuffer = 1;
				}
#else
				memcpy(&vowserv.voicedata_kernel_ptr[vowserv.voicedata_idx],
				       vowserv.voicddata_scp_ptr, VOW_VOICE_DATA_LENGTH_BYTES);
#endif
				...
				if (vowserv.voicedata_idx >= (vowserv.voicedata_user_size >> 1)) {
					...
					vowserv.voicedata_idx = 0;
					ret =
					    copy_to_user((void __user *)vowserv.voicedata_user_addr,
...
void VowDrv_SetFlag(VOW_FLAG_TYPE type, bool set)
{
	...
	vowserv.ipimsgwait = true;
	while (!vow_ipi_sendmsg
	       (AP_IPIMSG_VOW_SET_FLAG, (void *)&vowserv.vow_info_dsp[0], 8, 0, 0))
		;
...
static ssize_t VowDrv_write(struct file *fp, const char __user *data, size_t count,
			    loff_t *offset)
{
	return 0;
}
...
static ssize_t VowDrv_read(struct file *fp, char __user *data, size_t count, loff_t *offset)
{
	...
	read_count =
	    copy_to_user((void __user *)data, &VOW_EINT_DATA_STRUCT, sizeof(VOW_EINT_DATA_STRUCT));
...
static int VowDrv_resume(struct platform_device *dev)
{
...
static const struct miscdevice VowDrv_misc_device = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = VOW_DEVNAME,
	.fops = &VOW_fops,
};

struct const struct dev_pm_ops VowDrv_pm_ops = {
	.suspend = NULL,
	.resume = NULL,
	.freeze = NULL,
	.thaw = NULL,
