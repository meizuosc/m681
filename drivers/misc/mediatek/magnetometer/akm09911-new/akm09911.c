...
/* akm0991x.c - akm0991x compass driver
 * 
 *
...
/*----------------------------------------------------------------------------*/
#define DEBUG 1
#define AKM0991x_DEV_NAME         "akm0991x"
#define DRIVER_VERSION          "1.0.1"
/*----------------------------------------------------------------------------*/
#define AKM0991x_DEBUG		1
#define AKM0991x_RETRY_COUNT	10
#define AKM0991x_DEFAULT_DELAY	100
...
static short akmd_delay = AKM0991x_DEFAULT_DELAY;
...
static int akm0991x_init_flag =0;
static struct i2c_client *this_client = NULL;

/*----------------------------------------------------------------------------*/
static const struct i2c_device_id akm0991x_i2c_id[] = {{AKM0991x_DEV_NAME,0},{}};
//static struct i2c_board_info __initdata i2c_akm0991x={ I2C_BOARD_INFO("akm0991x", (AKM0991x_I2C_ADDRESS>>1))};
static struct i2c_board_info __initdata i2c_akm0991x={ I2C_BOARD_INFO("akm0991x", 0x0C)};
...
struct mag_hw* get_cust_mag_hw_native(void)
{
    return &cust_mag_hw;
}

/*----------------------------------------------------------------------------*/
static int akm0991x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id); 
static int akm0991x_i2c_remove(struct i2c_client *client);
static int akm0991x_i2c_detect(struct i2c_client *client, struct i2c_board_info *info);
#ifndef	CONFIG_HAS_EARLYSUSPEND
static int akm_probe(struct platform_device *pdev);
static int akm_remove(struct platform_device *pdev);
#endif
static int akm0991x_local_init(void);
static int akm0991x_remove(void);

static struct mag_init_info akm0991x_init_info = {
    .name = "akm0991x",
    .init = akm0991x_local_init,
    .uninit = akm0991x_remove,
};
...
/*----------------------------------------------------------------------------*/
static struct i2c_driver akm0991x_i2c_driver = {
    .driver = {
        .name  = AKM0991x_DEV_NAME,
    },
	.probe      = akm0991x_i2c_probe,
	.remove     = akm0991x_i2c_remove,
	.detect     = akm0991x_i2c_detect,
#if !defined(CONFIG_HAS_EARLYSUSPEND)
	.suspend    = akm0991x_suspend,
	.resume     = akm0991x_resume,
#endif 
	.id_table = akm0991x_i2c_id,
};
...
static DEFINE_MUTEX(akm0991x_i2c_mutex);

static void akm0991x_power(struct mag_hw *hw, unsigned int on) 
{
	...
	{        
		...
		{
			if(!hwPowerOn(hw->power_id, hw->power_vol, "akm0991x")) 
			{
		...
		{
			if(!hwPowerDown(hw->power_id, "akm0991x")) 
			{
...
static long AKI2C_RxData(char *rxData, int length)
{
	...
	struct akm0991x_i2c_data *data = i2c_get_clientdata(client);
	char addr = rxData[0];
#endif
    ...
    mutex_lock(&akm0991x_i2c_mutex);
	for(loop_i = 0; loop_i < AKM0991x_RETRY_COUNT; loop_i++)
	{
	...
	if(loop_i >= AKM0991x_RETRY_COUNT)
	{
	    mutex_unlock(&akm0991x_i2c_mutex);
		MAG_ERR("%s retry over %d\n", __func__, AKM0991x_RETRY_COUNT);
		return -EIO;
	}
	mutex_unlock(&akm0991x_i2c_mutex);
#if DEBUG
...
static long AKECS_SetMode_SngMeasure(void)
{
	...
	buffer[1] = AK0991x_MODE_SNG_MEASURE; 
	#endif
...
static long AKECS_SetMode_SelfTest(void)
{
	...
	/* Set measure mode */
	buffer[0] = AK0991x_REG_CNTL2;
	buffer[1] = AK0991x_MODE_SELF_TEST;  
	/* Set data */
	#endif
	return AKI2C_TxData(buffer, 2);
}
static long AKECS_SetMode_FUSEAccess(void)
{
	...
	/* Set measure mode */
	buffer[0] = AK0991x_REG_CNTL2;
	buffer[1] = AK0991x_MODE_FUSE_ACCESS;
	/* Set data */
	#endif
	return AKI2C_TxData(buffer, 2);
}
static int AKECS_SetMode_PowerDown(void)
{
	...
	/* Set powerdown mode */
	buffer[0] = AK0991x_REG_CNTL2;
	buffer[1] = AK0991x_MODE_POWERDOWN;
	/* Set data */
	#endif
	return AKI2C_TxData(buffer, 2);
}
...
static void AKECS_SaveData(int *buf)
{
	...
	{
		MAGN_LOG("Get daemon data: %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d!\n",
...
/*----------------------------------------------------------------------------*/
static int akm0991x_ReadChipInfo(char *buf, int bufsize)
{
	if((!buf)||(bufsize <= AKM0991x_BUFSIZE -1))
	{
		return -1;
	}
	if(!this_client)
...
int FST_AK0991x(void)
{
	...
	// TEST
	TEST_DATA(TLIMIT_NO_RST_WIA1_0991x,   TLIMIT_TN_RST_WIA1_0991x,   (int)i2cData[0],  TLIMIT_LO_RST_WIA1_0991x,   TLIMIT_HI_RST_WIA1_0991x,   &pf_total);
	TEST_DATA(TLIMIT_NO_RST_WIA2_0991x,   TLIMIT_TN_RST_WIA2_0991x,   (int)i2cData[1],  TLIMIT_LO_RST_WIA2_0991x,   TLIMIT_HI_RST_WIA2_0991x,   &pf_total);
	...
	asaz = AK09916_ASA;
	#else
	if (AKECS_SetMode(AK0991x_MODE_FUSE_ACCESS) < 0) {
		MAGN_LOG("%s:%d Error.\n", __FUNCTION__, __LINE__);
		return 0;
	}
	...
	#endif
	// TEST
	TEST_DATA(TLIMIT_NO_ASAX_0991x, TLIMIT_TN_ASAX_0991x, asax, TLIMIT_LO_ASAX_0991x, TLIMIT_HI_ASAX_0991x, &pf_total);
	TEST_DATA(TLIMIT_NO_ASAY_0991x, TLIMIT_TN_ASAY_0991x, asay, TLIMIT_LO_ASAY_0991x, TLIMIT_HI_ASAY_0991x, &pf_total);
	TEST_DATA(TLIMIT_NO_ASAZ_0991x, TLIMIT_TN_ASAZ_0991x, asaz, TLIMIT_LO_ASAZ_0991x, TLIMIT_HI_ASAZ_0991x, &pf_total);

	// Set to PowerDown mode
	if (AKECS_SetMode(AK0991x_MODE_POWERDOWN) < 0) {
		MAGN_LOG("%s:%d Error.\n", __FUNCTION__, __LINE__);
		return 0;
	}
	...
	// Set to SNG measurement pattern (Set CNTL register)
	if (AKECS_SetMode(AK0991x_MODE_SNG_MEASURE) < 0) {
		MAGN_LOG("%s:%d Error.\n", __FUNCTION__, __LINE__);
		return 0;
	}
	...
	// Get measurement data from AK0991x
	...
	// TEST
	i2cData[0] &= 0x7F;
	TEST_DATA(TLIMIT_NO_SNG_ST1_0991x,  TLIMIT_TN_SNG_ST1_0991x,  (int)i2cData[0], TLIMIT_LO_SNG_ST1_0991x,  TLIMIT_HI_SNG_ST1_0991x,  &pf_total);

	// TEST
	TEST_DATA(TLIMIT_NO_SNG_HX_0991x,   TLIMIT_TN_SNG_HX_0991x,   hdata[0],          TLIMIT_LO_SNG_HX_0991x,   TLIMIT_HI_SNG_HX_0991x,   &pf_total);
	TEST_DATA(TLIMIT_NO_SNG_HY_0991x,   TLIMIT_TN_SNG_HY_0991x,   hdata[1],          TLIMIT_LO_SNG_HY_0991x,   TLIMIT_HI_SNG_HY_0991x,   &pf_total);
	TEST_DATA(TLIMIT_NO_SNG_HZ_0991x,   TLIMIT_TN_SNG_HZ_0991x,   hdata[2],          TLIMIT_LO_SNG_HZ_0991x,   TLIMIT_HI_SNG_HZ_0991x,   &pf_total);
	TEST_DATA(TLIMIT_NO_SNG_ST2_0991x,  TLIMIT_TN_SNG_ST2_0991x,  (int)i2cData[8], TLIMIT_LO_SNG_ST2_0991x,  TLIMIT_HI_SNG_ST2_0991x,  &pf_total);

	// Set to Self-test mode (Set CNTL register)
	if (AKECS_SetMode(AK0991x_MODE_SELF_TEST) < 0) {
		MAGN_LOG("%s:%d Error.\n", __FUNCTION__, __LINE__);
		return 0;
	}
	...
	// TEST
	i2cData[0] &= 0x7F;
	TEST_DATA(TLIMIT_NO_SLF_ST1_0991x, TLIMIT_TN_SLF_ST1_0991x, (int)i2cData[0], TLIMIT_LO_SLF_ST1_0991x, TLIMIT_HI_SLF_ST1_0991x, &pf_total);
	...
	#ifdef AKM_Device_AK09916
	TEST_DATA(
			  TLIMIT_NO_SLF_RVHX_0991x,
			  TLIMIT_TN_SLF_RVHX_0991x,
			  (hdata[0])*(asax/128),
			  TLIMIT_LO_SLF_RVHX_0991x,
			  TLIMIT_HI_SLF_RVHX_0991x,
			  &pf_total
			  );

	TEST_DATA(
			  TLIMIT_NO_SLF_RVHY_0991x,
			  TLIMIT_TN_SLF_RVHY_0991x,
			  (hdata[1])*(asay/128),
			  TLIMIT_LO_SLF_RVHY_0991x,
			  TLIMIT_HI_SLF_RVHY_0991x,
			  &pf_total
			  );

	TEST_DATA(
			  TLIMIT_NO_SLF_RVHZ_0991x,
			  TLIMIT_TN_SLF_RVHZ_0991x,
			  (hdata[2])*(asaz/128),
			  TLIMIT_LO_SLF_RVHZ_0991x,
			  TLIMIT_HI_SLF_RVHZ_0991x,
			  &pf_total
			  );
	#else
	TEST_DATA(
	...
	#endif
		TEST_DATA(
			TLIMIT_NO_SLF_ST2_0991x,
			TLIMIT_TN_SLF_ST2_0991x,
			(int)i2cData[8],
			TLIMIT_LO_SLF_ST2_0991x,
			TLIMIT_HI_SLF_ST2_0991x,
			&pf_total
			);
...
{
	char strbuf[AKM0991x_BUFSIZE];
	sprintf(strbuf, "akmd0991x");
...
{
	char strbuf[AKM0991x_BUFSIZE];
	akm0991x_ReadChipInfo(strbuf, AKM0991x_BUFSIZE);
	return sprintf(buf, "%s\n", strbuf);        
}
/*----------------------------------------------------------------------------*/
...
{
	ssize_t res;
	struct akm0991x_i2c_data *obj = i2c_get_clientdata(this_client);
	if(NULL == obj)
	{
		MAG_ERR( "akm0991x_i2c_data is null!!\n");
		return 0;
	}	
...
{
    ssize_t  _tLength = 0;
    struct mag_hw   *_ptAccelHw = get_cust_mag_hw_native();
...
{
    int _nDirection = 0;
	struct akm0991x_i2c_data *_pt_i2c_obj = i2c_get_clientdata(this_client);
...
/*----------------------------------------------------------------------------*/
static struct driver_attribute *akm0991x_attr_list[] = {
    &driver_attr_daemon,
...
/*----------------------------------------------------------------------------*/
static int akm0991x_create_attr(struct device_driver *driver) 
{
	int idx, err = 0;
	int num = (int)(sizeof(akm0991x_attr_list)/sizeof(akm0991x_attr_list[0]));
	if (driver == NULL)
	{
		return -EINVAL;
	}
	...
	{
		if((err = driver_create_file(driver, akm0991x_attr_list[idx])))
		{            
			MAG_ERR( "driver_create_file (%s) = %d\n", akm0991x_attr_list[idx]->attr.name, err);
			break;
		}
	}    
	return err;
...
/*----------------------------------------------------------------------------*/
static int akm0991x_delete_attr(struct device_driver *driver)
{
	int idx ,err = 0;
	int num = (int)(sizeof(akm0991x_attr_list)/sizeof(akm0991x_attr_list[0]));
	...
	for(idx = 0; idx < num; idx++)
	{
		driver_remove_file(driver, akm0991x_attr_list[idx]);
	}
...
/*----------------------------------------------------------------------------*/
static int akm0991x_open(struct inode *inode, struct file *file)
{    
	struct akm0991x_i2c_data *obj = i2c_get_clientdata(this_client);    
	int ret = -1;	
	...
	{
		MAGN_LOG("Open device node:akm0991x\n");
	}
...
/*----------------------------------------------------------------------------*/
static int akm0991x_release(struct inode *inode, struct file *file)
{
	struct akm0991x_i2c_data *obj = i2c_get_clientdata(this_client);
	atomic_dec(&dev_open_count);
	if(atomic_read(&obj->trace) & AMK_CTR_DEBUG)
	{
		MAGN_LOG("Release device node:akm0991x\n");
	}	
	return 0;
}
...
/*----------------------------------------------------------------------------*/
//static int akm0991x_ioctl(struct inode *inode, struct file *file, unsigned int cmd,unsigned long arg)
static long akm0991x_unlocked_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
{
	...
	char buff[AKM0991x_BUFSIZE];				/* for chip information */
	...
	switch (cmd)
	{
	   ...
	   case ECS_IOCTL_RESET:
		...
		case ECS_IOCTL_GET_CONF:
			...
			#else
			ret = AKECS_SetMode(AK0991x_MODE_FUSE_ACCESS);
			#endif
			if (ret < 0)
				return ret;
			#ifdef AKM_Device_AK8963
			...
			#else
			sense_conf[0] = AK0991x_FUSE_ASAX;
			#endif
			#ifdef AKM_Device_AK09916
			...
			#else
			ret = AKECS_SetMode(AK0991x_MODE_POWERDOWN);
			#endif
			if (ret < 0)
				return ret;
		...
		case ECS_IOCTL_GET_DELAY_0991x:
		...
		case ECS_IOCTL_GET_LAYOUT_0991x:
		...
		case MSENSOR_IOCTL_READ_CHIPINFO:
			...
			akm0991x_ReadChipInfo(buff, AKM0991x_BUFSIZE);
		...
		case MSENSOR_IOCTL_READ_SENSORDATA:			
			...
			AKECS_GetRawData(buff, AKM0991x_BUFSIZE);
...
#ifdef CONFIG_COMPAT
static long akm0991x_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	long ret, i;
	...
	switch (cmd) {
	    ...
	    case COMPAT_ECS_IOCTL_SET_YPR_09911:
				 ret = file->f_op->unlocked_ioctl(file, ECS_IOCTL_SET_YPR_0991x,
								(unsigned long)(arg32));
				 if (ret){
					MAGN_LOG("ECS_IOCTL_SET_YPR_0991x unlocked_ioctl failed.");
					return ret;
				 }
		 ...
		 case COMPAT_ECS_IOCTL_GET_DELAY_09911:
			 ret = file->f_op->unlocked_ioctl(file, ECS_IOCTL_GET_DELAY_0991x,
							(unsigned long)(arg32));
			 if (ret){
			 	MAGN_LOG("ECS_IOCTL_GET_DELAY_0991x unlocked_ioctl failed.");
				return ret;
			 }
		 ...
		 case COMPAT_ECS_IOCTL_GET_LAYOUT_09911:
			 ret = file->f_op->unlocked_ioctl(file, ECS_IOCTL_GET_LAYOUT_0991x,
							(unsigned long)arg32);
			 if (ret){
			 	MAGN_LOG("ECS_IOCTL_GET_LAYOUT_0991x unlocked_ioctl failed.");
				return ret;
			 }
...
/*----------------------------------------------------------------------------*/
static struct file_operations akm0991x_fops = {
	.owner = THIS_MODULE,
	.open = akm0991x_open,
	.release = akm0991x_release,
	//.unlocked_ioctl = akm0991x_ioctl,
	.unlocked_ioctl = akm0991x_unlocked_ioctl,
#ifdef CONFIG_COMPAT
		.compat_ioctl = akm0991x_compat_ioctl,
#endif
};
/*----------------------------------------------------------------------------*/
static struct miscdevice akm0991x_device = {
    .minor = MISC_DYNAMIC_MINOR,
    .name = "msensor",
    .fops = &akm0991x_fops,
};
/*----------------------------------------------------------------------------*/
int akm0991x_operate(void* self, uint32_t command, void* buff_in, int size_in,
...
/*----------------------------------------------------------------------------*/
int akm0991x_orientation_operate(void* self, uint32_t command, void* buff_in, int size_in,
		void* buff_out, int size_out, int* actualout)
{
	int err = 0;
	int value;
	...
	if(atomic_read(&data->trace) & AMK_FUN_DEBUG)
	{
//		AKMFUNC("akm0991x_orientation_operate");
	}	
#endif
	...
	switch (command)
	{
		...
		case SENSOR_ENABLE:
			...
			else
			{
				...
				value = *(int *)buff_in;
			//	MAG_ERR( "akm0991x_orientation_operate SENSOR_ENABLE=%d  mEnabled=%d\n",value,mEnabled);
...
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
int akm0991x_gyroscope_operate(void* self, uint32_t command, void* buff_in, int size_in,
		void* buff_out, int size_out, int* actualout)
{
	int err = 0;
	int value;
	...
	if(atomic_read(&data->trace) & AMK_FUN_DEBUG)
	{
//		AKMFUNC("akm0991x_gyroscope_operate");
	}	
#endif
	...
	switch (command)
	{
		...
		case SENSOR_ENABLE:
			...
			else
			{
				...
				value = *(int *)buff_in;
			//	MAG_ERR( "akm0991x_gyroscope_operate SENSOR_ENABLE=%d  mEnabled=%d\n",value,mEnabled);
				if (mEnabled <= 0) {
						    if(value == 1)
						    {
						     atomic_set(&o_flag, 1);
...
/*----------------------------------------------------------------------------*/
int akm0991x_rotation_vector_operate(void* self, uint32_t command, void* buff_in, int size_in,
		void* buff_out, int size_out, int* actualout)
{
	int err = 0;
	int value;
	...
	if(atomic_read(&data->trace) & AMK_FUN_DEBUG)
	{
//		AKMFUNC("akm0991x_rotation_vector_operate");
	}	
#endif
...
/*----------------------------------------------------------------------------*/
int akm0991x_gravity_operate(void* self, uint32_t command, void* buff_in, int size_in,
		void* buff_out, int size_out, int* actualout)
{
	int err = 0;
	int value;
	...
	if(atomic_read(&data->trace) & AMK_FUN_DEBUG)
	{
//		AKMFUNC("akm0991x_gravity_operate");
	}	
#endif
...
/*----------------------------------------------------------------------------*/
int akm0991x_linear_accelration_operate(void* self, uint32_t command, void* buff_in, int size_in,
		void* buff_out, int size_out, int* actualout)
{
	int err = 0;
	int value;
	...
	if(atomic_read(&data->trace) & AMK_FUN_DEBUG)
	{
//		AKMFUNC("akm0991x_linear_accelration_operate");
	}	
#endif
...
#ifndef	CONFIG_HAS_EARLYSUSPEND
/*----------------------------------------------------------------------------*/
static int akm0991x_suspend(struct i2c_client *client, pm_message_t msg) 
{
	int err;
	struct akm0991x_i2c_data *obj = i2c_get_clientdata(client)
	    

	if(msg.event == PM_EVENT_SUSPEND)
	{
		akm0991x_power(obj->hw, 0);
	}
	return 0;
}
/*----------------------------------------------------------------------------*/
static int akm0991x_resume(struct i2c_client *client)
{
	int err;
	struct akm0991x_i2c_data *obj = i2c_get_clientdata(client)


	akm0991x_power(obj->hw, 1);
...
/*----------------------------------------------------------------------------*/
static void akm0991x_early_suspend(struct early_suspend *h) 
{
	struct akm0991x_i2c_data *obj = container_of(h, struct akm0991x_i2c_data, early_drv);   
	int err = 0;
	...
	if ((err = AKECS_SetMode(AK0991x_MODE_POWERDOWN)) < 0) {
		MAGN_LOG("%s:%d Error.\n", __FUNCTION__, __LINE__);
		return;
	}
...
/*----------------------------------------------------------------------------*/
static void akm0991x_late_resume(struct early_suspend *h)
{
	struct akm0991x_i2c_data *obj = container_of(h, struct akm0991x_i2c_data, early_drv);         
	int err;
	...
	if ((err = AKECS_SetMode(AK0991x_MODE_SNG_MEASURE)) < 0) {
		MAGN_LOG("%s:%d Error.\n", __FUNCTION__, __LINE__);
		return;
		}
}
...
static int akm0991x_i2c_detect(struct i2c_client *client, struct i2c_board_info *info) 
{    
	strcpy(info->type, AKM0991x_DEV_NAME);
	return 0;
}

static int akm0991x_m_enable(int en)
{
	int value = 0;
	int err = 0;
	value = en;
	factory_mode = 1;
		printk("hugo%s enable:%d\n", __func__, value);
	if(value == 1)
	{
		atomic_set(&m_flag, 1);
		atomic_set(&open_flag, 1);
...
static int akm0991x_m_set_delay(u64 ns)
{
	int value = 0;
...
static int akm0991x_m_open_report_data(int open)
{
	return 0;
}

static int akm0991x_m_get_data(int* x ,int* y,int* z, int* status)
{
	mutex_lock(&sensor_data_mutex);
...
static int akm0991x_o_enable(int en)
{
	int value = 0;
	int err = 0;
	value = en;
...
static int akm0991x_o_set_delay(u64 ns)
{
	int value = 0;
	value = (int)ns/1000/1000;
	if(value <= 10)
...
static int akm0991x_o_open_report_data(int open)
{
	return 0;
}

static int akm0991x_o_get_data(int* x ,int* y,int* z, int* status)
{
	mutex_lock(&sensor_data_mutex);
...
static int akm0991x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct akm0991x_i2c_data *data;
	struct mag_control_path ctl={0};
	struct mag_data_path mag_data={0};
	struct hwmsen_object sobj_m, sobj_o;
	
	MAGN_FUN();
	if(!(data = kzalloc(sizeof(struct akm0991x_i2c_data), GFP_KERNEL)))
	{
		err = -ENOMEM;
		goto exit;
	}
	...
	MAG_ERR("AKM09911x 1\n");
	data->hw = get_cust_mag_hw_native();		
...
/*
	...
	mt_set_gpio_out((GPIO96 | 0x80000000),GPIO_OUT_ZERO);
	mt_set_gpio_out((GPIO96 | 0x80000000),GPIO_OUT_ONE);
	msleep(50);
*/	
	/* Check connection */
	...
	if(err < 0)
	{
		MAG_ERR("AKM0991x akm0991x_probe: check device connect error\n");
		goto exit_init_failed;
	}
	

	/* Register sysfs attribute */
	if((err = akm0991x_create_attr(&(akm0991x_init_info.platform_diver_addr->driver))))
	{
		MAG_ERR("create attribute err = %d\n", err);
		goto exit_sysfs_create_group_failed;
	}

	if((err = misc_register(&akm0991x_device)))
	{
		MAG_ERR("akm0991x_device register failed\n");
		goto exit_misc_device_register_failed;	
	}    

	MAG_ERR("AKM09911x 2\n");
	ctl.is_use_common_factory = false;
	ctl.m_enable = akm0991x_m_enable;
	ctl.m_set_delay  = akm0991x_m_set_delay;
	ctl.m_open_report_data = akm0991x_m_open_report_data;
	ctl.o_enable = akm0991x_o_enable;
	ctl.o_set_delay  = akm0991x_o_set_delay;
	ctl.o_open_report_data = akm0991x_o_open_report_data;
...
/*	
	...
	mag_data.get_data_o = akm0991x_o_get_data;
	mag_data.get_data_m = akm0991x_m_get_data;
...
*/
	sobj_m.self = data;
	sobj_m.polling = 1;
	sobj_m.sensor_operate = akm0991x_operate;
	if(err = hwmsen_attach(ID_MAGNETIC, &sobj_m))
	{
		printk(KERN_ERR "akm0991x magnetic attach fail = %d\n", err);
		goto exit_sysfs_create_group_failed;
	}
	sobj_o.self = data;
	sobj_o.polling = 1;
	sobj_o.sensor_operate = akm0991x_orientation_operate;
	if(err = hwmsen_attach(ID_ORIENTATION, &sobj_o))
	{
		printk(KERN_ERR "akm0991x attach fail = %d\n", err);
		goto exit_sysfs_create_group_failed;
	}

#if CONFIG_HAS_EARLYSUSPEND
	data->early_drv.level    = EARLY_SUSPEND_LEVEL_STOP_DRAWING - 2,
	data->early_drv.suspend  = akm0991x_early_suspend,
	data->early_drv.resume   = akm0991x_late_resume,    
	register_early_suspend(&data->early_drv);
#endif

	MAG_ERR("%s: OK\n", __func__);
	akm0991x_init_flag = 1;  
	return 0;
...
static int akm0991x_i2c_remove(struct i2c_client *client)
{
	...
	if((err = akm0991x_delete_attr(&(akm0991x_init_info.platform_diver_addr->driver))))
	{
		MAG_ERR("akm0991x_delete_attr fail: %d\n", err);
	}
	
	this_client = NULL;
	i2c_unregister_device(client);
	kfree(i2c_get_clientdata(client));	
	misc_deregister(&akm0991x_device);    
	return 0;
}
...
static int akm0991x_remove(void)
{
	struct mag_hw *hw = get_cust_mag_hw_native();
 
	akm0991x_power(hw, 0);    
	atomic_set(&dev_open_count, 0);  
	i2c_del_driver(&akm0991x_i2c_driver);
	return 0;
}
...
static int	akm0991x_local_init(void)
{
	...
	akm0991x_power(hw, 1);
	if(i2c_add_driver(&akm0991x_i2c_driver))
	{
		MAG_ERR("i2c_add_driver error\n");
		return -1;
	}
	if(-1 == akm0991x_init_flag)
	{
	   return -1;
	}
	return 0;
...
/*----------------------------------------------------------------------------*/
static int __init akm0991x_init(void)
{
    struct mag_hw *hw = get_cust_mag_hw_native();
	MAGN_LOG("[%s]: i2c_number=%d\n",__func__,hw->i2c_num); 
	i2c_register_board_info(hw->i2c_num, &i2c_akm0991x, 1);
	mag_driver_add(&akm0991x_init_info);
	return 0; 
}
/*----------------------------------------------------------------------------*/
static void __exit akm0991x_exit(void)
{	
	MAGN_FUN();
}
/*----------------------------------------------------------------------------*/
module_init(akm0991x_init);
module_exit(akm0991x_exit);

MODULE_AUTHOR("viral wang <viral_wang@htc.com>");
MODULE_DESCRIPTION("AKM0991x compass driver");
MODULE_LICENSE("GPL");
