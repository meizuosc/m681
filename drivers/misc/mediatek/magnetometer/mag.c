                                 ...
                                 struct device_attribute *attr, char *buf) 
{
	...
	MAG_LOG("mag mag_dev_data o_div value: %d\n", div);
	...
	if(cxt->mag_ctl.is_support_batch){
		...
		else if (!strncmp(buf, "0", 1))
		{
			cxt->is_batch_enable = false;
                ...
                {
                    if(false == cxt->mag_ctl.is_report_input_direct && 0 != (cxt->active_data_sensor&ID_M_V_MAGNETIC))
                    {
                        startTimer(&cxt->hrTimer, atomic_read(&cxt->delay), true);
                        cxt->is_polling_run = true;
                    }
	...
	if(cxt->mag_ctl.is_support_batch){
		...
		else if (!strncmp(buf, "0", 1))
		{
			cxt->is_batch_enable = false;
                if(false == cxt->is_polling_run)
                {
                    if(false == cxt->mag_ctl.is_report_input_direct && 0 != (cxt->active_data_sensor&ID_M_V_ORIENTATION))
                    {
                        startTimer(&cxt->hrTimer, atomic_read(&cxt->delay), true);
                        cxt->is_polling_run = true;
                    }
