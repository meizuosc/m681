...
static int yamaha532_local_init(void);
static int yamaha532_remove(void);

extern int yamaha532_o_probe(struct platform_device *pdev);
extern int yamaha532_o_remove(struct platform_device *pdev);
extern int yamaha532_o_suspend(struct platform_device *pdev, pm_message_t state);
extern int yamaha532_o_resume(struct platform_device *pdev);

static int yamaha532_m_enable(int en);
static int yamaha532_m_set_delay(u64 ns);
static int yamaha532_m_open_report_data(int open);
static int yamaha532_m_get_data(int* x ,int* y,int* z, int* status);
extern int yamaha532_o_enable(int en);
extern int yamaha532_o_set_delay(u64 ns);
extern int yamaha532_o_open_report_data(int open);
extern int yamaha532_o_get_data(int* x ,int* y,int* z, int* status);

static int yamaha532_init_flag =-1; // 0<==>OK -1 <==> fail

static struct mag_init_info yamaha532_init_info = {
    .name = "yamaha532",
    .init = yamaha532_local_init,
    .uninit = yamaha532_remove,
};
...
static long compat_yamaha530_unlocked_ioctl(struct file *filp, unsigned int cmd,unsigned long arg)
{
	long ret;
	...
	if (!filp->f_op || !filp->f_op->unlocked_ioctl) {
		MSE_ERR("compat_ion_ioctl file has no f_op or no f_op->unlocked_ioctl.\n");
		return -ENOTTY;
	}
	...
	switch (cmd)
	{
		...
		case COMPAT_MSENSOR_IOCTL_READ_FACTORY_SENSORDATA: {
			MSE_LOG("compat_ion_ioctl : MSENSOR_IOCTL_XXX command is 0x%x\n", cmd);
			return filp->f_op->unlocked_ioctl(filp, cmd,
				(unsigned long)compat_ptr(arg));
		}
		default: {
			MSE_ERR("compat_ion_ioctl : No such command!! 0x%x\n", cmd);
			return -ENOIOCTLCMD;
		}
	}
}
...
#if IS_ENABLED(CONFIG_COMPAT)
	.compat_ioctl = compat_yamaha530_unlocked_ioctl,
#endif
};
/*----------------------------------------------------------------------------*/
...
static int yamaha530_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (yamaha532_o_probe(NULL))
    {
        pr_err("yamaha532_o_probe err\n");
        rt = -ENOMEM;
        goto err;
    }
	...
	if(yamaha530_create_attr(&(yamaha532_init_info.platform_diver_addr->driver)))
	{
	...
	ctl.m_enable = yamaha532_m_enable;
	ctl.m_set_delay  = yamaha532_m_set_delay;
	ctl.m_open_report_data = yamaha532_m_open_report_data;
	ctl.o_enable = yamaha532_o_enable;
	ctl.o_set_delay  = yamaha532_o_set_delay;
	ctl.o_open_report_data = yamaha532_o_open_report_data;
	ctl.is_report_input_direct = false;
	ctl.is_support_batch = data->hw->is_batch_supported;
	
	rt = mag_register_control_path(&ctl);
	if(rt)
	{
	...
	mag_data.div_m = 1000;
	mag_data.div_o = 1000;
	mag_data.get_data_o = yamaha532_o_get_data;
	mag_data.get_data_m = yamaha532_m_get_data;

	rt = mag_register_data_path(&mag_data);
	if(rt)
	{
    ...
    yamaha532_init_flag = 1;
    
	pr_debug("yamaha530 i2c probe ok!\n");
    return 0;
...
static int yamaha532_m_get_data(int* x ,int* y,int* z, int* status)
{
    ...
    *x = atomic_read(&data->last_data[0]),
    *y = atomic_read(&data->last_data[1]),
    *z = atomic_read(&data->last_data[2]),
    *status = atomic_read(&data->last_status);
...
static int yamaha530_i2c_remove(struct i2c_client *client)
{
	...
	if(data != NULL)
	{
		...
		if((err = yamaha530_delete_attr(&(yamaha532_init_info.platform_diver_addr->driver))))
	    {
...
#endif
	mag_driver_add(&yamaha532_init_info);
	return 0;    
}
/*----------------------------------------------------------------------------*/
