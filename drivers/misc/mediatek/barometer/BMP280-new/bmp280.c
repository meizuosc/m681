...
#define BAR_FUN(f)               pr_debug(KERN_INFO BAR_TAG"%s\n", __func__)
#define BAR_ERR(fmt, args...) \
	pr_err(KERN_ERR BAR_TAG"%s %d : "fmt, __func__, __LINE__, ##args)
#define BAR_LOG(fmt, args...)    pr_debug(KERN_INFO BAR_TAG fmt, ##args)
...
static int bmp_init_flag =-1; // 0<==>OK -1 <==> fail
			...
			u8 addr, u8 *data, u8 len)
{
	...
	while (left > 0) {
		...
		while (i2c_transfer(client->adapter, &msg[0], 2) != 2) {
			...
			if (retry == 20) {
				BAR_ERR("i2c read register=%#x length=%d"
					"failed\n", addr + offset, len);
				return -EIO;
			}
		}
...
static int bmp_i2c_write_block(struct i2c_client *client, u8 addr,
			u8 *data, u8 len)
{
	...
	while (left > 0) {
		...
		while (i2c_transfer(client->adapter, &msg, 1) != 1) {
			...
			if (retry == 20) {
				BAR_ERR("i2c write register=%#x length=%d"
					"failed\n", buffer[0], len);
				return -EIO;
			} else
		...
		enum BMP_POWERMODE_ENUM power_mode)
{
	...
	if (obj->sensor_type == BMP280_TYPE) {/* BMP280 */
		...
		err = bmp_i2c_read_block(client,
			BMP280_CTRLMEAS_REG_MODE__REG, &data, 1);
		data = BMP_SET_BITSLICE(data,
			BMP280_CTRLMEAS_REG_MODE, actual_power_mode);
		...
		enum BMP_FILTER_ENUM filter)
{
	...
	if (obj->sensor_type == BMP280_TYPE) {/* BMP280 */
		...
		err = bmp_i2c_read_block(client,
			BMP280_CONFIG_REG_FILTER__REG, &data, 1);
		data = BMP_SET_BITSLICE(data,
			BMP280_CONFIG_REG_FILTER, actual_filter);
		err += bmp_i2c_write_block(client,
			BMP280_CONFIG_REG_FILTER__REG, &data, 1);
	}
		...
		enum BMP_OVERSAMPLING_ENUM oversampling_p)
{
	...
	if (obj->sensor_type == BMP280_TYPE) {/* BMP280 */
		...
		err = bmp_i2c_read_block(client,
			BMP280_CTRLMEAS_REG_OSRSP__REG, &data, 1);
		data = BMP_SET_BITSLICE(data,
			BMP280_CTRLMEAS_REG_OSRSP, actual_oversampling_p);
		err += bmp_i2c_write_block(client,
			BMP280_CTRLMEAS_REG_OSRSP__REG, &data, 1);
	}
		...
		enum BMP_OVERSAMPLING_ENUM oversampling_t)
{
	...
	if (obj->sensor_type == BMP280_TYPE) {/* BMP280 */
		...
		err = bmp_i2c_read_block(client,
			BMP280_CTRLMEAS_REG_OSRST__REG, &data, 1);
		data = BMP_SET_BITSLICE(data,
			BMP280_CTRLMEAS_REG_OSRST, actual_oversampling_t);
		err += bmp_i2c_write_block(client,
			BMP280_CTRLMEAS_REG_OSRST__REG, &data, 1);
	}
	...
	if (obj->sensor_type == BMP280_TYPE) {/* BMP280 */
		...
		v_x2_u32r  = (((
			...
			*
			...
			((BMP280_S32_t)obj->bmp280_cali.dig_T3)) >> 14;
...
static int bmp_get_pressure(struct i2c_client *client, char *buf, int bufsize)
{
	...
	if (obj->last_temp_measurement +
		...
		status = bmp_get_temperature(client,
				temp_buf, BMP_BUFSIZE);/* update t_fine */
		if (status != 0)
			goto exit;
	...
	if (obj->sensor_type == BMP280_TYPE) {/* BMP280 */
		...
		v_x1_u32r = (((BMP280_S64_t)obj->bmp280_cali.dig_P9) *
			(p >> 13) * (p >> 13)) >> 25;
		v_x2_u32r = (((BMP280_S64_t)obj->bmp280_cali.dig_P8) *
			p) >> 19;
...
static int bmp280_verify_i2c_disable_switch(struct bmp_i2c_data *obj)
{
	...
	err = bmp_i2c_read_block(obj->client,
...
static long compat_bmp_unlocked_ioctl(struct file *filp, unsigned int cmd,
	unsigned long arg)
{
