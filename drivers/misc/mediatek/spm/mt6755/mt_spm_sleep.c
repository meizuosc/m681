...
#define DFS_UP_COUNT		(spm_vcore_dvfs + 0x58)
#define DFS_UP_COUNT_PHYS	(spm_vcore_dvfs_phy + 0x58)
#define DFS_DOWN_COUNT		(spm_vcore_dvfs + 0x60)
#define DFS_DOWN_COUNT_PHYS	(spm_vcore_dvfs_phy + 0x60)
...
enum spm_suspend_step {
	SPM_SUSPEND_ENTER = 0,
	SPM_SUSPEND_ENTER_UART_SLEEP,
	SPM_SUSPEND_ENTER_SPM_FLOW,
	SPM_SUSPEND_B3,
	SPM_SUSPEND_B4,
	SPM_SUSPEND_B5,
	SPM_SUSPEND_B6,
	...
	SPM_SUSPEND_LEAVE_SPM_FLOW,
	SPM_SUSPEND_ENTER_UART_AWAKE,/* bit 10 */
	SPM_SUSPEND_DRAMC_CHK, /* bit 11 */
	SPM_SUSPEND_B12,
	SPM_SUSPEND_B13,
	SPM_SUSPEND_B14,
	SPM_SUSPEND_PCMDESC_CHK, /* bit 15 */
	SPM_SUSPEND_DFS_UP_COUNT, /* bit 16 */
	SPM_SUSPEND_B17,
	SPM_SUSPEND_B18,
	SPM_SUSPEND_B19,
	SPM_SUSPEND_B20,
	...
	SPM_SUSPEND_B22,
	SPM_SUSPEND_B23,
	SPM_SUSPEND_DFS_DOWN_COUNT, /* bit 24 */
	SPM_SUSPEND_B25,
	SPM_SUSPEND_B26,
	SPM_SUSPEND_B27,
	SPM_SUSPEND_B28,
...
static void spm_suspend_pre_process(struct pwr_ctrl *pwrctrl)
{
	...
	aee_rr_rec_spm_suspend_val(aee_rr_curr_spm_suspend_val() |
			    ((spm_read(DFS_UP_COUNT) & 0xFF) << 16) |
			    ((spm_read(DFS_DOWN_COUNT) & 0xFF) << 24));
#endif
}
...
void suspend_dramc_chk(int step)
{
	u32 chk_val = spm_read(spm_ddrphy_base + 0x390);

	if (chk_val & 0xFF000000) { /* check bit 24 ~ 31 */
		aee_rr_rec_spm_suspend_val(aee_rr_curr_spm_suspend_val() | (1 << (SPM_SUSPEND_DRAMC_CHK + step)));
		pr_err("suspend_dramc_chk: %d, 0x%08x\n", step, chk_val);
	}
}

void suspend_pcmdesc_chk(const struct pcm_desc *pcmdesc)
{
	static int flag;
	...
	if (flag) {
		if (chk_cum != curr_chk_cum) {
			aee_rr_rec_spm_suspend_val(1 << SPM_SUSPEND_PCMDESC_CHK);
			pr_err("suspend_pcmdesc_chk: 0x%08x, 0x%08x\n", chk_cum, curr_chk_cum);
		}
	} else {
		chk_cum = curr_chk_cum;
		flag = 1;
...
wake_reason_t spm_go_to_sleep(u32 spm_flags, u32 spm_data)
{
...
#if SPM_AEE_RR_REC
	aee_rr_rec_spm_suspend_val(aee_rr_curr_spm_suspend_val() | (1 << SPM_SUSPEND_ENTER_UART_SLEEP));
#endif
...
#if SPM_AEE_RR_REC
	aee_rr_rec_spm_suspend_val(aee_rr_curr_spm_suspend_val() | (1 << SPM_SUSPEND_ENTER_SPM_FLOW));
	suspend_dramc_chk(1);
	suspend_pcmdesc_chk(pcmdesc);
#endif
	...
	aee_rr_rec_spm_suspend_val(aee_rr_curr_spm_suspend_val() | (1 << SPM_SUSPEND_ENTER_WFI) |
				(1 << SPM_SUSPEND_B3) | (1 << SPM_SUSPEND_B4) |
				(1 << SPM_SUSPEND_B5) | (1 << SPM_SUSPEND_B6));
	suspend_dramc_chk(2);
#endif
...
#if SPM_AEE_RR_REC
	aee_rr_rec_spm_suspend_val(aee_rr_curr_spm_suspend_val() | (1 << SPM_SUSPEND_ENTER_UART_AWAKE));
#endif
...
#if SPM_AEE_RR_REC
	aee_rr_rec_spm_suspend_val(aee_rr_curr_spm_suspend_val() | (1 << SPM_SUSPEND_LEAVE_SPM_FLOW));
#endif
