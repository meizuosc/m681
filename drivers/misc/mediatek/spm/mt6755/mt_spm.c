...
void __iomem *spm_vcore_dvfs;
void __iomem *spm_vcore_dvfs_phy;
u32 gpio_base_addr;
u32 spm_irq_0 = 197;
u32 spm_irq_1 = 198;
u32 spm_irq_2 = 199;
...
#define VCORE_DVFS_NODE "mediatek,vcore_dvfs"
	...
	if (!node) {
		spm_err("error: cannot find node " VCORE_DVFS_NODE);
		goto vcore_dvfs_exit;
	}
	if (of_address_to_resource(node, 0, &r)) {
		spm_err("error: cannot get phys addr" VCORE_DVFS_NODE);
		goto vcore_dvfs_exit;
	}
	spm_vcore_dvfs_phy = r.start;

	spm_vcore_dvfs = (unsigned long)of_iomap(node, 0);
	if (!spm_vcore_dvfs) {
		spm_err("error: cannot iomap " VCORE_DVFS_NODE);
		goto vcore_dvfs_exit;
	}

	spm_err("spm_vcore_dvfs = %p\n", spm_vcore_dvfs);
