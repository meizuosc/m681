...
enum spm_deepidle_step {
	...
	SPM_DEEPIDLE_LEAVE_SPM_FLOW,
	SPM_DEEPIDLE_ENTER_UART_AWAKE, /* bit 10 */
	SPM_DEEPIDLE_DRAMC_CHK, /* bit 11 */
	SPM_DEEPIDLE_B12,
	SPM_DEEPIDLE_B13,
	SPM_DEEPIDLE_B14,
	SPM_DEEPIDLE_PCMDESC_CHK, /* bit 15 */
	SPM_DEEPIDLE_DFS_UP_COUNT, /* bit 16 */
	SPM_DEEPIDLE_B17,
	SPM_DEEPIDLE_B18,
	SPM_DEEPIDLE_B19,
	SPM_DEEPIDLE_B20,
	...
	SPM_DEEPIDLE_B22,
	SPM_DEEPIDLE_B23,
	SPM_DEEPIDLE_DFS_DOWN_COUNT, /* bit 24 */
	SPM_DEEPIDLE_B25,
	SPM_DEEPIDLE_B26,
	SPM_DEEPIDLE_B27,
	SPM_DEEPIDLE_B28,
...
void deepidle_dramc_chk(int step)
{
	u32 chk_val = spm_read(spm_ddrphy_base + 0x390);

	if (chk_val & 0xFF000000) { /* check bit 24 ~ 31 */
		aee_rr_rec_deepidle_val(aee_rr_curr_deepidle_val() | (1 << (SPM_DEEPIDLE_DRAMC_CHK + step)));
		pr_err("deepidle_dramc_chk: %d, 0x%08x\n", step, chk_val);
	}
}

void deepidle_pcmdesc_chk(const struct pcm_desc *pcmdesc)
{
	static int flag;
	...
	if (flag) {
		if (chk_cum != curr_chk_cum) {
			aee_rr_rec_deepidle_val(1 << SPM_DEEPIDLE_PCMDESC_CHK);
			pr_err("deepidle_pcmdesc_chk: 0x%08x, 0x%08x\n", chk_cum, curr_chk_cum);
		}
	} else {
		chk_cum = curr_chk_cum;
		flag = 1;
...
wake_reason_t spm_go_to_dpidle(u32 spm_flags, u32 spm_data, u32 dump_log)
{
...
#if SPM_AEE_RR_REC
	aee_rr_rec_deepidle_val(aee_rr_curr_deepidle_val() | (1 << SPM_DEEPIDLE_ENTER_SPM_FLOW));
	deepidle_dramc_chk(1);
	deepidle_pcmdesc_chk(pcmdesc);
#endif
	...
	aee_rr_rec_deepidle_val(aee_rr_curr_deepidle_val() | (1 << SPM_DEEPIDLE_ENTER_WFI) |
				(1 << SPM_DEEPIDLE_B3) | (1 << SPM_DEEPIDLE_B4) |
				(1 << SPM_DEEPIDLE_B5) | (1 << SPM_DEEPIDLE_B6));
	deepidle_dramc_chk(2);
#endif
...
#if SPM_AEE_RR_REC
	aee_rr_rec_deepidle_val(aee_rr_curr_deepidle_val() | (1 << SPM_DEEPIDLE_LEAVE_SPM_FLOW));
#endif
