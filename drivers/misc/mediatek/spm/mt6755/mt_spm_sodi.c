...
enum spm_sodi_step {
	...
	SPM_SODI_ENTER_UART_AWAKE,/* bit 10 */
	SPM_SODI_DRAMC_CHK, /* bit 11 */
	SPM_SODI_B12,
	SPM_SODI_B13,
	SPM_SODI_B14,
	SPM_SODI_PCMDESC_CHK, /* bit 15 */
	SPM_SODI_DFS_UP_COUNT, /* bit 16 */
	SPM_SODI_B17,
	SPM_SODI_B18,
	SPM_SODI_B19,
	SPM_SODI_B20,
	...
	SPM_SODI_B22,
	SPM_SODI_B23,
	SPM_SODI_DFS_DOWN_COUNT, /* bit 24 */
	SPM_SODI_B25,
	SPM_SODI_B26,
	SPM_SODI_B27,
	SPM_SODI_B28,
...
static struct pwr_ctrl sodi_ctrl = {
	...
	.r0_ctrl_en = 1,
	.r7_ctrl_en = 1,
	.infra_dcm_lock = 1, /* set to be 0 if SODI original */
	.wfi_op = WFI_OP_AND,
...
void sodi_dramc_chk(int step)
{
	u32 chk_val = spm_read(spm_ddrphy_base + 0x390);

	if (chk_val & 0xFF000000) { /* check bit 24 ~ 31 */
		aee_rr_rec_sodi_val(aee_rr_curr_sodi_val() | (1 << (SPM_SODI_DRAMC_CHK + step)));
		pr_err("sodi_dramc_chk: %d, 0x%08x\n", step, chk_val);
	}
}

void sodi_pcmdesc_chk(const struct pcm_desc *pcmdesc)
{
	static int flag;
	...
	if (flag) {
		if (chk_cum != curr_chk_cum) {
			aee_rr_rec_sodi_val(1 << SPM_SODI_PCMDESC_CHK);
			pr_err("sodi_pcmdesc_chk: 0x%08x, 0x%08x\n", chk_cum, curr_chk_cum);
		}
	} else {
		chk_cum = curr_chk_cum;
		flag = 1;
	...
	if (!(sodi_flags & (1 << 1))) {
		...
		if (need_log_out == 1) {
			...
			if ((wakesta.assert_pc != 0) || (wakesta.r12 == 0)) {
				...
				sodi_err("vcore_status = %d, self_refresh = 0x%x, sw_flag = 0x%x, 0x%x, %s, sodi_cnt = %d, self_refresh_cnt = 0x%x\n",
						vcore_status, spm_read(SPM_PASR_DPD_0), spm_read(SPM_SW_FLAG),
						spm_read(DUMMY1_PWR_CON), pcmdesc->version,
						logout_sodi_cnt, logout_selfrefresh_cnt);

				sodi_err("wake up by SPM assert, spm_pc = 0x%0x, r13 = 0x%x, debug_flag = 0x%x\n",
						wakesta.assert_pc, wakesta.r13, wakesta.debug_flag);

				sodi_err("r12 = 0x%x, r12_ext = 0x%x, raw_sta = 0x%x, idle_sta = 0x%x, event_reg = 0x%x, isr = 0x%x\n",
				...
				sodi_warn("vcore_status = %d, self_refresh = 0x%x, sw_flag = 0x%x, 0x%x, %s, sodi_cnt = %d, self_refresh_cnt = 0x%x\n",
						vcore_status, spm_read(SPM_PASR_DPD_0), spm_read(SPM_SW_FLAG),
						spm_read(DUMMY1_PWR_CON), pcmdesc->version,
						logout_sodi_cnt, logout_selfrefresh_cnt);

				sodi_warn("wake up by %s, timer_out = %u, r13 = 0x%x, debug_flag = 0x%x\n",
						buf, wakesta.timer_out, wakesta.r13, wakesta.debug_flag);

				sodi_warn("r12 = 0x%x, r12_ext = 0x%x, raw_sta = 0x%x, idle_sta = 0x%x, event_reg = 0x%x, isr = 0x%x\n",
						wakesta.r12, wakesta.r12_ext, wakesta.raw_sta, wakesta.idle_sta,
						wakesta.event_reg, wakesta.isr);
			}
