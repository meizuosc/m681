				    ...
				    DEV_INT32 maxp)
{
	 ...
	 */
	pr_debug("write_fifo_burst=ep_num=%d, lenght=%d, buf=%p, maxp=%d\n", ep_num, length, buf,
	       maxp);
#endif
			    ...
			    DEV_INT8 interval, DEV_INT8 slot, DEV_INT8 burst, DEV_INT8 mult)
{
	 ...
	 * It means the value of Tx/RxFIFOSz _MUST_ be a power of 2 (2^n). It can _NOT_ be 5,48,180 the kind of values.
	 * TX/RXFIFOSEGSIZE should be equal or bigger than 4. The Tx/RxFIFO size of 2^n bytes also should be equal
	 * or bigger than TX/RXMAXPKTSZ. This EndPoint occupy total memory size  (TX/RX_SLOT + 1 )*2^TX/RXFIFOSEGSIZE bytes.
	 */
	if (maxp <= 16)
