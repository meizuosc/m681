...
#else
static struct wakeup_source mu3d_mtk_test_wakelock;
#endif
...
#else
	if (!wake_lock_active(&mu3d_mtk_test_wakelock))
		__pm_stay_awake(&mu3d_mtk_test_wakelock);
#endif
...
#else
	if (wake_lock_active(&mu3d_mtk_test_wakelock))
		__pm_relax(&mu3d_mtk_test_wakelock);
#endif
...
#else
	wakeup_source_init(&mu3d_mtk_test_wakelock, WAKE_LOCK_SUSPEND, "mu3d_mtk_test.wakelock");
#endif
...
#define CMD_NUM 10
	...
	for (i = 0; i < CMD_NUM; i++) {
		if ((!strcmp(_arPCmdTbl_dev[i].name, argv[0]))
		    && (_arPCmdTbl_dev[i].cb_func != NULL))
...
int usbif_u3d_test_send_event(char *event)
{
	...
	printk("usbif_u3d_test_send_event - sending event - %s in %s\n", udev_event,
	       kobject_get_path(&mu3d_test_uevent_device.this_device->kobj, GFP_KERNEL));
