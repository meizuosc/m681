...
static DEV_UINT8 bAddress_Offset;
...
void u3d_power_mode(DEV_INT32 mode, DEV_INT8 u1_value, DEV_INT8 u2_value, DEV_INT8 en_u1,
		    DEV_INT8 en_u2)
{
	...
	if (mode == 1) {
		...
		os_writel(U3D_LINK_POWER_CONTROL,
			  os_readl(U3D_LINK_POWER_CONTROL) & ~(SW_U1_REQUEST_ENABLE |
	...
	if (mode == 2) {
		...
		os_writel(U3D_LINK_POWER_CONTROL,
			  os_readl(U3D_LINK_POWER_CONTROL) & ~(SW_U1_REQUEST_ENABLE |
							       SW_U2_REQUEST_ENABLE));
		os_writel(U3D_LINK_POWER_CONTROL,
			  os_readl(U3D_LINK_POWER_CONTROL) | SW_U2_REQUEST_ENABLE);
		temp = os_readl(U3D_LINK_UX_INACT_TIMER);
	...
	if (en_u2) {
		os_writel(U3D_LINK_POWER_CONTROL,
			  os_readl(U3D_LINK_POWER_CONTROL) | SW_U2_ACCEPT_ENABLE |
			  SW_U2_REQUEST_ENABLE);
	}
	if (mode == 4) {
		os_ms_delay(200);
...
/* USBIF */
	...
	os_writel(U3D_EP0DMARLCOUNT,
		  os_readl(U3D_EP0DMARLCOUNT) | ((burst_mode & 0x3) << DMA_BURST_OFST));
...
void u3d_ep_start_transfer(DEV_INT32 ep_num, USB_DIR dir)
{
	...
	if (ep_setting->enabled) {
		...
		if ((dir == USB_TX) && (ep_num != 0)) {
			...
			need_zlp = ((req->actual == req->count) && (!(req->count % maxp))
				    && ep_num != 0);
...
void dev_power_mode(DEV_INT32 mode, DEV_INT8 u1_value, DEV_INT8 u2_value, DEV_INT8 en_u1,
		    DEV_INT8 en_u2)
{
...
void dev_send_one_packet(DEV_INT32 ep_tx)
{
	...
	req->dma_adr = mapping;
	req->count = 1024;
	mu3d_hal_insert_transfer_gpd(ep_tx, USB_TX, req->dma_adr, req->count, true, true, false,
				     false, 1024);
...
void dev_u1u2_en_cond(DEV_INT8 opt, DEV_INT8 cond, DEV_INT32 ep_rx, DEV_INT32 ep_tx)
{
	...
	if (cond) {
		if (opt == 2) {
			mu3d_hal_insert_transfer_gpd(ep_tx, USB_TX, treq->dma_adr, treq->count,
						     true, true, false, zlp, maxp);
			mu3d_hal_resume_qmu(ep_tx, USB_TX);
		}
		if (opt == 3) {
			mu3d_hal_insert_transfer_gpd(ep_rx, USB_RX, rreq->dma_adr, rreq->count,
						     true, true, false, zlp, maxp);
...
void dev_u1u2_en_ctrl(DEV_INT8 type, DEV_INT8 u_num, DEV_INT8 opt, DEV_INT8 cond, DEV_INT8 u1_value,
		      DEV_INT8 u2_value)
{
...
#define LPM_INACT_BMU_RX 4
	...
	/* LPM_MODE */
	os_writel(U3D_POWER_MANAGEMENT, (os_readl(U3D_POWER_MANAGEMENT) & ~LPM_MODE)
		  |
		  (((lpm_info->lpm_mode ==
		     LPM_MODE_HW_LPM) ? LPM_MODE_NORMAL : lpm_info->lpm_mode) << 8));
...
#else
	os_writel(U3D_MAC_U2_EN_CTRL, (os_readl(U3D_MAC_U2_EN_CTRL) & ~(0x1f << 16))
		...
		if (req->complete == 1) {
			...
			req->currentCount =
			    ((req->count - req->actual) >
			     ep_setting->maxp) ? ep_setting->maxp : req->count - req->actual;
			...
			if (req->actual >= req->count) {
				...
				dma_sync_single_for_cpu(&mu3d_if_dev, (dma_addr_t) req->dma_adr,
							USB_BUF_SIZE, DMA_BIDIRECTIONAL);
				dma_unmap_single(&mu3d_if_dev, req->dma_adr, USB_BUF_SIZE,
						 DMA_BIDIRECTIONAL);
				os_printk(K_DEBUG,
				...
				u3d_config_dma0(0, USB_TX,
						(DEV_UINT32) ((u32) req->dma_adr + req->actual),
						req->currentCount);
			}
		}
	}
...
void u3d_ep0_rx(void)
{
	...
	if (g_ep0_mode == PIO_MODE) {
		...
		if (req->currentCount == 0) {
			...
			u3d_config_dma0(0, USB_RX, (DEV_UINT32) ((u32) req->dma_adr + req->actual),
					req->currentCount);
		}
	}
		...
		if (Request->bmRequestType & USB_DIR_IN) {
			...
			if ((Request->bRequest == USB_REQ_SET_FEATURE) &&
				...
				os_writel(U3D_EP0CSR,
					  os_readl(U3D_EP0CSR) | EP0_SETUPPKTRDY | EP0_DATAEND);
				return;
			}
...
#ifdef BOUNDARY_4K
		loopback_buffer =
		    g_loopback_buffer[1] + (0x1000 -
					    (DEV_INT32) (unsigned long)g_loopback_buffer[1] %
					    0x1000) - 0x08 + bAddress_Offset;
		bAddress_Offset++;
		bAddress_Offset %= 4;
		req->buf = loopback_buffer;
#else
...
void u3d_dma_handler(DEV_INT32 DMAIntSts)
{
	...
	if (DMAIntSts & EP0DMAISR) {	/* ep0 */
		...
		if ((os_readl(U3D_EP0CSR) & EP0_SETUPPKTRDY)	/* SETUPEND case; restarts from EP0_IDLE */
			...
			mapping = req->dma_adr;
			dma_sync_single_for_cpu(&mu3d_if_dev, mapping, USB_BUF_SIZE,
						DMA_BIDIRECTIONAL);
			...
			if ((Request->bmRequestType & USB_TYPE_MASK) == USB_TYPE_STANDARD) {
				...
				if ((Request->bmRequestType & USB_RECIP_MASK) == USB_RECIP_DEVICE) {
					...
					if (Request->bRequest == USB_REQ_GET_STATUS) {
						...
						mapping =
						    dma_map_single(&mu3d_if_dev, req->buf,
								   USB_BUF_SIZE, DMA_BIDIRECTIONAL);
						dma_sync_single_for_device(&mu3d_if_dev, mapping,
									   USB_BUF_SIZE,
									   DMA_BIDIRECTIONAL);
						req->dma_adr = mapping;
						...
						g_ep0_state = EP0_TX;
						os_writel(U3D_EP0CSR,
							  os_readl(U3D_EP0CSR) | EP0_SETUPPKTRDY |
							  EP0_DPHTX);
					}
#ifdef SUPPORT_OTG
					/* 0x1 */
					...
					if (Request->bRequest == USB_REQ_SET_FEATURE) {
						...
						printk
						    ("Request wValue = 0x%x, Request wIndex = 0x%x\n",
					...
					if (Request->bRequest == USB_REQ_SET_CONFIGURATION) {
						...
						mb();
						printk
						    ("==DEV - USB_REQ_SET_CONFIGURATION== done, g_otg_config %d\n",
						     g_otg_config);
#endif
						os_writel(U3D_EP0CSR,
							  os_readl(U3D_EP0CSR) | EP0_SETUPPKTRDY |
				...
				if ((Request->bmRequestType & USB_RECIP_MASK) == USB_RECIP_ENDPOINT) {
					if (Request->bmRequestType & USB_DIR_IN) {
							...
							*(ptr1 + 1) = 0x00;
							...
							mapping =
							    dma_map_single(&mu3d_if_dev, req->buf,
									   USB_BUF_SIZE,
									   DMA_BIDIRECTIONAL);
							dma_sync_single_for_device(&mu3d_if_dev,
										   mapping,
										   USB_BUF_SIZE,
										   DMA_BIDIRECTIONAL);
							req->dma_adr = mapping;
						...
						if ((Request->bRequest == USB_REQ_SET_FEATURE) &&
						    (Request->wValue == ENDPOINT_HALT)) {
							printk
							    ("==EP - USB_REQ_SET_FEATURE to EP HALT==\n");
							u3d_stall_all();
							g_device_halt = 1;
							os_writel(U3D_EP0CSR,
								  os_readl(U3D_EP0CSR) |
						...
						if ((Request->bRequest == USB_REQ_CLEAR_FEATURE) &&
						    (Request->wValue == ENDPOINT_HALT)) {
							printk
							    ("==EP - USB_REQ_CLEAR_FEATURE to EP HALT==\n");
							u3d_clear_stall_all();
							g_device_halt = 0;
							os_writel(U3D_EP0CSR,
								  os_readl(U3D_EP0CSR) |
						...
						if ((Request->bRequest == USB_REQ_EP0_STALL) &&
						    (Request->wValue == ENDPOINT_HALT)) {
							printk
							    ("==EP - USB_REQ_EP0_STALL to EP HALT==\n");
							u3d_send_ep0_stall();
						}
			...
			if (Request->bmRequestType == USB_TYPE_VENDOR) {
				if (Request->bmRequestType & USB_DIR_IN) {
					...
					if (Request->bRequest == AT_CMD_ACK) {
						...
						*(ptr1 + 5) = 0;
						*(ptr1 + 6) =
						    dev_stschk(Request->wIndex, Request->wValue);
						*(ptr1 + 7) = 0;
					...
					mapping =
					    dma_map_single(&mu3d_if_dev, req->buf, USB_BUF_SIZE,
							   DMA_BIDIRECTIONAL);
					dma_sync_single_for_device(&mu3d_if_dev, mapping,
								   USB_BUF_SIZE, DMA_BIDIRECTIONAL);
					...
					loopback_buffer =
					    g_loopback_buffer[1] + (0x1000 -
								    (DEV_INT32) (unsigned long)
								    g_loopback_buffer[1] % 0x1000) -
					    0x08 + bAddress_Offset;
					bAddress_Offset++;
					bAddress_Offset %= 4;
					req->buf = loopback_buffer;
#else
			...
			if ((count < ep_setting->maxp) || (req->actual == req->count)) {
				...
				Request->bCommand = AT_CMD->tsfun;
				dma_sync_single_for_cpu(&mu3d_if_dev, req->dma_adr, USB_BUF_SIZE,
							DMA_BIDIRECTIONAL);
				dma_unmap_single(&mu3d_if_dev, req->dma_adr, USB_BUF_SIZE,
						 DMA_BIDIRECTIONAL);
			}
