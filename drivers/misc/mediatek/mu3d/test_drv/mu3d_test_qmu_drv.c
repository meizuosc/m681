...
void dev_insert_stress_gpd(USB_DIR dir, DEV_INT32 ep_num, DEV_UINT8 IOC)
{
	...
	_fgbypassGPD++;
	if (!(_fgbypassGPD % 5)) {
		mu3d_hal_insert_transfer_gpd(ep_num, dir, req->dma_adr, req->count, true, IOC, true,
					     zlp, maxp);
	} else
#endif
	{
		mu3d_hal_insert_transfer_gpd(ep_num, dir, req->dma_adr, req->count, true, IOC,
					     false, zlp, maxp);
...
void dev_prepare_gpd_short(DEV_INT32 num, USB_DIR dir, DEV_INT32 ep_num, DEV_UINT8 *buf)
{
	...
	if (dir == USB_TX) {
		...
		mu3d_hal_insert_transfer_gpd(ep_num, dir, req->dma_adr, 256, true, false, false,
					     zlp, maxp);

		req->dma_adr += TransferLength;
		for (i = 1; i < num; i++) {
			IOC = ((i % STRESS_IOC_TH) == (STRESS_IOC_TH / 2)) ? true : false;
			mu3d_hal_insert_transfer_gpd(ep_num, dir, req->dma_adr, req->count, true,
						     IOC, false, zlp, maxp);
	...
	if (dir == USB_TX)
		os_printk(K_DEBUG, "TX[%d] GPD Start : 0x%08X\n", ep_num,
			  os_readl(USB_QMU_TQSAR(ep_num)));
	else
		os_printk(K_DEBUG, "RX[%d] GPD Start : 0x%08X\n", ep_num,
			  os_readl(USB_QMU_RQSAR(ep_num)));
...
void dev_qmu_loopback_ext(DEV_INT32 ep_rx, DEV_INT32 ep_tx)
{
...
#ifdef BOUNDARY_4K
	qmu_loopback_buffer =
	    g_loopback_buffer[0] + (0x1000 -
				    (DEV_INT32) (unsigned long)g_loopback_buffer[0] % 0x1000) -
	    0x20 + bDramOffset;
	...
	for (i = 0; i < gpd_num; i++) {
		...
		mu3d_hal_insert_transfer_gpd(ep_rx, USB_RX, (rreq->dma_adr + i * gpd_buf_size),
					     gpd_buf_size, true, true, false, zlp, maxp);
		os_get_random_bytes(&bps_num, 1);
		bps_num %= 3;
		for (j = 0; j < bps_num; j++) {
			mu3d_hal_insert_transfer_gpd(ep_rx, USB_RX, rreq->dma_adr, gpd_buf_size,
						     true, true, true, zlp, maxp);
		}
	}
	...
	for (i = 0; i < gpd_num; i++) {
		...
		if (treq->count > gpd_buf_size) {
			mu3d_hal_insert_transfer_gpd(ep_tx, USB_TX,
						     (treq->dma_adr + i * gpd_buf_size),
						     gpd_buf_size, true, true, false, zlp, maxp);
		} else {
			mu3d_hal_insert_transfer_gpd(ep_tx, USB_TX,
						     (treq->dma_adr + i * gpd_buf_size),
						     treq->count, true, true, false, zlp, maxp);
		}
		...
		for (j = 0; j < bps_num; j++) {
			mu3d_hal_insert_transfer_gpd(ep_tx, USB_TX, treq->dma_adr, gpd_buf_size,
						     true, true, true, zlp, maxp);
		}
...
void dev_qmu_rx(DEV_INT32 ep_rx)
{
	...
	maxp = USB_ReadCsr32(U3D_RX1CSR0, ep_rx) & RX_RXMAXPKTSZ;
	for (i = 0; i < gpd_num; i++) {
		mu3d_hal_insert_transfer_gpd(ep_rx, USB_RX, (req->dma_adr + i * gpd_buf_size),
					     gpd_buf_size, true, true, false, zlp, maxp);
	}
	mu3d_hal_resume_qmu(ep_rx, USB_RX);
	os_ms_delay(500);
...
void dev_set_dma_busrt_limiter(DEV_INT8 busrt, DEV_INT8 limiter)
{
	...
	temp =
	    ((busrt << DMA_BURST_OFST) & DMA_BURST) | ((limiter << DMALIMITER_OFST) & DMALIMITER);
 ...
 */
TGPD *prepare_tx_stress_gpd(TGPD *gpd, dma_addr_t pBuf, DEV_UINT32 data_length, DEV_UINT8 ep_num,
			    DEV_UINT8 _is_bdp, DEV_UINT8 isHWO, DEV_UINT8 IOC)
{
	DEV_UINT32 offset;
	DEV_INT32 i, bd_num;
	...
	if (!_is_bdp) {
		...
		for (i = 0; i < bd_num; i++) {
			if (i == (bd_num - 1)) {
				...
				if (bBD_Extension) {
					dma_sync_single_for_cpu(NULL, (dma_addr_t) pBuf,
								g_dma_buffer_size,
								DMA_BIDIRECTIONAL);
					_tmp = TBD_GET_EXT(bd);
					os_memcpy(_tmp, os_phys_to_virt(pBuffer), bBD_Extension);
					dma_sync_single_for_device(NULL, (dma_addr_t) pBuf,
								   g_dma_buffer_size,
								   DMA_BIDIRECTIONAL);
				}
	...
	if (bGPD_Extension) {
		...
		os_memcpy(_tmp, (DEV_UINT8 *) os_phys_to_virt((void *)(unsigned long)pBuf),
			  bGPD_Extension);
...
void resume_gpd_hwo(USB_DIR dir, DEV_INT32 Q_num)
{
	...
	if (dir == USB_RX) {
		gpd_current =
		    (TGPD *) gpd_current + STRESS_IOC_TH * Rx_gpd_IOC[Q_num] * sizeof(TGPD);
	}
	if (dir == USB_TX) {
		gpd_current =
		    (TGPD *) gpd_current + STRESS_IOC_TH * Rx_gpd_IOC[Q_num] * sizeof(TGPD);
		gpd_current =
		    (TGPD *) gpd_current + STRESS_IOC_TH * Rx_gpd_IOC[Q_num] * AT_GPD_EXT_LEN;
	}
 ...
 * @args - arg1: gpd address, arg2: data buffer address, arg3: data length, arg4: ep number, arg5: with bd or not, arg6: write hwo bit or not,  arg7: write ioc bit or not
 */
TGPD *prepare_rx_stress_gpd(TGPD *gpd, dma_addr_t pBuf, DEV_UINT32 data_len, DEV_UINT8 ep_num,
			    DEV_UINT8 _is_bdp, DEV_UINT8 isHWO, DEV_UINT8 IOC)
{
	DEV_UINT32 offset;
	DEV_INT32 i, bd_num;
	...
	os_printk(K_DEBUG,
		  "Rx gpd info { HWO %d, Next_GPD %p ,DataBufferLength %d, DataBuffer %p, Recived Len %d, Endpoint %d, TGL %d, ZLP %d}\r\n",
 ...
 */
void insert_stress_gpd(DEV_INT32 ep_num, USB_DIR dir, dma_addr_t buf, DEV_UINT32 count,
		       DEV_UINT8 isHWO, DEV_UINT8 IOC)
{
	...
	if (dir == USB_TX) {
		...
		if (method == 1) {	/* set txpktrdy twice to ensure zlp will be sent */
			...
			USB_WriteCsr32(U3D_TX1CSR0, Q_num,
				       USB_ReadCsr32(U3D_TX1CSR0, Q_num) & ~TX_DMAREQEN);
			USB_WriteCsr32(U3D_TX1CSR0, Q_num,
				       USB_ReadCsr32(U3D_TX1CSR0, Q_num) | TX_TXPKTRDY);
			USB_WriteCsr32(U3D_TX1CSR0, Q_num,
				       USB_ReadCsr32(U3D_TX1CSR0, Q_num) | TX_DMAREQEN);
			while ((USB_ReadCsr32(U3D_TX1CSR0, Q_num) & TX_FIFOFULL))
				NULL;
		...
		if (!(USB_ReadCsr32(U3D_RX1CSR0, Q_num) & RX_FIFOEMPTY)) {
			...
			USB_WriteCsr32(U3D_RX1CSR0, Q_num,
				       USB_ReadCsr32(U3D_RX1CSR0, Q_num) | RX_RXPKTRDY);
		}
		spin_unlock_irqrestore(&lock, flags);
...
void proc_qmu_rx(DEV_INT32 ep_num)
{
	...
	if (g_run_stress) {
		...
		os_printk(K_INFO,
			  "ep_num : %d ,Rx_gpd_last : 0x%p, gpd_current : 0x%p, gpd_end : 0x%p \r\n",
		...
		dma_sync_single_for_cpu(NULL, mu3d_hal_gpd_virt_to_phys(gpd, USB_RX, ep_num),
		...
		while (gpd != gpd_current && !TGPD_IS_FLAGS_HWO(gpd)) {
			...
			if (TGPD_IS_FORMAT_BDP(gpd)) {
				...
				dma_sync_single_for_cpu(NULL, (dma_addr_t) (unsigned long)bd,
			...
			os_printk(K_INFO,
				  ...
				  (DEV_UINT32) TGPD_GET_FLAG(gpd), TGPD_GET_NEXT(gpd),
				  (DEV_UINT32) TGPD_GET_DataBUF_LEN(gpd), TGPD_GET_DATA(gpd),
				  (DEV_UINT32) TGPD_GET_BUF_LEN(gpd),
				  (DEV_UINT32) TGPD_GET_EPaddr(gpd), (DEV_UINT32) TGPD_GET_TGL(gpd),
				  (DEV_UINT32) TGPD_GET_ZLP(gpd));

			gpd = TGPD_GET_NEXT(gpd);

			/* invalidate next GPD data in CPU */
			if (gpd)
				dma_sync_single_for_cpu(NULL, (dma_addr_t) (unsigned long)gpd,
							sizeof(TGPD), DMA_BIDIRECTIONAL);
...
void proc_qmu_tx(DEV_INT32 ep_num)
{
	...
	if (g_run_stress) {
		...
		os_printk(K_DEBUG, "Tx_gpd_last 0x%p, gpd_current 0x%p, gpd_end 0x%p, \r\n", gpd,
			  gpd_current, Tx_gpd_end[ep_num]);

		if (gpd == gpd_current) {
			return;
		}
		/* flush data from device to CPU */
		dma_sync_single_for_cpu(NULL, mu3d_hal_gpd_virt_to_phys(gpd, USB_TX, ep_num),
					sizeof(TGPD), DMA_BIDIRECTIONAL);

		while (gpd != gpd_current && !TGPD_IS_FLAGS_HWO(gpd)) {
			os_printk(K_DEBUG,
				  "Tx gpd %p info { HWO %d, BPD %d, Next_GPD %p , DataBuffer %p, BufferLen %d, Endpoint %d}\r\n",
				  gpd, (DEV_UINT32) TGPD_GET_FLAG(gpd),
				  (DEV_UINT32) TGPD_GET_FORMAT(gpd), TGPD_GET_NEXT(gpd),
				  TGPD_GET_DATA(gpd), (DEV_UINT32) TGPD_GET_BUF_LEN(gpd),
...
irqreturn_t u3d_inter_handler(int irq, void *dev_id)
{
	...
	os_printk(K_DEBUG,
		  "Interrupt: IntrUsb [%x] IntrTx[%x] IntrRx [%x] IntrDMA[%x] IntrQMU [%x] IntrLTSSM [%x]\r\n",
	...
	os_printk(K_DEBUG,
		  "Intr: CMN[%x] Tx[%x] Rx[%x] DMA[%x] QMU[%x] QMUDn[%x] LTSSM[%x] SpdChg[%x]\n",
		  dwIntrUsbValue, wIntrTxValue, wIntrRxValue, dwDmaIntrValue, wIntrQMUValue,
...
#ifdef POWER_SAVING_MODE
#ifdef SUPPORT_U3
		os_writel(U3D_SSUSB_U3_CTRL_0P,
			  os_readl(U3D_SSUSB_U3_CTRL_0P) & ~SSUSB_U3_PORT_PDN);
#endif
		os_writel(U3D_SSUSB_U2_CTRL_0P,
			  os_readl(U3D_SSUSB_U2_CTRL_0P) & ~SSUSB_U2_PORT_PDN);
		os_writel(U3D_SSUSB_IP_PW_CTRL2,
			  os_readl(U3D_SSUSB_IP_PW_CTRL2) & ~SSUSB_IP_DEV_PDN);
#endif
	...
	if (dwLtssmValue) {
...
#ifdef POWER_SAVING_MODE
			os_writel(U3D_SSUSB_U2_CTRL_0P,
				  os_readl(U3D_SSUSB_U2_CTRL_0P) & ~SSUSB_U2_PORT_DIS);
#endif
		...
		if (dwLtssmValue & U3_RESUME_INTR) {
			...
			os_writel(U3D_LINK_POWER_CONTROL,
				  os_readl(U3D_LINK_POWER_CONTROL) | UX_EXIT);
		}
#endif
	}
#endif
...
/* chiachun... */
	...
	if (dwIntrUsbValue & LPM_INTR) {
		...
		os_printk(K_NOTICE, "BESL: %x, %x <= %x <= %x\n", dwTemp & 0xf, (dwTemp >> 8) & 0xf,
			  (dwTemp >> 12) & 0xf, (dwTemp >> 4) & 0xf);
