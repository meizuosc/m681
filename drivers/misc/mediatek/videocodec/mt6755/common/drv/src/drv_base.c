		...
		if (oal_hw_context[i].u4VCodecThreadNum != VCODEC_THREAD_MAX_NUM) {
			for (j = 0; j < oal_hw_context[i].u4VCodecThreadNum; j++) {
				if (oal_hw_context[i].u4VCodecThreadID[j] == curr_tid) {
					MFV_LOGD("[VCODEC][search_HWLockSlot_ByTID]\n");
					MFV_LOGD("Lookup curr HW Locker is ObjId %d in index%d\n",
						 curr_tid, i);
					return i;
				}
			}
...
VAL_INT32_T search_HWLockSlot_ByHandle(VAL_ULONG_T ulpa, VAL_HANDLE_T handle)
{
	...
	/* dump debug info */
	MFV_LOGE("search_HWLockSlot_ByHandle");
	for (i = 0; i < VCODEC_MULTIPLE_INSTANCE_NUM / 2; i++) {
		/* Add one line comment for avoid kernel coding style, WARNING:BRACES: */
		MFV_LOGE("[%d] 0x%lx", i, oal_hw_context[i].pvHandle);
	}
...
VAL_VCODEC_OAL_HW_CONTEXT_T *setCurr_HWLockSlot(VAL_ULONG_T ulpa, VAL_UINT32_T tid)
{
		...
		if (oal_hw_context[i].u4VCodecThreadNum != VCODEC_THREAD_MAX_NUM) {
			for (j = 0; j < oal_hw_context[i].u4VCodecThreadNum; j++) {
				if (oal_hw_context[i].u4VCodecThreadID[j] == current->pid) {
					...
					MFV_LOGD("[VCODEC][setCurr_HWLockSlot] setCurr %d Slot\n",
						 i);
					return &oal_hw_context[i];
				}
			}
	...
	MFV_LOGE("[VCODEC][ERROR] setCurr_HWLockSlot All %d Slots unavaliable\n",
							  ...
							  VAL_UINT32_T *a_prIndex)
{
	...
	for (i = 0; i < VCODEC_MULTIPLE_INSTANCE_NUM; i++) {
		if (oal_hw_context[i].u4VCodecThreadNum != VCODEC_THREAD_MAX_NUM) {
			for (j = 0; j < oal_hw_context[i].u4VCodecThreadNum; j++) {
				MFV_LOGD("[VCODEC][setCurr_HWLockSlot_Thread_ID]\n");
				MFV_LOGD("Dump curr slot %d, ThreadID[%d] = %d\n",
	...
	{
		MFV_LOGE("[VCODEC][ERROR] setCurr_HWLockSlot_Thread_ID All %d Slots unavaliable\n",
			 VCODEC_MULTIPLE_INSTANCE_NUM);
...
VAL_VCODEC_OAL_HW_CONTEXT_T *freeCurr_HWLockSlot(VAL_ULONG_T ulpa)
{
	...
	for (i = 0; i < VCODEC_MULTIPLE_INSTANCE_NUM; i++) {
		if (oal_hw_context[i].ObjId == ulpa) {
			...
			MFV_LOGD("[VCODEC] freeCurr_HWLockSlot %d Slot\n", i);
			return &oal_hw_context[i];
		}
	}

	MFV_LOGE("[VCODEC][ERROR] freeCurr_HWLockSlot can't find pid in HWLockSlot\n");
	return 0;
}
			    ...
			    VAL_UINT32_T a_u4VCodecThreadNum, VAL_UINT32_T *a_pu4VCodecThreadID)
{
	...
	MFV_LOGD("[VCODEC] Add_NonCacheMemoryList +, KVA = 0x%lx, KPA = 0x%lx, Size = 0x%lx\n",
	...
	if (u4I == VCODEC_MULTIPLE_INSTANCE_NUM_x_10) {
		/* Add one line comment for avoid kernel coding style, WARNING:BRACES: */
		MFV_LOGE("[VCODEC][ERROR] CAN'T ADD Add_NonCacheMemoryList, List is FULL!!\n");
	}

	MFV_LOGD("[VCODEC] Add_NonCacheMemoryList -\n");
}
...
void Free_NonCacheMemoryList(VAL_ULONG_T a_ulKVA, VAL_ULONG_T a_ulKPA)
{
	...
	MFV_LOGD("[VCODEC] Free_NonCacheMemoryList +, KVA = 0x%lx, KPA = 0x%lx\n", a_ulKVA,
		 a_ulKPA);

	for (u4I = 0; u4I < VCODEC_MULTIPLE_INSTANCE_NUM_x_10; u4I++) {
		if ((grNonCacheMemoryList[u4I].ulKVA == a_ulKVA)
		    && (grNonCacheMemoryList[u4I].ulKPA == a_ulKPA)) {
			MFV_LOGD("[VCODEC] Free Free_NonCacheMemoryList index = %d\n", u4I);
	...
	MFV_LOGD("[VCODEC]Free_NonCacheMemoryList -\n");
}
...
void Force_Free_NonCacheMemoryList(VAL_UINT32_T a_u4Tid)
{
	...
	MFV_LOGD("[VCODEC] Force_Free_NonCacheMemoryList +, curr_id = %d", a_u4Tid);
...
VAL_ULONG_T Search_NonCacheMemoryList_By_KPA(VAL_ULONG_T a_ulKPA)
{
	...
	MFV_LOGD("[VCODEC] Search_NonCacheMemoryList_By_KPA +, KPA = 0x%lx, ulVA_Offset = 0x%lx\n",
		 a_ulKPA, ulVA_Offset);

	for (u4I = 0; u4I < VCODEC_MULTIPLE_INSTANCE_NUM_x_10; u4I++) {
		if (grNonCacheMemoryList[u4I].ulKPA == (a_ulKPA - ulVA_Offset)) {
			MFV_LOGD("[VCODEC] Find Search_NonCacheMemoryList_By_KPA index = %d\n",
				 u4I);
			break;
		}
	}
	...
	MFV_LOGD("[VCODEC] Search_NonCacheMemoryList_By_KPA, ulVA = 0x%lx -\n",
