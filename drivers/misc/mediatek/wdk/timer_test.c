...
static int msleep_test(int length, int times)
{
	...
	t2 = sched_clock();
	pr_alert("msleep_test,  msleep(%d) test %d times\n", length, times);
	pr_alert("msleep_test: t1 =%lld, t2=%lld, delta=%lld\n", t1, t2, t2 - t1);
...
static int udelay_test(int length, int times)
{
	...
	t2 = sched_clock();
	pr_alert("udelay_test,  udelay_test(%d) test %d times\n", length, times);

	pr_alert("udelay_test :t1 =%lld, t2=%lld, delta=%lld\n", t1, t2, t2 - t1);
...
static int mdelay_test(int length, int times)
{
	...
	t2 = sched_clock();
	pr_alert("mdelay_test,  mdelay_test(%d) test %d times\n", length, times);

	pr_alert("mdelay_test:t1 =%lld, t2=%lld, delta=%lld\n", t1, t2, t2 - t1);
...
static int phycical_count_test(void)
{
	...
	pr_alert("phycical_count_test start\n");
	while (1) {
	...
	pr_alert("phycical_count_test end\n");
}
...
static irqreturn_t cpuxgpt_test_irq_handler(int irq, void *dev_id)
{
	...
	g_cpuxgpt0_called = 1;
	pr_alert("cpuxgpt irq:%d called\n", irq);
	pr_alert("cpuxgpt t2(%lld),t1(%lld),delta(%lld)\n", cpuxgpt_t2, cpuxgpt_t1,
		 cpuxgpt_t2 - cpuxgpt_t1);
	return IRQ_HANDLED;
}
...
#define TIMEOUT (13000000)	/* 1s = 13M */
	...
	g_cpuxgpt0_called = 0;
	pr_alert("cpuxgpt0-div1 t2(%lld),t1(%lld),delta(%lld)\n", cpuxgpt_t2, cpuxgpt_t1,
		 cpuxgpt_t2 - cpuxgpt_t1);
	...
	g_cpuxgpt0_called = 0;
	pr_alert("cpuxgpt0-div2 t2(%lld),t1(%lld),delta(%lld)\n", cpuxgpt_t2, cpuxgpt_t1,
		 cpuxgpt_t2 - cpuxgpt_t1);
	...
	g_cpuxgpt0_called = 0;
	pr_alert("cpuxgpt0-div4 t2(%lld),t1(%lld),delta(%lld)\n", cpuxgpt_t2, cpuxgpt_t1,
		 cpuxgpt_t2 - cpuxgpt_t1);
#endif
}
...
/*
	...
	read_cntpct(cntpct_lo1, cntpct_hi1);
	localtimer_get_phy_count();
	pr_alert("11111 ca7 counter(%u,%u) == %lld\n", cntpct_hi1,cntpct_lo1,localtimer_get_phy_count() );


	read_cntpct(cntpct_lo1, cntpct_hi1);
	pr_alert("22222 ca7 counter(%u,%u) == %lld\n", cntpct_hi1,cntpct_lo1,localtimer_get_phy_count() );
	read_cntpct(cntpct_lo1, cntpct_hi1);
	pr_alert("33333 ca7 counter(%u,%u) == %lld\n", cntpct_hi1,cntpct_lo1,localtimer_get_phy_count() );
	read_cntpct(cntpct_lo1, cntpct_hi1);
	pr_alert("44444 ca7 counter(%u,%u) == %lld\n", cntpct_hi1,cntpct_lo1,localtimer_get_phy_count() );
*/
...
{
	pr_alert("1:cpuxgpt_interrupt_test\n");
	pr_alert("2:cpuxgpt_frequency_test\n");
	pr_alert("3: cpuxgpt_halt_on_debug_test\n");
...
static int ktimer_thread_test(void *arg)
{
	...
	for (;;) {
		...
		pr_alert("timer test debug start, cpu:%d\n", cpu);
		/* phycical_count_test(); */
		cpuxgpt_run_case(test_case);

		if (ts_msleep != 0) {
			/* pr_alert("CPU:%d, msleep(%d) test\n", cpu,ts_msleep); */
			if (0 != msleep_times) {
		...
		read_cntpct(cntpct_lo1, cntpct_hi1);
		pr_alert("ca7 counter(%u,%u) == %lld\n", cntpct_hi1, cntpct_lo1,
			 localtimer_get_phy_count());

		/* pr_alert("cpuxgpt(%d) set 100ms timer,idx=%d\n" ,idx%8,idx); */
		/* cpu_xgpt_set_timer(idx%8,100000000); */
		/* idx++; */

		pr_alert("timer test debug end, cpu:%d\n", cpu);
#if 0
		pr_alert("fwq2 before set\n");
		dump_gpt_reg();
		pr_alert("set gpt3 5s oneshot\n");
		...
		if (cntpct_hi1 == last_cntpct_hi1 && cntpct_lo1 == last_cntpct_lo1) {
			pr_alert("fwq local count error !!!\n");
		}
		last_cntpct_hi1 = cntpct_hi1;
		last_cntpct_lo1 = cntpct_lo1;
		pr_alert("ca7 counter(%u,%u), cpu:%d\n", cntpct_hi1, cntpct_lo1);
		pr_alert("timer test debug end, cpu:%d\n", cpu);
#endif
	}
	return 0;
...
static enum hrtimer_restart hrtimer_test_func(struct hrtimer *timer)
{
	...
	hr_t2 = sched_clock();
	pr_alert("[hrtimer_test_func] t1=%lld,t2 =%lld\n", hr_t1, hr_t2);

	pr_alert("[hrtimer_test_func] delta=%lld,\n", hr_t2 - hr_t1);
...
static int hrtimer_test_case(void)
{
	...
	pr_alert("hrtimer_test_case:ts_hrtimer_sec=%ld, ts_hrtimer_nsecs=%ld\n", ts_hrtimer_sec,
		 ts_hrtimer_nsecs);
	hr_t1 = sched_clock();
