...
static	int mrdump_rst_source;
static	int mrdump_en, mrdump_mode;
...
{
	seq_printf(s, "%d %d %d\n",mrdump_rst_source,mrdump_mode,mrdump_en);
	return 0;
}
				     ...
				     loff_t *data)
{
	...
	ret = sscanf(mrdump_cmd_buf, "%d %d %d", &mrdump_rst_source, &mrdump_mode, &mrdump_en);
	if (ret != 3)
	...
	if (mrdump_rst_source == MRDUMP_SYSRESETB) {
		ret = my_wd_api->wd_debug_key_sysrst_config(mrdump_en, mrdump_mode);
	} else if (mrdump_rst_source == MRDUMP_EINTRST) {
		ret = my_wd_api->wd_debug_key_eint_config(mrdump_en, mrdump_mode);
	} else {
...
static int cpuhpg_dbg_lastcpu, cpuhpg_dbg_act;
static unsigned long long cpuhpg_dbg_t;
void wk_cpu_update_bit_flag(int cpu, int plug_status)
{
	/* add debug info to record the last cpu hotplug action */
	spin_lock(&lock);
	cpuhpg_dbg_lastcpu = cpu;
	cpuhpg_dbg_act = plug_status;
	cpuhpg_dbg_t = sched_clock();
	spin_unlock(&lock);
...
static int kwdt_thread(void *arg)
{
	...
	for (;;) {
		...
		/*
		   ...
		   pr_debug(KERN_ERR "%s, HW Gating, 0x374:0x%x, 0x378:%x\n", __FUNCTION__,
		   ...
		   pr_debug(KERN_ERR "%s, HW Gating, Base::%p, 0x374:0x%x, 0x378:%x\n", __FUNCTION__,
		   dram_nao_base,
		   DRV_Reg32(dram_nao_base + 0x374),
		   DRV_Reg32(dram_nao_base + 0x378));
		   } else {
		   pr_debug(KERN_ERR "can't find DRAMC0_NA compatible node\n");
		   return -1;
		   }
		   }
		   #endif
		...
		spin_unlock(&lock);
		/* pr_debug("fwq loc_wk_wdt(%x),loc_wk_wdt->ready(%d)\n",loc_wk_wdt ,loc_wk_wdt->ready); */
		if (loc_wk_wdt && loc_wk_wdt->ready && g_enable) {
			...
			local_bit, cpu, wk_check_kick_bit(), cpuhpg_dbg_lastcpu, cpuhpg_dbg_act, cpuhpg_dbg_t, sched_clock());
