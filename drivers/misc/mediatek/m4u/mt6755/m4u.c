...
//cloud, temp workaround
        ...
        if((pa & (PAGE_SIZE - 1))!=0)
        {
            M4ULOG_MID("pa error, pa: 0x%llx, va: 0x%lx, align: 0x%lx\n", (unsigned long long)pa, va_tmp, va_align);
        }
...
int m4u_mva_map_kernel(unsigned int mva, unsigned int size, unsigned long *map_va, unsigned int *map_size)
{
    ...
    if (pages == NULL)
    {
	M4UMSG("mva_map_kernel:error to vmalloc for %lu\n", sizeof(struct page *) * page_num);
    }
...
{
	int ret = 0;
	M4U_MOUDLE_STRUCT *pM4u_module;
	M4U_PORT_STRUCT *pM4u_port;
	M4U_PORT_ID PortID;
	M4U_CACHE_STRUCT *pM4u_cache_data;
	M4U_DMA_STRUCT *pM4u_dma_data;
	m4u_client_t *client = filp->private_data;

	pM4u_module = kmalloc(sizeof(M4U_MOUDLE_STRUCT), GFP_ATOMIC);

	if (!pM4u_module) {
		M4UMSG("kmalloc for m4u_module_port fail\n");
		return -ENOMEM;
	}

	pM4u_port = kmalloc(sizeof(M4U_PORT_STRUCT), GFP_ATOMIC);

	if (!pM4u_port) {
		M4UMSG("kmalloc for m4u_port_struct fail\n");
		return -ENOMEM;
	}

	pM4u_cache_data = kmalloc(sizeof(M4U_CACHE_STRUCT), GFP_ATOMIC);

	if (!pM4u_cache_data) {
		M4UMSG("kmalloc for m4u_cache_struct fail\n");
		return -ENOMEM;
	}

	pM4u_dma_data = kmalloc(sizeof(M4U_DMA_STRUCT), GFP_ATOMIC);

	if (!pM4u_dma_data) {
		M4UMSG("kmalloc for m4u_dma_struct fail\n");
		return -ENOMEM;
	}

	switch (cmd) {
	case MTK_M4U_T_ALLOC_MVA:
		ret = copy_from_user(pM4u_module, (void *) arg, sizeof(M4U_MOUDLE_STRUCT));
		if (ret) {
			M4UMSG("MTK_M4U_T_ALLOC_MVA,copy_from_user failed:%d\n", ret);
			return -EFAULT;
		}

		ret = m4u_alloc_mva(client, pM4u_module->port, pM4u_module->BufAddr,
			NULL, pM4u_module->BufSize, pM4u_module->prot, pM4u_module->flags, &(pM4u_module->MVAStart));

		if (ret)
			return ret;

		ret = copy_to_user(&(((M4U_MOUDLE_STRUCT *) arg)->MVAStart),
			&(pM4u_module->MVAStart), sizeof(unsigned int));
		if (ret) {
			M4UMSG("MTK_M4U_T_ALLOC_MVA,copy_from_user failed:%d\n", ret);
			return -EFAULT;
		}
	...
	case MTK_M4U_T_DEALLOC_MVA:
		{
			...
			ret = m4u_dealloc_mva(client, pM4u_module->port, pM4u_module->MVAStart);
			if (ret)
				return ret;
		}
		break;
	case MTK_M4U_T_CACHE_SYNC:
		ret = copy_from_user(pM4u_cache_data, (void *) arg, sizeof(M4U_CACHE_STRUCT));
		if (ret) {
			M4UMSG("m4u_cache_sync,copy_from_user failed:%d\n", ret);
			return -EFAULT;
		}
		ret = m4u_cache_sync(client, pM4u_cache_data->port, pM4u_cache_data->va,
			pM4u_cache_data->size, pM4u_cache_data->mva, pM4u_cache_data->eCacheSync);
		break;
	case MTK_M4U_T_DMA_OP:
		ret = copy_from_user(pM4u_dma_data, (void *) arg, sizeof(M4U_DMA_STRUCT));
		if (ret) {
			M4UMSG("m4u dma map/unmap area,copy_from_user failed:%d\n", ret);
			return -EFAULT;
		}

		ret = m4u_dma_op(client, pM4u_dma_data->port, pM4u_dma_data->va,
			pM4u_dma_data->size, pM4u_dma_data->mva,
			pM4u_dma_data->eDMAType, pM4u_dma_data->eDMADir);
		break;
	case MTK_M4U_T_CONFIG_PORT:
		ret = copy_from_user(pM4u_port, (void *) arg, sizeof(M4U_PORT_STRUCT));
		if (ret) {
	...
	case MTK_M4U_T_CONFIG_PORT_ARRAY:
		{
			struct m4u_port_array *pPort_array;

			pPort_array = kmalloc(sizeof(struct m4u_port_array), GFP_ATOMIC);

			if (!pM4u_dma_data) {
				M4UMSG("kmalloc for m4u_port_array fail\n");
				return -ENOMEM;
			}
			ret = copy_from_user(pPort_array, (void *) arg, sizeof(struct m4u_port_array));
			if (ret) {
				M4UMSG("MTK_M4U_T_CONFIG_PORT,copy_from_user failed:%d\n", ret);
				return -EFAULT;
			}
			...
			mutex_lock(&gM4u_sec_init);
#endif
			ret = m4u_config_port_array(pPort_array);
#ifdef M4U_TEE_SERVICE_ENABLE
			mutex_unlock(&gM4u_sec_init);
#endif
			kfree(pPort_array);
	...
	case MTK_M4U_T_CONFIG_MAU:
		{
			...
			pMAU = kmalloc(sizeof(M4U_MAU_STRUCT), GFP_ATOMIC);

			if (!pMAU) {
				M4UMSG("kmalloc for m4u_mau_struct fail\n");
				return -ENOMEM;
			}

			ret = copy_from_user(pMAU, (void *) arg, sizeof(M4U_MAU_STRUCT));
			if (ret) {
	...
	case MTK_M4U_T_CONFIG_TF:
		{
			M4U_TF_STRUCT *pM4UTF;

			pM4UTF = kmalloc(sizeof(M4U_TF_STRUCT), GFP_ATOMIC);

			if (!pM4UTF) {
				M4UMSG("kmalloc for m4u_tf_struct fail\n");
				return -ENOMEM;
			}

			ret = copy_from_user(pM4UTF, (void *) arg, sizeof(M4U_TF_STRUCT));
			if (ret) {
				M4UMSG("MTK_M4U_T_CONFIG_TF,copy_from_user failed:%d\n", ret);
				return -EFAULT;
			}

			ret = m4u_enable_tf(pM4UTF->port, pM4UTF->fgEnable);
...
static int m4u_probe(struct platform_device *pdev)
{
	...
	if (err)
		M4UINFO("[DTS] get m4u platform_device id fail!!\n");
    }
	...
	if (IS_ERR(gM4uDev->infra_m4u)) {
		M4UINFO("cannot get infra m4u clock\n");
