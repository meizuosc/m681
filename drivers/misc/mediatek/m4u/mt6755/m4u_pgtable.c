                    ...
                    struct sg_table *sg_table, unsigned int size, unsigned int prot)
{
	...
	for_each_sg(sg_table->sgl, sg, sg_table->nents, i) 
    {
        ...
        if(ret)
        {
		M4UMSG("%s: ret: %d, i: %d, sg->dma: 0x%pad, sg->phy: 0x%lu, sg->offset: 0x%x\n",
			__func__, ret, i, &sg_dma_address(sg), (unsigned long)sg_phys(sg), sg->offset);
            goto err_out;
        }
        else
        {
