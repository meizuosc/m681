...
#include <linux/of_irq.h>
	...
	/* toprgu_base will be used by mt_wdt.h, so don't change this name */
#endif

#ifdef MTK_WDT_NON_RST_REG2
int mvg_get_rgu_counter(void)
{
	int result;
	volatile unsigned int *reg = (unsigned int *)MTK_WDT_NON_RST_REG2;
...
int mvg_set_rgu_counter(int val)
{
	...
	i = (PWR_LOSS_MAGIC_PATTERN << 16) |
		PWR_LOSS_COUNTER((unsigned int)val);
	*reg = i;
...
int pwr_loss_open(struct inode *inode, struct file *file)
{
	...
	pr_debug("%s Power Loss Test: Open operation !\n", TAG);
#endif
	return 0;
}
...
{
#ifdef PWR_LOSS_DEBUG
	pr_debug("%s Power Loss Test: Release operation !\n", TAG);
#endif
	return 0;
}
...
long pwr_loss_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
...
#ifdef PWR_LOSS_DEBUG
	/*pr_debug("%s Power Loss Test: IOCTL operation ! CMD=%d\n",
		TAG, cmd);*/
#endif

	switch (cmd) {
	case PRINT_REBOOT_TIMES:
		ret  = copy_from_user((int *)l_buf, (int *)arg,
			sizeof(int));
		if (ret != 0) {
	...
	case PRINT_GENERAL_INFO:
		ret  = copy_from_user(l_buf, (int *)arg,
			(sizeof(l_buf[0])*(sizeof(l_buf))));
		if (ret != 0)
			pr_err("%s Power Loss Test: IOCTL->PRINT_REBOOT_TIMES %d Bytes can't be copied\n",
				TAG, ret);
	...
	case MVG_ADDR_RANGE_ADD:
		ret  = copy_from_user(&addr_entry, (int *)arg,
			sizeof(struct mvg_addr));
		if (ret != 0) {
	...
	case MVG_EMMC_SET_DELAY_TABLE:
		ret = mvg_emmc_get_set_delay_table(&power_loss_info,
			(unsigned char *)arg);
		break;
#endif
...
#ifdef PWR_LOSS_DEBUG
	pr_debug("%s Power Loss Test: sleep time =%d\n", TAG, sleep_time);
#endif
...
#ifdef PWR_LOSS_DEBUG
	pr_debug("%s Power Loss Test: sleep time = 100sec\n", TAG);
#endif
...
static ssize_t power_loss_debug_proc_write(struct file *file,
	...
	if (1 == sscanf(cmd_buf, "%x", &wdt_reboot_support)) {
		if (wdt_reboot_support < 0) {
			...
			power_loss_info.wdt_reboot_support =
				(wdt_reboot_support == WDT_REBOOT_OFF) ?
				WDT_REBOOT_OFF : WDT_REBOOT_ON;
			up_write(&power_loss_info.rwsem);

			pr_debug("%s [****PWR_LOSS_DEBUG****]\n", TAG);
...
static int power_loss_debug_init(void)
{
	...
	power_loss_debug = proc_create("power_loss_debug", 0660, NULL,
		&power_loss_debug_fops);

	if (power_loss_debug) {
		pr_debug("%s [%s]: Successfully create /proc/power_loss_debug\n",
			TAG, __func__);
	} else {
...
static int __init power_loss_init(void)
{
	...
	pr_debug("%s Power Loss Test Module Init\n", TAG);

	err = alloc_chrdev_region(&sg_pwr_loss_devno, PWR_LOSS_FIRST_MINOR,
		PWR_LOSS_MAX_MINOR_COUNT, PWR_LOSS_DEVNAME);
	if (err != 0) {
...
#ifdef PWR_LOSS_DEBUG
	pr_debug("%s Power Loss Test: MAJOR =%d, MINOR=%d\n", TAG,
	...
	sg_pwr_loss_dev_file = device_create(sg_pwr_loss_dev_class,
		NULL, sg_pwr_loss_devno, NULL, PWR_LOSS_DEVNAME);
	...
	if (mvg_toprgu_node == NULL) {
		mvg_toprgu_node = of_find_compatible_node(NULL, NULL,
			"mediatek,TOPRGU");
		pr_err("mvg_toprgu_node at %p\n", mvg_toprgu_node);
		if (mvg_toprgu_node) {
			toprgu_base = of_iomap(mvg_toprgu_node, 0);
...
{
#ifdef PWR_LOSS_DEBUG
	pr_debug("%s Power Loss Test: Module Exit\n", TAG);
#endif
...
void mvg_set_curr_case_name(char *str)
{
	...
	pr_err("%s set case name to %s\n", TAG,
		power_loss_info.current_case_name);
}
void mvg_set_curr_group_name(char *str)
{
	...
	pr_err("%s set group name to %s\n", TAG,
		power_loss_info.current_group_name);
}
...
void mvg_nand_set_program_wdt(int trigger)
{
	...
	pr_err("%s set nand_program_wdt_enable to %X\n", TAG,
		host->pl.nand_program_wdt_enable);
}
...
static struct hd_struct *mvg_get_part(char *name)
{
	dev_t devt;
	int partno;
...
static void mvg_put_part(struct hd_struct *part)
{
	disk_put_part(part);
}
	...
	for (i = 0; i < PART_NUM; i++) {
		if (name && info) {
			if (strcmp(name, (char *)lastest_part[i].name) == 0) {
				memcpy(info, &lastest_part[i],
					sizeof(pt_resident));
#ifdef CONFIG_MTK_EMMC_SUPPORT
				/* Note: for emmc, the physical address
					 start from preloader-end */
				if (i > 0)
...
void mvg_wdt_reset(void)
{
#ifdef CONFIG_OF
	unsigned int wdt_mode_val;
	/*WDT_RESTART register*/
	DRV_WriteReg32(MTK_WDT_RESTART, MTK_WDT_RESTART_KEY);
	/*WDT_MODE register*/
	wdt_mode_val = DRV_Reg32(toprgu_base);
	/* clear autorestart bit: autoretart:
	   1, bypass power key, 0: not bypass power key */
	wdt_mode_val &= (~MTK_WDT_MODE_AUTO_RESTART);
	/* make sure WDT mode is hw reboot mode, can not config isr mode */
	...
	wdt_mode_val = wdt_mode_val | (MTK_WDT_MODE_KEY | MTK_WDT_MODE_EXTEN |
						MTK_WDT_MODE_AUTO_RESTART);
	DRV_WriteReg32(toprgu_base, wdt_mode_val);
	/*WDT_SWRST register*/
	DRV_WriteReg32(MTK_WDT_SWRST, MTK_WDT_SWRST_KEY);
#else
	/*FIX ME: wdt_arch_reset() has extra delay,
		  the better way is to write register directly.
	/*wdt_arch_reset(0xFF); */
	#if 1
	unsigned int ap_rgu_remap_base =
		(unsigned int)ioremap_nocache(0x10007000, 0x90);
	#else
