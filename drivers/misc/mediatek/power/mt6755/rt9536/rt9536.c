...
int charging_ic_set_chargingmode(enum charging_ic_charging_mode mode)
{
	...
	/*
	   ...
	   pr_debug( "[charger_rt9536] ::idx = %3d: mode = %d, pullsel = %d, dout = %d, pullen = %d, dir = %d\n",
...
void charging_ic_active_default(void)
{
	...
	if (charging_ic_status == POWER_SUPPLY_TYPE_USB)
		pr_debug("[charger_rt9536] :: it's already %s mode!!\n", __func__);
		return;
...
void charging_ic_set_ta_mode(void)
{
	...
	pr_debug("[charger_rt9536] :: charging_ic_set_ta_mode\n");
...
void charging_ic_set_factory_mode(void)
{
	...
	pr_debug("[charger_rt9536] :: charging_ic_set_factory_mode\n");
...
void rt9536_charging_enable(unsigned int set_current, unsigned int enable)
{
	if (enable) {
		...
		pr_debug("[charger_rt9536] :: %s, current(%d), enable(%d)\n", __func__, set_current,
			 enable);
	} else {
		charging_ic_deactive();
		pr_debug("[charger_rt9536] :: %s, enable(%d)\n", __func__, enable);
	}
...
unsigned char rt9536_check_eoc_status(void)
{
	...
	if (eoc_status == 1) {
		pr_debug("[charger_rt9536] :: (%s) eoc_status(%d)\n", __func__, eoc_status);
		return 1;
	}
...
static int charging_ic_probe(struct platform_device *dev)
{
	...
	pr_debug("[charger_rt9536] :: charging IC Initialization is done\n");
...
{
	pr_debug("[charger_rt9536] :: charging_ic_suspend\n");
...
{
	pr_debug("[charger_rt9536] :: charging_ic_resume\n");
...
static int __init charging_ic_init(void)
{
	...
	pr_debug("[charger_rt9536] Charging IC Driver Init\n");

	ret = platform_device_register(&charger_ic_dev);
	if (ret) {
		pr_debug("[charger_rt9536] Unable to device register(%d)\n", ret);
		return ret;
	}
...
{
	pr_debug("[charger_rt9536] Charging IC Driver Exit\n");
