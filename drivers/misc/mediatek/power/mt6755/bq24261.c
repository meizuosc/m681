				    ...
				    kal_uint8 SHIFT)
{
	...
	/* Check bq24261_read_byte(RegNum, &bq24261_reg);
	pr_debug("[bq24261_config_interface] Check Reg[%x]=0x%x\n", RegNum, bq24261_reg);
	*/
...
void bq24261_dump_register(void)
{
	...
	for (i = 0; i < bq24261_REG_NUM; i++) {
		...
		pr_debug("[0x%x]=0x%x ", i, bq24261_reg[i]);
	}
	pr_debug("\n");
}
				    ...
				    const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0) {
		...
		reg_address = kstrtoul(buf, &pvalue, 16); /* reg_address = simple_strtoul(buf, &pvalue, 16); */
...
static struct i2c_board_info i2c_bq24261 __initdata = { I2C_BOARD_INFO("bq24261", (bq24261_SLAVE_ADDR_WRITE >> 1)) };
