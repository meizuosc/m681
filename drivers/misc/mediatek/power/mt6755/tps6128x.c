...
/**********************************************************
  ...
  *
  *********************************************************/
kal_uint32 tps6128x_read_interface(kal_uint8 RegNum, kal_uint8 *val, kal_uint8 MASK,
				   kal_uint8 SHIFT)
{
	...
	pr_debug("[tps6128x_read_interface] Reg[%x]=0x%x\n", RegNum, tps6128x_reg);

	tps6128x_reg &= (MASK << SHIFT);
	*val = (tps6128x_reg >> SHIFT);

	pr_debug("[tps6128x_read_interface] val=0x%x\n", *val);

	return ret;
}

kal_uint32 tps6128x_config_interface(kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK,
				     kal_uint8 SHIFT)
{
	kal_uint8 tps6128x_reg = 0;
	int ret = 0;
	...
	ret = tps6128x_read_byte(RegNum, &tps6128x_reg);
	pr_debug("[tps6128x_config_interface] Reg[%x]=0x%x\n", RegNum, tps6128x_reg);

	tps6128x_reg &= ~(MASK << SHIFT);
	tps6128x_reg |= (val << SHIFT);

	ret = tps6128x_write_byte(RegNum, tps6128x_reg);
	pr_debug("[tps6128x_config_interface] write Reg[%x]=0x%x\n", RegNum, tps6128x_reg);

	/* Check */
	/* tps6128x_read_byte(RegNum, &tps6128x_reg); */
	/* pr_debug("[tps6128x_config_interface] Check Reg[%x]=0x%x\n", RegNum, tps6128x_reg); */
...
void tps6128x_dump_register(void)
{
	...
	for (i = 0; i < tps6128x_REG_NUM; i++) {
		...
		pr_debug("[0x%x]=0x%x ", i, tps6128x_reg[i]);
	}
	pr_debug("\n");
}
...
{
	pr_debug("g_tps6128x_driver_ready=%d\n", g_tps6128x_driver_ready);
...
{
	pr_debug("g_tps6128x_hw_exist=%d\n", g_tps6128x_hw_exist);
...
void tps6128x_hw_component_detect(void)
{
	...
	pr_debug("[tps6128x_hw_component_detect] exist=%d, Reg[0x3]=0x%x\n",
		 g_tps6128x_hw_exist, val);
}

void tps6128x_hw_init(void)
{
	pr_debug("[tps6128x_hw_init] From Johnson\n");
	...
	pr_debug("[tps6128x_hw_init] After HW init\n");
...
static int tps6128x_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	pr_debug("[tps6128x_driver_probe]\n");
	...
	pr_debug("[tps6128x_driver_probe] g_tps6128x_hw_exist=%d, g_tps6128x_driver_ready=%d\n",
		 g_tps6128x_hw_exist, g_tps6128x_driver_ready);

	if (g_tps6128x_hw_exist == 0) {
		pr_debug("[tps6128x_driver_probe] return err\n");
		return err;
	}
...
{
	pr_debug("[show_tps6128x_access] 0x%x\n", g_reg_value_tps6128x);
	return sprintf(buf, "%u\n", g_reg_value_tps6128x);
}

static ssize_t store_tps6128x_access(struct device *dev, struct device_attribute *attr,
				     const char *buf, size_t size)
{
	...
	pr_debug("[store_tps6128x_access]\n");

	if (buf != NULL && size != 0) {
		pr_debug("[store_tps6128x_access] buf is %s and size is %zu\n", buf, size);
		reg_address = simple_strtoul(buf, &pvalue, 16);

		if (size > 3) {
			reg_value = simple_strtoul((pvalue + 1), NULL, 16);
			pr_debug
			    ("[store_tps6128x_access] write tps6128x reg 0x%x with value 0x%x !\n",
			     reg_address, reg_value);
			ret = tps6128x_config_interface(reg_address, reg_value, 0xFF, 0x0);
		} else {
			ret =
			    tps6128x_read_interface(reg_address, &g_reg_value_tps6128x, 0xFF, 0x0);
			pr_debug
			    ("[store_tps6128x_access] read tps6128x reg 0x%x with value 0x%x !\n",
			     reg_address, g_reg_value_tps6128x);
			pr_debug
			    ("[store_tps6128x_access] Please use \"cat tps6128x_access\" to get value\r\n");
		}
	}
	return size;
}
...
static int tps6128x_user_space_probe(struct platform_device *dev)
{
	...
	pr_debug("******** tps6128x_user_space_probe!! ********\n");
...
static struct i2c_board_info i2c_tps6128x __initdata = {
I2C_BOARD_INFO("tps6128x", (tps6128x_SLAVE_ADDR_WRITE >> 1)) };
#endif

static int __init tps6128x_init(void)
{
#if defined(CONFIG_POWER_EXT) || defined(CONFIG_MTK_FPGA) || defined(DISABLE_TPS6128X)
	pr_debug("[tps6128x_init] not support or disable\n");
	g_tps6128x_hw_exist = 0;
	g_tps6128x_driver_ready = 1;

#else
	int ret = 0;
	pr_debug("[tps6128x_init] init start. ch=%d\n", tps6128x_BUSNUM);

	i2c_register_board_info(tps6128x_BUSNUM, &i2c_tps6128x, 1);

	if (i2c_add_driver(&tps6128x_driver) != 0)
		pr_debug("[tps6128x_init] failed to register tps6128x i2c driver.\n");
	else
		pr_debug("[tps6128x_init] Success to register tps6128x i2c driver.\n");


	/* tps6128x user space access interface */
	ret = platform_device_register(&tps6128x_user_space_device);
	if (ret) {
		pr_debug("****[tps6128x_init] Unable to device register(%d)\n", ret);
		return ret;
	}
	ret = platform_driver_register(&tps6128x_user_space_driver);
	if (ret) {
		pr_debug("****[tps6128x_init] Unable to register driver (%d)\n", ret);
		return ret;
	}
#endif
