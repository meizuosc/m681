...
#define LONG_PWRKEY_PRESS_TIME_UNIT     5000UL     /*500ms */
#define LONG_PWRKEY_PRESS_TIME_NS       1000000UL /*500ms */
#endif
...
{
	printk("[pwrkey_int_handler_r] Release pwrkey %d\n",
		pmic_get_register_value(PMIC_PWRKEY_DEB));
#if defined(CONFIG_MTK_KERNEL_POWER_OFF_CHARGING)
	if (g_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT && timer_pre != 0) {
		timer_pos = sched_clock();//ns
		if (timer_pos - timer_pre >=
		    LONG_PWRKEY_PRESS_TIME_UNIT * LONG_PWRKEY_PRESS_TIME_NS) {//5S
...
{
#ifdef VPA_OC
	/*pmic_enable_interrupt(23, 1, "PMIC");*/
#endif
}
...
#ifdef VPA_OC
	pmic_register_interrupt_callback(23, vpa_oc_int_handler);
#endif
