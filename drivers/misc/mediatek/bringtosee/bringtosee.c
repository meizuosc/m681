...
int bts_notify()
{
	...
	input_report_key(cxt->idev, KEY_POWER, 1);
	input_sync(cxt->idev);
	input_report_key(cxt->idev, KEY_POWER, 0);
	input_sync(cxt->idev); 
...
static void bts_early_suspend(struct early_suspend *h) 
{
	...
	resume_enable_status = cxt->is_active_data;
	if(atomic_read(&bts_context_obj->wake)) //not wake up, disable in early suspend
	{
		bts_real_enable(1);
	}
	else //not wake up, disable in early suspend
	{
		bts_real_enable(0);
	}
...
static void bts_late_resume(struct early_suspend *h)
{
	...
	{
		bts_real_enable(resume_enable_status);
	}
