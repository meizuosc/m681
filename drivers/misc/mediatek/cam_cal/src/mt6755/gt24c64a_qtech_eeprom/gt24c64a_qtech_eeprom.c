...
#include "gt24c64a_qtech_eeprom.h"
...
#define CAM_CAL_I2C_BUSNUM_QTECH 2
...
*
********************************************************************************/
#define CAM_CAL_DRVNAME_QTECH "gt24c64a_qtech"
#define CAM_CAL_I2C_GROUP_ID 0
/*******************************************************************************
*
********************************************************************************/
static struct i2c_board_info kd_cam_cal_dev __initdata = {
	I2C_BOARD_INFO(CAM_CAL_DRVNAME_QTECH, 0xAa >> 1)    //a0-a1-aa
};
...
bool selective_read_byte_qtech(u32 addr, BYTE *data, u16 i2c_id)
{
...
int selective_read_region_qtech(u32 addr, BYTE *data, u16 i2c_id, u32 size)
{
	...
	while (size_to_read > 0) {
		if (selective_read_byte_qtech(addr, (u8 *)buff, CATC24C16_QTECH_DEVICE_ID)) {
			addr += 1;
			buff += 1;
			size_to_read -= 1;
			ret += 1;
...
#endif
{
	...
	case CAM_CALIOC_G_READ:
		...
		i4RetValue = selective_read_region_qtech(ptempbuf->u4Offset, pu1Params,
		CATC24C16_QTECH_DEVICE_ID, ptempbuf->u4Length);
...
static inline int RegisterCAM_CALCharDrv(void)
{
	...
	if (alloc_chrdev_region(&g_CAM_CALdevno, 0, 1, CAM_CAL_DRVNAME_QTECH)) {
		CAM_CALDB("[S24CAM_CAL] Allocate device no failed\n");

		return -EAGAIN;
	}
#else
	if (register_chrdev_region(g_CAM_CALdevno , 1 , CAM_CAL_DRVNAME_QTECH)) {
	...
	CAM_CAL_class = class_create(THIS_MODULE, "CAM_CALdrv_QTECH");
	if (IS_ERR(CAM_CAL_class)) {
		int ret = PTR_ERR(CAM_CAL_class);
		CAM_CALDB("Unable to create class, err = %d\n", ret);
		return ret;
	}
	CAM_CAL_device = device_create(CAM_CAL_class, NULL, g_CAM_CALdevno, NULL, CAM_CAL_DRVNAME_QTECH);
...
static const struct i2c_device_id CAM_CAL_i2c_id[] = {{CAM_CAL_DRVNAME_QTECH, 0}, {} };
...
static struct i2c_driver CAM_CAL_i2c_driver = {
	...
	.driver.name = CAM_CAL_DRVNAME_QTECH,
...
static int CAM_CAL_i2c_detect(struct i2c_client *client, int kind, struct i2c_board_info *info)
{
	strcpy(info->type, CAM_CAL_DRVNAME_QTECH);
	return 0;
}
#endif
static int CAM_CAL_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	g_pstI2Cclient->addr = CATC24C16_QTECH_DEVICE_ID >> 1;
...
{
	i2c_register_board_info(CAM_CAL_I2C_BUSNUM_QTECH, &kd_cam_cal_dev, 1);
	printk("come to %s,%d,allenyao\n",__func__,__LINE__);
