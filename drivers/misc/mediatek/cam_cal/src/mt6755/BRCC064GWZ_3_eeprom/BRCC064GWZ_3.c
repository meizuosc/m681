...
int iWriteEEPROM(u16 a_u2Addr  , u32 a_u4Bytes, u8 *puDataInBytes)
{
	...
	if (a_u4Bytes + 2 > 8) {
		EEPROMDB("[BRCC064GWZ_3_eeprom] exceed I2c-mt65xx.c 8 bytes limitation (include address 2 Byte)\n");
		return -1;
	}
...
int iReadDataFromM24C08F(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char *pinputdata)
{
	...
	for (loopCount = 0; loopCount < Read_NUMofEEPROM; loopCount++) {
		do {
			if (16 <= loop[loopCount]) {
				EEPROMDB("[BRCC064GWZ_3_eeprom]1 loopCount=%d loop[loopCount]=%d puSendCmd[0]=%x puSendCmd[1]=%x, EEPROM(%x)\n", loopCount , loop[loopCount], puSendCmd[0], puSendCmd[1], EEPROM_Address[loopCount]);
				...
				EEPROMDB("[BRCC064GWZ_3_eeprom]2 loopCount=%d loop[loopCount]=%d puSendCmd[0]=%x puSendCmd[1]=%x\n", loopCount , loop[loopCount], puSendCmd[0], puSendCmd[1]);
