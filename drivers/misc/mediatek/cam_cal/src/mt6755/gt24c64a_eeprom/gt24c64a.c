...
#define CAM_CAL_I2C_BUSNUM	2	//wangjie, M91
...
*
********************************************************************************/
static struct i2c_board_info __initdata kd_cam_cal_dev={ I2C_BOARD_INFO(CAM_CAL_DRVNAME, GT24C64A_DEVICE_ID>>1) };
...
bool selective_read_byte(u32 addr, BYTE* data, u16 i2c_id)
{
	...
	CAM_CALDB("selective_read_byte addr = 0x%x, data = 0x%x", addr, *data);
	
	return true;
}

int first_lsc_readflag=1;
u32 lscsize=0x5e4;
unsigned lsc_start_addr=0x4b;
unsigned char lscdata[0x5e4];
int selective_read_region(u32 addr, BYTE* data,u16 i2c_id,u32 size)
{
	...
	//CAM_CALDB("selective_read_region addr = 0x%x, size = %d, data read = %d\n", addr, size, ret);
	return ret;
}
...
#endif
{
    ...
    switch(a_u4Command)
    {
        ...
        case CAM_CALIOC_G_READ:
            ...
            i4RetValue = selective_read_region(ptempbuf->u4Offset, pu1Params, GT24C64A_DEVICE_ID, ptempbuf->u4Length);
...
static int CAM_CAL_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id) {
    ...
    g_pstI2Cclient->addr = GT24C64A_DEVICE_ID>>1;
