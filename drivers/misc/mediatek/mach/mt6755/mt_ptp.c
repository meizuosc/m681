...
#else
	#define eem_emerg(fmt, args...)     pr_emerg(KERN_EMERG EEM_TAG KERN_CONT fmt, ##args)
	#define eem_alert(fmt, args...)     pr_alert(KERN_ALERT EEM_TAG KERN_CONT fmt, ##args)
	#define eem_crit(fmt, args...)      pr_crit(KERN_CRIT EEM_TAG KERN_CONT fmt, ##args)
	#define eem_error(fmt, args...)     pr_err(KERN_ERR EEM_TAG KERN_CONT fmt, ##args)
	#define eem_warning(fmt, args...)   pr_warn(KERN_WARNING EEM_TAG KERN_CONT fmt, ##args)
	#define eem_notice(fmt, args...)    pr_notice(KERN_NOTICE EEM_TAG KERN_CONT fmt, ##args)
	#define eem_info(fmt, args...)      pr_info(KERN_NOTICE EEM_TAG KERN_CONT fmt, ##args)
	#define eem_debug(fmt, args...)     pr_debug(KERN_DEBUG EEM_TAG KERN_CONT fmt, ##args)
#endif
...
static inline void handle_mon_mode_isr(struct eem_det *det)
{
	...
	*/
	...
	if (((det->t250 & 0xff)  > 0x4b) && ((det->t250  & 0xff) < 0xd3)) {
		eem_isr_info("thermal sensor init has not been completed. "
			     "(temp = 0x%08X)\n", det->t250);
		goto out;
	}
	...
	for_each_det_ctrl(det, ctrl) {
		{
			...
			eem_debug(KERN_ERR "%s, vboot = %d, VBOOT = %d\n",
