...
static fh_pll_t g_fh_pll[FH_PLL_NUM] = {
	{FH_FH_ENABLE_SSC,	FH_PLL_ENABLE,		0,	ARMCA15PLL_DEF_FREQ,	0}, /* Enable SSC for RF desense */
	{FH_FH_ENABLE_SSC,	FH_PLL_ENABLE,		0,	ARMCA7PLL_DEF_FREQ,		0}, /* Enable SSC for RF desense */
	{FH_FH_DISABLE,		FH_PLL_ENABLE,		0,	MAINPLL_DEF_FREQ,		0},
	{FH_FH_DISABLE,		FH_PLL_ENABLE,		0,	MEMPLL_DEF_FREQ,		0},
	{FH_FH_DISABLE,		FH_PLL_ENABLE,		0,	MMPLL_DEF_FREQ,			0},	/* MMPLL SSC causes GPU perf fail */
	...
	{FH_FH_DISABLE,		FH_PLL_ENABLE,		0,	TVDPLL_DEF_FREQ,		0}	/* TVDPLL SSC causes MHL fail */
};
...
static void mt_fh_hal_popod_save(void)
{
	...
	if (g_fh_pll[pll_id].fh_status == FH_FH_ENABLE_SSC) {
		...
		fh_write32(g_reg_pll_con1[pll_id],
					(fh_read32(g_reg_dds[pll_id])&MASK21b)|
					(fh_read32(MAINPLL_CON1)&0xFFE00000)|(BIT32));
...
static int fh_dumpregs_proc_read(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "\r\n APMIXEDSYS RSV_RW0_CON 0x%08x RSV_RW1_CON 0x%08x\r\n", fh_read32((unsigned long) g_apmixed_base+0x900), fh_read32((unsigned long) g_apmixed_base+0x904));
...
static int __fh_debug_proc_read(struct seq_file* m, void* v, fh_pll_t * pll)
{
	...
	seq_printf(m, "===============================================\r\n");
	seq_printf(m, "id=ARMCA15PLL=ARMCA7PLL=MAINPLL=MEMPLL=MMPLL=MPLL=VENCPLL=MSDCPLL=TVDPLL\r\n");
