...
*                     - stats_dump_enabled
***********************************************************/
PROC_FOPS_RW_UNSIGNED_INT_FULL_FUNC(enabled, hps_ctxt.enabled, mutex_lock(&hps_ctxt.lock); ,
				    hps_ctxt_reset_stas_nolock(); mutex_unlock(&hps_ctxt.lock););
PROC_FOPS_RW_UNSIGNED_INT_FULL_FUNC(early_suspend_enabled, hps_ctxt.early_suspend_enabled,
				    mutex_lock(&hps_ctxt.lock); , mutex_unlock(&hps_ctxt.lock););
PROC_FOPS_RW_UNSIGNED_INT_FULL_FUNC(suspend_enabled, hps_ctxt.suspend_enabled,
				    mutex_lock(&hps_ctxt.lock); , mutex_unlock(&hps_ctxt.lock););
PROC_FOPS_RW_UNSIGNED_INT_FULL_FUNC(cur_dump_enabled, hps_ctxt.cur_dump_enabled,
				    mutex_lock(&hps_ctxt.lock); , mutex_unlock(&hps_ctxt.lock););
PROC_FOPS_RW_UNSIGNED_INT_FULL_FUNC(stats_dump_enabled, hps_ctxt.stats_dump_enabled,
				    mutex_lock(&hps_ctxt.lock); , mutex_unlock(&hps_ctxt.lock););
...
*                     - tlp_times
***********************************************************/
PROC_FOPS_RW_UNSIGNED_INT_FULL_FUNC(up_threshold, hps_ctxt.up_threshold,
				    mutex_lock(&hps_ctxt.lock); , hps_ctxt_reset_stas_nolock();
				    mutex_unlock(&hps_ctxt.lock););
PROC_FOPS_RW_UNSIGNED_INT_FULL_FUNC(up_times, hps_ctxt.up_times, mutex_lock(&hps_ctxt.lock); ,
				    hps_ctxt_reset_stas_nolock(); mutex_unlock(&hps_ctxt.lock););
PROC_FOPS_RW_UNSIGNED_INT_FULL_FUNC(down_threshold, hps_ctxt.down_threshold,
				    mutex_lock(&hps_ctxt.lock); , hps_ctxt_reset_stas_nolock();
				    mutex_unlock(&hps_ctxt.lock););
PROC_FOPS_RW_UNSIGNED_INT_FULL_FUNC(down_times, hps_ctxt.down_times, mutex_lock(&hps_ctxt.lock); ,
				    hps_ctxt_reset_stas_nolock(); mutex_unlock(&hps_ctxt.lock););
PROC_FOPS_RW_UNSIGNED_INT_FULL_FUNC(input_boost_enabled, hps_ctxt.input_boost_enabled,
				    mutex_lock(&hps_ctxt.lock); , mutex_unlock(&hps_ctxt.lock););
PROC_FOPS_RW_UNSIGNED_INT_FULL_FUNC(input_boost_cpu_num, hps_ctxt.input_boost_cpu_num,
				    mutex_lock(&hps_ctxt.lock); , mutex_unlock(&hps_ctxt.lock););
PROC_FOPS_RW_UNSIGNED_INT_FULL_FUNC(rush_boost_enabled, hps_ctxt.rush_boost_enabled,
				    mutex_lock(&hps_ctxt.lock); , mutex_unlock(&hps_ctxt.lock););
PROC_FOPS_RW_UNSIGNED_INT_FULL_FUNC(rush_boost_threshold, hps_ctxt.rush_boost_threshold,
				    mutex_lock(&hps_ctxt.lock); , hps_ctxt_reset_stas_nolock();
				    mutex_unlock(&hps_ctxt.lock););
PROC_FOPS_RW_UNSIGNED_INT_FULL_FUNC(rush_boost_times, hps_ctxt.rush_boost_times,
				    mutex_lock(&hps_ctxt.lock); , hps_ctxt_reset_stas_nolock();
				    mutex_unlock(&hps_ctxt.lock););
PROC_FOPS_RW_UNSIGNED_INT_FULL_FUNC(tlp_times, hps_ctxt.tlp_times, mutex_lock(&hps_ctxt.lock); ,
				    hps_ctxt_reset_stas_nolock(); mutex_unlock(&hps_ctxt.lock););
PROC_FOPS_RW_UNSIGNED_INT_FULL_FUNC(power_mode, hps_ctxt.power_mode, mutex_lock(&hps_ctxt.lock); ,
				    hps_ctxt_reset_stas_nolock(); mutex_unlock(&hps_ctxt.lock););
PROC_FOPS_RW_UNSIGNED_INT_FULL_FUNC(ppm_power_mode, hps_ctxt.ppm_power_mode,
				    mutex_lock(&hps_ctxt.lock); , hps_ctxt_reset_stas_nolock();
				    mutex_unlock(&hps_ctxt.lock););
...
static int hps_num_base_perf_serv_proc_write(struct file *file, const char __user *buffer,
...
static int hps_num_limit_thermal_proc_write(struct file *file, const char __user *buffer,
					    size_t count, loff_t *pos)
{
...
static int hps_num_limit_low_battery_proc_write(struct file *file, const char __user *buffer,
						size_t count, loff_t *pos)
{
	...
	if ((hps_ctxt.is_hmp || hps_ctxt.is_amp)
	    && (sscanf(desc, "%u %u", &little_num_limit_low_battery, &big_num_limit_low_battery) ==
		2)) {
		...
		return count;
		   ...
		   && (sscanf(desc, "%u", &little_num_limit_low_battery) == 1)) {
		if (little_num_limit_low_battery > num_possible_little_cpus()
		    || little_num_limit_low_battery < 1) {
...
static int hps_num_limit_ultra_power_saving_proc_write(struct file *file,
						       ...
						       loff_t *pos)
{
	...
	if ((hps_ctxt.is_hmp || hps_ctxt.is_amp)
	    &&
	    (sscanf
	     (desc, "%u %u", &little_num_limit_ultra_power_saving,
		...
		return count;
		   ...
		   && (sscanf(desc, "%u", &little_num_limit_ultra_power_saving) == 1)) {
		if (little_num_limit_ultra_power_saving > num_possible_little_cpus()
		    || little_num_limit_ultra_power_saving < 1) {
...
static int hps_num_limit_power_serv_proc_write(struct file *file, const char __user *buffer,
					       size_t count, loff_t *pos)
{
	...
	if ((hps_ctxt.is_hmp || hps_ctxt.is_amp)
	    && (sscanf(desc, "%u %u", &little_num_limit_power_serv, &big_num_limit_power_serv) ==
		2)) {
		...
		return count;
		   ...
		   && (sscanf(desc, "%u", &little_num_limit_power_serv) == 1)) {
		if (little_num_limit_power_serv > num_possible_little_cpus()
		    || little_num_limit_power_serv < 1) {
...
static int hps_test0_proc_write(struct file *file, const char __user *buffer, size_t count,
				loff_t *pos)
{
	int len = 0, test0 = 0;
	char desc[32];
	...
	if (sscanf(desc, "%u", &test0) == 1) {
		...
		case 3:
			...
			hps_warn("MP0_CPU0_PWR_CON: 0x%08x\n", REG_READ(MP0_CPU0_PWR_CON));
			hps_warn("MP0_CPU1_PWR_CON: 0x%08x\n", REG_READ(MP0_CPU1_PWR_CON));
			hps_warn("MP0_CPU2_PWR_CON: 0x%08x\n", REG_READ(MP0_CPU2_PWR_CON));
			hps_warn("MP0_CPU3_PWR_CON: 0x%08x\n", REG_READ(MP0_CPU3_PWR_CON));
			hps_warn("MP0_CPUTOP_PWR_CON: 0x%08x\n", REG_READ(MP0_CPUTOP_PWR_CON));
			hps_warn("MP0_CPU0_L1_PDN: 0x%08x\n", REG_READ(MP0_CPU0_L1_PDN));
			hps_warn("MP0_CPU1_L1_PDN: 0x%08x\n", REG_READ(MP0_CPU1_L1_PDN));
			hps_warn("MP0_CPU2_L1_PDN: 0x%08x\n", REG_READ(MP0_CPU2_L1_PDN));
			hps_warn("MP0_CPU3_L1_PDN: 0x%08x\n", REG_READ(MP0_CPU3_L1_PDN));
			hps_warn("MP0_CPUTOP_L2_PDN: 0x%08x\n", REG_READ(MP0_CPUTOP_L2_PDN));
			hps_warn("MP1_CPU0_PWR_CON: 0x%08x\n", REG_READ(MP1_CPU0_PWR_CON));
			hps_warn("MP1_CPU1_PWR_CON: 0x%08x\n", REG_READ(MP1_CPU1_PWR_CON));
			hps_warn("MP1_CPU2_PWR_CON: 0x%08x\n", REG_READ(MP1_CPU2_PWR_CON));
			hps_warn("MP1_CPU3_PWR_CON: 0x%08x\n", REG_READ(MP1_CPU3_PWR_CON));
			hps_warn("MP1_CPUTOP_PWR_CON: 0x%08x\n", REG_READ(MP1_CPUTOP_PWR_CON));
			hps_warn("MP1_CPU0_L1_PDN: 0x%08x\n", REG_READ(MP1_CPU0_L1_PDN));
			hps_warn("MP1_CPU1_L1_PDN: 0x%08x\n", REG_READ(MP1_CPU1_L1_PDN));
			hps_warn("MP1_CPU2_L1_PDN: 0x%08x\n", REG_READ(MP1_CPU2_L1_PDN));
			hps_warn("MP1_CPU3_L1_PDN: 0x%08x\n", REG_READ(MP1_CPU3_L1_PDN));
			hps_warn("MP1_CPUTOP_L2_PDN: 0x%08x\n", REG_READ(MP1_CPUTOP_L2_PDN));
			break;
		case 4:
...
static int hps_test1_proc_write(struct file *file, const char __user *buffer, size_t count,
				loff_t *pos)
{
	int len = 0, test1 = 0;
	char desc[32];
...
static int hps_memory_debug_proc_write(struct file *file, const char __user *buffer, size_t count,
				       loff_t *pos)
{
	int len = 0;
	char desc[32];
...
int hps_procfs_init(void)
{
	...
	if (hps_dir == NULL) {
		...
		for (i = 0; i < ARRAY_SIZE(entries); i++) {
			if (!proc_create
			    (entries[i].name, S_IRUGO | S_IWUSR | S_IWGRP, hps_dir,
			     entries[i].fops))
				hps_emerg("create /proc/cpufreq/%s failed\n", entries[i].name);
		}
	}

	g_memory_debug = MP1_CPUTOP_PWR_CON;
