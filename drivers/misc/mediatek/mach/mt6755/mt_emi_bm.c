...
void BM_Enable(const unsigned int enable)
{
	...
	mt_reg_sync_writel((value & ~(BUS_MON_PAUSE | BUS_MON_EN)) | (enable ? BUS_MON_EN : 0),
			   EMI_BMEN);
}
			 ...
			 const unsigned int trans_type)
{
	...
	if (counter_num == 1) {
		...
		value =
		    (readl(IOMEM(addr)) & ~(iMask << 16)) | ((trans_type & 0xFF) << 24) |
		    ((master & 0xFF) << 16);
	} else {
...
int BM_SetIDSelect(const unsigned int counter_num, const unsigned int id, const unsigned int enable)
{
	...
	value =
	    (readl(IOMEM(EMI_BMEN2)) & ~(1 << (counter_num - 1))) | (enable << (counter_num - 1));
...
int BM_SetUltraHighFilter(const unsigned int counter_num, const unsigned int enable)
{
	...
	value =
	    (readl(IOMEM(EMI_BMEN1)) & ~(1 << (counter_num - 1))) | (enable << (counter_num - 1));
...
int BM_SetLatencyCounter(void)
{
	...
	and emi_ttype9 -- emi_ttype15 change as total transaction counts for m0 -- m6 */
	value |= (0b10 << 24);
	mt_reg_sync_writel(value, EMI_BMEN2);
	return BM_REQ_OK;
}
