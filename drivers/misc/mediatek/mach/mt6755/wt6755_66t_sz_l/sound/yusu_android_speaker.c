...
#define PRINTK_YUSU_SPK(format, args...) pr_debug(format, ##args)
#else
#define PRINTK_YUSU_SPK(format, args...)
#endif
...
{
	PRINTK_YUSU_SPK("+Speaker_Init Success");
...
{
	PRINTK_YUSU_SPK(" Sound_SpeakerL_SetVolLevel level=%d\n", level);
}

void Sound_SpeakerR_SetVolLevel(int level)
{
	PRINTK_YUSU_SPK(" Sound_SpeakerR_SetVolLevel level=%d\n", level);
}

void Sound_Speaker_Turnon(int channel)
{
	PRINTK_YUSU_SPK("Sound_Speaker_Turnon channel = %d\n", channel);
	if (gsk_on)
		return;
...
{
	PRINTK_YUSU_SPK("Sound_Speaker_Turnoff channel = %d\n", channel);
	if(!gsk_on)
		return;
#if defined(AMP_CLASS_AB)
...
{
	PRINTK_YUSU_SPK("AudioDevice_Suspend\n");
	if(gsk_on)
	{
...
{
	PRINTK_YUSU_SPK("AudioDevice_Resume\n");
	if(gsk_resume)
...
{
	PRINTK_YUSU_SPK("AudioDevice_SpeakerLouderOpen\n");
	gsk_forceon = false;
	if(gsk_on)
		return;
	Sound_Speaker_Turnon(Channel_Stereo);
...
{
	PRINTK_YUSU_SPK("AudioDevice_SpeakerLouderClose\n");
...
{
	PRINTK_YUSU_SPK("AudioDevice_mute\n");
	if(gsk_on)
		Sound_Speaker_Turnoff(Channel_Stereo);
}
...
kal_int32 Sound_ExtFunction(const char* name, void* param, int param_size)
{
	...
	while (strcmp("End", ExtFunArray[i]) != 0) {
		/* while function not equal to "End" */
		if (strcmp(name, ExtFunArray[i]) == 0) {
			/* When function name equal to table, break */
			funNum = i;
			break;
		}
		i++;
