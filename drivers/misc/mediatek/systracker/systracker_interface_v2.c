...
int tracker_dump(char *buf)
{
...

	{
		...
		ptr += sprintf(ptr, "[TRACKER] BUS_DBG_CON = (0x%x, 0x%x), T0= 0x%x, T1 = 0x%x\n",
			track_entry.dbg_con, readl(IOMEM(BUS_DBG_CON)), readl(IOMEM(BUS_DBG_TIMER_CON0)), readl(IOMEM(BUS_DBG_TIMER_CON1)));
		for (i = 0; i < BUS_DBG_NUM_TRACKER; i++) {
			entry_address       = track_entry.ar_track_l[i];
			reg_value           = track_entry.ar_track_h[i];
			entry_valid         = extract_n2mbits(reg_value, 19, 19);
			...
			ptr += sprintf(ptr, "read entry = %d, valid = 0x%x, tid = 0x%x, read id = 0x%x,\
					address = 0x%x, data_size = 0x%x, burst_length = 0x%x\n",
					i,
					entry_valid,
					entry_tid,
					entry_id,
		...
		for (i = 0; i < BUS_DBG_NUM_TRACKER; i++) {
			...
			ptr += sprintf(ptr, "write entry = %d, valid = 0x%x, tid = 0x%x, read id = 0x%x,\
					address = 0x%x, data_size = 0x%x, burst_length = 0x%x\n",
					i,
					entry_valid,
					entry_tid,
...
void systracker_read_timeout_test(void)
{
	...
	if (mt_systracker_drv.systracker_read_timeout_test)
		...
		pr_err("mt_systracker_drv.systracker_read_timeout_test is NULL");
}

void systracker_write_timeout_test(void)
{
	pr_debug("we are going to have write timeout\n");
...
void systracker_timeout_withrecord_test(void)
{
	...
	if (mt_systracker_drv.systracker_withrecord_test)
		...
		pr_err("mt_systracker_drv.systracker_withrecord_test is NULL");
}
...
void systracker_notimeout_test(void)
{
	...
	if (mt_systracker_drv.systracker_notimeout_test)
		...
		pr_err("mt_systracker_drv.systracker_notimeout_test is NULL");
}
