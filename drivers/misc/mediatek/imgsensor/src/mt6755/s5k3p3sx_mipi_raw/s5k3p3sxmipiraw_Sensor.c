		...
		do {
			*sensor_id = ((read_cmos_sensor_8(0x0000) << 8) | read_cmos_sensor_8(0x0001));
			...
			if (*sensor_id ==imgsensor_info.sensor_id) {
				...
				RWB_Flag = (read_cmos_sensor_8(0x000D) >> 4);
				LOG_INF("RWB_Flag: 0x%x\n", RWB_Flag);
				if((RWB_Flag==0x2) || (RWB_Flag==0x3)){//0: RGB no pdaf; 1: RGB pdaf; 2: RWB no pdaf; 3: RWB  pdaf
					imgsensor_info.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_RWB_Wr;
					LOG_INF("RWB sensor of S5k3p3\n");
				}
				else{
					LOG_INF("RGB sensor of S5k3p3\n");
				}
				return ERROR_NONE;
			}
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			LOG_INF("Read sensor id fail, id: %x %x\n", imgsensor.i2c_write_id,sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
