...
static kal_uint32 OV5670_capture_shutter = 1;
static kal_uint32 OV5670_longexp_linelength = 1676;
static kal_uint16 OV5670_capture_gain = 64;
static kal_uint8 OV5670_long_exp_flag = 0;
...
#else
	.pre = {
		.pclk = 102857000,				//record different mode's pclk
		.linelength = 1676,				//record different mode's linelength
		.framelength = 2045,			//record different mode's framelength
		.startx = 1,					//record different mode's startx of grabwindow
		.starty = 0,					//record different mode's starty of grabwindow
...
static int check_otp_info(int index)
{
	...
	printk("-otp- OV5670 check_otp_info flag=%x\n",flag);
	//select group
	if(index==1)
	{
		flag=(flag>>6)&0x03;
	...
	printk("-otp- OV5670 check_otp_info flag1=%x\n",flag);
...
static int check_otp_wb(int index)
{
	...
	printk("-otp- OV5670 check_otp_wb flag=%x\n",flag);	
	//select group
	if(index==1)
	{
		flag=(flag>>6)&0x03;
...
static int read_otp_wb(int index,struct otp_struct *otp_ptr)
{
	...
	printk("-otp- read_otp_wb rg_ratio=%x\n",(*otp_ptr).rg_ratio);
	(*otp_ptr).bg_ratio=(read_cmos_sensor(start_addr+1)<<2)+((temp>>4)&0x03);
	printk("-otp- read_otp_wb bg_ratio=%x\n",(*otp_ptr).bg_ratio);
...
static int update_wb_gain(int R_gain,int B_gain,int G_gain)
{
	if(R_gain>0x400)
	{
	...
	printk("-otp- update_wb_gain R_gain=%x,G_gain=%x,B_gain=%x\n",R_gain,G_gain,B_gain);
	
	return 0;
}

static int update_otp_info(int *module_id)
{
	...
	if(NULL != module_id) {
		*module_id=current_otp.module_integrator_id;
		printk("-otp- update_otp_info module_id=%x\n",*module_id);
	}
...
static int update_otp_wb()
{
	...
	update_wb_gain(R_Gain,B_Gain,G_Gain);
	...
	write_cmos_sensor(0x3500, (shutter >> 12) & 0x0F);
       ...
       if(shutter < imgsensor_info.max_frame_length - imgsensor_info.margin) {
        	...
        	if (imgsensor.autoflicker_en) { 
              ...
              if(shutter == 4 && imgsensor.sensor_mode == IMGSENSOR_MODE_CAPTURE) {
                    LOG_INF("long exp restore");
                    write_cmos_sensor(0x0100, 0x00); 
                    write_cmos_sensor(0x3019, 0x01);
                    write_cmos_sensor(0x3502, (shutter << 4) & 0xF0);
                    write_cmos_sensor(0x3501, (shutter >> 4) & 0xFF);
                    write_cmos_sensor(0x3500, (shutter >> 12) & 0x0F);
            ...
            if(OV5670_longexp_linelength > 0) {
                line_length = OV5670_longexp_linelength;
            } else {
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if ((imgsensor.sensor_mode == IMGSENSOR_MODE_CAPTURE)
	    && (!OV5670_long_exp_flag) && mode_change) {
		write_cmos_sensor(0x0100,0x01);
		mode_change = 0;
	}
...
static void preview_setting(void)
{
	...
	write_cmos_sensor(0x400a, 0x02);
	write_cmos_sensor(0x400b, 0x00);
	write_cmos_sensor(0x4502, 0x44);//wangjie, 0x48
	write_cmos_sensor(0x4508, 0x55);
	write_cmos_sensor(0x4509, 0x55);
	write_cmos_sensor(0x450a, 0x00);
	write_cmos_sensor(0x4600, 0x00);
...
static void longexp_setting(kal_uint32 shutter,kal_uint16 gain)
{
    kal_uint32 line_length = 0,frame_length = 0;

    //use shutter to count line_length
    if(shutter < (imgsensor_info.max_frame_length - imgsensor_info.margin)) {
        if (shutter > imgsensor.min_frame_length - imgsensor_info.margin) {
        	frame_length = shutter + imgsensor_info.margin;
        } else {
    ...
    set_gain(gain);
    LOG_INF("long exp setting sensor mode = %d,shutter = %d,line_length = %d,frame_length = %d,gain = %d",
            imgsensor.sensor_mode,shutter,line_length,frame_length,gain);
}
	...
	if (currefps == 240) { //24fps for PIP
		...
		write_cmos_sensor(0x0100, 0x00); 
              if(OV5670_long_exp_flag) {
                longexp_setting(OV5670_capture_shutter, OV5670_capture_gain);
              } else {
                write_cmos_sensor(0x3501, 0x5f); //long exposure
                write_cmos_sensor(0x3502, 0xd0);  //long exposure
...
static void hd_1080p_video_setting(void)
{
  write_cmos_sensor(0x0100, 0x00);
...
//	update_otp_info(NULL);
	...
	OV5670_long_exp_flag = 0;
	OV5670_capture_shutter = 1;
	OV5670_capture_gain = 64;
	OV5670_longexp_linelength = 1676;
	spin_unlock(&imgsensor_drv_lock);
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
			...
			break;
		//add for longexp, wangjie
            case SENSOR_FEATURE_SET_LONGEXP_PARAM:
                OV5670_capture_shutter = *feature_data;
                OV5670_longexp_linelength = *(feature_data + 1);
                OV5670_capture_gain = (*(feature_data + 2)) & 0xffff;
                OV5670_long_exp_flag = 1;
                LOG_INF("SENSOR_FEATURE_SET_LONGEXP_PARAM shutter=%d, linelength = %d,Gain=%d\n",
                    OV5670_capture_shutter,OV5670_longexp_linelength,OV5670_capture_gain);
                break;

            case SENSOR_FEATURE_GET_CAP_PIXEL_CLOCK_FREQ:
                *feature_return_para_32 = imgsensor_info.cap.pclk;
                *feature_para_len=4;
                break;
            case SENSOR_FEATURE_GET_CAP_PERIOD:
                *feature_return_para_16++ = imgsensor_info.cap.linelength;
                *feature_return_para_16 = imgsensor_info.cap.framelength;
                *feature_para_len=4;
                break;
            case SENSOR_FEATURE_GET_MAXFRAMELENGTH_MARGIN:
                *feature_return_para_32++ = imgsensor_info.max_frame_length;
                *feature_return_para_32 = imgsensor_info.margin;
                *feature_para_len = 8;
                break;
            case SENSOR_FEATURE_CANCEL_LONGEXP:
                OV5670_long_exp_flag = 0;
                LOG_INF("SENSOR_FEATURE_CANCEL_LONGEXP flag=%d \n",OV5670_long_exp_flag);
                break;
		//add end
		default:
			break;
