...
#define S5K5E8_OTP_ENABLE	//wangjie, otp function
static int first_flag = 1;
static DEFINE_SPINLOCK(imgsensor_drv_lock);

#define shutter1 20000
#define shutter2 40000
#define shutter3 50000		
#define shutter4 60000
#define shutter6 80000
#define shutter8 100000	
...
static imgsensor_info_struct imgsensor_info = { 
	...
	.mipi_settle_delay_mode = MIPI_SETTLEDELAY_AUTO,//0,MIPI_SETTLEDELAY_AUTO; 1,MIPI_SETTLEDELAY_MANNUAL	//MTK, 2016.1.8
...
struct s5k5e8_otp_struct
{
    ...
    kal_uint8 vid;	//vcm id
    kal_uint8 lct;	//light color temperature
	
    kal_uint8 u_r_h;	//unit awb
    kal_uint8 u_r_l;
    kal_uint8 u_b_h;
    kal_uint8 u_b_l;
    kal_uint8 u_g_h;
...
**************************************************************************************************/
void s5k5e8_start_read_otp(u8 zone)
{
...
**************************************************************************************************/
void s5k5e8_stop_read_otp(void)
{
   	write_cmos_sensor(0x0A00, 0x04);//make initial state
...
/*************************************************************************************************
* Function    :  s5k5e8_check_otp_group_flag
* Description :  before read otp , set the reading block setting  
* Parameters  :  [BYTE] zone : OTP PAGE index , 0x00~0x0f
* Return      :  0, reading block setting err
                 1, reading block setting ok 
**************************************************************************************************/
int s5k5e8_check_otp_group_flag(u8 group)
{
	u8 val = 0;

	LOG_INF("s5k5e8_check_otp_group group = %d.\n",group);
	
	s5k5e8_start_read_otp(4);

	if(group == 2)
	{
		val = read_cmos_sensor(0x0A1B);
		LOG_INF("s5k5e8 otp group 2 flag = 0x%02x.\n",val);
	}
	else if(group == 1)
	{
		val = read_cmos_sensor(0x0A04);
		LOG_INF("s5k5e8 otp group 1 flag = 0x%02x.\n",val);
	}
...
bool s5k5e8_read_otp_wb(struct s5k5e8_otp_struct *otp)
{
	u8 group_id = 0;	
	U16 basic_address = 0x0A05;
	U16 temp_address = 0x0A05;
	
	if(0x01 == s5k5e8_check_otp_group_flag(2))
	{
		group_id = 2;
	}
	else if(0x01 == s5k5e8_check_otp_group_flag(1))
	{
		group_id = 1;
	}
	LOG_INF("s5k5e8 otp group_id = %d.\n",group_id);

	if(group_id != 0)
	{
		s5k5e8_start_read_otp(4);//page 4

		temp_address = basic_address + (group_id - 1) * 23;//0x0A05 or 0x0A1C
		...
		otp->vid = read_cmos_sensor(temp_address++);
		otp->lct = read_cmos_sensor(temp_address++);
		LOG_INF("mid=%d, year=%d, month=%d, day=%d.\n",otp->mid,otp->year,otp->month,otp->day);

		temp_address++;
		LOG_INF("OB: %d\n",read_cmos_sensor(temp_address++));
		
		otp->u_r_h = read_cmos_sensor(temp_address++);
		otp->u_r_l = read_cmos_sensor(temp_address++);
		otp->u_b_h = read_cmos_sensor(temp_address++);
		otp->u_b_l = read_cmos_sensor(temp_address++);
		otp->u_g_h = read_cmos_sensor(temp_address++);
		otp->u_g_l = read_cmos_sensor(temp_address++);
/*
		temp_address++;
		
		otp->g_r_h = read_cmos_sensor(temp_address++);
		otp->g_r_l = read_cmos_sensor(temp_address++);
		otp->g_b_h = read_cmos_sensor(temp_address++);
		otp->g_b_l = read_cmos_sensor(temp_address++);
		otp->g_g_h = read_cmos_sensor(temp_address++);
		otp->g_g_l = read_cmos_sensor(temp_address++);
*/
		s5k5e8_stop_read_otp();
		return 1;
	}
...
{
	kal_uint32 unit_r, unit_g, unit_b;
	kal_uint32 golden_r, golden_g, golden_b;
	kal_uint32 G_gain_R, G_gain_B;
	kal_uint32 R_to_G, B_to_G, G_to_G;
	kal_uint32 R_Gain, B_Gain, G_Gain;
	kal_uint32 Golden_RG, Golden_BG;
	
	unit_r = (otp->u_r_h << 8)|(otp->u_r_l);
	unit_g = (otp->u_g_h << 8)|(otp->u_g_l);
	unit_b = (otp->u_b_h << 8)|(otp->u_b_l);

	golden_r = 0x26c;//fix value
	golden_b = 0x23a;//fix value

	LOG_INF("unit_r=0x%x, unit_g=0x%x, unit_b=0x%x, golden_r=0x%x, golden_b=0x%x.\n"
            ,unit_r
            ,unit_g
            ,unit_b
            ,golden_r
	...
	Golden_RG=(golden_r/2);
	Golden_BG=(golden_b/2);
...
{
	kal_uint8 R_GainH, B_GainH, G_GainH;
	kal_uint8 R_GainL, B_GainL, G_GainL;
	kal_uint16 temp;
...
//add by allenyao
kal_uint8  s5k5e8_u_r_h=0,s5k5e8_u_r_l=0,s5k5e8_u_b_h=0,s5k5e8_u_b_l=0,s5k5e8_u_g_h=0,s5k5e8_u_g_l=0;
kal_uint8  s5k5e8_g_r_h=0,s5k5e8_g_r_l=0,s5k5e8_g_b_h=0,s5k5e8_g_b_l=0,s5k5e8_g_g_h=0,s5k5e8_g_g_l=0;
kal_uint8  s5k5e8_moudu_id=0;
//end
void s5k5e8_read_static_register_from_otp(void)
{
	memset(&current_5e8_otp, 0, sizeof(struct s5k5e8_otp_struct));
	
	if (s5k5e8_read_otp_wb(&current_5e8_otp)) {
		s5k5e8_algorithm_otp_wb1(&current_5e8_otp);
	}
	s5k5e8_u_r_h=current_5e8_otp.u_r_h;
	s5k5e8_u_r_l=current_5e8_otp.u_r_l;
	s5k5e8_u_b_h=current_5e8_otp.u_b_h;
	s5k5e8_u_b_l=current_5e8_otp.u_b_l;
	s5k5e8_u_g_h=current_5e8_otp.u_g_h;
	s5k5e8_u_g_l=current_5e8_otp.u_g_l;
	
	s5k5e8_g_r_h=current_5e8_otp.g_r_h;
	s5k5e8_g_r_l=current_5e8_otp.g_r_l;
	s5k5e8_g_b_h=current_5e8_otp.g_b_h;
	s5k5e8_g_b_l=current_5e8_otp.g_b_l;
	s5k5e8_g_g_h=current_5e8_otp.g_g_h;
	s5k5e8_g_g_l=current_5e8_otp.g_g_l;

	s5k5e8_moudu_id=current_5e8_otp.mid;
}

int s5k5e8_get_module_id(void)
{	
	return current_5e8_otp.mid;
}
...
/*************************************************************************************************
* Function    :  s5k5e8_otp_lsc_update
* Description :  Update lens correction            
**************************************************************************************************/
static void s5k5e8_otp_lsc_update(void)
{
	if(0x01 == s5k5e8_get_otp_LSC_flag(4))
	{
		//LSC correct
		write_cmos_sensor(0x0B00,0x01);
		write_cmos_sensor(0x3400,0x00);//0x00:lsc on, 0x01:lsc off
		LOG_INF("LSC Auto Correct OK!\n");
	}
	else
	{
		LOG_INF("LSC Auto Correct FAIL!\n");
	}
}
#endif
...
static void set_shutter(kal_uint16 shutter)
{
	...
	case shutter1:			   
		   ...
		   write_cmos_sensor(0x0340,0x2D);
		   write_cmos_sensor(0x0341,0xF9);
		   write_cmos_sensor(0x0342,0x0B);   
		   write_cmos_sensor(0x0343,0x28); 
		   write_cmos_sensor(0x0202,0x2D); 
		...
		case shutter2:			   
		   ...
		   write_cmos_sensor(0x0100,0x00);
		   write_cmos_sensor(0x0340,0x5B);
		   write_cmos_sensor(0x0341,0xEE);
		   write_cmos_sensor(0x0342,0x0B);   
		   write_cmos_sensor(0x0343,0x28); 
		   write_cmos_sensor(0x0202,0x5B); 
		   write_cmos_sensor(0x0203,0xE9); 
		   write_cmos_sensor(0x3C16,0x00); 
		   write_cmos_sensor(0x3C0D,0x04); 
		   write_cmos_sensor(0x0100,0x01); 
		   write_cmos_sensor(0x3C22,0x00); 
		...
		case shutter3: 
		   ...
		   long_shutter_flag = 1;
		   write_cmos_sensor(0x0100,0x00);
		   write_cmos_sensor(0x0340,0x72);
		   write_cmos_sensor(0x0341,0xE8);
		   write_cmos_sensor(0x0342,0x0B);   
		   write_cmos_sensor(0x0343,0x28); 
		   write_cmos_sensor(0x0202,0x72); 
		   write_cmos_sensor(0x0203,0xE3); 
		   write_cmos_sensor(0x3C16,0x00); 
		   write_cmos_sensor(0x3C0D,0x04); 
		   write_cmos_sensor(0x0100,0x01); 
		   write_cmos_sensor(0x3C22,0x00); 
		...
		case shutter4: 
		   ...
		   long_shutter_flag = 1;
		   write_cmos_sensor(0x0100,0x00);
		   write_cmos_sensor(0x0340,0x89);
		   write_cmos_sensor(0x0341,0xE3);
		   write_cmos_sensor(0x0342,0x0B);   
		   write_cmos_sensor(0x0343,0x28); 
		   write_cmos_sensor(0x0202,0x89); 
		 ...
		 case shutter6:	
			...
			long_shutter_flag = 1;
			write_cmos_sensor(0x0100,0x00);
			write_cmos_sensor(0x0340,0xB7);
			write_cmos_sensor(0x0341,0xD7);
			write_cmos_sensor(0x0342,0x0B);   
			write_cmos_sensor(0x0343,0x28); 
			write_cmos_sensor(0x0202,0xB7); 
			write_cmos_sensor(0x0203,0xD2); 
			write_cmos_sensor(0x3C16,0x00); 
			write_cmos_sensor(0x3C0D,0x04); 
			write_cmos_sensor(0x0100,0x01); 
			write_cmos_sensor(0x3C22,0x00); 
		 ...
		 case shutter8:	
			...
			long_shutter_flag = 1;
			write_cmos_sensor(0x0100,0x00);
			write_cmos_sensor(0x0340,0xE5);
			write_cmos_sensor(0x0341,0xCC);
			write_cmos_sensor(0x0342,0x0B);   
			write_cmos_sensor(0x0343,0x28); 
			write_cmos_sensor(0x0202,0xE5); 
			write_cmos_sensor(0x0203,0xC7); 
			write_cmos_sensor(0x3C16,0x00); 
			write_cmos_sensor(0x3C0D,0x04); 
			write_cmos_sensor(0x0100,0x01); 
			write_cmos_sensor(0x3C22,0x00); 
...
static void sensor_init(void)
{
   ...
   // +++++++++++++++++++++++++++//                                                               
	...
	write_cmos_sensor(0x323b,0x02);	//MTK, 2016.1.8
	//sensor otp bpc
...
static void exit_long_shutter(void)
{
	if(long_shutter_flag){
		write_cmos_sensor(0x0100,0x00);
...
*
*************************************************************************/
extern char camera_f_info[20];//add by wangjie for module info
...
static kal_uint32 open(void)
{
	...
	//#ifdef S5K5E8_OTP_ENABLE
	#if 0
	if(1 == current_5e8_otp.awb_valid)
		s5k5e8_write_otp_wb(&current_5e8_otp);//apply awb otp
	s5k5e8_otp_lsc_update();//apply lsc otp
	#endif
