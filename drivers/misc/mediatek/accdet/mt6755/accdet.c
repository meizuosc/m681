...
#else
		mt_eint_set_hw_debounce(gpiopin, accdet_dts_data.accdet_plugout_debounce);
#endif
#endif
		/* update the eint status */
		cur_eint_state = EINT_PIN_PLUG_IN;
...
static inline int accdet_setup_eint(void)
{
	...
	ACCDET_INFO
	    ("[Accdet]EINT finished, accdet_eint_num=%d, accdet_eint_debounce_en=%d, accdet_eint_polarity=%d\n",
...
int accdet_irq_handler(void)
{
	...
	if ((pmic_pwrap_read(ACCDET_IRQ_STS) & NEGV_IRQ_STATUS_BIT) == NEGV_IRQ_STATUS_BIT) {
		...
		while (((pmic_pwrap_read(ACCDET_IRQ_STS) & NEGV_IRQ_STATUS_BIT) && (accdet_timeout_ns(cur_time, ACCDET_TIME_OUT))))
		/*recover EINT irq clear bit*/
...
void accdet_get_dts_data(void)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,accdet");
	if (node) {
		...
		ACCDET_INFO("[Accdet]DTS.pwm_width = %x, DTS.pwm_thresh = %x\n DTS->deb0 = %x, DTS->deb1 = %x\n",
...
#if defined(CONFIG_MTK_LEGACY)
	pmic_pwrap_write(ACCDET_MICBIAS_REG, pmic_pwrap_read(ACCDET_MICBIAS_REG) | (ACCDET_MIC_VOL<<4) | 0x80);
#else
	pmic_pwrap_write(ACCDET_MICBIAS_REG, pmic_pwrap_read(ACCDET_MICBIAS_REG) | (accdet_dts_data.mic_mode_vol<<4) | 0x80);
#endif
	pmic_pwrap_write(ACCDET_RSV, 0x0010);
#ifdef ACCDET_EINT_IRQ
	pmic_pwrap_write(ACCDET_ADC_REG, pmic_pwrap_read(ACCDET_ADC_REG) | ACCDET_EINT_CON_EN); /*Internal connection between ACCDET and EINT*/
#endif
#ifdef ACCDET_NEGV_IRQ
