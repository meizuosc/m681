...
static int rawbulk_tty_write(struct tty_struct *tty, const unsigned char *buf, int count);
...
static int rawbulk_tty_tiocmset(struct tty_struct *tty, unsigned int set, unsigned int clear);
...
static inline struct usb_request_wrapper *get_req(struct list_head *head, spinlock_t *lock)
{
	unsigned long flags;
...
/* #define C2K_USB_PERF */
				...
				while (!signal_pending(current)) {
					...
					while (1) {
						ret =
						    rawbulk_push_upstream_buffer(transfer_id, buf,
										 len);
						if (ret == len || ret == 0) {
...
static int __init init(void)
{
	...
	if (legacy_sysfs_stuff) {
		...
		if (rc < 0)
			printk(KERN_ERR
			       "failed to create legacy bypass sys-stuff, but continue...\n");
	}
#endif				/* SUPPORT_LEGACY_CONTROL */
