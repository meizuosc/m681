...
EXT_DISP_PATH_MODE ext_disp_mode;
...
typedef struct {
	...
	EXTD_OVL_REQ_STATUS ovl_req_state;
	int init;
...
{
	int ret = ext_disp_cmdq_enabled() ? 1 : 0;
	return ret;
}
...
static int _convert_disp_input_to_ovl(OVL_CONFIG_STRUCT *dst, disp_input_config *src)
{
	...
	if (src->buffer_source == DISP_BUFFER_ALPHA) {
		...
		EXT_DISP_ERR("unknow source = %d", src->buffer_source);
...
int ext_disp_config_input_multiple(disp_session_input_config *input, int idx, unsigned int session)
{
	...
	if (pgc->state != EXTD_INIT && pgc->state != EXTD_RESUME && pgc->suspend_config != 1) {
		EXT_DISP_LOG("config ext disp is already sleeped, state:%d\n", pgc->state);
	...
	if (_should_config_ovl_input()) {
		for (i = 0; i < input->config_layer_num; i++) {
			...
			ret = _convert_disp_input_to_ovl(&(data_config->ovl_config[config_layer_id]), &(input->config[i]));
...
{
	EXTD_OVL_REQ_STATUS ret = EXTD_OVL_NO_REQ;
