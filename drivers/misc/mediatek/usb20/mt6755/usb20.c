...
static DEFINE_MUTEX(musb_connect_lock);
...
static void mt_usb_enable(struct musb *musb)
{
	...
	if (!mtk_usb_power) {
		if (down_interruptible(&power_clock_lock))
			xlog_printk(ANDROID_LOG_ERROR, "USB20",
...
void mt_usb_connect(void)
{
	...
	if (cable_mode != CABLE_MODE_NORMAL) {
		...
		mtk_musb->power = true;
		mutex_unlock(&musb_connect_lock);
		return;
	}
...
void mt_usb_check_reconnect(void)
{
	...
	if (usb_connected) {
		musb_stop(mtk_musb);
		musb_start(mtk_musb);
		printk("[MUSB] USB check reconnect: RECONNECT, IDDIG noise\n");
	} else {
		musb_stop(mtk_musb);
		printk("[MUSB] USB check reconnect: STOP\n");
	}
	mutex_unlock(&musb_connect_lock);
}
