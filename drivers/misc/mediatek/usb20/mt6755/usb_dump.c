 ...
 */
static bool com_opend;	/* Whether USB com port is opend */
...
static char *usb_hs_config_desc;
static char *usb_fs_config_desc;
...
static struct usb_config_descriptor config_desc = {
	...
	(sizeof(struct usb_config_descriptor)) +
	    ...
	    (sizeof(struct usb_endpoint_descriptor) * 3) + (sizeof(struct usb_cdc_union_desc)),
	2,
	1,
	0,
	0xc0,
...
static void recv_data(u32 ep_num, u8 *pbuffer, u16 data_len)
{
	...
	while (count < data_len) {
		...
		if (ep_num != 0) {
			...
			while (usb_readw(MUSB_IECSR + MUSB_RXCSR) & MUSB_RXCSR_RXPKTRDY);	/* wait for EP0 tx success */
			...
			while (usb_readw(MUSB_IECSR + MUSB_CSR0) & MUSB_CSR0_RXPKTRDY);	/* wait for EP0 tx success */
...
/* int time_out = 50; */
	...
	if (ep_num == 0) {
		...
		while (count < data_len) {
			...
			while (usb_readw(MUSB_IECSR + MUSB_CSR0) & MUSB_CSR0_TXPKTRDY);	/* wait for EP0 tx success */
		}
	} else {
		...
		while (count < data_len) {
			i = 0;
			while ((usb_readw(MUSB_IECSR + MUSB_TXCSR) & MUSB_TXCSR_TXPKTRDY));	/* if some data has been in fifo before call printk, we must wait for tx complete */
			if (i >= time_out || IS_DISCONNECT || IS_SUSPEND) {
				printk("count = 0x%x, IS_DISCONNECT=%d, IS_SUSPEND=%d\n", count,
				       IS_DISCONNECT, IS_SUSPEND);
