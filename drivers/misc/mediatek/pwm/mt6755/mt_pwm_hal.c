...
unsigned long PWM_register[PWM_NUM] = {
	...
	(PWM_BASE + 0x00d0),	/* PWM3 register base    15 registers */
};
#endif
...
void mt_pwm_power_on_hal(struct clk *clk, BOOL pmic_pad, unsigned long *power_flag)
{
	...
	clk_en_ret = clk_prepare_enable(clk);
	if (clk_en_ret) {
...
void mt_pwm_init_power_flag(unsigned long *power_flag)
{
	...
	PWM_register[PWM4] = (unsigned long)pwm_base + 0x00d0;	/* PWM3 register base,   15 registers */
#endif
/*
    U32 idx=0;
    ...
    PWMDBG("mt_pwm_init_power_flag 0x%x\n", *power_flag);*/
}
...
{
/* mt6755 doesn't support delay mode. */
...
{				/*
				   ...
				   PWMDBG("PWM: clk_div = %x, clk_src = %x, pwm_no = %x\n",
