/* mediatek/platform/mt6755/kernel/drivers/uart/uart.c
 *
 * (C) Copyright 2008
...
#include <mach/mt_clk_id.h>	/* dpidle use */
...
#if defined(CONFIG_MTK_CLKMGR)
	 .set_bit = PDN_FOR_UART1, .clr_bit = PDN_FOR_UART1,
#endif
	.pll_id = PDN_FOR_UART1, .sysrq = FALSE, .hw_flow = TRUE, .vff = TRUE,
	 },
	{
	 .tx_mode = UART_TX_VFIFO_DMA, .rx_mode = UART_RX_VFIFO_DMA, .dma_mode = UART_DMA_MODE_0,
	 .tx_trig = UART_FCR_TXFIFO_1B_TRI, .rx_trig = UART_FCR_RXFIFO_12B_TRI,
...
#if defined(CONFIG_MTK_CLKMGR)
	 .set_bit = PDN_FOR_UART2, .clr_bit = PDN_FOR_UART2,
#endif
	.pll_id = PDN_FOR_UART2, .sysrq = FALSE, .hw_flow = TRUE, .vff = TRUE,
	 },
#if 0
	{
	 .tx_mode = UART_TX_VFIFO_DMA, .rx_mode = UART_RX_VFIFO_DMA, .dma_mode = UART_DMA_MODE_0,
...
static void mtk_uart_cal_baud(struct mtk_uart *uart, int baudrate, int highspeed)
{
	...
	if (highspeed == 0) {
		 ...
		 * In order to improve compatibilty, the guard time register
