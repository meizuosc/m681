...
void mt_cirq_dump_reg(void)
{
	...
	for (cirq_num = 0; cirq_num < CIRQ_IRQ_NUM; cirq_num++) {
		...
		if (mask == 0) {
			...
			irq_iter = CIRQ_TO_IRQ_NUM(cirq_num) - __check_irq_type[0].num;
...
int __init mt_cirq_init(void)
{
	...
	if (!node)
		pr_debug(KERN_ERR "[CIRQ] find SYS_CIRQ node failed!!!\n");
	else {
	...
	ret = driver_register(&mt_cirq_drv.platform.driver);
	if (ret == 0)
		pr_debug("[CIRQ] CIRQ init done...\n");

#ifdef LDVT
	ret = driver_create_file(&mt_cirq_drv.platform.driver, &driver_attr_cirq_dvt);
	if (ret == 0)
		pr_debug("[CIRQ] CIRQ create sysfs file for dvt done...\n");
#endif

	ret = driver_create_file(&mt_cirq_drv.platform.driver,
