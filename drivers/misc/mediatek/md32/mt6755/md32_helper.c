...
int get_md32_semaphore(int flag)
{
	...
	if (ret < 0)
		pr_debug("[MD32] get md32 semaphore %d TIMEOUT...!\n", flag);
...
int release_md32_semaphore(int flag)
{
	...
	if (read_back == 0)
		...
		pr_debug("[MD32] release md32 semaphore %d failed!\n", flag);
...
#endif
	default:
		pr_debug("[MD32] helper get unkown action %ld\n", action);
		break;
	}
	return NOTIFY_OK;
}
...
ssize_t md32_get_log_buf(unsigned char *md32_log_buf, size_t b_len)
{
	...
	if (!md32_log_buf) {
		pr_debug("[MD32] input null buffer\n");
		goto out;
	}

	if (b_len > log_buf_max_len) {
		pr_debug("[MD32] b_len %zu > log_buf_max_len %d\n", b_len, log_buf_max_len);
		b_len = log_buf_max_len;
	}
...
static ssize_t md32_log_read(struct file *file, char __user *data, size_t len, loff_t *ppos)
{
	...
	if (g_md32_log_buf == NULL || log_buf_max_len != last_log_buf_max_len) {
		g_md32_log_buf =
		    (unsigned char *)krealloc(g_md32_log_buf, log_buf_max_len + 1, GFP_KERNEL);

		last_log_buf_max_len = log_buf_max_len;

		if (!g_md32_log_buf) {
			pr_debug("[MD32] %s, kmalloc fail, len=0x%08x\n", __func__,
				 log_buf_max_len);
			return 0;
		}
	}
...
void buf_full_ipi_handler(int id, void *data, unsigned int len)
{
	...
	while (ret_len > 0) {
		...
		pr_debug("[MD32 LOG] %s", g_md32_log_buf_2);
		idx += print_len;
		ret_len -= print_len;
	}
#endif
...
{
	unsigned int *md32_time = (unsigned int *)data;
	pr_debug("[MD32] time_sync_handler %u ms\n", (*md32_time) % 100000000);
}
...
{
	FLAG_LOG_INFO *flag_info = (FLAG_LOG_INFO *) data;
	unsigned int cnt;
...
int get_md32_img_sz(const char *IMAGE_PATH)
{
	...
	if (IS_ERR(filp)) {
		pr_debug("[MD32] Open MD32 image %s FAIL!\n", IMAGE_PATH);
		return -1;
	} else {
...
int md32_tcm_swap(int group_id)
{
	...
	if (swap_lock == 1) {
		app_program_start_addr =
		    readl(MD32_DTCM + g_group_info.app_ptr_p + sizeof(unsigned int) * group_id * 2);
		app_program_end_addr =
		    readl(MD32_DTCM + g_group_info.app_ptr_p +
			  (sizeof(unsigned int) * group_id) * 2 + sizeof(unsigned int));
		app_program_sz = app_program_end_addr - app_program_start_addr + 1;

		app_data_start_addr =
		    readl(MD32_DTCM + g_group_info.app_ptr_d + sizeof(unsigned int) * group_id * 2);
		app_data_end_addr =
		    readl(MD32_DTCM + g_group_info.app_ptr_d +
			  (sizeof(unsigned int) * group_id * 2) + sizeof(unsigned int));
		...
		swap_out_data_end_addr =
		    readl(MD32_DTCM + g_group_info.app_ptr_d +
			  sizeof(unsigned int) * g_current_group * 2 + sizeof(unsigned int));
		swap_out_data_sz = swap_out_data_end_addr - swap_out_data_start_addr + 1;

		/* swap out stop-app data into swap_buf */
		memcpy(swap_buf[g_current_group], MD32_DTCM + g_group_info.app_d_area,
		       swap_out_data_sz);


		/* swap in start-app program/data into tcm */

		memcpy((MD32_PTCM + g_group_info.app_p_area),
		       (md32_program_image + app_program_start_addr), app_program_sz);
...
void swap_ipi_handler(int id, void *data, unsigned int len)
{
	...
	if (info_type == INFO_GROUP) {
		...
		app_program_end_addr =
		    readl(MD32_DTCM + g_group_info.app_ptr_p +
			  (sizeof(unsigned int) * GROUP_BASIC) * 2 + sizeof(unsigned int));
		...
		for (i = 0; i < g_group_info.group_num; i++) {
			...
			app_data_end_addr =
			    readl(MD32_DTCM + g_group_info.app_ptr_d +
				  (sizeof(unsigned int) * i * 2) + sizeof(unsigned int));
			app_data_sz = app_data_end_addr - app_data_start_addr + 1;
			swap_buf[i] =
		...
		if (self_trigger_swap.prepare_result == 0)
		...
		pr_debug("[MD32 swap IPI] unknown info type %d !\n", info_type);
	}
}
#endif
...
int reboot_load_md32(void)
{
	...
	if (d_sz < 0) {
		pr_debug("[MD32] MD32 boot up failed --> can not get data image size\n");
		ret = -1;
		goto error;
	}
	d_sz = ((d_sz + 63) >> 6) << 6;
	...
	if (!md32_data_image) {
		pr_debug("[MD32] MD32 boot up failed --> can't allocate memory\n");
		ret = -1;
		goto error;
	}
	...
	if (ret < 0) {
		pr_debug("[MD32] MD32 boot up failed --> load data image failed!\n");
		ret = -1;
		goto error;
	}
	...
	if (p_sz < 0) {
		pr_debug("[MD32] MD32 boot up failed --> can not get program image size\n");
		ret = -1;
		goto error;
	}
	p_sz = ((p_sz + 63) >> 6) << 6;
	...
	if (ret < 0) {
		pr_debug("[MD32] MD32 boot up failed --> load program image failed!\n");
		ret = -1;
		goto error;
	}
	...
	ret = load_md32(MD32_DATA_IMAGE_PATH, md32_data_image);
	if (ret < 0) {
		pr_debug("MD32 boot up failed --> load data image failed!\n");
		return;
	}
	...
	ret = load_md32(MD32_PROGRAM_IMAGE_PATH, md32_program_image);
	if (ret < 0) {
		pr_debug("MD32 boot up failed --> load program image failed!\n");
		return;
	}
...
error:
	pr_debug("[MD32] boot up failed!!! free images\n");
			       ...
			       size_t n)
{
	...
	if (sw_rstn == 0x0) {
		...
		if (reboot_load_md32() < 0) {
			pr_debug("[MD32] Enable MD32 fail\n");
			return -EINVAL;
		}
	} else {
					   ...
					   char *buf)
{
	...
	if (md32_mobile_log_ready) {
		enable_md32_mobile_log =
		    readl((void __iomem *)(MD32_DTCM + enable_md32_mobile_log_addr));
	}
...
int md32_dt_init(void)
{
	...
	if (!node) {
		pr_debug("[MD32] Can't find node: mediatek,SCP_SRAM\n");
		return -1;
	}
	...
	if (!mreg->sram) {
		pr_debug("[MD32] Unable to ioremap mregisters\n");
		return -1;
	}

	node = of_find_compatible_node(NULL, NULL, "mediatek,SCP_CFGREG");
	if (!node)
		pr_debug("[MD32] Can't find node: mediatek,SCP_CFGRE\n");
	...
	if (!mreg->cfg) {
		pr_debug("[MD32] Unable to ioremap registers\n");
		return -1;
	}

	node = of_find_compatible_node(NULL, NULL, "mediatek,SCP_CLK_CTRL");
	if (!node)
		pr_debug("[MD32] Can't find node: mediatek,SCP_CLK_CTRL\n");
...
static int md32_pm_event(struct notifier_block *notifier, unsigned long pm_event, void *unused)
{
	...
	case PM_POST_HIBERNATION:
		...
		if (retval < 0) {
			retval = -EINVAL;
			pr_debug("[MD32] md32_pm_event MD32 reboot Fail\n");
		}
		return NOTIFY_DONE;
	}
	return NOTIFY_OK;
...
static int __init md32_init(void)
{
	...
	ret = md32_dt_init();
	if (ret) {
		pr_debug("[MD32] Device Init Fail\n");
		return -1;
	}

	md32_send_buff = (unsigned char *)kmalloc((size_t) 64, GFP_KERNEL);
	if (!md32_send_buff) {
		pr_debug("[MD32] Fail --> can't allocate send buff\n");
		return -1;
	}

	md32_recv_buff = (unsigned char *)kmalloc((size_t) 64, GFP_KERNEL);
	if (!md32_recv_buff) {
		pr_debug("[MD32] Fail --> can't allocate recv buff\n");
		return -1;
	}
	...
	/* register time sync IPI */
	md32_ipi_registration(IPI_TIME_SYNC, time_sync_handler, "time_sync");
	...
	if (unlikely(ret != 0)) {
		pr_debug("[MD32] misc register failed\n");
		ret = -1;
	}

	ret = create_files();
	if (unlikely(ret != 0))
		pr_debug("[MD32] create files failed\n");
	...
	if (ret)
		pr_debug("[MD32] failed to register PM notifier %d\n", ret);
