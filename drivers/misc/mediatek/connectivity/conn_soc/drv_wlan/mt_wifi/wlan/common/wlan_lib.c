 ...
 * ease physically continous memory demands separate kalMemAlloc() into virtually-continous
 * and physically-continous type to ease slab system pressure
 *
 * 12 31 2010 cp.wu
		 ...
		 IN P_REG_INFO_T prRegInfo, IN PVOID pvFwImageMapFile, IN UINT_32 u4FwImageFileLength)
{
	...
	if (u4Status == WLAN_STATUS_SUCCESS) {
		...
		case RAM_CODE_DOWNLOAD_FAIL:
			...
			kalSendAeeWarning("[Wait firmware ready fail or ram code download fail!]", __func__);
...
CFG_DEFAULT_SETTING_TABLE_T g_DefaultCfgTable[] =	{
	/*Item Number |  String |Item type | default value | min value | max value */
	 /* Set PNO enable or disable*/
	/*0xff:don't care, 0: forced disabe, 1: forced enable */
	{CMD_PNO_ENABLE, (const char *)"PnoEnable", ITEM_TYPE_HEX, 0xff, 0, 0xff},

	 /* Set PNO scan time*/
	/* unit: second, if value== 0, means don't care*/
	{CMD_PNO_SCAN_PERIOD, (const char *)"PnoScanPeriod", ITEM_TYPE_DEC, 0, 0, 65500},

	/* Set Channel Plan*/
	/* Channel Plan , if value== 255, mean don't care
		Please using "arSupportedRegDomains" for channel plan.
	*/
	{CMD_SCN_CHANNEL_PLAN, (const char *)"ScnChannelPlan", ITEM_TYPE_HEX, 0xFF, 0, 0xFF},

	/* Set Scan dwell time*/
	/* if value== 0xffff, mean don't care
	*/
	{CMD_SCN_DWELL_TIME, (const char *)"ScnDwellTime", ITEM_TYPE_HEX, 0xFFFF, 0, 0xFFFF},

	/* Set Scan stop*/
	/* 0:mean don't care,  1: forced stop scan*/
	{CMD_SCN_STOP_SCAN, (const char *)"ScnStopScan", ITEM_TYPE_DEC, 0, 0, 1},
};

/* For String Only*/
/*Item Number |  String |Item type | default value | min Length | max Length */
CFG_DEFAULT_SETTING_STR_TABLE_T g_DefaultStrCfgTable[] = {
	/*Test*/
	{CMD_STR_TEST_STR, (const char *)"Test", ITEM_TYPE_STR, (const char *)"1234", 0, 128},
};

#define NIC_NUM_REG_PARAMS	CMD_MAX/*(sizeof (g_DefaultCfgTable) / sizeof(CFG_DEFAULT_SETTING_TABLE_T))*/
#define NIC_NUM_STR_REG_PARAMS	CMD_STR_MAX/*(sizeof (g_DefaultStrCfgTable) / sizeof(CFG_DEFAULT_SETTING_STR_TABLE_T))*/

/*Easy debug begin*/
VOID StringTypeSetting(IN P_ADAPTER_T prAdapter, UINT_32 itemNum,  PUINT_32 pCurrentValue, BOOLEAN bSet)
{
	switch (itemNum) {
	case CMD_STR_TEST_STR:
	if (bSet) {
		memset(prAdapter->CfgSetting.TestStr, 0, sizeof(prAdapter->CfgSetting.TestStr));
		memcpy(prAdapter->CfgSetting.TestStr, pCurrentValue, sizeof(prAdapter->CfgSetting.TestStr));
	} else {
		prAdapter->QueryInfo.ptr = (PUINT_32) prAdapter->CfgSetting.TestStr;
		prAdapter->QueryInfo.Length = sizeof(prAdapter->CfgSetting.TestStr);
		prAdapter->QueryInfo.Type = ITEM_TYPE_STR;
	}
	break;
	default:
		break;
...
VOID IntegerTypeSetting(IN P_ADAPTER_T prAdapter, UINT_32 itemNum, UINT_32 CurrentValue, BOOLEAN bSet)
{
	switch (itemNum) {
	case CMD_PNO_ENABLE:
		if (bSet) {
			prAdapter->CfgSetting.PnoEnable = (UINT_8)CurrentValue;
		} else {
			prAdapter->QueryInfo.Value = prAdapter->CfgSetting.PnoEnable;
			prAdapter->QueryInfo.Length = sizeof(prAdapter->CfgSetting.PnoEnable);
		}
		break;
	case CMD_PNO_SCAN_PERIOD:
		if (bSet)
			prAdapter->CfgSetting.PnoScanPeriod = (UINT_32)CurrentValue;
		else {
			prAdapter->QueryInfo.Value = prAdapter->CfgSetting.PnoScanPeriod;
			prAdapter->QueryInfo.Length = sizeof(prAdapter->CfgSetting.PnoScanPeriod);
		}
		break;
	case CMD_SCN_CHANNEL_PLAN:
		if (bSet) {
			prAdapter->CfgSetting.ScnChannelPlan = (UINT_8)CurrentValue;
			rlmDomainSendCmd(prAdapter, FALSE);
		} else {
			prAdapter->QueryInfo.Value = prAdapter->CfgSetting.ScnChannelPlan;
			prAdapter->QueryInfo.Length = sizeof(prAdapter->CfgSetting.ScnChannelPlan);
		}
		break;
	case CMD_SCN_DWELL_TIME:
		if (bSet)
			prAdapter->CfgSetting.ScnDwellTime = (UINT_16)CurrentValue;
		else {
			prAdapter->QueryInfo.Value = prAdapter->CfgSetting.ScnDwellTime;
			prAdapter->QueryInfo.Length = sizeof(prAdapter->CfgSetting.ScnDwellTime);
		}
		break;
	case CMD_SCN_STOP_SCAN:
		if (bSet)
			prAdapter->CfgSetting.ScnStopScan = (UINT_8)CurrentValue;
		else {
			prAdapter->QueryInfo.Value = prAdapter->CfgSetting.ScnStopScan;
			prAdapter->QueryInfo.Length = sizeof(prAdapter->CfgSetting.ScnStopScan);
		}
		break;
	default:
		break;
...
VOID RecordCfgSetting(IN P_ADAPTER_T prAdapter, UINT_32 itemNum, UINT_8 itemType, PUINT_32 pCurrentValue, BOOLEAN bSet)
{
	if (bSet == FALSE) {
		prAdapter->QueryInfo.Length = 0;
		prAdapter->QueryInfo.Value = 0;
		prAdapter->QueryInfo.ptr = NULL;
		prAdapter->QueryInfo.Type = ITEM_TYPE_HEX;
	}

	if (itemType < ITEM_TYPE_STR) {
		UINT_32 CurrentValue = *pCurrentValue;
		IntegerTypeSetting(prAdapter, itemNum, CurrentValue, bSet);
	} else
		StringTypeSetting(prAdapter, itemNum, pCurrentValue, bSet);
}

VOID InitDefaultSetting(IN P_ADAPTER_T prAdapter)
{
	UINT_16 i = 0;
	UINT_32 CurrentValue = 0;
	UINT_8 itemType = ITEM_TYPE_DEC;
	UINT_32 itemNum = 0xffffffff;

	DBGLOG(INIT, LOUD, "==> EasyDbg InitDefaultSetting total items %d\n", NIC_NUM_REG_PARAMS);

	for (i = 0; i < NIC_NUM_REG_PARAMS; i++) {
		CurrentValue = g_DefaultCfgTable[i].defaultValue;
		itemNum = g_DefaultCfgTable[i].itemNum;
		itemType = g_DefaultCfgTable[i].itemType;
		RecordCfgSetting(prAdapter, itemNum, itemType, &CurrentValue, TRUE);
	}

	for (i = 0; i < NIC_NUM_STR_REG_PARAMS; i++) {
		itemNum = g_DefaultStrCfgTable[i].itemNum;
		itemType = g_DefaultStrCfgTable[i].itemType;
		RecordCfgSetting(prAdapter, itemNum, itemType, (PUINT_32) g_DefaultStrCfgTable[i].DefString, TRUE);
	}
	DBGLOG(INIT, LOUD, "<== EasyDbg InitDefaultSetting\n");
}


VOID UpdateSettingFromExternal(IN P_ADAPTER_T prAdapter, P_CMD_HEADER_T prtGenBuff, BOOLEAN bSet)
{
	UINT_16 i = 0, j = 0;
	UINT_8 itemSetNum = prtGenBuff->itemNum;
	UINT_8 SetItemType = 0;
	UINT_32 CurrentValue = 0;
	PUINT_32 pCurrentValue = NULL;
	P_CMD_FORMAT_V1_T prtCmdForMatBuf = (P_CMD_FORMAT_V1_T)prtGenBuff->buffer;

	SetItemType = prtCmdForMatBuf->itemType;


	DBGLOG(INIT, LOUD, "==> EasyDbg UpdateSetting total items %d,  Int:%d, Str:%d, bSet %d, type %d\n",
		itemSetNum,
		NIC_NUM_REG_PARAMS,
		NIC_NUM_STR_REG_PARAMS,
		bSet,
		SetItemType);

	for (j = 0; j < itemSetNum; j++) {
		for (i = 0; i < NIC_NUM_REG_PARAMS; i++) {
			UINT_32 itemNum = 0xffffffff;
			...
			/*Integer case:*/
				if ((strcmp(g_DefaultCfgTable[i].String, prtCmdForMatBuf->itemString) == 0)) {
				UINT_32 settingValue = 0;

				if (bSet == FALSE) {
					UINT_32 Value = 0;

					RecordCfgSetting(prAdapter, itemNum, g_DefaultCfgTable[i].itemType, &Value, bSet);
					return;
				}

				if (prtCmdForMatBuf->itemValueLength <= 4) {
					UINT_32 minValue = 0, maxValue = 0;

					/* 1 byte value.*/
					if (prtCmdForMatBuf->itemValueLength == 1)
						settingValue = prtCmdForMatBuf->itemValue[0];
					/* 2 byte value. */
					else if (prtCmdForMatBuf->itemValueLength == 2)
						settingValue = *(UINT_16 *)(prtCmdForMatBuf->itemValue);
					/*4 byte value. */
					else if (prtCmdForMatBuf->itemValueLength <= 4)
						settingValue = *(UINT_32 *)(prtCmdForMatBuf->itemValue);

					minValue = g_DefaultCfgTable[i].minValue;
					maxValue = g_DefaultCfgTable[i].maxValue;

					/*make sure the setting Value was within the range.*/
					if (settingValue > maxValue)
						settingValue = maxValue;
					else if (settingValue <= minValue)
						settingValue = minValue;

					/*update new value from cmd or wifi.cfg file.*/
					CurrentValue = settingValue;
							pCurrentValue = &CurrentValue;

					/*using itemNum to know which item's to set.*/
					RecordCfgSetting(prAdapter, itemNum, prtCmdForMatBuf->itemType, pCurrentValue, bSet);
				}
			}
		}
		...
		for (i = 0; i < NIC_NUM_STR_REG_PARAMS; i++) {
			UINT_32 itemNum = 0xffffffff;
			PUINT_8 pStrValue = (PUINT_8) g_DefaultStrCfgTable[i].DefString;
			itemNum = g_DefaultStrCfgTable[i].itemNum;

			/*String case:*/
			if ((strcmp(g_DefaultStrCfgTable[i].String, prtCmdForMatBuf->itemString) == 0)) {

				if (bSet == FALSE) {
					CFG_QUERY_FORMAT_T CfgQueryInfo;
					UINT_32 Value = 0;

					RecordCfgSetting(prAdapter, itemNum, g_DefaultStrCfgTable[i].itemType, &Value, bSet);
					return;
				}

				if (prtCmdForMatBuf->itemType == ITEM_TYPE_STR)
					pStrValue = prtCmdForMatBuf->itemValue;

				/*using itemNum to know which item's to set.*/
				RecordCfgSetting(prAdapter, itemNum, prtCmdForMatBuf->itemType, pStrValue, bSet);
			}
		}
		prtCmdForMatBuf++;
	}

	DBGLOG(INIT, LOUD, "<== EasyDbg UpdateSettingFromExternal\n");

	return;
}

VOID wlanGetSetCfgFromExt (IN P_ADAPTER_T prAdapter, P_CMD_HEADER_T prtCmdHeader)
{
	UINT_8 i;
	P_CMD_FORMAT_V1_T ptr = (P_CMD_FORMAT_V1_T)prtCmdHeader->buffer;

	DBGLOG(INIT, LOUD, "EasyDbg wlanGetSetCfgFromExt, Type %d, itemNum %d,  cmdBufferLen %d \n", prtCmdHeader->cmdType, prtCmdHeader->itemNum, prtCmdHeader->cmdBufferLen);

	for (i = 0; i < prtCmdHeader->itemNum; i++)
		ptr++;

	if (prtCmdHeader->cmdBufferLen != ((prtCmdHeader->itemNum) * CMD_FORMAT_V1_LENGTH))
		DBGLOG(INIT, WARN, "EasyDbg cmdBufferLen buffer length not match %d, %d \n", prtCmdHeader->cmdBufferLen, ((prtCmdHeader->itemNum) * CMD_FORMAT_V1_LENGTH));

	if (prtCmdHeader->cmdBufferLen > MAX_CMD_BUFFER_LENGTH)
		DBGLOG(INIT, WARN, "EasyDbg cmdBufferLen buffer length too long %d \n", prtCmdHeader->cmdBufferLen);

	if (prtCmdHeader->itemNum > MAX_CMD_ITEM_MAX)
		DBGLOG(INIT, WARN, "EasyDbg Cmd Item number too much %d \n", prtCmdHeader->itemNum);

	if (prtCmdHeader->cmdType == CMD_TYPE_SET) {
		UpdateSettingFromExternal(prAdapter, prtCmdHeader, TRUE);
	} else if (prtCmdHeader->cmdType == CMD_TYPE_QUERY) {

		UINT_32 Length = 0;
		P_CFG_QUERY_FORMAT_T pGetValue = NULL;

		kalMemSet(&prAdapter->QueryInfo, 0, sizeof(CFG_QUERY_FORMAT_T));

		UpdateSettingFromExternal(prAdapter, prtCmdHeader, FALSE);

		pGetValue = &prAdapter->QueryInfo;

		Length = pGetValue->Length;

		if (pGetValue->Length <= 4) {
			memset(prtCmdHeader->buffer, 0, sizeof(prtCmdHeader->buffer));
			sprintf(prtCmdHeader->buffer, "0x%x\n", pGetValue->Value);
		} else {
			UINT_16 i = 0, j = 0;
			memset(prtCmdHeader->buffer, 0x20, sizeof(prtCmdHeader->buffer));
			if (pGetValue->Type != ITEM_TYPE_STR) {
				for (i = 0; i < (Length/4); i++) {
					sprintf(prtCmdHeader->buffer+j, "%x ", (UINT_32)*(pGetValue->ptr+i));
					j += 9;

					if ((i+1)%4 == 0) {
						sprintf(prtCmdHeader->buffer+j, "\n");
						j += 1;
					}
				}
			} else
				memcpy(prtCmdHeader->buffer, pGetValue->ptr, pGetValue->Length);
		}
	}
}
...
WLAN_STATUS wlanCfgParseToFW(PCHAR *args, PCHAR args_size, UCHAR nargs, PCHAR buffer, UCHAR times)
{
	...
	for (i = 0; i < cmd_v1.itemStringLength; i++)
		pr_debug("%c", cmd_v1.itemString[i]);
	pr_debug("\n");

	pr_debug("cmd_v1.itemType = %d\n", cmd_v1.itemType);
#endif
	if (cmd_v1.itemType == ITEM_TYPE_DEC || cmd_v1.itemType == ITEM_TYPE_HEX) {
		...
		for (i = args_size[ED_VALUE_SITE] - 1 - startOffset; i >= 0; i--) {
...
WLAN_STATUS wlanCfgParse(IN P_ADAPTER_T prAdapter, PUINT_8 pucConfigBuf, UINT_32 u4ConfigBufLen, BOOLEAN isFwConfig)
{
	...
	CHAR **args;
	INT_32 nargs;
	CHAR   apcArgv_size[WLAN_CFG_ARGV_MAX];
	UCHAR  times = 0;
	...
	pcmdV1Header = (P_ADAPTER_T) kalMemAlloc(sizeof(CMD_HEADER_T), VIR_MEM_TYPE);
	...
	for (;;) {
		switch (wlanCfgFindNextToken(&state)) {
		case STATE_EOF:
			if (nargs == 2 && !isFwConfig)
				wlanCfgParseAddEntry(prAdapter, args[0], NULL, args[1], NULL);
			else if (nargs == 3 && isFwConfig) {
				WLAN_STATUS ret;
				ret = wlanCfgParseToFW(args, apcArgv_size, nargs, pcmdV1Header->buffer, times);
				if (ret == WLAN_STATUS_SUCCESS) {
					times++;
					pcmdV1Header->cmdBufferLen += sizeof(CMD_FORMAT_V1_T);
				}
			}
			goto exit;
		...
		case STATE_NEWLINE:
			if (nargs == 2 && !isFwConfig)
				...
				if (times == MAX_CMD_ITEM_MAX) {
					/* Send to FW */
					pcmdV1Header->itemNum = times;
					wlanGetSetCfgFromExt(prAdapter, pcmdV1Header);
					...
					rStatus = wlanSendSetQueryCmd(
						...
						(PUINT_8) pcmdV1Header,	/* pucInfoBuffer */
						NULL,	/* pvSetQueryBuffer */
						0	/* u4SetQueryBufferLen */
					);

					if (rStatus == WLAN_STATUS_FAILURE)
						DBGLOG(INIT, ERROR, "kalIoctl wifiSefCFG fail 0x%x\n", rStatus);

					memset(pcmdV1Header->buffer, 0, MAX_CMD_BUFFER_LENGTH);
					pcmdV1Header->cmdBufferLen = 0;
					times = 0;
				}
			}

			memset(apcArgv_size, 0, WLAN_CFG_ARGV_MAX);
			memset(apcArgv, 0, WLAN_CFG_ARGV_MAX);
			nargs = 0;
			break;
		...
		case STATE_TEXT:
			if (nargs < WLAN_CFG_ARGV_MAX) {
				args[nargs++] = state.text;
				apcArgv_size[nargs-1] = state.textsize;
				state.textsize = 0;
			}
			break;
		}
...
#define STR2BYTE(s) (((((PUINT_8)s)[0]-'0')*10)+(((PUINT_8)s)[1]-'0'))
	...
	if (wlanCfgGet(prAdapter, "country", aucValue, "", 0) == WLAN_STATUS_SUCCESS) {
		DBGLOG(INIT, LOUD, "CFG_FILE: Found Country Key, Vaule=%s\n", aucValue);
		prAdapter->rWifiVar.rConnSettings.u2CountryCode =
