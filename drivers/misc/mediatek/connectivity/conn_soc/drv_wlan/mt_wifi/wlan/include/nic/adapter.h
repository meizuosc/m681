...
typedef struct _CONNECTION_SETTINGS_T {
	...
	EMUN_RECONNECT_LEVEL_T eReConnectLevel;
...
struct _BSS_INFO_T {
	...
	BOOLEAN fgDetectConnectedAP;
	UINT_8  ucDetectFailCount;
...
struct _ADAPTER_T {
	...
	CFG_SETTING_T CfgSetting;

	CFG_QUERY_FORMAT_T QueryInfo;
	...
	UINT_32 u4AirDelayTotal;	/*  dbg priviledge power mode, always keep in active */
#if (CFG_SUPPORT_SUSPEND_SLEEP == 1)
	atomic_t	fgIsSuspended;
#endif
	struct PERF_MONITOR_T rPerMonitor;
	UINT_8			ucBcnTimeoutLostBcn;
};				/* end of _ADAPTER_T */
