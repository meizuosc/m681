		...
		case SCAN_CHANNEL_SPECIFIED:
			{
				...
				if (prChnlReqInfo->eChannelReqType == CHANNEL_REQ_TYPE_GO_START_BSS && !fgIsPureAP) {
		...
		case SCAN_CHANNEL_P2P_SOCIAL:
			{
				...
				/* For compatible. */
				if (EQUAL_SSID
				    (aucP2pSsid, P2P_WILDCARD_SSID_LEN, prScanReq->aucSSID, prScanReq->ucSSIDLength)) {
		   ...
		   IN P_P2P_MGMT_TX_REQ_INFO_T prMgmtTxReqInfo, IN P_MSDU_INFO_T prMgmtTxMsdu, IN UINT_64 u8Cookie)
{
	...
	do {
		...
		case MAC_FRAME_PROBE_RSP:
			DBGLOG(P2P, INFO, "p2pFuncTxMgmtFrame:  TX MAC_FRAME_PROBE_RSP\n");
...
BOOLEAN p2pFuncValidateAssocReq(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb, OUT PUINT_16 pu2StatusCode)
{
	...
	do {
		...
		*pu2StatusCode = STATUS_CODE_REQ_DECLINED;
		...
		DBGLOG(P2P, INFO, "AssocReq: Current WfdCfgSettings wfd_en %u wfd_info 0x%x  wfd_policy 0x%x wfd_flag 0x%x\n",
