...
VOID qmReassignTcResource(IN P_ADAPTER_T prAdapter)
{
	...
	if (i4TotalResourceDemand <= 0) {
		...
		/* total TC resouces are enough, no extra TC resource is needed */

		/* adjust used TC resource to average TC resouces + min reserve TC resources */
		for (i = 0; i < TC_NUM; i++) {
	...
	/* 4 <3> Case 2: Demand > Total Resource --> Guarantee a minimum amount of resource for each TC */
		...
		for (i = 0; i < TC_NUM; i++) {
			...
			/*
			   ...
			   current average queue len + min TC resouces
			 */
