	      ...
	      IN struct iw_request_info *prIwReqInfo, IN struct iw_freq *prIwFreq, IN char *pcExtra)
{
	...
	/*
	   DBGLOG(REQ, TRACE, "set m:%d, e:%d, i:%d, flags:%d\n",
	      ...
	      IN struct iw_request_info *prIwrInfo, IN OUT struct iw_point *prData, IN char *pcExtra)
{
	...
	/* wait scan done */
	/* DBGLOG(REQ, TRACE, "wait for scan results\n"); */
	...
	if (rStatus == WLAN_STATUS_INVALID_LENGTH) {
		...
		/* DBGLOG(REQ, TRACE, "[wifi] buf:%d result:%ld\n", pData->length, u4BufLen); */
	...
	for (i = 0; i < prList->u4NumberOfItems; ++i) {
		...
		for (j = 0; j < PARAM_MAX_LEN_RATES_EX; ++j) {
			...
			if (u4BufIndex > sizeof(aucRatesBuf)) {
				/* DBGLOG(REQ, TRACE, "rate info too long\n"); */
				break;
			}
#endif
		}
	       ...
	       IN struct iw_request_info *prIwrInfo, IN struct iw_point *prEssid, IN char *pcExtra)
{
	...
	if (prGlueInfo->rWpaInfo.u4WpaVersion == IW_AUTH_WPA_VERSION_DISABLED) {
		...
		/* DBGLOG(REQ, TRACE, "IW_AUTH_WPA_VERSION_DISABLED->Param_AuthMode%s\n", */
		...
		case IW_AUTH_KEY_MGMT_802_1X:
			...
			/* DBGLOG(REQ, TRACE, "IW_AUTH_KEY_MGMT_802_1X->AUTH_MODE_WPA%s\n", */
		...
		case IW_AUTH_KEY_MGMT_PSK:
			...
			/* DBGLOG(REQ, TRACE, "IW_AUTH_KEY_MGMT_PSK->AUTH_MODE_WPA%sPSK\n", */
...
/* //DBGLOG(REQ, TRACE, "IW_AUTH_KEY_MGMT_WPA_NONE->AUTH_MODE_WPA_NONE\n"); */
/* break; */
/* #endif */
#if CFG_SUPPORT_802_11W
...
#endif
		default:
			/* DBGLOG(REQ, TRACE, DRV_NAME"strange IW_AUTH_KEY_MGMT : %ld set auto switch\n", */
	...
	if (cipher & IW_AUTH_CIPHER_CCMP) {
		/* DBGLOG(REQ, TRACE, "IW_AUTH_CIPHER_CCMP->ENUM_ENCRYPTION3_ENABLED\n"); */
		eEncStatus = ENUM_ENCRYPTION3_ENABLED;
	} else if (cipher & IW_AUTH_CIPHER_TKIP) {
		/* DBGLOG(REQ, TRACE, "IW_AUTH_CIPHER_TKIP->ENUM_ENCRYPTION2_ENABLED\n"); */
		eEncStatus = ENUM_ENCRYPTION2_ENABLED;
	} else if (cipher & (IW_AUTH_CIPHER_WEP104 | IW_AUTH_CIPHER_WEP40)) {
		/* DBGLOG(REQ, TRACE, "IW_AUTH_CIPHER_WEPx->ENUM_ENCRYPTION1_ENABLED\n"); */
		eEncStatus = ENUM_ENCRYPTION1_ENABLED;
	} else if (cipher & IW_AUTH_CIPHER_NONE) {
		/* DBGLOG(REQ, TRACE, "IW_AUTH_CIPHER_NONE->ENUM_ENCRYPTION_DISABLED\n"); */
		if (prGlueInfo->rWpaInfo.fgPrivacyInvoke)
			eEncStatus = ENUM_ENCRYPTION1_ENABLED;
		else
			eEncStatus = ENUM_ENCRYPTION_DISABLED;
	} else {
		/* DBGLOG(REQ, TRACE, "unknown IW_AUTH_CIPHER->Param_EncryptionDisabled\n"); */
		eEncStatus = ENUM_ENCRYPTION_DISABLED;
	}
	...
	if (kalIoctl(prGlueInfo,
		...
		/* DBGLOG(REQ, TRACE, "Fail to set ssid\n"); */
		return -EFAULT;
	}
	      ...
	      IN struct iw_request_info *prIwReqInfo, IN struct iw_param *prRate, IN char *pcExtra)
{
	...
	/*
	   DBGLOG(REQ, TRACE, "value = %d, fixed = %d, disable = %d, flags = %d\n",
	...
	if (prRate->value < 0) {
		if (prRate->fixed == 0) {
			...
			/* DBGLOG(REQ, TRACE, "wlanoidQuerySupportedRates():u4BufLen = %ld\n", u4BufLen); */
		...
		IN struct iw_request_info *prIwrInfo, IN struct iw_point *prEnc, IN char *pcExtra)
{
	...
	if (prEnc->length == 5 || prEnc->length == 13 || prEnc->length == 16) {
		...
		if (rStatus != WLAN_STATUS_SUCCESS) {
			/* DBGLOG(REQ, TRACE, DRV_NAME"wlanoidSetAuthMode fail 0x%lx\n", rStatus); */
			return -EFAULT;
		}
		...
		if (rStatus != WLAN_STATUS_SUCCESS) {
			/* DBGLOG(REQ, TRACE, DRV_NAME"wlanoidSetEncryptionStatus fail 0x%lx\n", rStatus); */
			return -EFAULT;
		}
	       ...
	       IN struct iw_request_info *prIwrInfo, IN struct iw_param *prPower, IN char *pcExtra)
{
	...
	/* DBGLOG(REQ, TRACE, "wext_set_power value(%d) disabled(%d) flag(0x%x)\n", */
	...
	if (rStatus != WLAN_STATUS_SUCCESS) {
		/* DBGLOG(REQ, TRACE, DRV_NAME"wlanoidSet802dot11PowerSaveProfile fail 0x%lx\n", rStatus); */
		return -EFAULT;
	}
#endif
	return 0;
	       ...
	       IN struct iw_request_info *prIwrInfo, OUT struct iw_param *prPower, IN char *pcExtra)
{
	...
	/* DBGLOG(REQ, TRACE, "wext_get_power value(%d) disabled(%d) flag(0x%x)\n", */
	/* prPower->value, prPower->disabled, prPower->flags); */
	      ...
	      IN struct iw_request_info *prIwrInfo, IN struct iw_param *prAuth, IN char *pcExtra)
{
...
#if CFG_SUPPORT_802_11W
	case IW_AUTH_MFP:
		/* DBGLOG(REQ, TRACE, "wext_set_auth IW_AUTH_MFP=%d\n", prAuth->value); */
	...
	default:
		/*
		   DBGLOG(REQ, TRACE, "[wifi] unsupported IW_AUTH_INDEX :%d\n", prAuth->flags);
		 */
		break;
	}
	return 0;
		    ...
		    IN struct iw_request_info *prIwrInfo, IN struct iw_point *prEnc, IN char *pcExtra)
{
	...
	if (prIWEncExt->alg == IW_ENCODE_ALG_SMS4) {
		if (prEnc->flags & IW_ENCODE_DISABLED) {
			/* DBGLOG(REQ, TRACE, "[wapi] IW_ENCODE_DISABLED\n"); */
			return 0;
		}
		/* KeyID */
		...
		if (prWpiKey->ucKeyID > 1) {
			/* key id is out of range */
			/* DBGLOG(REQ, TRACE, "[wapi] add key error: key_id invalid %d\n", prWpiKey->ucKeyID); */
			return -EINVAL;
		}

		if (prIWEncExt->key_len != 32) {
			/* key length not valid */
			/* DBGLOG(REQ, TRACE, "[wapi] add key error: key_len invalid %d\n", prIWEncExt->key_len); */
			return -EINVAL;
		}
		/* DBGLOG(REQ, TRACE, "[wapi] %d ext_flags %d\n", prEnc->flags, prIWEncExt->ext_flags); */
		...
		if (rStatus != WLAN_STATUS_SUCCESS) {
			/* Do nothing */
			/* DBGLOG(REQ, TRACE, "[wapi] add key error:%lx\n", rStatus); */
		}
	} else
#endif
	{
		...
		if ((prEnc->flags & IW_ENCODE_MODE) == IW_ENCODE_DISABLED) {
			...
			/*
			   DBGLOG(REQ, TRACE, "IW_ENCODE_DISABLED: ID:%d, Addr:[ %pM ]\n",
...
int wext_support_ioctl(IN struct net_device *prDev, IN struct ifreq *prIfReq, IN int i4Cmd)
{
	...
	case SIOCGIWESSID:	/* 0x8B1B, get SSID */
		...
		if (u4ExtraSize != IW_ESSID_MAX_SIZE && u4ExtraSize != IW_ESSID_MAX_SIZE + 1) {
			DBGLOG(REQ, ERROR, "[wifi] iwr->u.essid.length:%d error\n", u4ExtraSize);
		...
		if (copy_from_user(prExtraBuf, iwr->u.data.pointer, u4ExtraSize)) {
			...
			if (rStatus != WLAN_STATUS_SUCCESS) {
				...
				if (wextSrchDesiredWPSIE(prExtraBuf,
					...
					if (rStatus != WLAN_STATUS_SUCCESS) {
						/* DBGLOG(REQ, TRACE, "[WSC] set WSC assoc info
						error:%lx\n", rStatus); */
					}
				}
#endif
	...
	case SIOCSIWPMKSA:	/* 0x8B36, pmksa cache operation */
		...
		if (iwr->u.data.pointer) {
			...
			if (copy_from_user(prExtraBuf, iwr->u.data.pointer, sizeof(struct iw_pmksa))) {
				...
				case IW_PMKSA_ADD:
					/*
					   DBGLOG(REQ, TRACE, "IW_PMKSA_ADD [ %pM ]\n",
				...
				case IW_PMKSA_REMOVE:
					/*
					   DBGLOG(REQ, TRACE, "IW_PMKSA_REMOVE [ %pM ]\n",
					   (((struct iw_pmksa *)buf)->bssid.sa_data));
					 */
					break;
				case IW_PMKSA_FLUSH:
					/*
					   DBGLOG(REQ, TRACE, "IW_PMKSA_FLUSH\n");
					 */
	...
	default:
		/* DBGLOG(REQ, TRACE, "unsupported IOCTL: 0x%x\n", i4Cmd); */
		ret = -EOPNOTSUPP;
		break;
	}

	/* DBGLOG(REQ, TRACE, "%ld CMD:0x%x ret:%d\n", jiffies_to_msecs(jiffies), i4Cmd, ret); */
			 ...
			 IN unsigned int u4Cmd, IN unsigned char *pucData, IN unsigned int u4dataLen)
{
	...
	case IWEVASSOCREQIE:
		...
		if (wextSrchDesiredWPAIE(pucData, u4dataLen, 0x30, &pucDesiredIE)) {
			...
			/* WAPI IE found */
			/* DBGLOG(REQ, TRACE, "wextSrchDesiredWAPIIE!!\n"); */
#endif
		} else {
		...
		/* DBGLOG(REQ, TRACE, "assoc info buffer size needed:%d\n", infoElemLen * 2 + 17); */
...
#else
			/* DBGLOG(REQ, TRACE, "IWEVPMKIDCAND event skipped, WE < 18\n"); */
			goto skip_indicate_event;
#endif
		} else {
			/* DBGLOG(REQ, TRACE, "IWEVPMKIDCAND event skipped, NOT WPA2\n"); */
			goto skip_indicate_event;
		}
		break;
	...
	default:
		/* DBGLOG(REQ, TRACE, "Unsupported wext event:%x\n", cmd); */
		goto skip_indicate_event;
	}
