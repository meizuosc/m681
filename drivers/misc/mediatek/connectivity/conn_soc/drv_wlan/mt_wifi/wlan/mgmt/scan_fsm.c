...
VOID scnSendScanReqExtCh(IN P_ADAPTER_T prAdapter)
{
	...
	if (prScanParam->eScanChannel == SCAN_CHANNEL_SPECIFIED) {
		...
		kalMemZero(&prScanParam->arChnlInfoList[0], MAXIMUM_OPERATION_CHANNEL_LIST * sizeof(RF_CHANNEL_INFO_T));
    }
...
VOID scnSendScanReq(IN P_ADAPTER_T prAdapter)
{
	...
	if (prScanParam->ucChannelListNum > 32 && prScanParam->eScanChannel == SCAN_CHANNEL_SPECIFIED) {
		...
		if (prScanParam->eScanChannel == SCAN_CHANNEL_SPECIFIED) {
			...
			prCmdScanReq->ucChannelListNum = 0;
			for(i = 0 ; i < 32 ; i++) {
				prCmdScanReq->arChannelList[i].ucBand = 0;
				prCmdScanReq->arChannelList[i].ucChannelNum = 0;
			}
		}
#if CFG_ENABLE_WIFI_DIRECT
		if (prScanParam->eNetTypeIndex == NETWORK_TYPE_P2P_INDEX)
		...
		if (prScanParam->eNetTypeIndex == NETWORK_TYPE_AIS_INDEX)
			prCmdScanReq->u2ChannelDwellTime = CFG_FAST_SCAN_DWELL_TIME;
				...
				if (prAdapter->CfgSetting.ScnDwellTime != 0xffff)
					prCmdScanReq->u2ChannelDwellTime = prAdapter->CfgSetting.ScnDwellTime;
...
BOOLEAN scnFsmPSCNSetParam(IN P_ADAPTER_T prAdapter, IN P_CMD_SET_PSCAN_PARAM prCmdPscnParam)
{
	...
	prCmdPscnParam->u4BasePeriod = prCmdPscnParam->u4BasePeriod;
		...
		if (prAdapter->CfgSetting.PnoScanPeriod > 0)
			prCmdPscnParam->u4BasePeriod = prAdapter->CfgSetting.PnoScanPeriod;
#if 0
	DBGLOG(SCN, TRACE,
