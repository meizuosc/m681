    ...
    IN P_GLUE_INFO_T prGlueInfo)
{
	...
	*/ 
	/*printk("mtk_cfg80211_testmode_set_packet_filter\nDrop_IPv4(%02llx)\nDrop_IPv6(%02llx)\nDrop_SNAP(%02llx)\n",
...
int
mtk_cfg80211_testmode_set_always_scan_state(
    IN struct wiphy *wiphy,
    IN void *data,
    IN int len,
    IN P_GLUE_INFO_T prGlueInfo)
{
	WLAN_STATUS rStatus = WLAN_STATUS_SUCCESS;
	INT_32 i4Status = -EINVAL;
	P_NL80211_DRIVER_ALWAYSSCAN_PARAMS prParams;
	UINT_8 state = 0xff;
	UINT_32 u4BufLen = 0;
	...
	if (data && len)
		prParams = (P_NL80211_DRIVER_ALWAYSSCAN_PARAMS)data;
	
	if ((prParams->alwaysscanstate != 0) &&
		(prParams->alwaysscanstate != 1)) {
		DBGLOG(QM, TRACE, "%s invalid alwaysscanstate must 0 or 1\n", __func__);
		return -EINVAL;
	}
	
	state = prParams->alwaysscanstate;
...
int mtk_cfg80211_testmode_cmd(IN struct wiphy *wiphy, IN void *data, IN int len)
{
	...
	if (prParams) {
		...
		case TESTMODE_CMD_ID_ALWAYSSCANSTATE:
			i4Status = mtk_cfg80211_testmode_set_always_scan_state(wiphy, data, len, prGlueInfo);
			break;
