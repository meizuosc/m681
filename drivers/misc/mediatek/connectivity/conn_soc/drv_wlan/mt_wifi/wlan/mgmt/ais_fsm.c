...
#define AIS_ROAMING_SCAN_CHANNEL_DWELL_TIME 100
...
#endif
			prAisBssInfo = &(prAdapter->rWifiVar.arBssInfo[NETWORK_TYPE_AIS_INDEX]);
			if (!prAisBssInfo->fgDetectConnectedAP) {
				prAisBssInfo->ucDetectFailCount++;
			} else {
				prAisBssInfo->ucDetectFailCount = 0;
			}

			if (prAisBssInfo->ucDetectFailCount > 5) {
				prAisBssInfo->ucDetectFailCount = 0;
				DBGLOG(INIT, WARN, "Detect no AP, trigger beacon timeout!\n");
	...
	if (!prConnSettings->fgIsScanReqIssued) {
		...
		if (prAisFsmInfo->eCurrentState == AIS_STATE_NORMAL_TR) {
			if (prAisBssInfo->eCurrentOPMode == OP_MODE_INFRASTRUCTURE
				...
				/* start online scan */
				wlanClearScanningResult(prAdapter);
				prAisBssInfo->fgDetectConnectedAP = FALSE;
			      ...
			      IN P_MSDU_INFO_T prMsduInfo, IN ENUM_TX_RESULT_CODE_T rTxDoneStatus)
{
	...
	do {
		...
		if (rTxDoneStatus != TX_RESULT_SUCCESS) {
			...
			fgIsSuccess = TRUE;
			/* DBGLOG(AIS, STATE, "Mgmt Frame TX Done.\n"); */
		}
		   ...
		   IN P_AIS_MGMT_TX_REQ_INFO_T prMgmtTxReqInfo, IN P_MSDU_INFO_T prMgmtTxMsdu, IN UINT_64 u8Cookie)
{
	...
	do {
		...
		if (prStaRec != NULL) {
			/* Do nothing */
			/* DBGLOG(AIS, ERROR, "Mgmt with station record: %pM .\n", prStaRec->aucMacAddr); */
		}
