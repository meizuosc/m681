...
typedef enum _ENUM_CMD_ID_T {
	...
	CMD_ID_BEACON_CONFIG = 0x49,		/* 0x49 (Set) */
...
typedef enum _ENUM_EVENT_ID_T {
	...
	EVENT_ID_GET_CMD_INFO = 0x70, /* 0x70 (Query - EVENT_ID_GET_CMD_INFO) */ /* query info from cmd.*/
...
typedef struct _CMD_RLM_BCN_INFO_T {
	UINT_32		u4Version;
	UINT_32		u4BcnTT_NumOfContLostThreshold;
	BOOLEAN		fgBcnTT_fgKeepAwakeInLastStage;
    UINT_32     u4Reserved[100]; /* for future */
} CMD_RLM_BCN_INFO_T;
