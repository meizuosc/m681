...
#if CFG_AUTO_CHANNEL_SEL_SUPPORT
		case 0x30:
			i4Status = mtk_p2p_cfg80211_testmode_get_lte_channel(wiphy, data, len, prGlueInfo);
#endif
...
#if CFG_AUTO_CHANNEL_SEL_SUPPORT
int mtk_p2p_cfg80211_testmode_get_lte_channel(IN struct wiphy *wiphy, IN void *data, IN int len, IN P_GLUE_INFO_T prGlueInfo)
{
#define MAXMUN_2G_CHA_NUM 14
#define CHN_DIRTY_WEIGHT_UPPERBOUND 4
	...
	WLAN_STATUS rStatus = WLAN_STATUS_SUCCESS;
	/*P_DOMAIN_INFO_ENTRY prDomainInfo;S*/
	/*RF_CHANNEL_INFO_T aucChannelList[MAXMUN_2G_CHA_NUM];*/
	UINT_8 ucMax_2G_Chn_List = 14, ucIdx = 0;
	UINT_16 u2APNumScore = 0, u2UpThreshold = 0, u2LowThreshold = 0, ucInnerIdx = 0;
	UINT_32 u4BufLen, u4LteSafeChnBitMask_2G = 0;
	UINT_32 u4AcsChnReport[4];

	P_PARAM_GET_CHN_LOAD prGetChnLoad, prQueryLteChn;
	PARAM_PREFER_CHN_INFO rPreferChannels[1] = { {0, 0xFFFF, {0} } };
	PARAM_PREFER_CHN_INFO arChannelLoadingWeightScore_2G[MAXMUN_2G_CHA_NUM];
	...
	if (!skb) {
		DBGLOG(P2P, ERROR, "Allocate skb failed:%x\n", rStatus);
		return -ENOMEM;
	}
...
#if 0
	/* ToDo:
	 * 1. Get the Maximun channel List for 2.4G Bands in current country domain
	 */
	prDomainInfo = rlmDomainGetDomainInfo(prGlueInfo->prAdapter);
	ASSERT(prDomainInfo);
	rlmDomainGetChnlList(prGlueInfo->prAdapter,
			BAND_2G4, MAXMUN_2G_CHA_NUM, &ucMax_2G_Chn_List, aucChannelList);
#else
	ucMax_2G_Chn_List = 11;
#endif

	prGetChnLoad = &(prGlueInfo->prAdapter->rWifiVar.rChnLoadInfo);

	/* 2. Calculate each channel direty score */
	for (ucIdx = 0; ucIdx < ucMax_2G_Chn_List; ucIdx++) {
		/* Current channel dirty score */
		...
		if (ucIdx < 3) {
			...
			u2UpThreshold = 3;
		} else if (ucIdx >= (ucMax_2G_Chn_List - 3)) {
			u2LowThreshold = 3;
			u2UpThreshold = ucMax_2G_Chn_List - (ucIdx + 1);
		}

		/* Lower channel dirty score */
		...
		arChannelLoadingWeightScore_2G[ucIdx].u2APNum = u2APNumScore;
	...
	/* 4. Find out the best channel, skip LTE unsafe channel */
	for (ucIdx = 0; ucIdx < ucMax_2G_Chn_List; ucIdx++) {
		if (!(u4LteSafeChnBitMask_2G & BIT(ucIdx + 1)))
			continue;

		if (rPreferChannels[0].u2APNum >= arChannelLoadingWeightScore_2G[ucIdx].u2APNum) {
			rPreferChannels[0].ucChannel = ucIdx + 1;
			rPreferChannels[0].u2APNum = arChannelLoadingWeightScore_2G[ucIdx].u2APNum;
		}
	}

	u4AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_2G_BASE_1 - 1] = fgIsReady ? BIT(31) : 0;
	if (rPreferChannels[0].ucChannel > 0)
		u4AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_2G_BASE_1 - 1] |= BIT(rPreferChannels[0].ucChannel - 1);

	/* ToDo: Support 5G channel selection */
	u4AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_5G_BASE_34 - 1] = 0x11223344;
	u4AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_5G_BASE_149 - 1] = 0x55667788;
	u4AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_5G_BASE_184 - 1] = 0x99AABBCC;

acs_report:
	NLA_PUT_U32(skb, NL80211_TESTMODE_AVAILABLE_CHAN_2G_BASE_1,
		    u4AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_2G_BASE_1 - 1]);
	NLA_PUT_U32(skb, NL80211_TESTMODE_AVAILABLE_CHAN_5G_BASE_34,
		    u4AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_5G_BASE_34 - 1]);
	NLA_PUT_U32(skb, NL80211_TESTMODE_AVAILABLE_CHAN_5G_BASE_149,
		    u4AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_5G_BASE_149 - 1]);
	NLA_PUT_U32(skb, NL80211_TESTMODE_AVAILABLE_CHAN_5G_BASE_184,
		    u4AcsChnReport[NL80211_TESTMODE_AVAILABLE_CHAN_5G_BASE_184 - 1]);
