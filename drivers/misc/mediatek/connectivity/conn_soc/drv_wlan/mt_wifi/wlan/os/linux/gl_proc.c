...
#define MODULE_NAME_LEN_1 5
	...
	UINT_32 u4Enabled;
	UINT_8 aucModuleArray[CFG_NUM][MODULE_NAME_LEN_1] = {"ARP", "DNS", "TCP", "UDP"};
	...
	while (temp) {
		...
		for (i = 0; i < CFG_NUM; i++) {
			if (kalStrniCmp(aucModule, aucModuleArray[i], MODULE_NAME_LEN_1) == 0) {
				if (u4Enabled)
					u2SetTxDoneCfg |= 1 << i;
				else
					u2ClsTxDoneCfg |= 1 << i;
...
static ssize_t procfile_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (*f_pos > 0) {
...
#endif
		{
			...
			DBGLOG(INIT, TRACE, "%d bytes avaliable\n", gCoexBuf1.availSize);
...
cfgWrite(struct file *filp, const char *buf, unsigned long count, void *data) {
	...
	case 3: /* Set cfg */
		if (u4CopySize > 0)
			wlanCfgParse(gprGlueInfo->prAdapter, (PUINT8)&aucCfgBuf, u4CopySize, TRUE);
		break;
