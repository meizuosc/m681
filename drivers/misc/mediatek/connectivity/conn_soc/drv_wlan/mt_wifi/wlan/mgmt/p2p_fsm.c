...
** [ALPS00410124] [Rose][Free Test][KE][rlmUpdateParamsForAP]The device reboot automaticly
...
VOID p2pFsmRunEventStopAP(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	...
	do {
		...
		DBGLOG(P2P, INFO, "p2pFsmRunEventStopAP, Re activate P2P Network.\n");
		nicDeactivateNetwork(prAdapter, NETWORK_TYPE_P2P_INDEX);
			   ...
			   IN P_MSDU_INFO_T prMsduInfo, IN ENUM_TX_RESULT_CODE_T rTxDoneStatus)
{
	...
	do {
		...
		DBGLOG(P2P, INFO, "Deauth TX Done, prStaRec %p, status: %d\n",
			prStaRec, rTxDoneStatus);
		...
		if ((prP2pBssInfo->eCurrentOPMode != OP_MODE_ACCESS_POINT) ||
			...
			DBGLOG(P2P, INFO, "No More Client, Media Status DISCONNECTED\n");
			p2pChangeMediaState(prAdapter, PARAM_MEDIA_STATE_DISCONNECTED);
		}
			      ...
			      IN P_MSDU_INFO_T prMsduInfo, IN ENUM_TX_RESULT_CODE_T rTxDoneStatus)
{
	...
	do {
		...
		if (rTxDoneStatus != TX_RESULT_SUCCESS) {
			DBGLOG(P2P, INFO, "Mgmt Frame TX Fail, Status:%d.\n", rTxDoneStatus);
		} else {
			fgIsSuccess = TRUE;
...
VOID p2pFsmRunEventRxDeauthentication(IN P_ADAPTER_T prAdapter, IN P_STA_RECORD_T prStaRec, IN P_SW_RFB_T prSwRfb)
{
	...
	do {
		...
		DBGLOG(P2P, INFO, "RX Deauth, Deauth Reason:%d\n", u2ReasonCode);
...
VOID p2pFsmRunEventRxDisassociation(IN P_ADAPTER_T prAdapter, IN P_STA_RECORD_T prStaRec, IN P_SW_RFB_T prSwRfb)
{
	...
	do {
		...
		DBGLOG(P2P, TRACE, "RX Disassoc, Disassoc reason %d\n", u2ReasonCode);
...
VOID p2pFsmRunEventWfdSettingUpdate(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	...
	DBGLOG(P2P, TRACE, "p2pFsmRunEventWfdSettingUpdate\n");
	...
	do {
		...
		DBGLOG(P2P, INFO, "p2pFsmRunEventWfdSettingUpdate, WFD Enalbe %x info %x state %x flag %x adv %x\n",
