 ...
 * continous buffer instead of physically contineous one
 *
 * 01 04 2011 cp.wu
 * [WCXRP00000338] [MT6620 Wi-Fi][Driver] Separate kalMemAlloc into kmalloc
 * and vmalloc implementations to ease physically continous memory demands
...
WLAN_STATUS scanProcessBeaconAndProbeResp(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
	...
	if (prBssDesc) {
		...
		if (prAisBssInfo->eConnectionState == PARAM_MEDIA_STATE_CONNECTED &&
			...
			prAisBssInfo->fgDetectConnectedAP = TRUE;
		}
		...
		if (prAdapter->fgIsP2PRegistered) {
			/*
			 * Sanity check:
			 * Do not indicate BSS to supplicant if BSS is on 5G
			 * channel while driver is configured without 5G channel
			 * support
			 */
			if (prBssDesc && prBssDesc->eBand == BAND_5G &&
				!prAdapter->fgEnable5GBand) {
				DBGLOG(SCN, INFO, "P2P: scanned BSS on 5G channel while 5G not support, ignore it!\n");
				return rStatus;
			}
