...
wlanoidSetConnect(IN P_ADAPTER_T prAdapter, IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	if (fgIsValidSsid) {
		...
		prConnSettings->eReConnectLevel = RECONNECT_LEVEL_INTER_SET;
		       ...
		       IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = 0;
	...
	/* prepare message to AIS */
	prAdapter->rWifiVar.rConnSettings.fgIsConnReqIssued = FALSE;
	prAdapter->rWifiVar.rConnSettings.eReConnectLevel = RECONNECT_LEVEL_INTER_SET;
		    ...
		    IN PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	CMD_GET_LTE_SAFE_CHN_T rQueryLteSaftChn;

	DBGLOG(P2P, INFO, "[ACS]wlanoidQueryLteSafeChannel\n");
