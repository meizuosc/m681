...
kalIndicateStatusAndComplete(IN P_GLUE_INFO_T prGlueInfo, IN WLAN_STATUS eStatus, IN PVOID pvBuf, IN UINT_32 u4BufLen)
{
	...
	case WLAN_STATUS_MEDIA_CONNECT:
		...
		if (prGlueInfo->fgIsRegistered == TRUE) {
			...
			if (bss == NULL) {
				...
				if (prBssDesc != NULL) {
					bss = cfg80211_inform_bss(priv_to_wiphy(prGlueInfo), prChannel, arBssid,
								0,	/* TSF */
								WLAN_CAPABILITY_ESS,
	...
	case WLAN_STATUS_MEDIA_DISCONNECT_LOCALLY:
		...
		DBGLOG(AIS, INFO, " %s netif_carrier_off %d\n",
...
BOOLEAN kalIsWakeupByWlan(P_ADAPTER_T  prAdapter)
{
	...
	return !!(spm_get_last_wakeup_src() & WAKE_SRC_R12_CONN2AP_SPM_WAKEUP_B); /* for jade only */
}
#endif
