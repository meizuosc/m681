...
#define L2_BUF_SIZE (384*KBYTES)
	...
	core_dmp->compressor = wcn_compressor_init("core_dump_compressor", L1_BUF_SIZE, 13*packet_num*KBYTES);
...
INT32 stp_dbg_cpupcr_infor_format(PPUINT8 buf, PUINT32 str_len)
{
	...
	if (!g_stp_dbg_cpupcr->wifiVer[0] && g_u2FwIDVersion == 0)
		len += osal_sprintf(*buf + len, "<wifi>NULL</wifi>\n\t");
	else if (g_u2FwIDVersion != 0) {
		len += osal_sprintf(*buf + len, "<wifi>0x%X.%X</wifi>\n\t", (UINT8)((g_u2FwIDVersion & 0xFF00)>>8),
			(UINT8)(g_u2FwIDVersion & 0xFF));
	}
	else
