...
INT32 wmt_dbg_fwinfor_from_emi(INT32 par1, INT32 par2, INT32 par3)
{
	...
	osal_memset(&buf[0], 0, BUF_LEN_MAX);
...
#else
static ssize_t wmt_dev_dbg_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	INT32 len = 0;
	...
	if (off > 0) {
		...
		if (gCoexBuf.availSize <= 0) {
			...
			len = osal_sprintf(page,
					"no data available, please run echo 15 xx > /proc/driver/wmt_psm first\n");
		} else {
			INT32 i = 0;
			/*we do not check page buffer, because there are only 100 bytes in g_coex_buf,
			* no reason page buffer is not enough, a bomb is placed here on unexpected
			* condition
			*/
...
static ssize_t wmt_dev_dbg_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
...
#else
static ssize_t wmt_dev_proc_for_aee_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	UINT32 len = 0;
...
long WMT_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case WMT_IOCTL_SET_PATCH_INFO:{
			...
			WMT_DBG_FUNC(
				"patch dl seq %d,name %s,addres info 0x%02x,0x%02x,0x%02x,0x%02x\n",
