...
static UINT32 gDbgLevel = BT_LOG_WARN;
...
long BT_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case COMBO_IOCTL_FW_ASSERT:
		...
		BT_WARN_FUNC("BT Set fw assert......, reason:%lu\n", arg);
...
static int BT_open(struct inode *inode, struct file *file)
{
	...
	if (mtk_wcn_stp_is_ready()) {
		...
		platform_load_nvram_data(BT_NVRAM_CUSTOM_NAME, (PINT8) &g_nvram_btdata, sizeof(g_nvram_btdata));
