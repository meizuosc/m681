...
static INT32 (*fp_wmt_tra_sdio_update)(VOID) = NULL;
...
#if (WMT_PLAT_APEX == 0)
extern INT32 mtk_wcn_sdio_irq_flag_set(INT32 falg);
#else
INT32 mtk_wcn_sdio_irq_flag_set(INT32 falg)
{
	return 0;
}
#endif
	...
	for (i = 0; i < array_size; i++) {
		...
		for (j = 0; j < clt_info_size; j++)
			memset(&g_hif_sdio_ds_info_list[i].clt_info[j],
			       sizeof(MTK_WCN_HIF_SDIO_DS_CLT_INFO), 0);
...
INT32 mtk_wcn_hif_sdio_client_reg(const MTK_WCN_HIF_SDIO_CLTINFO *pinfo)
{
	...
	for (i = 0; i < pinfo->func_tbl_size; i++) {
		...
		if (ret) {
			HIF_SDIO_WARN_FUNC
			    ("vendor id(0x%x), device id(0x%x), and fun_num(%d) of sdio_func are duplicated in g_hif_sdio_clt_drv_list!\n",
			     pinfo->func_tbl[i].manf_id, pinfo->func_tbl[i].card_id,
			     pinfo->func_tbl[i].func_num);
			goto out;
		}
		...
		for (j = 0; j < CFG_CLIENT_COUNT; j++) {
			...
			if ((g_hif_sdio_clt_drv_list[clt_index].func_info->manf_id ==
				...
				HIF_SDIO_INFO_FUNC("Reset sdio block size: %d!\n",
						   g_hif_sdio_clt_drv_list[clt_index].func_info->
						   blk_sz);
				sdio_claim_host(g_hif_sdio_probed_func_list[j].func);
...
static VOID hif_sdio_clt_probe_worker(struct work_struct *work)
{
	...
	clt_worker_info_p->registinfo_p->sdio_cltinfo->
	    hif_clt_probe(CLTCTX(card_id, func_num, blk_sz, prob_idx),
 ...
 * \retval >= 0  return found index
 */
static INT32 hif_sdio_find_probed_list_index_by_id_func(UINT16 vendor, UINT16 device, UINT16 func_num)
{
	INT32 i;
	for (i = 0; i < CFG_CLIENT_COUNT; i++) {
		if (g_hif_sdio_probed_func_list[i].func) {
	...
	if (i == CFG_CLIENT_COUNT) {
		/*
		   pr_warn(KERN_INFO DRV_NAME "Cannot find vendor:0x%x, device:0x%x, func_num:0x%x, i=%d\n",
...
static INT32 hif_sdio_probe(struct sdio_func *func, const struct sdio_device_id *id)
{
	...
	if (hif_sdio_probed_funcp->clt_idx >= 0 && hif_sdio_probed_funcp->clt_idx < CFG_CLIENT_COUNT) {	/* The clt contex has been registed */
		sdio_claim_host(func);
		idx = hif_sdio_probed_funcp->clt_idx;
		ret =
		    sdio_set_block_size(func,
					g_hif_sdio_clt_drv_list[idx].func_info->blk_sz);
		sdio_release_host(func);
...
static VOID hif_sdio_remove(struct sdio_func *func)
{
	...
	if (registed_list_index >= 0) {
		g_hif_sdio_clt_drv_list[registed_list_index].sdio_cltinfo->
		    hif_clt_remove(CLTCTX
				   (func->device, func->num, func->cur_blksize, probed_list_index));
	}
#endif
...
INT32 hif_sdio_wifi_off(VOID)
{
 ...
 wifi_off_exist:
	...
	sdio_release_host(func);
	if (ret) {
		pr_warn(KERN_WARNING DRV_NAME "sdio_release_irq for wifi fail(%d)\n", ret);
	} else {
		pr_warn(KERN_INFO DRV_NAME "sdio_release_irq for wifi ok\n");
	}
#endif
...
INT32 mtk_wcn_hif_sdio_bus_set_power(MTK_WCN_HIF_SDIO_CLTCTX ctx, UINT32 pwrState)
{
	...
	probe_index = CLTCTX_IDX(ctx);
	if (CFG_CLIENT_COUNT > probe_index && probe_index >= 0)
		func = g_hif_sdio_probed_func_list[probe_index].func;
	else {
