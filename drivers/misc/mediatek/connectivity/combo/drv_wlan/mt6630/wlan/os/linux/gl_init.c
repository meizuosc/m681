...
static struct wiphy_vendor_command mtk_wlan_vendor_ops[] = {
	{
	 {
...
static struct nl80211_vendor_cmd_info mtk_wlan_vendor_events[] = {
	{
	.vendor_id = GOOGLE_OUI,
...
void wlanMonWorkHandler(struct work_struct *work)
{
	...
	if (prGlueInfo->fgIsEnableMon) {
		...
		prGlueInfo->prMonDevHandler =
		    alloc_netdev_mq(sizeof(NETDEV_PRIVATE_GLUE_INFO), NIC_MONITOR_INF_NAME, ether_setup,
				    CFG_MAX_TXQ_NUM);
...
int set_p2p_mode_handler(struct net_device *netdev, PARAM_CUSTOM_P2P_SET_STRUC_T p2pmode)
{
	...
	rWlanStatus = kalIoctl(prGlueInfo,
			       wlanoidSetP2pMode,
			       (PVOID) &rSetP2P, sizeof(PARAM_CUSTOM_P2P_SET_STRUC_T), FALSE, FALSE, TRUE, &u4BufLen);
