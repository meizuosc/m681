 ...
 *  INTERNAL FUNCTION
 **************************************************************************/
static a_ctx_t *a_alloc_ctx(unsigned char *key, unsigned int keyLen);
static void a_free_ctx(a_ctx_t *ctx);

inline ulong a_subword(ulong w);
inline ulong a_rotword(ulong w);
static void a_key_exp(a_ctx_t *ctx);

inline unsigned char a_mul_manual(unsigned char a, unsigned char b);	/* use a_mul instead */
static void a_subbytes(a_ctx_t *ctx);
static void a_shi_row(a_ctx_t *ctx);
static void a_mix_col(a_ctx_t *ctx);
static void a_add_key(a_ctx_t *ctx, int round);

static void a_sub_b(a_ctx_t *ctx);
static void a_inv_shi_row(a_ctx_t *ctx);
static void a_inv_mix_col(a_ctx_t *ctx);
...
inline ulong a_subword(ulong w)
{
	return g_a_sbox[w & 0x000000ff] |
		...
		(g_a_sbox[(w & 0x00ff0000) >> 16] << 16) | (g_a_sbox[(w & 0xff000000) >> 24] << 24);
...
inline ulong a_rotword(ulong w)
{
	return ((w & 0x000000ff) << 24) |
		((w & 0x0000ff00) >> 8) | ((w & 0x00ff0000) >> 8) | ((w & 0xff000000) >> 8);
...
inline unsigned char a_mul_manual(unsigned char a, unsigned char b)
{
...
void a_mix_col(a_ctx_t *ctx)
{
	...
	for (i = 0; i < 4; i++) {
		nstate[0][i] = a_mul(0x02, ctx->state[0][i]) ^
			a_mul(0x03, ctx->state[1][i]) ^ ctx->state[2][i] ^ ctx->state[3][i];
		nstate[1][i] = ctx->state[0][i] ^
			a_mul(0x02, ctx->state[1][i]) ^
			a_mul(0x03, ctx->state[2][i]) ^ ctx->state[3][i];
		nstate[2][i] = ctx->state[0][i] ^
			ctx->state[1][i] ^
			a_mul(0x02, ctx->state[2][i]) ^ a_mul(0x03, ctx->state[3][i]);
		nstate[3][i] = a_mul(0x03, ctx->state[0][i]) ^
			ctx->state[1][i] ^ ctx->state[2][i] ^ a_mul(0x02, ctx->state[3][i]);
	}
...
void a_inv_mix_col(a_ctx_t *ctx)
{
	...
	for (i = 0; i < 4; i++) {
		nstate[0][i] = a_mul(0x0e, ctx->state[0][i]) ^
			...
			a_mul(0x0d, ctx->state[2][i]) ^ a_mul(0x09, ctx->state[3][i]);
		nstate[1][i] = a_mul(0x09, ctx->state[0][i]) ^
			a_mul(0x0e, ctx->state[1][i]) ^
			a_mul(0x0b, ctx->state[2][i]) ^ a_mul(0x0d, ctx->state[3][i]);
		nstate[2][i] = a_mul(0x0d, ctx->state[0][i]) ^
			a_mul(0x09, ctx->state[1][i]) ^
			a_mul(0x0e, ctx->state[2][i]) ^ a_mul(0x0b, ctx->state[3][i]);
		nstate[3][i] = a_mul(0x0b, ctx->state[0][i]) ^
			a_mul(0x0d, ctx->state[1][i]) ^
			a_mul(0x09, ctx->state[2][i]) ^ a_mul(0x0e, ctx->state[3][i]);
	}
 ...
 **************************************************************************/
int aes_legacy_enc(unsigned char *input_buf, unsigned int input_len, unsigned char *output_buf, unsigned int output_len)
{
	unsigned int i = 0;
 ...
 **************************************************************************/
int aes_legacy_dec(unsigned char *input_buf, unsigned int input_len, unsigned char *output_buf, unsigned int output_len)
{
	unsigned int i = 0;
