...
static void A_F(ulong **RK, ulong *X0, ulong *X1, ulong *X2, ulong *X3, ulong Y0, ulong Y1, ulong Y2, ulong Y3)
{
	*X0 = **RK ^ FT0[(Y0) & MASK] ^ FT1[(Y1 >>  8) & MASK] ^ FT2[(Y2 >> 16) & MASK] ^ FT3[(Y3 >> 24) & MASK];
	(*RK)++;
	*X1 = **RK ^ FT0[(Y1) & MASK] ^ FT1[(Y2 >>  8) & MASK] ^ FT2[(Y3 >> 16) & MASK] ^ FT3[(Y0 >> 24) & MASK];
	(*RK)++;
	*X2 = **RK ^ FT0[(Y2) & MASK] ^ FT1[(Y3 >>  8) & MASK] ^ FT2[(Y0 >> 16) & MASK] ^ FT3[(Y1 >> 24) & MASK];
	(*RK)++;
	*X3 = **RK ^ FT0[(Y3) & MASK] ^ FT1[(Y0 >>  8) & MASK] ^ FT2[(Y1 >> 16) & MASK] ^ FT3[(Y2 >> 24) & MASK];
	(*RK)++;

	return;
}

static void A_R(ulong **RK, ulong *X0, ulong *X1, ulong *X2, ulong *X3, ulong Y0, ulong Y1, ulong Y2, ulong Y3)
{
	*X0 = **RK ^ RT0[(Y0) & MASK] ^ RT1[(Y3 >>  8) & MASK] ^ RT2[(Y2 >> 16) & MASK] ^ RT3[(Y1 >> 24) & MASK];
	(*RK)++;
	*X1 = **RK ^ RT0[(Y1) & MASK] ^ RT1[(Y0 >>  8) & MASK] ^ RT2[(Y3 >> 16) & MASK] ^ RT3[(Y2 >> 24) & MASK];
	(*RK)++;
	*X2 = **RK ^ RT0[(Y2) & MASK] ^ RT1[(Y1 >>  8) & MASK] ^ RT2[(Y0 >> 16) & MASK] ^ RT3[(Y3 >> 24) & MASK];
	(*RK)++;
	*X3 = **RK ^ RT0[(Y3) & MASK] ^ RT1[(Y2 >>  8) & MASK] ^ RT2[(Y1 >> 16) & MASK] ^ RT3[(Y0 >> 24) & MASK];
	(*RK)++;
 ...
 *  MTK SECRET
 **************************************************************************/
static unsigned int g_AES_IV[4] = {0x6c8d3259, 0x86911412, 0x55975412, 0x6c8d3257};
static unsigned int g_AES_IV_TEMP[4] = {0x0, 0x0, 0x0, 0x0};
unsigned int g_AES_Key[4] = {0x0, 0x0, 0x0, 0x0};
...
void G_U_LE(ulong *n, unsigned char *b, unsigned int i)
{
	unsigned int j;
...
void P_U_LE(ulong n, unsigned char *b, unsigned int i)
{
	unsigned int j;
	for (j = 0; j < 4; j++)
		b[i + j] = ((unsigned char)n >> (8 * i));
}
...
static void a_gen_tables(void)
{
	...
	for (i = 0; i < T_SZ; i++) {
		...
		RT0[i] = ((ulong) MUL(0x0E, x)) ^
			((ulong) MUL(0x09, x) << 8) ^
			((ulong) MUL(0x0D, x) << 16) ^ ((ulong) MUL(0x0B, x) << 24);
	...
	*RK++ = *SK++;
	...
	for (i = ctx->nr - 1, SK -= 8; i > 0; i--, SK -= 8) {
		for (j = 0; j < 4; j++, SK++) {
			*RK++ = RT0[FS[(*SK) & MASK]] ^
				RT1[FS[(*SK >> 8) & MASK]] ^
				RT2[FS[(*SK >> 16) & MASK]] ^ RT3[FS[(*SK >> 24) & MASK]];
		}
	}
...
int a_crypt_ecb(a_ctx *ctx, int mode, const unsigned char input[16], unsigned char output[16])
{
	int i;
	...
	if (mode == AES_DECRYPT) {
		for (i = (ctx->nr >> 1) - 1; i > 0; i--) {
			A_R(&RK, &Y0, &Y1, &Y2, &Y3, X0, X1, X2, X3);
			A_R(&RK, &X0, &X1, &X2, &X3, Y0, Y1, Y2, Y3);
		}
		...
		X0 = *RK++ ^
			...
			((ulong) RS[(Y2 >> 16) & MASK] << 16) ^ ((ulong) RS[(Y1 >> 24) & MASK] << 24);

		X1 = *RK++ ^
			((ulong) RS[(Y1) & MASK]) ^
			((ulong) RS[(Y0 >> 8) & MASK] << 8) ^
			((ulong) RS[(Y3 >> 16) & MASK] << 16) ^ ((ulong) RS[(Y2 >> 24) & MASK] << 24);

		X2 = *RK++ ^
			((ulong) RS[(Y2) & MASK]) ^
			((ulong) RS[(Y1 >> 8) & MASK] << 8) ^
			((ulong) RS[(Y0 >> 16) & MASK] << 16) ^ ((ulong) RS[(Y3 >> 24) & MASK] << 24);

		X3 = *RK++ ^
			((ulong) RS[(Y3) & MASK]) ^
			((ulong) RS[(Y2 >> 8) & MASK] << 8) ^
			((ulong) RS[(Y1 >> 16) & MASK] << 16) ^ ((ulong) RS[(Y0 >> 24) & MASK] << 24);
	} else {		/* AES_ENCRYPT */

		for (i = (ctx->nr >> 1) - 1; i > 0; i--) {
			A_F(&RK, &Y0, &Y1, &Y2, &Y3, X0, X1, X2, X3);
			A_F(&RK, &X0, &X1, &X2, &X3, Y0, Y1, Y2, Y3);
		}
		...
		X0 = *RK++ ^
			...
			((ulong) FS[(Y2 >> 16) & MASK] << 16) ^ ((ulong) FS[(Y3 >> 24) & MASK] << 24);

		X1 = *RK++ ^
			((ulong) FS[(Y1) & MASK]) ^
			((ulong) FS[(Y2 >> 8) & MASK] << 8) ^
			((ulong) FS[(Y3 >> 16) & MASK] << 16) ^ ((ulong) FS[(Y0 >> 24) & MASK] << 24);

		X2 = *RK++ ^
			((ulong) FS[(Y2) & MASK]) ^
			((ulong) FS[(Y3 >> 8) & MASK] << 8) ^
			((ulong) FS[(Y0 >> 16) & MASK] << 16) ^ ((ulong) FS[(Y1 >> 24) & MASK] << 24);

		X3 = *RK++ ^
			((ulong) FS[(Y3) & MASK]) ^
			((ulong) FS[(Y0 >> 8) & MASK] << 8) ^
			((ulong) FS[(Y1 >> 16) & MASK] << 16) ^ ((ulong) FS[(Y2 >> 24) & MASK] << 24);
	}
...
int a_crypt_cbc(a_ctx *ctx,
		int mode, size_t length, unsigned char iv[16], const unsigned char *input, unsigned char *output)
{
	int i;
...
int aes_so_enc(unsigned char *ip_buf, unsigned int ip_len, unsigned char *op_buf, unsigned int op_len)
{
	...
	for (i = 0; i != ip_len; i += CIPHER_BLOCK_SIZE) {
		ret = a_crypt_cbc(&aes, AES_ENCRYPT, CIPHER_BLOCK_SIZE,
					(unsigned char *) g_AES_IV_TEMP, ip_buf + i, op_buf + i);
		if (ret != 0) {
