...
static void sec_cipherfmt_dump_buffer(unsigned char *buf, unsigned int len)
{
	unsigned int i = 0;
...
int sec_cipherfmt_check_cipher(ASF_FILE fp, unsigned int start_off, unsigned int *img_len)
{
	...
	if (!ASF_FILE_ERROR(fp)) {
		...
		/* get header */
		read_sz = ASF_READ(fp, (char *)&cipher_header, CIPHER_IMG_HEADER_SIZE);
		if (CIPHER_IMG_HEADER_SIZE != read_sz) {
			SMSG(true, "[%s] read size '%d' != '%d'\n", MOD, read_sz,
			     CIPHER_IMG_HEADER_SIZE);
			    ...
			    unsigned int *data_offset)
{
	...
	SMSG(true, "[%s] sec_cipherfmt_decrypted - cipher_offset is %d (0x%x)\n", MOD,
	     cipher_header.cipher_offset, cipher_header.cipher_offset);
	SMSG(true, "[%s] sec_cipherfmt_decrypted - cipher_length %d (0x%x)\n", MOD,
	     cipher_header.cipher_length, cipher_header.cipher_length);

	/* by pass cipher offset, cause this part is not encrypted */
	if (cipher_header.cipher_offset) {
		/* get header */
		read_sz = ASF_READ(fp, (char *)buf_ptr, cipher_header.cipher_offset);
		if (cipher_header.cipher_offset != read_sz) {
			SMSG(true, "[%s] read start size '%d' != '%d'\n", MOD, read_sz,
			     cipher_header.cipher_offset);
			ret = ERR_IMAGE_CIPHER_READ_FAIL;
	...
	if (cipher_header.cipher_length) {
		...
		while (read_offset < end_pos) {
			...
			read_sz = ASF_READ(fp, (char *)tmp_buf, CIPHER_BLOCK_SIZE);
			if (CIPHER_BLOCK_SIZE != read_sz) {
				SMSG(true, "[%s] read cipher size '%d' != '%d'(try)\n", MOD,
				     read_sz, CIPHER_BLOCK_SIZE);
				ret = ERR_IMAGE_CIPHER_READ_FAIL;
		...
		if (try_read_len) {
			memset(tmp_buf, 0x00, CIPHER_BLOCK_SIZE);
			read_sz = ASF_READ(fp, (char *)tmp_buf, try_read_len);
			if (try_read_len != read_sz) {
				SMSG(true, "[%s] read cipher size '%d' != '%d'(remain)\n", MOD,
				     read_sz, try_read_len);
				ret = ERR_IMAGE_CIPHER_READ_FAIL;
				goto _read_cipher_size_wrong;
			}
#if DUMP_MORE_FOR_DEBUG
...
#if DUMP_MORE_FOR_DEBUG
			SMSG(true,
			     "[%s] sec_cipherfmt_decrypted - dump decipher part: 0x%x (try)\n", MOD,
			     read_offset);
	...
	if (total_len > 0) {
		read_sz = ASF_READ(fp, (char *)buf_ptr, total_len);
		if (total_len != read_sz) {
