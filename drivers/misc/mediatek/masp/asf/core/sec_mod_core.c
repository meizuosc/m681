...
long sec_core_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case SEC_SECCFG_ENCRYPT:
		...
		if (SEC_CFG_END_PATTERN == seccfg.v1.end_pattern) {
			...
			sec_update_lks(seccfg.v1.sw_sec_lock_try, seccfg.v1.sw_sec_lock_done,
				       seccfg.v1.attr == ATTR_DISABLE_IMG_CHECK);
			masp_hal_sp_hacc_enc((unsigned char *)&seccfg.v1.image_info, cipher_len,
					     rom_info.m_SEC_CTRL.m_seccfg_ac_en, HACC_USER1, FALSE);
			...
			sec_update_lks(seccfg.v3.sw_sec_lock_try, seccfg.v3.sw_sec_lock_done,
				       seccfg.v3.seccfg_attr == ATTR_DISABLE_IMG_CHECK);
			masp_hal_sp_hacc_enc((unsigned char *)&seccfg.v3.image_info, cipher_len,
					     rom_info.m_SEC_CTRL.m_seccfg_ac_en, HACC_USER1, FALSE);
		} else {
	...
	case SEC_SECCFG_DECRYPT:
		...
		if (SEC_CFG_END_PATTERN == seccfg.v1.end_pattern) {
			...
			cipher_len = get_seccfg_cipher_len();
			masp_hal_sp_hacc_dec((unsigned char *)&seccfg.v1.image_info, cipher_len,
					     rom_info.m_SEC_CTRL.m_seccfg_ac_en, HACC_USER1, FALSE);
			sec_update_lks(seccfg.v1.sw_sec_lock_try, seccfg.v1.sw_sec_lock_done,
				       seccfg.v1.attr == ATTR_DISABLE_IMG_CHECK);
		} else if (SEC_CFG_END_PATTERN == seccfg.v3.end_pattern) {
			...
			cipher_len = get_seccfg_cipher_len();
			masp_hal_sp_hacc_dec((unsigned char *)&seccfg.v3.image_info, cipher_len,
					     rom_info.m_SEC_CTRL.m_seccfg_ac_en, HACC_USER1, FALSE);
			sec_update_lks(seccfg.v3.sw_sec_lock_try, seccfg.v3.sw_sec_lock_done,
				       seccfg.v3.seccfg_attr == ATTR_DISABLE_IMG_CHECK);
		} else {
	...
	case SEC_READ_ROM_INFO:
		...
		ret =
		    osal_copy_to_user((void __user *)arg, (void *)&rom_info, sizeof(AND_ROMINFO_T));
		break;
