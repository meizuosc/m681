...
static u64 sec_get_u64(unsigned int high, unsigned int low)
{
	u64 value = 0;
 ...
 **************************************************************************/
static int sec_signfmt_image_read_64(ASF_FILE fp, char *part_name, u64 seek_offset, char *read_buf,
				     unsigned int read_len)
{
	...
	/* read from mtd */
		...
		ret = sec_dev_read_image(pl2part(part_name),
						(char *)read_buf,
						seek_offset, read_len, NORMAL_ROM);
		if (SEC_OK != ret) {
			SMSG(true,
			     "[%s] read mtd '%s' fail at image offset 0x%llx with length 0x%x\n",
...
static int sec_signfmt_gen_hash_by_chunk_64(ASF_FILE img_fd, char *part_name, u64 img_hash_off,
					    u64 img_hash_len, unsigned char *final_hash_buf,
					    SEC_CRYPTO_HASH_TYPE hash_type, unsigned int chunk_size)
{
	...
	br = sec_signfmt_image_read_64(img_fd, part_name, seek_pos * sizeof(char),
 ...
 **************************************************************************/
static unsigned int sec_signfmt_search_extension_v4(unsigned char *ext, unsigned int ext_len,
					      SEC_IMG_EXTENSTION_SET *ext_set)
{
	...
	while (d_ptr < end_ptr) {
		...
		case SEC_EXT_HDR_FRAG_CFG:
			...
			ext_set->hash_only_64 =
			    (SEC_EXTENSTION_HASH_ONLY_64 **) ASF_MALLOC(ext_set->frag->frag_count *
									sizeof
									(SEC_EXTENSTION_HASH_ONLY_64
									 *));
			break;
		case SEC_EXT_HDR_HASH_ONLY_64:
			ext_set->hash_only_64[hash_only_idx] =
			    (SEC_EXTENSTION_HASH_ONLY_64 *) d_ptr;
			d_ptr +=
			    sizeof(SEC_EXTENSTION_HASH_ONLY_64) +
			    get_hash_size(ext_set->hash_only_64[hash_only_idx]->sub_type);
			hash_only_idx++;
			break;
	...
	if (ext_set->crypto == NULL || ext_set->frag == NULL || ext_set->hash_only_64 == NULL
	    || ext_set->end == NULL) {
					    ...
					    SEC_IMG_HEADER *file_img_hdr_p, char *ext_buf)
{
	...
	if (ASF_FILE_NULL == fp) {
		ext_crypto_ptr =
		    (SEC_EXTENSTION_CRYPTO *) (ext_buf + img_if->ext_offset +
					       img_if->header.v4.signature_length);
	...
	/* get from file's extension header */
		...
		crypto_hdr_offset =
		    sec_get_u64(file_img_hdr->image_length_high, file_img_hdr->image_length_low);
					...
					char *final_hash_buf, unsigned int hash_len, char *ext_buf)
{
	...
	sec_signfmt_dump_buffer((unsigned char *) img_hdr, sizeof(SEC_IMG_HEADER_V4));
	...
	ret = sec_signfmt_search_extension_v4(ext_hdr_buf, ext_hdr_len, &ext_set);
	if (SEC_OK != ret) {
	...
	for (i = 0; i < ext_set.frag->frag_count; i++) {
		...
		if (ext_set.frag->chunk_size == 0) {
			real_chunk_size =
			    ext_set.hash_only_64[i]->hash_len_64 & 0x00000000FFFFFFFFULL;
		} else {
			real_chunk_size = ext_set.frag->chunk_size;
		}
		if (sec_signfmt_gen_hash_by_chunk_64
		    (ASF_FILE_NULL, part_name, ext_set.hash_only_64[i]->hash_offset_64,
		     ext_set.hash_only_64[i]->hash_len_64, tmp_ptr,
		     ext_set.hash_only_64[i]->sub_type, real_chunk_size) != 0) {
...
int sec_signfmt_verify_file_v4(ASF_FILE fp, SEC_IMG_HEADER *file_img_hdr_p)
{
...
#if DUMP_MORE_FOR_DEBUG
	SMSG(sec_info.bMsg, "[%s] Dump header ============> START\n", MOD);
	sec_signfmt_dump_buffer((unsigned char *) file_img_hdr, sizeof(SEC_IMG_HEADER_V4));
	SMSG(sec_info.bMsg, "[%s] Dump header ============> END\n", MOD);
#endif

	if (SEC_IMG_MAGIC != file_img_hdr->magic_number) {
		SMSG(true, "[%s] magic number is invalid '0x%x'\n", MOD,
		     file_img_hdr->magic_number);
		ret = ERR_SIGN_FORMAT_MAGIC_WRONG;
		goto _magic_wrong_err;
	}
	...
	memset(&ext_set, 0x00, sizeof(SEC_IMG_EXTENSTION_SET));
	ext_hdr_offset =
	    sec_get_u64(file_img_hdr->image_length_high,
			file_img_hdr->image_length_low) + file_img_hdr->image_offset +
	    file_img_hdr->signature_length;
	...
	for (i = 0; i < ext_set.frag->frag_count; i++) {
		...
		if (sec_signfmt_gen_hash_by_chunk_64
		    (fp, NULL, SEC_IMG_HEADER_SIZE + ext_set.hash_only_64[i]->hash_offset_64,
		     ext_set.hash_only_64[i]->hash_len_64, tmp_ptr,
		     ext_set.hash_only_64[i]->sub_type, real_chunk_size) != 0) {
			ret = ERR_SIGN_FORMAT_CAL_HASH_BY_CHUNK_FAIL;
			goto _gen_hash_by_chunk_fail;
