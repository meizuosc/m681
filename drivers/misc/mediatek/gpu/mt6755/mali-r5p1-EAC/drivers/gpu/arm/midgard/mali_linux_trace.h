 ...
 */
TRACE_EVENT(mali_job_slots_event, TP_PROTO(unsigned int event_id, unsigned int tgid, unsigned int pid, unsigned char atom_id, u32 job_id), TP_ARGS(event_id, tgid, pid, atom_id, job_id), TP_STRUCT__entry(__field(unsigned int, event_id)
																		       __field(unsigned int, tgid)
																		       __field(unsigned int, pid)
																		       __field(unsigned char, atom_id)
																	           __field(u32, job_id)
	    ), TP_fast_assign(__entry->event_id = event_id; __entry->tgid = tgid; __entry->pid = pid; __entry->atom_id = atom_id; __entry->job_id = job_id;), TP_printk("event=%u tgid=%u pid=%u atom_id=%u job_id=%u", __entry->event_id, __entry->tgid, __entry->pid, __entry->atom_id, __entry->job_id)
	);
...
TRACE_EVENT(mali_pm_status, TP_PROTO(unsigned int event_id, unsigned long long value), TP_ARGS(event_id, value), TP_STRUCT__entry(__field(unsigned int, event_id)
	    ...
	    ), TP_fast_assign(__entry->event_id = event_id; __entry->value = value;), TP_printk("event %u = %llu", __entry->event_id, __entry->value)
	);
...
TRACE_EVENT(mali_page_fault_insert_pages, TP_PROTO(int event_id, unsigned long value), TP_ARGS(event_id, value), TP_STRUCT__entry(__field(int, event_id)
	    ...
	    ), TP_fast_assign(__entry->event_id = event_id; __entry->value = value;), TP_printk("event %d = %lu", __entry->event_id, __entry->value)
	);
...
TRACE_EVENT(mali_sw_counter, TP_PROTO(unsigned int event_id, signed long long value), TP_ARGS(event_id, value), TP_STRUCT__entry(__field(int, event_id)
	    ...
	    ), TP_fast_assign(__entry->event_id = event_id; __entry->value = value;), TP_printk("event %d = %lld", __entry->event_id, __entry->value)
	);
