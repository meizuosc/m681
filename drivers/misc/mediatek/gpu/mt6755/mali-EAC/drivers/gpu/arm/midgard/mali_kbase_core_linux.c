...
int mtk_gpu_dvfs(void)
{
    ...
    for(i=0 ; i<mt6755_gpu_freq_limit_data[iCurrentGPUPlatformID].virtual_freq_index_count ; i++)
    {
        if( iCurrentFreqID <=mt6755_gpu_freq_limit_data[iCurrentGPUPlatformID].virtual_freq_index[i] )
        {
    ...
    iTargetVirtualFreqID = _mtk_dvfs_index_clipping(iTargetVirtualFreqID, 0, mt6755_gpu_freq_limit_data[iCurrentGPUPlatformID].virtual_freq_index_count-1);

    iTargetFreqID = mt6755_gpu_freq_limit_data[iCurrentGPUPlatformID].virtual_freq_index[iTargetVirtualFreqID];
...
probe_begin:
	...
	if (err){
	    pr_alert("[MALI]Using platform_device_add_data err, [%s][l:%d] \n",  __func__, __LINE__);
	    goto out;
	//	return err; // remove return for probe re-try machinization
	}
	...
	if (!kbdev) {
		pr_alert("[Mali]Can't allocate device\n");
		err = -ENOMEM;
		goto out;
	}
#ifdef CONFIG_MALI_NO_MALI
	...
	if (MALI_ERROR_NONE != mali_err) {
		pr_alert("[Mali]Can't initialize dummy model\n");
		err = -ENOMEM;
		goto out_midg;
	}
#endif /* CONFIG_MALI_NO_MALI */
	...
	if (NULL == platform_data) {
		pr_alert("[Mali]Platform data not specified\n");
		err = -ENOENT;
		goto out_free_dev;
	}

	if (MALI_TRUE != kbasep_validate_configuration_attributes(kbdev, platform_data)) {
		pr_alert("[Mali]Configuration attributes failed to validate\n");
		err = -EINVAL;
		goto out_free_dev;
	}
	...
	for (i = 0; i < 3; i++) {
		...
		if (!irq_res) {
			pr_alert("[Mali]No IRQ resource at index %d\n", i);
			err = -ENOENT;
			goto out_free_dev;
		}
		...
		if (!strcmp(irq_res->name, "JOB")) {
			...
			pr_alert("Invalid irq res name: '%s'\n",
				irq_res->name);
			err = -EINVAL;
			goto out_free_dev;
		}
	...
	if (!reg_res) {
		pr_alert("[Mali]Invalid register resource\n");
		err = -ENOENT;
		goto out_free_dev;
	}
	...
	if (IS_ERR_OR_NULL(kbdev->clock)) {
		pr_alert("[Mali]Continuing without Mali clock control\n");
		kbdev->clock = NULL;
		/* Allow probe to continue without clock. */
	} else {
		err = clk_prepare_enable(kbdev->clock);
		if (err) {
			pr_alert("[Mali]Failed to prepare and enable clock (%d)\n", err);
			goto out_clock_get;
		}
	}
...
//#endif 
	...
	if (MALI_ERROR_NONE != kbase_device_init(kbdev)) {
		pr_alert("Can't initialize device\n");
	...
	if (err) {
		pr_alert("[Mali]Failed kbase_common_device_init\n");
		goto out_term_dev;
	}
...
out:
	...
	pr_alert("[Mali]Failed probe inite to re-try three , probe_count = %d !!\n",probe_count);
	BUG_ON(probe_count>=3);
	return err;
}
...
void kbase_trace_mali_pm_status(u32 event, u64 value)
{
	...
	case KBASE_PM_CORE_SHADER:
		met_tag_oneshot(0, "MALI_SHADER_STATUS", (unsigned int)value);
		break;
	case KBASE_PM_CORE_TILER:
		met_tag_oneshot(0, "MALI_TILER_STATUS", (unsigned int)value);
		break;
	case KBASE_PM_CORE_L2:
		met_tag_oneshot(0, "MALI_L2_STATUS", (unsigned int)value);
		break;
	}
...
void kbase_trace_mali_pm_power_off(u32 event, u64 value)
{
	switch (event) {
	case KBASE_PM_CORE_SHADER:
		met_tag_oneshot(0, "MALI_SHADER_OFF", (unsigned int)value);
		break;
	case KBASE_PM_CORE_TILER:
		met_tag_oneshot(0, "MALI_TILER_OFF", (unsigned int)value);
		break;
	case KBASE_PM_CORE_L2:
		met_tag_oneshot(0, "MALI_L2_OFF", (unsigned int)value);
		break;
	}
...
void kbase_trace_mali_pm_power_on(u32 event, u64 value)
{
	switch (event) {
	case KBASE_PM_CORE_SHADER:
		met_tag_oneshot(0, "MALI_SHADER_ON", (unsigned int)value);
		break;
	case KBASE_PM_CORE_TILER:
		met_tag_oneshot(0, "MALI_TILER_ON", (unsigned int)value);
		break;
	case KBASE_PM_CORE_L2:
		met_tag_oneshot(0, "MALI_L2_ON", (unsigned int)value);
		break;
	}
...
void kbase_trace_mali_job_slots_event(u32 event, const struct kbase_context *kctx, u8 atom_id)
{
	trace_mali_job_slots_event(event, (kctx != NULL ? kctx->tgid : 0), (kctx != NULL ? kctx->pid : 0), atom_id, \
									(kctx != NULL ? kctx->jctx.atoms[atom_id].work_id : 0));
}
