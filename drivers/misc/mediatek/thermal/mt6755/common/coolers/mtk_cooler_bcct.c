...
#include <mach/charging.h> /*<< for mt_get_charger_type and CHARGER_TYPE */
#include "cust_charging.h" /*<< drivers/misc/mediatek/<IC>/<project>/power/cust_charging.h */
#include <tmp_battery.h> /*<< drivers/misc/mediatek/thermal/mt6755/inc/tmp_battery.h */
...
static void mtk_cl_bcct_set_bcct_limit(void)
{
	...
	if (min_limit != cl_bcct_cur_limit) {
		...
		if (65535 <= cl_bcct_cur_limit) {
			/*	TODO: temp use mtk_cooler_bcct to test limiting input current */
#ifdef CONFIG_MTK_SWITCH_INPUT_OUTPUT_CURRENT_SUPPORT
			set_chr_input_current_limit(-1);
#endif
