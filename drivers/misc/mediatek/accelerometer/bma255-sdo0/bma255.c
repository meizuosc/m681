...
static struct i2c_board_info bma255_i2c_info __initdata = { I2C_BOARD_INFO(BMA255_DEV_NAME, BMA255_I2C_ADDR)};
...
static int bma255_smbus_read_byte_block(\
struct i2c_client *client, \
...
static int bma255_read_accel_xyz(struct i2c_client *client, struct bma255acc *acc)
{
	...
	acc->x = BMA255_GET_BITSLICE(data[0], BMA255_ACC_X_LSB) | \
	 (BMA255_GET_BITSLICE(data[1], BMA255_ACC_X_MSB)<<(BMA255_ACC_X_LSB__LEN));
	acc->x = acc->x << (sizeof(short)*8-(BMA255_ACC_X_LSB__LEN + BMA255_ACC_X_MSB__LEN));
	acc->x = acc->x >> (sizeof(short)*8-(BMA255_ACC_X_LSB__LEN + BMA255_ACC_X_MSB__LEN));

	acc->y = BMA255_GET_BITSLICE(data[2], BMA255_ACC_Y_LSB) | \
	 (BMA255_GET_BITSLICE(data[3], BMA255_ACC_Y_MSB)<<(BMA255_ACC_Y_LSB__LEN));
	acc->y = acc->y << (sizeof(short)*8- \
	 (BMA255_ACC_Y_LSB__LEN + BMA255_ACC_Y_MSB__LEN));
	acc->y = acc->y >> (sizeof(short)*8- \
	 (BMA255_ACC_Y_LSB__LEN + BMA255_ACC_Y_MSB__LEN));

	acc->z = BMA255_GET_BITSLICE(data[4], BMA255_ACC_Z_LSB) \
	  | (BMA255_GET_BITSLICE(data[5], BMA255_ACC_Z_MSB)<<(BMA255_ACC_Z_LSB__LEN));
	acc->z = acc->z << (sizeof(short)*8- \
	  (BMA255_ACC_Z_LSB__LEN + BMA255_ACC_Z_MSB__LEN));
	acc->z = acc->z >> (sizeof(short)*8- \
	  (BMA255_ACC_Z_LSB__LEN + BMA255_ACC_Z_MSB__LEN));
...
static ssize_t bma255_runCalibration_store(struct device *dev, \
...
static DRIVER_ATTR(fast_calibration_x, S_IRUGO|S_IWUSR|S_IWGRP, \
bma255_fast_calibration_x_show, bma255_fast_calibration_x_store);
static DRIVER_ATTR(fast_calibration_y, S_IRUGO|S_IWUSR|S_IWGRP, \
bma255_fast_calibration_y_show, bma255_fast_calibration_y_store);
static DRIVER_ATTR(fast_calibration_z, S_IRUGO|S_IWUSR|S_IWGRP, \
bma255_fast_calibration_z_show, bma255_fast_calibration_z_store);
static DRIVER_ATTR(run_fast_calibration, S_IRUGO|S_IWUSR|S_IWGRP, \
bma255_runCalibration_show, bma255_runCalibration_store);
static DRIVER_ATTR(eeprom_writing, S_IRUGO|S_IWUSR|S_IWGRP, \
bma255_eeprom_writing_show, bma255_eeprom_writing_store);
static DRIVER_ATTR(bandwidth, S_IRUGO|S_IWUSR|S_IWGRP, \
bma255_bandwidth_show, bma255_bandwidth_store);
static DRIVER_ATTR(selftest, S_IRUGO|S_IWUSR|S_IWGRP,\
 bma255_SelfTest_show, bma255_SelfTest_store);
#if 1 /* motion sensor cal backup */
static DRIVER_ATTR(cali_backup, S_IWUSR|S_IRUGO|S_IWGRP, \
show_cali_backup_value, NULL);
#endif
#if 0
