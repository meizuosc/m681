...
int scan_image_list(int md_id, char fmt[], int out_img_list[], int img_list_size)
{
	...
	if (md_type) {
		...
		CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "md_type: %d, image nume:%d\n", md_type, img_num);
		return img_num;
	}
	...
	for (i = 0; i < (sizeof(type_str) / sizeof(char *)); i++) {
		...
		CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "Find:%s\n", full_path);
		...
		if (IS_ERR(filp)) {
			...
			filp = filp_open(full_path, O_RDONLY, 0644);
			if (IS_ERR(filp)) {
				CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "%s not found(%d,%d)\n", full_path, img_num, i);
				continue;
			}
		}
		/* Run here means open image success */
		filp_close(filp, NULL);
		CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "Image:%s found\n", full_path);
		if (img_num < img_list_size)
			out_img_list[img_num] = i;
		img_num++;
	}

	CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "md_type: %d, image nume:%d\n", out_img_list[0], img_num);
	return img_num;
}
...
static int check_md_header_v3(int md_id, void *parse_addr, struct ccci_image_info *image)
{
	...
	CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "**********************MD image check V3 %d***************************\n",
				(int)sizeof(struct md_check_header_v3));
	ret = strncmp(head->check_header, MD_HEADER_MAGIC_NO, 12);
	if (ret) {
		CCCI_UTIL_ERR_MSG_WITH_ID_REP(md_id, "md check header not exist!\n");
		ret = -CCCI_ERR_LOAD_IMG_CHECK_HEAD;
	} else {
		if (head->header_verno < 3) {
			CCCI_UTIL_ERR_MSG_WITH_ID_REP(md_id, "[Error]md check header version mis-match to AP:[%d]!\n",
						  head->header_verno);
		} else {
			...
			if (head->header_verno >= 2) {
				...
				if (head->mem_size == md_size) {
					...
					md_size_check = true;
					CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id,
			...
			if (md_type_check && md_plat_check && md_sys_match && md_size_check) {
				CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "Modem header check OK!\n");
			} else {
				CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "[Error]Modem header check fail!\n");
				if (!md_type_check)
					CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "[Reason]MD type(2G/3G) mis-match to AP!\n");

				if (!md_plat_check)
					CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "[Reason]MD platform mis-match to AP!\n");

				if (!md_sys_match)
					CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "[Reason]MD image is not for MD SYS%d!\n",
								  md_id + 1);
			...
			CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "(MD)[type]=%s, (AP)[type]=%s\n", image->img_info.image_type,
						  image->ap_info.image_type);
			CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "(MD)[plat]=%s, (AP)[plat]=%s\n", image->img_info.platform,
						  image->ap_info.platform);
			if (head->header_verno >= 2) {
				CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "(MD)[size]=%x, (AP)[size]=%x\n",
				...
				CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "(MD)[img_size]=%x, (AP)[img_size]=%x\n",
							  head->md_img_size, image->size);
			}
			if (head->header_verno >= 3) {
				image->dsp_offset = head->dsp_img_offset;
				image->dsp_size = head->dsp_img_size;
				CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "DSP image offset=%x size=%x\n", image->dsp_offset,
							  image->dsp_size);
				if (image->dsp_offset == 0xCDCDCDAA) {
					CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "DSP on EMI disabled\n");
				} else if (((image->dsp_offset&0xFFFF) != 0) && (head->header_verno == 3)) {
					   CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "DSP image offset not 64KB align\n");
					   ret = -CCCI_ERR_LOAD_IMG_MD_CHECK;
				} else if (image->dsp_offset + image->dsp_size > md_size) {
					CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "DSP image size too large %x\n", md_size);
					ret = -CCCI_ERR_LOAD_IMG_MD_CHECK;
				}
			} else {
				image->dsp_offset = 0;
			...
			CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "(MD)[build_ver]=%s, [build_time]=%s\n",
						  image->img_info.build_ver, image->img_info.build_time);
			CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "(MD)[product_ver]=%s\n", image->img_info.product_ver);
		}
	}
	CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "**********************MD image check V3***************************\n");
...
static int md_check_header_parser(int md_id, void *parse_addr, struct ccci_image_info *image)
{
	...
	CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "MD image header size = %d\n", header_size);

	if (header_size == sizeof(struct md_check_header_v3)) { /* v3, v4 */
		headv12 = headv34 = &md_img_header_v4[md_id];
		head = (struct md_check_header *)headv34;
		header_up = 4;
	} else if (header_size == sizeof(struct md_check_header_v5)) {/* v5 */
		headv12 = headv34 = headv5 = &md_img_header_v5[md_id];
		head = (struct md_check_header *)headv5;
		header_up = 5;
	} else { /* Default Load v1/2 */
		/* if (header_size == sizeof(struct md_check_header)) {*//* v1, v2 */
		headv12 = &md_img_header[md_id];
		head = (struct md_check_header *)headv12;
		header_up = 2;
	}
	...
	CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "**********************MD image check v%d %d***************************\n",
	...
	CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "**********************MD image check end***************************\n");
...
static int check_md_header(int md_id, void *parse_addr, struct ccci_image_info *image)
{
	...
	CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "**********************MD image check %d***************************\n",
				  (int)sizeof(struct md_check_header));
	ret = strncmp(head->check_header, MD_HEADER_MAGIC_NO, 12);
	if (ret) {
		CCCI_UTIL_ERR_MSG_WITH_ID_REP(md_id, "md check header not exist! %d\n", header_size);
		ret = -CCCI_ERR_LOAD_IMG_CHECK_HEAD;
	} else {
		if (head->header_verno > 2) {
			CCCI_UTIL_ERR_MSG_WITH_ID_REP(md_id, "[Error]md check header version mis-match to AP:[%d]!\n",
	...
	CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "**********************MD image check***************************\n");
...
static int signature_check_v2(int md_id, char *file_path, unsigned int *sec_tail_length)
{
	...
	if (masp_ccci_signfmt_verify_file(file_path, &bypass_sec_header_offset, &sec_total_len) == 0) {
		...
		/*-- check return value */
		CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "sign check ret value 0x%x, 0x%x!\n", bypass_sec_header_offset,
					  sec_total_len);
		if (bypass_sec_header_offset > sec_total_len) {
			CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "sign check fail(0x%x, 0x%x!)!\n", bypass_sec_header_offset,
						  sec_total_len);
			return -CCCI_ERR_LOAD_IMG_SIGN_FAIL;
		} else {
			CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "sign check success(0x%x, 0x%x)!\n", bypass_sec_header_offset,
						  sec_total_len);
		...
		CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "sign check fail!\n");
		return -CCCI_ERR_LOAD_IMG_SIGN_FAIL;
	}
}
				   ...
				   unsigned int cipher_img_offset, unsigned int cipher_img_len)
{
	...
	if (addr == NULL) {
		CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "ioremap image fialed!\n");
	...
	if (SEC_OK !=
		...
		CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "cipher image decrypt fail!\n");
...
static void get_md_postfix(int md_id, char k[], char buf[], char buf_ex[])
{
	...
	if ((md_id < 0) || (md_id > MAX_MD_NUM)) {
		CCCI_UTIL_ERR_MSG_WITH_ID_REP(md_id, "wrong MD ID to get postfix\n");
		return;
	}
	...
	if ((curr_ubin_id != 0) && (md_id == MD_SYS1)) {
		if (buf) {
			...
			CCCI_UTIL_ERR_MSG_WITH_ID_REP(md_id, "MD%d image postfix=%s\n", md_id + 1, buf);
		}
	...
	case MD_SYS5:
		...
		break;
	default:
		CCCI_UTIL_ERR_MSG_WITH_ID_REP(md_id, "request MD ID %d not supported\n", md_id);
		break;
	}

	if ((feature_val == 0) || (feature_val >= MAX_IMG_NUM)) {
		CCCI_UTIL_ERR_MSG_WITH_ID_REP(md_id, "request MD type %d not supported\n", feature_val);
...
static int find_img_to_open(int md_id, MD_IMG_TYPE img_type, char active_file_path[], char active_post_fix[])
{
	...
	if (img_type == IMG_MD) {	/* Gen MD image name */
		...
		CCCI_UTIL_ERR_MSG_WITH_ID_REP(md_id, "Invalid img type%d\n", img_type);
	...
	CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "Find ubin img @default\n");
	for (i = modem_ultg; i <= MAX_IMG_NUM; i++) {
		CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "Curr i:%d \n", i);
		if (img_type == IMG_MD)
		...
		CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "try to open %s\n", img_name[0]);
	...
	if (i > MAX_IMG_NUM) {
		CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "Find ubin img @default fail\n");
		return -CCCI_ERR_LOAD_IMG_NOT_FOUND;
	}
	active_post_fix[0] = '\0';
	CCCI_UTIL_ERR_MSG_WITH_ID_REP(md_id, "No Image file found\n");
	{
		char title[100];
		char info[100];
...
static int load_md_from_partition(int md_id, struct ccci_image_info *img);
static int load_std_firmware(int md_id, struct file *filp, struct ccci_image_info *img)
{
	...
	ret = load_md_from_partition(md_id, img);
	if (ret == 0) /* Using partition data first */
		goto _prt_ld_done;
...
_prt_ld_done:
	...
	set_fs(curr_fs);
	CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "Load %s (size=0x%x) to 0x%pa\n", img->file_name, read_size, &load_addr);
	return read_size;
...
static int load_image(int md_id, struct ccci_image_info *img_inf, char post_fix[])
{
	...
	if (IS_ERR(filp)) {
		CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "open %s fail: %ld\n", img->file_name, PTR_ERR(filp));
		ret = -CCCI_ERR_LOAD_IMG_FILE_OPEN;
		filp = NULL;
		goto out;
	} else {
	...
	if (img_type == IMG_MD) {
		...
		CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "signature_check offset:%d, tail:%d\n", offset, sec_tail_length);
		if (offset < 0) {
			CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "signature_check failed ret=%d\n", offset);
			ret = offset;
			goto out;
		}
#endif
		...
		if (masp_ccci_is_cipherfmt(fp_id, offset, &img_len)) {
			CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "cipher image\n");
			ret = load_cipher_firmware_v2(md_id, fp_id, img, offset, img_len);
			if (ret < 0) {
				CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "load_cipher_firmware failed:ret=%d!\n", ret);
				goto out;
			}
			CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "load_cipher_firmware done! (=%d)\n", ret);
		} else {
#endif
			CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "Not cipher image\n");
			ret = load_std_firmware(md_id, filp, img);
			if (ret < 0) {
				CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "load_firmware failed: ret=%d!\n", ret);
				goto out;
			}
#ifdef ENABLE_MD_IMG_SECURITY_FEATURE
		}
		...
		ret = load_std_firmware(md_id, filp, img);
		if (ret < 0) {
			CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "load_firmware for %s failed:ret=%d!\n", img->file_name, ret);
			goto out;
		}
	} else if (img_type == IMG_ARMV7) {
		/* load armv7 */
...
int ccci_load_firmware(int md_id, void *img_inf, char img_err_str[], char post_fix[])
{
	...
	if (ret < 0) {
		...
		CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "fail to load firmware!\n");
	}
	...
	if (img_err_str != NULL) {
		if (ret == -CCCI_ERR_LOAD_IMG_SIGN_FAIL) {
			...
			CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "signature check fail!\n");
		} else if (ret == -CCCI_ERR_LOAD_IMG_CIPHER_FAIL) {
			snprintf(img_err_str, IMG_ERR_STR_LEN, "%s Cipher chekc fail\n", img_ptr->file_name);
			CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "cipher check fail!\n");
...
static const int md1_capability_array[] = {
	0, 0, 0, 0, 0, 0, 0, 0,
	((1 << modem_ulwtg) | (1 << modem_ultg) | (0 << modem_ulwg) | (0 << modem_ulwcg) | (1 << modem_ulwctg)),	/* ultg */
	((1 << modem_ulwtg) | (0 << modem_ultg) | (1 << modem_ulwg) | (1 << modem_ulwcg) | (1 << modem_ulwctg)),	/* ulwg */
	((1 << modem_ulwtg) | (0 << modem_ultg) | (1 << modem_ulwg) | (1 << modem_ulwcg) | (1 << modem_ulwctg)),	/* ulwtg */
	((0 << modem_ulwtg) | (0 << modem_ultg) | (0 << modem_ulwg) | (1 << modem_ulwcg) | (1 << modem_ulwctg)),	/* ulwcg */
	((0 << modem_ulwtg) | (0 << modem_ultg) | (0 << modem_ulwg) | (0 << modem_ulwcg) | (1 << modem_ulwctg)),	/* ulwctg */
	((1 << modem_ulwtg) | (1 << modem_ultg) | (0 << modem_ulwg) | (0 << modem_ulwcg) | (1 << modem_ulwctg)),	/* ulttg */
	((1 << modem_ulwtg) | (0 << modem_ultg) | (1 << modem_ulwg) | (1 << modem_ulwcg) | (1 << modem_ulwctg)),	/* ulfwg */
	((0 << modem_ulwtg) | (0 << modem_ultg) | (0 << modem_ulwg) | (1 << modem_ulwcg) | (1 << modem_ulwctg)),	/* ulfwcg */
	((0 << modem_ulwtg) | (0 << modem_ultg) | (0 << modem_ulwg) | (0 << modem_ulwcg) | (1 << modem_ulwctg)),	/* ulctg */
	((0 << modem_ulwtg) | (0 << modem_ultg) | (0 << modem_ulwg) | (0 << modem_ulwcg) | (1 << modem_ulwctg)),	/* ultctg */
};
...
int load_md_header_info(int md_id, char *name, unsigned char buff[], unsigned int buff_size, unsigned int *img_size)
{
	char env_path[64];
	...
	part = get_part(name);
	if (!part) {
		CCCI_UTIL_ERR_MSG_WITH_ID_REP(md_id, "Not find partition: %s\n", name);
		ret = -1;
		goto _parse_exit;
	}
	...
	CCCI_UTIL_ERR_MSG_WITH_ID_REP(md_id, "env_path: %s\n", env_path);

	filp = filp_open(env_path, O_RDONLY, 0444);
	if (IS_ERR(filp)) {
		CCCI_UTIL_ERR_MSG_WITH_ID_REP(md_id, "open partition: %s, fail(%ld)\n", name, PTR_ERR(filp));
		ret = -2;
		filp = NULL;
		goto _parse_exit;
	}

	/* begin to find check header */
	curr_fs = get_fs();
	set_fs(KERNEL_DS);
	filp->f_pos = 0;
	ret = (int)filp->f_op->read(filp, buff, sizeof(prt_img_hdr_t), &filp->f_pos);
	if (ret != sizeof(prt_img_hdr_t)) {
		CCCI_UTIL_ERR_MSG_WITH_ID_REP(md_id, "load hdr fail(%d)\n", ret);
		ret = -3;
		goto _parse_exit_opened;
	}

	p_hdr = (prt_img_hdr_t *)buff;
	if (p_hdr->info.magic != IMG_MAGIC) {
		CCCI_UTIL_ERR_MSG_WITH_ID_REP(md_id, "invald magic(%x)\n", p_hdr->info.magic);
		ret = -4;
		goto _parse_exit_opened;
	}
	if (p_hdr->info.dsize < 1024*1024) {
		CCCI_UTIL_ERR_MSG_WITH_ID_REP(md_id, "img size abnormal,size(0x%x)\n", p_hdr->info.dsize);
		ret = -5;
		goto _parse_exit_opened;
	}

	/* load check header */
	hdr_size = p_hdr->info.hdr_size;
	raw_size = p_hdr->info.dsize;
	*img_size = raw_size;

	CCCI_UTIL_ERR_MSG_WITH_ID_REP(md_id, "dump p_hdr info for modem\n");
	CCCI_UTIL_ERR_MSG_WITH_ID_REP(md_id, " p_hdr->info.magic:%x\n", p_hdr->info.magic);
	CCCI_UTIL_ERR_MSG_WITH_ID_REP(md_id, " p_hdr->info.dsize:%x\n", p_hdr->info.dsize);
	CCCI_UTIL_ERR_MSG_WITH_ID_REP(md_id, " p_hdr->info.name:%s\n", p_hdr->info.name);
	CCCI_UTIL_ERR_MSG_WITH_ID_REP(md_id, " p_hdr->info.mode:%x\n", p_hdr->info.mode);
	CCCI_UTIL_ERR_MSG_WITH_ID_REP(md_id, " p_hdr->info.hdr_size:%x\n", p_hdr->info.hdr_size);

	filp->f_pos = hdr_size + raw_size - buff_size;
	ret = (int)filp->f_op->read(filp, buff, buff_size, &filp->f_pos);
	if (ret != buff_size) {
		CCCI_UTIL_ERR_MSG_WITH_ID_REP(md_id, "load chk_hdr fail(%d)\n", raw_size);
		ret = -6;
		goto _parse_exit_opened;
	}
	ret = 0;
...
int ccci_get_md_check_hdr_inf(int md_id, void *img_inf, char post_fix[])
{
	...
	char *img_str;
	char *buf;
	struct md_check_header *head;
	struct md_check_header_v5 *head_v5;
	unsigned int header_size;
	unsigned int md_type = 0;

	buf = kmalloc(1024, GFP_KERNEL);
	if (buf == NULL) {
		CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "fail to allocate memor for chk_hdr\n");
		return -1;
	}
	...
	if (ret < 0) {
		CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "fail to load header(%d)!\n", ret);
		kfree(buf);
		return -1;
	}
	...
	if (ret < 0) {
		CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "check header fail(%d)!\n", ret);
		kfree(buf);
		return -1;
	}
	...
	CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "check header str[%s]!\n", img_str);

	if (md_id == MD_SYS1) {
		curr_ubin_id = md_type;
		CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "type @ header(%d)!\n", curr_ubin_id);
	}

	snprintf(post_fix, IMG_POSTFIX_LEN, "%d_%s_n", md_id+1, img_ptr->img_info.image_type);

	CCCI_UTIL_INF_MSG_WITH_ID_REP(md_id, "post fix[%s]!\n", post_fix);
...
static const char *get_partition_name(int md_id, int img_type)
{
	switch (md_id) {
	case MD_SYS1:
		if (img_type == IMG_MD)
...
int load_md_from_partition(int md_id, struct ccci_image_info *img)
{
	char env_path[64];
	struct hd_struct *part = NULL;
	struct file *filp_local = NULL;
	int ret;
	mm_segment_t curr_fs;
	unsigned int hdr_size;
	prt_img_hdr_t *p_hdr;
	...
	buf = kmalloc(sizeof(prt_img_hdr_t), GFP_KERNEL);
	if (buf == NULL) {
		CCCI_UTIL_ERR_MSG_WITH_ID_REP(md_id, "allocate memory fail\n");
		ret = -CCCI_ERR_LOAD_IMG_FILE_READ;
		return ret;
	}

	name = get_partition_name(md_id, img->type);
	if (name == NULL) {
		CCCI_UTIL_ERR_MSG_WITH_ID_REP(md_id, "find patition name fail\n");
		ret = -CCCI_ERR_LOAD_IMG_FILE_READ;
		goto _parse_exit;
	}

	part = (char *)get_part(name);
	if (!part) {
		CCCI_UTIL_ERR_MSG_WITH_ID_REP(md_id, "Not find partition: %s\n", name);
		ret = -CCCI_ERR_LOAD_IMG_FILE_READ;
		goto _parse_exit;
	...
	filp_local = filp_open(env_path, O_RDONLY, 0444);
	if (IS_ERR(filp_local)) {
		CCCI_UTIL_ERR_MSG_WITH_ID_REP(md_id, "open partition: %s, fail(%ld)\n", name, PTR_ERR(filp_local));
		ret = -CCCI_ERR_LOAD_IMG_FILE_READ;
		filp_local = NULL;
		goto _parse_exit;
	}
	...
	set_fs(KERNEL_DS);
	filp_local->f_pos = 0;
	ret = (int)filp_local->f_op->read(filp_local, buf, sizeof(prt_img_hdr_t), &filp_local->f_pos);
	if (ret != sizeof(prt_img_hdr_t)) {
		CCCI_UTIL_ERR_MSG_WITH_ID_REP(md_id, "load hdr fail(%d)\n", ret);
		ret = -CCCI_ERR_LOAD_IMG_FILE_READ;
		goto _parse_exit_opened;
	...
	while (1) {
		...
		if (size_per_read > (img_size-read_size))
			...
			curr_read = size_per_read;
		ret = (int)filp_local->f_op->read(filp_local, start, curr_read, &filp_local->f_pos);
		/*make sure image size isn't 0 */
		if ((ret < 0) || (ret > curr_read) || ((ret == 0) && (read_size == 0))) {
			CCCI_UTIL_ERR_MSG_WITH_ID(md_id, "image read fail: size=%d+\n", ret);
			ret = -CCCI_ERR_LOAD_IMG_FILE_READ;
...
_parse_exit:
	...
	if (filp_local != NULL)
		filp_close(filp_local, current->files);
