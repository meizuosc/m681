...
static void cal_md_settings_v2(struct device_node *node)
{
	...
	/* lk_dt_info_collect may set "LK_LOAD_MD_EN" flag if found dt node sucess*/
	if (lk_load_img_status & LK_LOAD_MD_EN)
		return;
...
int ccci_reserve_mem_of_init(struct reserved_mem *rmem, unsigned long node, const char *uname)
{
	...
	if (strstr(CCCI_MD1_MEM_RESERVED_KEY, uname))
		md_id = MD_SYS1;
	else if (strstr(CCCI_MD2_MEM_RESERVED_KEY, uname))
		md_id = MD_SYS2;
	else if (strstr(CCCI_MD3_MEM_RESERVED_KEY, uname))
		md_id = MD_SYS3;
	else if (strstr(CCCI_MD1MD3_SMEM_RESERVED_KEY, uname)) {
