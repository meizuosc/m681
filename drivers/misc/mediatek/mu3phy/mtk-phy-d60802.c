...
PHY_INT32 phy_init_d60802(struct u3phy_info *info)
{
	...
	U3PhyWriteField32(((phys_addr_t) &info->u2phy_regs_d->usbphyacr6)
			  , D60802_RG_USB20_OTG_VBUSCMP_EN_OFST, D60802_RG_USB20_OTG_VBUSCMP_EN,
			  0x1);
	...
	U3PhyWriteField32(((phys_addr_t) &info->u3phyd_bank2_regs_d->b2_phyd_top1)
			  , D60802_RG_SSUSB_FORCE_TX_EIDLE_LP_EN_OFST,
			  D60802_RG_SSUSB_FORCE_TX_EIDLE_LP_EN, 0x1);
	U3PhyWriteField32(((phys_addr_t) &info->u3phyd_bank2_regs_d->b2_phyd_top1)
	...
	U3PhyWriteField32(((phys_addr_t) &info->u3phya_regs_d->reg5)
			  , D60802_reg5_FLD_RG_SSUSB_RXDET_VTHSEL_L_OFST,
			  D60802_reg5_FLD_RG_SSUSB_RXDET_VTHSEL_L, 0x2);

	/* modify Tx det Rx Vth to work around the threshold back to 200mV */
	U3PhyWriteField32(((phys_addr_t) &info->u3phya_regs_d->reg5)
			  , D60802_reg5_FLD_RG_SSUSB_RXDET_VTHSEL_H_OFST,
			  D60802_reg5_FLD_RG_SSUSB_RXDET_VTHSEL_H, 0x2);

	/*************phya da part*****************/
	/* set to pass SSC min in electrical compliance */
	U3PhyWriteField32(((phys_addr_t) &info->u3phya_da_regs_d->reg21)
			  , D60802_reg21_FLD_RG_SSUSB_PLL_SSC_DELTA_U3_OFST,
			  D60802_reg21_FLD_RG_SSUSB_PLL_SSC_DELTA_U3, 0x47);

	/* set R step 1 = 2 to improve Rx JT */
	U3PhyWriteField32(((phys_addr_t) &info->u3phya_da_regs_d->reg32)
			  , D60802_reg32_FLD_RG_SSUSB_EQ_RSTEP1_U3_OFST,
			  D60802_reg32_FLD_RG_SSUSB_EQ_RSTEP1_U3, 0x2);
...
PHY_INT32 phy_change_pipe_phase_d60802(struct u3phy_info *info, PHY_INT32 phy_drv,
...
static void EyeScanHelper_RunTest(struct u3phy_info *info)
{
	...
	U3PhyWriteField32(((phys_addr_t) &info->u3phyd_regs_d->eq_eye0)
			  , D60802_RG_SSUSB_EQ_EYE_CNT_EN_OFST, D60802_RG_SSUSB_EQ_EYE_CNT_EN, 0);

	/* Run the test */
	/* RG_SSUSB_RX_EYE_CNT_EN = 1 */
	U3PhyWriteField32(((phys_addr_t) &info->u3phyd_regs_d->eq_eye0)
			  , D60802_RG_SSUSB_EQ_EYE_CNT_EN_OFST, D60802_RG_SSUSB_EQ_EYE_CNT_EN, 1);

	/* Wait til it's done */
	/* RGS_SSUSB_RX_EYE_CNT_RDY */
#ifdef CONFIG_U3_PHY_GPIO_SUPPORT
	while (!U3PhyReadField32(((phys_addr_t) &info->u3phyd_regs_d->phya_rx_mon4)
				 , D60802_RGS_SSUSB_EQ_EYE_CNT_RDY_OFST,
				 D60802_RGS_SSUSB_EQ_EYE_CNT_RDY))
					NULL;
#else
...
PHY_INT32 eyescan_init_d60802(struct u3phy_info *info)
{
	...
	U3PhyWriteField32(((phys_addr_t) &info->u3phya_regs_d->reg9)
			  , D60802_reg9_FLD_RG_SSUSB_CDR_EPEN_OFST,
			  D60802_reg9_FLD_RG_SSUSB_CDR_EPEN, 1);
	U3PhyWriteField32(((phys_addr_t) &info->u3phyd_regs_d->phyd_mix3)
			  , D60802_RG_SSUSB_FORCE_CDR_PI_PWD_OFST, D60802_RG_SSUSB_FORCE_CDR_PI_PWD,
			  1);
	return PHY_TRUE;
}

PHY_INT32 phy_eyescan_d60802(struct u3phy_info *info, PHY_INT32 x_t1, PHY_INT32 y_t1,
			     ...
			     PHY_INT32 num_ignore_cnt)
{
	...
	pr_debug("x_t1: %x, y_t1: %x, x_br: %x, y_br: %x, delta_x: %x, delta_y: %x,	eye_cnt: %x, num_cnt: %x, PI_cal_en: %x, num_ignore_cnt: %x\n",
		x_t1, y_t1, x_br, y_br, delta_x, delta_y, eye_cnt,
		num_cnt, PI_cal_en, num_ignore_cnt);

	/* force SIGDET to OFF */
#ifdef CONFIG_U3_PHY_GPIO_SUPPORT
	/* RG_SSUSB_RX_SIGDET_SEL = 1 */
	U3PhyWriteField32(((phys_addr_t) &info->u3phyd_regs_d->phyd_reserved)
			  , D60802_RG_SSUSB_RX_SIGDET_SEL_OFST, D60802_RG_SSUSB_RX_SIGDET_SEL, 1);
	/* RG_SSUSB_RX_SIGDET_EN = 0 */
	U3PhyWriteField32(((phys_addr_t) &info->u3phyd_regs_d->phyd_reserved)
			  , D60802_RG_SSUSB_RX_SIGDET_EN_OFST, D60802_RG_SSUSB_RX_SIGDET_EN, 0);
#else
	/* RG_SSUSB_RX_SIGDET_SEL = 1 */
	U3PhyWriteField32(((phys_addr_t) &info->u3phyd_bank2_regs_d->b2_phyd_misc0)
			  , D60802_RG_SSUSB_RX_SIGDET_EN_SEL_OFST, D60802_RG_SSUSB_RX_SIGDET_EN_SEL, 1);
	/* RG_SSUSB_RX_SIGDET_EN = 0 */
	U3PhyWriteField32(((phys_addr_t) &info->u3phyd_bank2_regs_d->b2_phyd_misc0)
			  , D60802_RG_SSUSB_RX_SIGDET_EN_OFST, D60802_RG_SSUSB_RX_SIGDET_EN, 0);
#endif
	...
	U3PhyWriteField32(((phys_addr_t) &info->u3phyd_regs_d->eq_eye1)
			  , D60802_RG_SSUSB_EQ_SIGDET_OFST, D60802_RG_SSUSB_EQ_SIGDET, 0);

	/* RX_TRI_DET_EN to Disable */
	/* RG_SSUSB_RX_TRI_DET_EN = 0 */
	U3PhyWriteField32(((phys_addr_t) &info->u3phyd_regs_d->eq3)
			  , D60802_RG_SSUSB_EQ_TRI_DET_EN_OFST, D60802_RG_SSUSB_EQ_TRI_DET_EN, 0);

	/* RG_SSUSB_EYE_MON_EN = 1 */
	U3PhyWriteField32(((phys_addr_t) &info->u3phyd_regs_d->eq_eye0)
			  , D60802_RG_SSUSB_EQ_EYE_MON_EN_OFST, D60802_RG_SSUSB_EQ_EYE_MON_EN, 1);
	/* RG_SSUSB_RX_EYE_XOFFSET = 0 */
	U3PhyWriteField32(((phys_addr_t) &info->u3phyd_regs_d->eq_eye0)
			  , D60802_RG_SSUSB_EQ_EYE_XOFFSET_OFST, D60802_RG_SSUSB_EQ_EYE_XOFFSET, 0);
	/* RG_SSUSB_RX_EYE0_Y = 0 */
	U3PhyWriteField32(((phys_addr_t) &info->u3phyd_regs_d->eq_eye0)
			  , D60802_RG_SSUSB_EQ_EYE0_Y_OFST, D60802_RG_SSUSB_EQ_EYE0_Y, 0);
	/* RG_SSUSB_RX_EYE1_Y = 0 */
	U3PhyWriteField32(((phys_addr_t) &info->u3phyd_regs_d->eq_eye0)
			  , D60802_RG_SSUSB_EQ_EYE1_Y_OFST, D60802_RG_SSUSB_EQ_EYE1_Y, 0);


	if (PI_cal_en) {
		/* PI Calibration */
#ifdef CONFIG_U3_PHY_GPIO_SUPPORT
		/* RG_SSUSB_RX_PI_CAL_MANUAL_SEL = 1 */
		U3PhyWriteField32(((phys_addr_t) &info->u3phyd_regs_d->phyd_reserved)
				  , D60802_RG_SSUSB_RX_PI_CAL_MANUAL_SEL_OFST, D60802_RG_SSUSB_RX_PI_CAL_MANUAL_SEL, 1);
		/* RG_SSUSB_RX_PI_CAL_MANUAL_EN = 0 */
		U3PhyWriteField32(((phys_addr_t) &info->u3phyd_regs_d->phyd_reserved)
				  , D60802_RG_SSUSB_RX_PI_CAL_MANUAL_EN_OFST, D60802_RG_SSUSB_RX_PI_CAL_MANUAL_EN, 0);
		/* RG_SSUSB_RX_PI_CAL_MANUAL_EN = 1 */
		U3PhyWriteField32(((phys_addr_t) &info->u3phyd_regs_d->phyd_reserved)
				  , D60802_RG_SSUSB_RX_PI_CAL_MANUAL_EN_OFST, D60802_RG_SSUSB_RX_PI_CAL_MANUAL_EN, 1);
		...
		/* read RGS_SSUSB_RX_PILPO */
		_bPIResult = U3PhyReadField32(((phys_addr_t) &info->u3phyd_regs_d->phya_rx_mon4)
					      , D60802_RGS_SSUSB_EQ_PILPO_OFST, D60802_RGS_SSUSB_EQ_PILPO);
#else
		/* RG_SSUSB_RX_PI_CAL_MANUAL_SEL = 1 */
		U3PhyWriteField32(((phys_addr_t) &info->u3phyd_bank2_regs_d->b2_phyd_misc0)
				  , D60802_RG_SSUSB_RX_PI_CAL_EN_SEL_OFST, D60802_RG_SSUSB_RX_PI_CAL_EN_SEL, 1);
		/* RG_SSUSB_RX_PI_CAL_MANUAL_EN = 0 */
		U3PhyWriteField32(((phys_addr_t) &info->u3phyd_bank2_regs_d->b2_phyd_misc0)
				  , D60802_RG_SSUSB_RX_PI_CAL_EN_OFST, D60802_RG_SSUSB_RX_PI_CAL_EN, 0);
		/* RG_SSUSB_RX_PI_CAL_MANUAL_EN = 1 */
		U3PhyWriteField32(((phys_addr_t) &info->u3phyd_bank2_regs_d->b2_phyd_misc0)
				  , D60802_RG_SSUSB_RX_PI_CAL_EN_OFST, D60802_RG_SSUSB_RX_PI_CAL_EN, 1);
	...
	U3PhyWriteField32(((phys_addr_t) &info->u3phyd_regs_d->eq_eye2)
			  , D60802_RG_SSUSB_EQ_EYE_CNT_OFST, D60802_RG_SSUSB_EQ_EYE_CNT, eye_cnt);
#else
	/* RG_SSUSB_RX_EYE_CNT */
	...
	/* Eye Monitor Feature */
	/* RG_SSUSB_RX_EYE_MASK = 0x3ff */
	U3PhyWriteField32(((phys_addr_t) &info->u3phyd_regs_d->eq_eye1)
			  , D60802_RG_SSUSB_EQ_EYE_MASK_OFST, D60802_RG_SSUSB_EQ_EYE_MASK, 0x3ff);
	/* RG_SSUSB_EYE_MON_EN = 1 */
	U3PhyWriteField32(((phys_addr_t) &info->u3phyd_regs_d->eq_eye0)
			  , D60802_RG_SSUSB_EQ_EYE_MON_EN_OFST, D60802_RG_SSUSB_EQ_EYE_MON_EN, 1);
	...
	for (cOfst = 0; cOfst >= -64; cOfst--) {
		...
		U3PhyWriteField32(((phys_addr_t) &info->u3phyd_regs_d->eq_eye0)
				  , D60802_RG_SSUSB_EQ_EYE_XOFFSET_OFST, D60802_RG_SSUSB_EQ_EYE_XOFFSET, cOfst);
	}
	for (cOfst = 0; cOfst < 64; cOfst++) {
		/* RG_SSUSB_RX_EYE0_Y */
		U3PhyWriteField32(((phys_addr_t) &info->u3phyd_regs_d->eq_eye0)
	...
	while (isContinue) {
		...
		if (_fgXChged == true) {
			/* RG_SSUSB_RX_EYE_XOFFSET */
			U3PhyWriteField32(((phys_addr_t) &info->u3phyd_regs_d->eq_eye0)
					  , D60802_RG_SSUSB_EQ_EYE_XOFFSET_OFST, D60802_RG_SSUSB_EQ_EYE_XOFFSET, cX);
		}
		/* Set Y */
		/* RG_SSUSB_RX_EYE0_Y */
		U3PhyWriteField32(((phys_addr_t) &info->u3phyd_regs_d->eq_eye0)
				  , D60802_RG_SSUSB_EQ_EYE0_Y_OFST, D60802_RG_SSUSB_EQ_EYE0_Y, cY);
		/* RG_SSUSB_RX_EYE1_Y */
		U3PhyWriteField32(((phys_addr_t) &info->u3phyd_regs_d->eq_eye0)
				  , D60802_RG_SSUSB_EQ_EYE1_Y_OFST, D60802_RG_SSUSB_EQ_EYE1_Y, cY);
		...
		if (fgValid) {
			...
			for (bExtendCnt = 0; bExtendCnt < num_cnt; bExtendCnt++) {
...
#ifdef CONFIG_U3_PHY_GPIO_SUPPORT
				wErr0 =
				    U3PhyReadField32(((phys_addr_t) &info->u3phyd_regs_d->
						      phya_rx_mon3)
						     ,
						     D60802_RGS_SSUSB_EQ_EYE_MONITOR_ERRCNT_0_OFST,
						     D60802_RGS_SSUSB_EQ_EYE_MONITOR_ERRCNT_0);
				wErr1 =
				    U3PhyReadField32(((phys_addr_t) &info->u3phyd_regs_d->
						      phya_rx_mon3)
						     ,
						     D60802_RGS_SSUSB_EQ_EYE_MONITOR_ERRCNT_1_OFST,
						     D60802_RGS_SSUSB_EQ_EYE_MONITOR_ERRCNT_1);
#else
				wErr0 =
				    U3PhyReadField32(((phys_addr_t) &info->u3phyd_regs_d->
						      phya_rx_mon3)
				...
				/* pr_debug("cnt[%d] cur_x,y [0x%x][0x%x], cX,cY [0x%x][0x%x], ErrCnt[%d][%d]\n"
					, bExtendCnt, _bXcurr, _bYcurr, cX, cY, pwErrCnt0[bExtendCnt][_bXcurr][_bYcurr],
					pwErrCnt1[bExtendCnt][_bXcurr][_bYcurr]); */
			}
			/* pr_debug("cur_x,y [0x%x][0x%x], cX,cY [0x%x][0x%x], ErrCnt[%d][%d]\n",
				_bXcurr, _bYcurr, cX, cY,
				pwErrCnt0[0][_bXcurr][_bYcurr], pwErrCnt1[0][_bXcurr][_bYcurr]); */
		} else {
	...
	pr_debug("CurX [0x%x] CurY [0x%x]\n",
	       U3PhyReadField32(((phys_addr_t) &info->u3phyd_regs_d->eq_eye0),
				D60802_RG_SSUSB_EQ_EYE_XOFFSET_OFST, D60802_RG_SSUSB_EQ_EYE_XOFFSET)
	       , U3PhyReadField32(((phys_addr_t) &info->u3phyd_regs_d->eq_eye0),
				  D60802_RG_SSUSB_EQ_EYE0_Y_OFST, D60802_RG_SSUSB_EQ_EYE0_Y));
	...
	for (cOfst = 63; cOfst >= 0; cOfst--) {
		U3PhyWriteField32(((phys_addr_t) &info->u3phyd_regs_d->eq_eye0)
				  ...
				  cOfst);
		U3PhyWriteField32(((phys_addr_t) &info->u3phyd_regs_d->eq_eye0)
				  , D60802_RG_SSUSB_EQ_EYE1_Y_OFST, D60802_RG_SSUSB_EQ_EYE1_Y,
				  cOfst);
...
PHY_INT32 u2_slew_rate_calibration_d60802(struct u3phy_info *info)
{
	...
	if (u4FmOut == 0) {
		U3PhyWriteField32(((phys_addr_t) &info->u2phy_regs_d->usbphyacr5)
				  , D60802_RG_USB20_HSTX_SRCTRL_OFST, D60802_RG_USB20_HSTX_SRCTRL,
				  0x4);
