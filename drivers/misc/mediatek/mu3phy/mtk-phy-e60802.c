...
PHY_INT32 phy_change_pipe_phase_e60802(struct u3phy_info *info, PHY_INT32 phy_drv,
...
PHY_INT32 phy_eyescan_e60802(struct u3phy_info *info, PHY_INT32 x_t1, PHY_INT32 y_t1,
	...
	while (isContinue) {
		...
		if (fgValid) {
			...
			for (bExtendCnt = 0; bExtendCnt < num_cnt; bExtendCnt++) {
				EyeScanHelper_RunTest(info);
				wErr0 =
				    U3PhyReadField32(((phys_addr_t) &info->u3phyd_regs_e->
						      phya_rx_mon3)
						     ,
...
/* , bExtendCnt, _bXcurr, _bYcurr, cX, cY, pwErrCnt0[bExtendCnt][_bXcurr][_bYcurr], pwErrCnt1[bExtendCnt][_bXcurr][_bYcurr]); */
	...
	printk(KERN_ERR "CurX [0x%x] CurY [0x%x]\n",
	       U3PhyReadField32(((phys_addr_t) &info->u3phyd_regs_e->eq_eye0),
