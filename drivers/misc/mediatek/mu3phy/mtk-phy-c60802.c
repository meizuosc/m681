...
PHY_INT32 phy_init_c60802(struct u3phy_info *info)
{
	...
	U3PhyWriteField32(((PHY_UINT32) & info->u3phyd_bank2_regs_c->b2_phyd_top1)
			  , C60802_RG_SSUSB_FORCE_TX_EIDLE_LP_EN_OFST,
			  C60802_RG_SSUSB_FORCE_TX_EIDLE_LP_EN, 1);
	U3PhyWriteField32(((PHY_UINT32) & info->u3phyd_bank2_regs_c->b2_phyd_top1)
	...
	U3PhyWriteField32(((PHY_UINT32) & info->u3phya_da_regs_c->reg21)
			  , C60802_reg21_FLD_RG_SSUSB_PLL_SSC_DELTA_U3_OFST,
			  C60802_reg21_FLD_RG_SSUSB_PLL_SSC_DELTA_U3, 0x47);
...
PHY_INT32 phy_change_pipe_phase_c60802(struct u3phy_info *info, PHY_INT32 phy_drv,
...
static void EyeScanHelper_RunTest(struct u3phy_info *info)
{
	...
	while (!U3PhyReadField32(((PHY_UINT32) & info->u3phyd_regs_c->phya_rx_mon4)
				 , C60802_RGS_SSUSB_EQ_EYE_CNT_RDY_OFST,
				 C60802_RGS_SSUSB_EQ_EYE_CNT_RDY))
					NULL;
#else
...
PHY_INT32 eyescan_init_c60802(struct u3phy_info *info)
{
	...
	U3PhyWriteField32(((PHY_UINT32) & info->u3phya_regs_c->rega)
			  , C60802_regA_FLD_RG_SSUSB_CDR_EPEN_OFST,
			  C60802_regA_FLD_RG_SSUSB_CDR_EPEN, 1);
	U3PhyWriteField32(((PHY_UINT32) & info->u3phyd_regs_c->phyd_mix3)
			  , C60802_RG_SSUSB_FORCE_CDR_PI_PWD_OFST, C60802_RG_SSUSB_FORCE_CDR_PI_PWD,
			  1);
	return PHY_TRUE;
}

PHY_INT32 phy_eyescan_c60802(struct u3phy_info *info, PHY_INT32 x_t1, PHY_INT32 y_t1,
			     ...
			     PHY_INT32 num_ignore_cnt)
{
	...
	if (PI_cal_en) {
		...
		_bPIResult = U3PhyReadField32(((PHY_UINT32) &info->u3phyd_regs_c->phya_rx_mon4)
					      , C60802_RGS_SSUSB_EQ_PILPO_OFST,
					      C60802_RGS_SSUSB_EQ_PILPO);
#else
		U3PhyWriteField32(((PHY_UINT32) &info->u3phyd_bank2_regs_c->b2_phyd_misc0)
...
#ifdef CONFIG_U3_PHY_GPIO_SUPPORT
				wErr0 =
				    U3PhyReadField32(((PHY_UINT32) &info->u3phyd_regs_c->
						      phya_rx_mon3)
						     ,
						     C60802_RGS_SSUSB_EQ_EYE_MONITOR_ERRCNT_0_OFST,
						     C60802_RGS_SSUSB_EQ_EYE_MONITOR_ERRCNT_0);
				wErr1 =
				    U3PhyReadField32(((PHY_UINT32) &info->u3phyd_regs_c->
						      phya_rx_mon3)
						     ,
						     C60802_RGS_SSUSB_EQ_EYE_MONITOR_ERRCNT_1_OFST,
						     C60802_RGS_SSUSB_EQ_EYE_MONITOR_ERRCNT_1);
#else
				wErr0 =
				    U3PhyReadField32(((PHY_UINT32) &info->u3phyd_regs_c->
						      phya_rx_mon3)
...
/* , bExtendCnt, _bXcurr, _bYcurr, cX, cY, pwErrCnt0[bExtendCnt][_bXcurr][_bYcurr], pwErrCnt1[bExtendCnt][_bXcurr][_bYcurr]); */
...
#if 1
			pr_err("Xcurr [0x%x] Ycurr [0x%x]\n", _bXcurr, _bYcurr);
			pr_err("XcurrREG [0x%x] YcurrREG [0x%x]\n", cX, cY);
#endif
	...
	pr_debug("CurX [0x%x] CurY [0x%x]\n",
	       U3PhyReadField32(((PHY_UINT32) &info->u3phyd_regs_c->eq_eye0),
				C60802_RG_SSUSB_EQ_EYE_XOFFSET_OFST, C60802_RG_SSUSB_EQ_EYE_XOFFSET)
	       , U3PhyReadField32(((PHY_UINT32) &info->u3phyd_regs_c->eq_eye0),
				  C60802_RG_SSUSB_EQ_EYE0_Y_OFST, C60802_RG_SSUSB_EQ_EYE0_Y));
	...
	for (cOfst = 63; cOfst >= 0; cOfst--) {
		U3PhyWriteField32(((PHY_UINT32) &info->u3phyd_regs_c->eq_eye0)
				  ...
				  cOfst);
		U3PhyWriteField32(((PHY_UINT32) &info->u3phyd_regs_c->eq_eye0)
				  , C60802_RG_SSUSB_EQ_EYE1_Y_OFST, C60802_RG_SSUSB_EQ_EYE1_Y,
				  cOfst);
	...
	pr_debug("PI result: %d\n", _bPIResult);
	pr_debug("pwErrCnt0 addr: 0x%x\n", (PHY_UINT32) pwErrCnt0);
	pr_debug("pwErrCnt1 addr: 0x%x\n", (PHY_UINT32) pwErrCnt1);
	return PHY_TRUE;
}
...
PHY_INT32 u2_save_cur_re_c60802(struct u3phy_info *info)
{
	...
	/* wait 800us (1. force_suspendm=1, 2. set wanted utmi value, 3. rg_usb20_pll_stable=1) */
	U3PhyWriteField32(((PHY_UINT32) &info->u2phy_regs_c->u2phyacr2)
			  , C60802_RG_USB20_OTG_VBUSCMP_EN_OFST, C60802_RG_USB20_OTG_VBUSCMP_EN,
			  0x1);
...
PHY_INT32 u2_slew_rate_calibration_c60802(struct u3phy_info *info)
{
	...
	if (u4FmOut == 0) {
		U3PhyWriteField32(((PHY_UINT32) &info->u2phy_regs_c->u2phyacr0)
				  , C60802_RG_USB20_HSTX_SRCTRL_OFST, C60802_RG_USB20_HSTX_SRCTRL,
				  0x4);
