...
#include "isl29125.h"		//vvdn change
...
static s32 XYZCCM_RangeW[3][3] ={
	{	-1767L,	16406L,	-8475L},// X col
	{	-5108L,	20272L,	-9407L },// Y col
	{	-5293L,  2468L,  21845L}, // Z col
};
/*
...
*/
static s32 XYZCCM_RangeB[3][3] ={
	{	-3606L,	16422L,	-4974L},// X col
	{	-5658L,	18705L,	-4440L },// Y col
	{	-6040L,   3732L, 21845L}, // Z col
};
	...
	switch (adc_resolution) {
		case 12:
			switch(optical_range) {
				...
				case 10000:
					/* Switch to 375 lux */
					// if lux in switch range, ignore range switch
				...
				struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(val == 1) {
	    ...
	    if(!atomic_read(&isl29125_info->suspend_state)) {
		    queue_delayed_work(isl29125_info->wq,
				    &isl29125_info->sensor_dwork,
...
static ssize_t show_tp_type_flag(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", tp_type_flag);
}

static ssize_t store_tp_type_flag(struct device *dev,
				struct device_attribute *attr, const char *buf, size_t count)
{
	int data ;
	int flag;
	if( 1 == sscanf(buf,"%d",&data)){
		...
		printk("ALS huangzifan store_tp_type_flag: %s\n", buf);
	}	
	return count;
}
...
static DEVICE_ATTR(tp_type_flag, ISL29125_SYSFS_PERMISSIONS ,show_tp_type_flag, store_tp_type_flag);
...
int initialize_isl29125(struct i2c_client *client)
{
	unsigned char reg;
	int res;
...
//Add by huangzifan at 2015.10.22
	if( 0 == tp_type_flag){//At LK '0' mean BLACK,'1' mean WHITE.So we need to convert it at driver.Modify by huangzifan
		isl29125_info->tptype = 1;
	}else{
		isl29125_info->tptype = 0;
	}	
	...
	isl29125_info->first_flag = 1;
	 ...
	 info += sprintf(info,"ISL29125 ");
    	 info += sprintf(info,"InterS");
	...
	res = i2c_smbus_write_byte_data(client, CONFIG1_REG, 0x0D); 
	if( res < 0){
		printk("initialize_isl29125 i2c error!\n");
		return -1;
	}

	/* Default IR Active compenstation,
	   Disable IR compensation control */
	res = i2c_smbus_write_byte_data(client, CONFIG2_REG, isl29125_info->ir_comp);
	if( res < 0){
		printk("initialize_isl29125 i2c error!\n");
		return -1;
	}
	...
	res = i2c_smbus_write_byte_data(client, CONFIG3_REG, 0x1D); 
	if( res < 0){
		printk("initialize_isl29125 i2c error!\n");
		return -1;
	}
	...
	res = i2c_smbus_write_byte_data(client, LOW_THRESHOLD_LBYTE_REG, 0xCC);	
	if( res < 0){
		printk("initialize_isl29125 i2c error!\n");
		return -1;
	}

	res = i2c_smbus_write_byte_data(client, LOW_THRESHOLD_HBYTE_REG, 0x0C);	
	if( res < 0){
		printk("initialize_isl29125 i2c error!\n");
		return -1;
	}
	...
	res = i2c_smbus_write_byte_data(client, HIGH_THRESHOLD_LBYTE_REG, 0xCC);	
	if( res < 0){
		printk("initialize_isl29125 i2c error!\n");
		return -1;
	}

	res = i2c_smbus_write_byte_data(client, HIGH_THRESHOLD_HBYTE_REG, 0xCC);	
	if( res < 0){
		printk("initialize_isl29125 i2c error!\n");
		return -1;
	}
...
static void isl29125_work_handler(struct work_struct *work)
{
	...
	if (ret < 0) {
	    printk(KERN_ERR "[ISL] %s: Failed to read word\n", __FUNCTION__);
	    return;
	}
	...
	autorange(isl29125->raw_green_ircomp, lux);
	...
	*/
	...
	/*debounce processing : eg. 0,1,0,1 for range switch*/
	...
	suspend_state = atomic_read(&isl29125->suspend_state);
	pr_warn("[ISL] lux %llu, cache_green %lu, suspend_state %d\n",
			isl29125->lux, isl29125->cache_green, suspend_state);
	if(!suspend_state) { // not in suspend
	...
	if (isl29125_info->sensor_enable && !atomic_read(&isl29125->suspend_state)) {
		if(fast_flag > 0) {
			fast_flag--;
			isl29125->conversion_time = 50;
		} else {
			isl29125->conversion_time = DEFAULT_CONVERSION_TIME * 4;
		}
		queue_delayed_work(isl29125->wq, &isl29125->sensor_dwork,
				msecs_to_jiffies(isl29125->conversion_time));
	}
}
#endif
...
static void isl29125_work_handler(struct work_struct *work)
{
	...
	queue_delayed_work(isl29125->wq, &isl29125->sensor_dwork, msecs_to_jiffies(POLL_DELAY));	// restart timer
}
#endif
...
int isl29125_als_read(void)
{
	...
	static int fast_flag = 0;
	int final_lux = 0;
	struct isl29125_data_t *isl29125 = isl29125_info;
...
//  printk("huangzifan_als, isl29125->cache_green = %lu, isl29125->cache_red =%lu,isl29125->cache_blue =%lu, \n", isl29125->cache_green,isl29125->cache_red,isl29125->cache_blue);
...
//	isl29125->cct = cal_cct(isl29125);
	...
	isl29125->prev_lux = lux;
	if(lux < 0){
		printk("isl29125_als_read lux value is error\n");
		return -1;
	}
	final_lux = (int)lux;
	return final_lux;
...
EXPORT_SYMBOL(isl29125_als_read);


int isl29125_als_enable(void)
{
/*
	int error;
	gainrange = ALS_RANGE_16K;
	APS_LOG("gainrange = %d\n",gainrange);
	switch (gainrange)
	{
		case ALS_RANGE_64K:
...
//At by huangzifan at 2015.10.22. Check the TP type when enable.
	if( 0 == tp_type_flag){//At LK '0' mean BLACK,'1' mean WHITE.So we need to convert it at driver.Modify by huangzifan
		isl29125_info->tptype = 1; // DEFAULT BLACK
	}else{
		isl29125_info->tptype = 0;//WHITE
	}	
	printk("Isl29125_als_enable,isl29125_info->tptype = %d\n",isl29125_info->tptype);
...
EXPORT_SYMBOL(isl29125_als_enable);

int isl29125_als_disable(void)
{
/*
	int error;
...
EXPORT_SYMBOL(isl29125_als_disable);
...
*/
	...
	struct isl29125_data_t *obj = i2c_get_clientdata(client);  
	int err = 0;
	void __user *ptr = (void __user*) arg;
	int dat;
	uint32_t enable;
//	printk("huangzifan isl29125_unlocked_ioctl cmd= %d\n", cmd); 
	switch (cmd)
	{
...
*/
		...
		default:
			printk("isl29125_unlocked_ioctl. This commad  is  not supported\n");
			err = -ENOIOCTLCMD;
			break;
	}
...
static int isl_early_suspend(struct early_suspend *h)
{
	...
	atomic_set(&isl29125_info->suspend_state, 1);
...
{
	struct isl29125_data_t *obj = container_of(h, struct isl29125_data_t, early_drv);         
	...
	atomic_set(&isl_als_start, 1);
	atomic_set(&isl29125_info->suspend_state, 0);
	/*
	if (isl29125_info->sensor_enable)
	    queue_delayed_work(isl29125_info->wq,
			    &isl29125_info->sensor_dwork, msecs_to_jiffies(POLL_DELAY));
	*/
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	struct isl29125_data_t *obj = (struct isl29125_data_t *)self;
//	printk("huangzifan ISL_als_operate %d!\n",(int)command);
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			...
			{
				printk("get sensor als data 1!\n");
				sensor_data = (hwm_sensor_data *)buff_out;
				//printk("get sensor als data 2!\n");				
				obj->lux = isl29125_als_read();
				printk("get sensor als data 3!\n");
                #if defined(MTK_AAL_SUPPORT)
				//sensor_data->values[0] = ltr559_get_als_value(obj, obj->als);//wisky-lxh@20150206
				sensor_data->values[0] = (int)(obj->lux);
				#else
			...
			break;
		default:
			printk("light sensor operate function no this parameter %d!\n", command);
			err = -1;
			break;
	}
...
/*
	...
	isl29125->wq = create_singlethread_workqueue("isl29125");
	if(isl29125->wq == NULL) {
		pr_err("[ISL] create workqueue failed\n");
		goto err;
	}
*/
	/* Initialize the default configurations for isl29125 sensor device */ 
	ret = initialize_isl29125(client);
	if(ret < 0) {                                                                   
		printk("init isl29125 error\n");                    
		goto err;                                                           
	}     
	...
	isl29125->early_suspend.level = EARLY_SUSPEND_LEVEL_DISABLE_FB + 1;
...
#endif
	als_isl29125 = 1;
/*Remove it. The isl29125 has been registerd to NEW ARCH at pa12200001.c.Modify by huangzifan 2015.10.22
	isl29125->enable = 0;
	set_bit(CMC_BIT_ALS, &isl29125->enable);

	obj_als.self = isl29125;
	obj_als.polling = 1;
	obj_als.sensor_operate = ISL_als_operate;
	if(err = hwmsen_attach(ID_LIGHT, &obj_als))
	{
		printk("huangzifan attach fail = %d\n", err);
	//	goto exit_create_attr_failed;
	}
	printk("huangzifan isl_sensor_probe is ok!\n");
*/
	return 0;
err: 
	return -1;
