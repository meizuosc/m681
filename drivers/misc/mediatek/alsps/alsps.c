...
extern int ltr579_als_aal_read(void);
...
static int alsps_real_driver_init() 
{
	...
	for(i = 0; i < 2; i++)
	{
	  printk(KERN_ERR "alsps_real_driver_init i=%d\n",i);
	  if(0 != alsps_init_list[i])
	  {
	    printk(KERN_ERR " alsps try to init driver %s\n", alsps_init_list[i]->name);
	    err = alsps_init_list[i]->init();
		if(0 == err)
		{
		   printk("alsps real driver %s probe ok\n", alsps_init_list[i]->name);
	//	   break;
		}
	  }
	}
	...
	{
	   printk(" alsps_real_driver_init fail\n");
	   err=-1;
	}
	return err;
}
...
{
//Start:Reqxxx,liuyang3.wt,MODIFY,20160121,Modify for DRE request.
#if 0
	int ret = 0;
	struct alsps_context *cxt = NULL;
	int value = 0;
	...
	if( als_isl29125 == 1){
		...
		value = ltr579_als_aal_read();
	}
	ALSPS_LOG("alsps_aal_get_data lux value=%d\n", value); 
	return value;
//End:Start:Reqxxx,liuyang3.wt,MODIFY,20160121,Modify for DRE request.	
}
//***************************************************
