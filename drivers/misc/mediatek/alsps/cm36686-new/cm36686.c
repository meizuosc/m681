...
#ifdef CONFIG_64BIT
		atomic64_set(&obj->ps_deb_end, jiffies+atomic_read(&obj->ps_debounce)/(1000/HZ));
#else
...
#ifdef CONFIG_64BIT
		atomic64_set(&obj->als_deb_end, jiffies+atomic_read(&obj->als_debounce)/(1000/HZ));
#else
...
static ssize_t cm36686_show_config(struct device_driver *ddri, char *buf)
{
	...
	res = snprintf(buf, PAGE_SIZE, "(%d %d %d %d %d)\n", 
		...
		atomic_read(&cm36686_obj->ps_mask), atomic_read(&cm36686_obj->ps_thd_val),
...
static ssize_t cm36686_show_als(struct device_driver *ddri, char *buf)
{
	...
	if ((res = cm36686_read_als(cm36686_obj->client, &cm36686_obj->als)))
	{
...
int cm36686_setup_eint(struct i2c_client *client)
{
	...
	{
		of_property_read_u32_array(cm36686_obj->irq_node, "debounce", ints, ARRAY_SIZE(ints));
		...
		if (request_irq(cm36686_obj->irq, cm36686_eint_handler, IRQF_TRIGGER_NONE, "ALS-eint", NULL)) {
...
static int cm36686_init_client(struct i2c_client *client)
{
	...
	if (1 == obj->hw->polling_mode_als)
		databuf[1] = 0b01000001;
	else
		databuf[1] = 0b01000111;	
	...
	databuf[0] = CM36686_REG_PS_CONF1_2;
	databuf[1] = 0b01100011;
	if (1 == obj->hw->polling_mode_ps)
		databuf[2] = 0b00001000;
	else
		databuf[2] = 0b00001011;
	res = CM36686_i2c_master_operate(client, databuf, 0x3, I2C_FLAG_WRITE);
	if (res <= 0)
	{
		APS_ERR("i2c_master_send function err\n");
