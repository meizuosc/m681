...
static void fastcall_thread_no_freeze_fn (void *data) {
                set_nofreezable();
}
...
int mc_fastcall_init(struct mc_context *context)
{
	...
	struct kthread_work  work =  KTHREAD_WORK_INIT(work, fastcall_thread_no_freeze_fn);
...
#endif
	if (!queue_kthread_work(&fastcall_worker, &work)) {
		MCDRV_DBG_ERROR(mcd, "cannot set mc_fastcall to NO FREEZABLE");
		return 0;
	}
