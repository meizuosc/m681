...
void md_cd_traffic_monitor_func(unsigned long data)
{
	...
	for (i = 0; i < QUEUE_LEN(md_ctrl->txq); i++) {
		if (md_ctrl->txq[i].busy_count != 0) {
			CCCI_REP_MSG(md->index, TAG, "Txq%d busy count %d\n", i, md_ctrl->txq[i].busy_count);
			md_ctrl->txq[i].busy_count = 0;
		}
	}
#ifdef ENABLE_CLDMA_TIMER
	CCCI_REP_MSG(md->index, TAG, "traffic(tx_timer): [3]%llu %llu, [4]%llu %llu, [5]%llu %llu\n",
	...
	CCCI_REP_MSG(md->index, TAG, "net Rx skb queue:%u %u %u / %u %u %u\n",
...
static void cldma_dump_packet_history(struct ccci_modem *md)
{
	...
	for (i = 0; i < QUEUE_LEN(md_ctrl->txq); i++) {
		CCCI_HSTRY_MSG(md->index, 0, "Current txq%d pos: tr_done=%x, tx_xmit=%x\n", i,
		       (unsigned int)md_ctrl->txq[i].tr_done->gpd_addr,
		       (unsigned int)md_ctrl->txq[i].tx_xmit->gpd_addr);
	}
	for (i = 0; i < QUEUE_LEN(md_ctrl->rxq); i++) {
		CCCI_HSTRY_MSG(md->index, 0, "Current rxq%d pos: tr_done=%x, rx_refill=%x\n", i,
...
static inline void cldma_start(struct ccci_modem *md)
{
	...
	CCCI_NOTICE_MSG(md->index, TAG, "%s from %ps\n", __func__, __builtin_return_address(0));
	enable_irq(md_ctrl->cldma_irq_id);
	spin_lock_irqsave(&md_ctrl->cldma_timeout_lock, flags);
	/* set start address */
	for (i = 0; i < QUEUE_LEN(md_ctrl->txq); i++) {
		...
		for (i = 0; i < QUEUE_LEN(md_ctrl->rxq); i++) {
			...
			list_for_each_entry(req, &md_ctrl->rxq[i].tr_ring->gpd_ring, entry) {
				...
				if (req->skb == NULL) {
					...
					req->data_buffer_ptr_saved =
						dma_map_single(&md->plat_dev->dev, req->skb->data, skb_data_size(req->skb),
								DMA_FROM_DEVICE);
					rgpd->data_buff_bd_ptr = (u32) (req->data_buffer_ptr_saved);
					caculate_checksum((char *)rgpd, 0x81);
				}
...
static int md_cd_reset(struct ccci_modem *md)
{
	...
	if (md->flight_mode == MD_FIGHT_MODE_ENTER && atomic_read(&md_ctrl->ccif_irq_enabled) == 1) {
			...
			CCCI_INF_MSG(md->index, TAG, "Reset when MD state: %d, %d, %d\n",
					md->md_state, md->boot_stage, md->ex_stage);

	} else if (md->flight_mode != MD_FIGHT_MODE_ENTER &&
		md->md_state == EXCEPTION && md->boot_stage != MD_BOOT_STAGE_EXCEPTION && md->ex_stage != EX_INIT_DONE) {

		CCCI_INF_MSG(md->index, TAG, "Will reset after MD exception!\n");
		while (++count < (EX_EE_WHOLE_TIMEOUT * 50)) {
			...
			if ((md->md_state != EXCEPTION) ||
				...
				CCCI_INF_MSG(md->index, TAG, "Reset when MD exception!%d, %d, %d\n",
					md->md_state, md->boot_stage, md->ex_stage);
				break;
			}
			msleep(20);
...
static void dump_runtime_data_v2(struct ccci_modem *md, struct ap_query_md_feature *ap_feature)
{
	...
	CCCI_INF_MSG_RUNTIME(md->index, 0, "head_pattern 0x%x\n", ap_feature->head_pattern);

	for (i = BOOT_INFO; i < AP_RUNTIME_FEATURE_ID_MAX; i++) {
		CCCI_INF_MSG_RUNTIME(md->index, 0, "feature %u: mask %u, version %u\n",
				i, ap_feature->feature_set[i].support_mask, ap_feature->feature_set[i].version);
	}
	CCCI_INF_MSG_RUNTIME(md->index, 0, "share_memory_support 0x%x\n", ap_feature->share_memory_support);
	CCCI_INF_MSG_RUNTIME(md->index, 0, "ap_runtime_data_addr 0x%x\n", ap_feature->ap_runtime_data_addr);
	CCCI_INF_MSG_RUNTIME(md->index, 0, "ap_runtime_data_size 0x%x\n", ap_feature->ap_runtime_data_size);
	CCCI_INF_MSG_RUNTIME(md->index, 0, "md_runtime_data_addr 0x%x\n", ap_feature->md_runtime_data_addr);
	CCCI_INF_MSG_RUNTIME(md->index, 0, "md_runtime_data_size 0x%x\n", ap_feature->md_runtime_data_size);
	CCCI_INF_MSG_RUNTIME(md->index, 0, "set_md_mpu_start_addr 0x%x\n", ap_feature->set_md_mpu_start_addr);
	CCCI_INF_MSG_RUNTIME(md->index, 0, "set_md_mpu_total_size 0x%x\n", ap_feature->set_md_mpu_total_size);
	CCCI_INF_MSG_RUNTIME(md->index, 0, "tail_pattern 0x%x\n", ap_feature->tail_pattern);
}
...
static void dump_runtime_data(struct ccci_modem *md, struct modem_runtime *runtime)
{
	...
	*p = runtime->Platform_H;
	...
	CCCI_INF_MSG_RUNTIME(md->index, 0, "**********************************************\n");
	CCCI_INF_MSG_RUNTIME(md->index, 0, "Prefix					  %c%c%c%c\n", ctmp[0],
			ctmp[1], ctmp[2], ctmp[3]);
	CCCI_INF_MSG_RUNTIME(md->index, 0, "Platform_L				  %c%c%c%c\n", ctmp[4], ctmp[5],
		     ctmp[6], ctmp[7]);
	CCCI_INF_MSG_RUNTIME(md->index, 0, "Platform_H				  %c%c%c%c\n", ctmp[8], ctmp[9],
		     ctmp[10], ctmp[11]);
	CCCI_INF_MSG_RUNTIME(md->index, 0, "DriverVersion			   0x%x\n", runtime->DriverVersion);
	CCCI_INF_MSG_RUNTIME(md->index, 0, "BootChannel				 %d\n", runtime->BootChannel);
	CCCI_INF_MSG_RUNTIME(md->index, 0, "BootingStartID(Mode)		0x%x\n", runtime->BootingStartID);
	CCCI_INF_MSG_RUNTIME(md->index, 0, "BootAttributes			  %d\n", runtime->BootAttributes);
	CCCI_INF_MSG_RUNTIME(md->index, 0, "BootReadyID				 %d\n", runtime->BootReadyID);

	CCCI_INF_MSG_RUNTIME(md->index, 0, "ExceShareMemBase			0x%x\n", runtime->ExceShareMemBase);
	CCCI_INF_MSG_RUNTIME(md->index, 0, "ExceShareMemSize			0x%x\n", runtime->ExceShareMemSize);
	CCCI_INF_MSG_RUNTIME(md->index, 0, "TotalShareMemBase		   0x%x\n", runtime->TotalShareMemBase);
	CCCI_INF_MSG_RUNTIME(md->index, 0, "TotalShareMemSize		   0x%x\n", runtime->TotalShareMemSize);

	CCCI_INF_MSG_RUNTIME(md->index, 0, "CheckSum					%d\n", runtime->CheckSum);

	p = (int *)ctmp;
	*p = runtime->Postfix;
	CCCI_INF_MSG_RUNTIME(md->index, 0, "Postfix					 %c%c%c%c\n", ctmp[0],
			ctmp[1], ctmp[2], ctmp[3]);
	CCCI_INF_MSG_RUNTIME(md->index, 0, "**********************************************\n");
...
static int md_cd_send_runtime_data(struct ccci_modem *md, unsigned int sbp_code)
{
	...
	CCCI_ERR_MSG(md->index, TAG, "WMID:%d\n", runtime->feature_4_val[1]);
...
static int md_cd_dump_info(struct ccci_modem *md, MODEM_DUMP_FLAG flag, void *buff, int length)
{
	...
	if (flag & DUMP_FLAG_CCIF) {
		...
		CCCI_EXP_INF_MSG(md->index, TAG, "Dump CCIF SRAM (last 16bytes)\n");
	...
	if (flag & DUMP_FLAG_SMEM) {
		CCCI_EXP_INF_MSG(md->index, TAG, "Dump share memory\n");
	...
	if (flag&DUMP_FLAG_MD_WDT) {
		CCCI_EXP_INF_MSG(md->index, KERN, "Dump MD RGU registers\n");
		md_cd_lock_modem_clock_src(1);
#ifdef BASE_ADDR_MDRSTCTL
...
{
	int ret;
	ret = sscanf(buf, "%d", &md_rxd_switcher);
	if (ret < 0)
		CCCI_ERR_MSG(md->index, TAG, "sscanf retrun fail: %d\n", ret);
	return count;
}
