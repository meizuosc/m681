...
#define IS_PASS_SKB(md, qno)	\
		(!md->data_usb_bypass && (md->md_state != EXCEPTION || md->ex_stage != EX_INIT_DONE) && ((1<<qno) & NET_RX_QUEUE_MASK))
...
static void md_ccif_dump(unsigned char *title, struct ccci_modem *md)
{
	...
	if (title == NULL)
		CCCI_INF_MSG(md->index, TAG, "md_ccif_dump: not dump channel data\n");
	else
...
static void md_ccif_queue_dump(struct ccci_modem *md)
{
	int idx;
	struct md_ccif_ctrl *md_ctrl = (struct md_ccif_ctrl *)md->private_data;
	CCCI_INF_MSG(md->index, TAG, "Dump md_ctrl->channel_id 0x%lx\n", md_ctrl->channel_id);
...
static int md_ccif_op_start(struct ccci_modem *md)
{
	...
	if (md->config.setting & MD_SETTING_FIRST_BOOT) {
		CCCI_INF_MSG(md->index, TAG, "CCIF modem is first boot\n");
		...
		if (md1) {
			while(md1->config.setting & MD_SETTING_FIRST_BOOT)
				msleep(20);
			CCCI_INF_MSG(md->index, TAG, "wait for MD1 starting done\n");
		} else
			CCCI_ERR_MSG(md->index, TAG, "get MD1 modem struct fail\n");
		md_ccif_ring_buf_init(md);
		CCCI_INF_MSG(md->index, TAG, "modem capability 0x%x\n", md->capability);
...
static int md_ccif_op_reset(struct ccci_modem *md)
{
	...
	if (md->flight_mode != MD_FIGHT_MODE_ENTER &&
 ...
 retry:
	...
	if (ccci_ringbuf_writeable(md->index, queue->ringbuf, skb->len) > 0) {
		...
		if (IS_PASS_SKB(md, qno))
			...
			CCCI_ERR_MSG(md->index, TAG, "unexpected md state? only free skb for Q%d\n", queue->index);
			dev_kfree_skb_any(skb);
		}
		/*send ccif request */
