...
static inline void append_runtime_feature(char **p_rt_data, struct ccci_runtime_feature *rt_feature, char *data)
{
...
static unsigned int get_booting_start_id(struct ccci_modem *md)
{
	...
	CCCI_INF_MSG(md->index, KERN, "get_booting_start_id 0x%x\n", booting_start_id);
	...
	for (i = BOOT_INFO; i < FEATURE_COUNT; i++) {
		...
		if (rt_feature.support_info.support_mask == CCCI_FEATURE_MUST_SUPPORT) {
			...
			case BOOT_INFO:
				...
				append_runtime_feature(&rt_data, &rt_feature, (char *)&boot_info);
				break;
			...
			case EXCEPTION_SHARE_MEMORY:
				...
				append_runtime_feature(&rt_data, &rt_feature, (char *)&rt_shm);
				break;
			...
			case MISC_INFO_HIF_DMA_REMAP:
				...
				append_runtime_feature(&rt_data, &rt_feature, (char *)&rt_f_element);
				break;
			...
			case MISC_INFO_SBP_ID:
				...
				CCCI_INF_MSG(md->index, KERN, "wm_id: 0x%X, sbp_id:0x%X\n",
						rt_f_element.feature[1], rt_f_element.feature[0]);
				append_runtime_feature(&rt_data, &rt_feature, (char *)&rt_f_element);
				break;
			case MISC_INFO_CCCI:
				rt_feature.data_len = sizeof(struct ccci_misc_info_element);
			...
			case MISC_INFO_CLIB_TIME:
				...
				rt_f_element.feature[0] = t.tv_sec;
				rt_f_element.feature[1] = t.tv_sec >> 32;
...
#ifdef MD_UMOLY_EE_SUPPORT
				unsigned int RF_CLK_BUF1_DRIVING_CURR = CLK_BUF_DRIVING_CURR_1_4MA,
...
#ifdef MD_UMOLY_EE_SUPPORT
					of_property_read_u32_array(node, "mediatek,clkbuf-driving-current", vals, 4);
...
#ifdef MD_UMOLY_EE_SUPPORT
				clkbuf->ClkBuf_Driving[0] = RF_CLK_BUF1_DRIVING_CURR;
				clkbuf->ClkBuf_Driving[1] = RF_CLK_BUF2_DRIVING_CURR;
				clkbuf->ClkBuf_Driving[2] = RF_CLK_BUF3_DRIVING_CURR;
				clkbuf->ClkBuf_Driving[3] = RF_CLK_BUF4_DRIVING_CURR;
				CCCI_INF_MSG(md->index, RPC, "RF_CLK_BUF*_DRIVING_CURR %d, %d, %d, %d, AfcDac: %d\n", RF_CLK_BUF1_DRIVING_CURR,
					RF_CLK_BUF2_DRIVING_CURR, RF_CLK_BUF3_DRIVING_CURR, RF_CLK_BUF4_DRIVING_CURR, AfcDac);
#endif
			}
...
static void status_msg_handler(struct ccci_port *port, struct ccci_request *req)
{
	...
	CCCI_REP_MSG(port->modem->index, KERN, "modem status info seq=0x%X\n", *(((u32 *) ccci_h) + 2));
...
void md_status_poller_func(unsigned long data)
{
	...
	CCCI_REP_MSG(md->index, KERN, "poll modem status %d seq=0x%X\n", ret, md->seq_nums[OUT][CCCI_STATUS_TX]);
...
static void ccci_md_ee_info_dump(struct ccci_modem *md)
{
	...
	/* Dump MD EE info */
	CCCI_EXP_INF_MSG(md->index, KERN, "Dump MD EX log, 0x%x, 0x%x\n", debug_info->more_info, (unsigned int)md->boot_stage);
	if (debug_info->more_info == MD_EE_CASE_NORMAL && md->boot_stage == MD_BOOT_STAGE_0 && md->flight_mode == MD_FIGHT_MODE_NONE) {
...
err_exit:
	md->boot_stage = MD_BOOT_STAGE_EXCEPTION;
	/* update here to maintain handshake stage info druing exception handling */
...
static void ccci_md_exp_change(struct ccci_modem *md)
{
	...
	if ((debug_info->more_info == MD_EE_CASE_NORMAL) && (md->boot_stage == MD_BOOT_STAGE_0) && (md->flight_mode != MD_FIGHT_MODE_ENTER)) {
...
PL_CORE_PROC:
				...
				case MD_EX_PL_FATALERR_BUF:
					...
					if ((core_id == MD_CORE_NUM) &&
						(ee_type == (MD_EX_CC_C2K_EXCEPTION  - MD_EX_CC_INVALID_EXCEPTION + MD_EX_PL_FATALE_TOTAL)))
						debug_info->type = MD_EX_CC_C2K_EXCEPTION;/* the only one case: none offender, c2k ee */
					else
...
void md_ex_monitor_func(unsigned long data)
{
	...
	if ((1 << MD_EE_PASS_MSG_GET) & md->ee_info_flag)
		CCCI_EXP_MSG(md->index, KERN, "MD exception timer 2 has been set!\n");
...
void md_ex_monitor2_func(unsigned long data)
{
	...
	if (md_wdt_ee && md->index == MD_SYS3){
		CCCI_INF_MSG(md->index, KERN, "trigger force assert after WDT EE.\n");
...
static int swtp_switch_mode(struct swtp_t *swtp)
{
	...
	if (swtp == NULL) {
		CCCI_ERR_MSG(-1, KERN, "%s data is null\n", __func__);
		return -1;
	}
	...
	CCCI_INF_MSG(swtp->md_id, KERN, "%s mode %d\n", __func__, swtp->curr_mode);
...
static int swtp_send_tx_power_mode(struct swtp_t *swtp)
{
	...
	if (md_state != BOOTING && md_state != READY) {
		CCCI_ERR_MSG(swtp->md_id, KERN, "%s md_state=%d no ready\n", __func__, md_state);
		ret = 1;
		goto __ERR_HANDLE__;
	}
	if (swtp->md_id == 0)
		...
		CCCI_ERR_MSG(swtp->md_id, KERN, "%s md is no support\n", __func__);
		ret = 2;
		goto __ERR_HANDLE__;
	}

	if (ret >= 0)
		CCCI_INF_MSG(swtp->md_id, KERN, "%s send swtp to md ret=%d, mode=%d, rety_cnt=%d\n",
...
static int swtp_init(int md_id)
{
	...
	if (node) {
		...
		if (ret != 0) {
			CCCI_ERR_MSG(md_id, KERN, "swtp-eint IRQ LINE NOT AVAILABLE\n");
		} else {
			CCCI_INF_MSG(md_id, KERN,
		...
		CCCI_ERR_MSG(md_id, KERN, "%s can't find compatible node\n", __func__);
		ret = -1;
	}
