...
int port_ipc_ioctl(struct ccci_port *port, unsigned int cmd, unsigned long arg)
{
	...
	case CCCI_IPC_WAIT_TIME_UPDATE:
		...
		CCCI_REP_MSG(port->modem->index, IPC, "CCCI_IPC_WAIT_TIME_UPDATE wakeup\n");
		break;

	case CCCI_IPC_UPDATE_TIMEZONE:
		CCCI_REP_MSG(port->modem->index, IPC, "CCCI_IPC_UPDATE_TIMEZONE keep 0x%x\n", (unsigned int)arg);
		current_time_zone = (int)arg;
		break;
	};
	return ret;
...
int ccci_ipc_set_garbage_filter(struct ccci_modem *md, int reg)
{
	...
	if (req) {
		...
		local_para_ptr->msg_len =
		    garbage_lenth + sizeof(struct garbage_filter_header) + sizeof(struct local_para);
		/* copy gf header */
...
int send_new_time_to_md(int tz)
{
	...
	CCCI_REP_DUMP_MSG(-1, IPC, "Update time(R): [sec=0x%lx][timezone=0x%08x][des=0x%08x]\n", tv.tv_sec,
		     sys_tz.tz_minuteswest, sys_tz.tz_dsttime);
	CCCI_REP_DUMP_MSG(-1, IPC, "Update time(A): [L:0x%08x][H:0x%08x][0x%08x][0x%08x]\n", timeinfo[0], timeinfo[1],
		     timeinfo[2], timeinfo[3]);
	if (port_ipc_kernel_write(&in_ilm) < 0) {
		CCCI_REP_MSG(-1, IPC, "Update fail\n");
		return -1;
	} else {
		CCCI_REP_MSG(-1, IPC, "Update success\n");
		return 0;
	}
}
