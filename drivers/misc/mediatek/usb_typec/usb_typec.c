...
void StateMachineFUSB300(struct usbtypc *typec)
{
	...
	FUSB300Read(regStatus0, 2, &Registers.Status.Status);	/* Read the status */
	FUSB300Read(regInterrupta, 2, &Registers.Status.InterruptAdv);	/* Read the status */
	FUSB300Read(regStatus0a, 2, &Registers.Status.StatusAdv);	/* Read the status */
...
BOOL FUSB300Write(unsigned char regAddr, unsigned char length, unsigned char *data)
{
	...
	fusb300_i2c_w_reg8(typec_client, regAddr, val);
#else
	int i;
...
{
#if 0
	*data = fusb300_i2c_r_reg(typec_client, regAddr);
#else
	int i;
...
static int usb3_switch_en(struct usbtypc *typec, int on)
{
	...
	if (on == ENABLE) {	/*enable usb switch */
		retval |= mt_set_gpio_out(typec->u3_sw->en_gpio, GPIO_OUT_ZERO);
		typec->u3_sw->en = ENABLE;
	} else {		/*disable usb switch */
		retval |= mt_set_gpio_out(typec->u3_sw->en_gpio, GPIO_OUT_ONE);
		typec->u3_sw->en = DISABLE;
	}

	fusb_printk(K_DEBUG, "%s gpio=%d\n", __func__, mt_get_gpio_out(typec->u3_sw->en_gpio));
...
static int usb3_switch_sel(struct usbtypc *typec, int sel)
{
	...
	if (sel == UP_SIDE) {	/*select SW1 */
		retval |= mt_set_gpio_out(typec->u3_sw->sel_gpio, GPIO_OUT_ZERO);
		typec->u3_sw->sel = sel;
	} else if (sel == DOWN_SIDE) {		/*select SW2 */
		retval |= mt_set_gpio_out(typec->u3_sw->sel_gpio, GPIO_OUT_ONE);
		typec->u3_sw->sel = sel;
	}

	fusb_printk(K_DEBUG, "%s gpio=%d\n", __func__, mt_get_gpio_out(typec->u3_sw->sel_gpio));
...
int usb3_switch_init(struct usbtypc *typec)
{
	...
	typec->u3_sw->en_gpio = GPIO_USB_TYPEC_SW_EN_PIN;
	typec->u3_sw->en = 0;
	typec->u3_sw->sel_gpio = GPIO_USB_TYPEC_SW_SEL_PIN;
	typec->u3_sw->sel = 0;

	/*chip enable pin*/
	retval |= mt_set_gpio_mode(typec->u3_sw->en_gpio, GPIO_MODE_GPIO);
	retval |= mt_set_gpio_dir(typec->u3_sw->en_gpio, GPIO_DIR_OUT);
	retval |= mt_set_gpio_out(typec->u3_sw->en_gpio, GPIO_OUT_ZERO);

	/*dir selection */
	retval |= mt_set_gpio_mode(typec->u3_sw->sel_gpio, GPIO_MODE_GPIO);
	retval |= mt_set_gpio_dir(typec->u3_sw->sel_gpio, GPIO_DIR_OUT);
	retval |= mt_set_gpio_out(typec->u3_sw->sel_gpio, GPIO_OUT_ZERO);

	fusb_printk(K_DEBUG, "en_gpio=%d, mode=%d, out=%d\n", typec->u3_sw->en_gpio,
		    mt_get_gpio_mode(typec->u3_sw->en_gpio),
		    mt_get_gpio_out(typec->u3_sw->en_gpio));

	fusb_printk(K_DEBUG, "sel_gpio=%d, mode=%d, out=%d\n", typec->u3_sw->sel_gpio,
		    mt_get_gpio_mode(typec->u3_sw->sel_gpio),
		    mt_get_gpio_out(typec->u3_sw->sel_gpio));
...
{
	int retval = 0;
	int u3_eq_c1 = GPIO_USB_REDRI_SW_C1_PIN;
	int u3_eq_c2 = GPIO_USB_REDRI_SW_C2_PIN;
	...
	retval |= mt_set_gpio_mode(typec->u_rd->c1_gpio, GPIO_MODE_GPIO);
	retval |= mt_set_gpio_dir(typec->u_rd->c1_gpio, GPIO_DIR_OUT);
	retval |= mt_set_gpio_out(typec->u_rd->c1_gpio, GPIO_OUT_ONE);

	retval |= mt_set_gpio_mode(typec->u_rd->c2_gpio, GPIO_MODE_GPIO);
	retval |= mt_set_gpio_dir(typec->u_rd->c2_gpio, GPIO_DIR_OUT);
	retval |= mt_set_gpio_out(typec->u_rd->c2_gpio, GPIO_OUT_ZERO);

	fusb_printk(K_DEBUG, "c1_gpio=%d, mode=%d, out=%d\n", typec->u_rd->c1_gpio,
		    mt_get_gpio_mode(typec->u_rd->c1_gpio), mt_get_gpio_out(typec->u_rd->c1_gpio));

	fusb_printk(K_DEBUG, "c2_gpio=%d, mode=%d, out=%d\n", typec->u_rd->c2_gpio,
		    mt_get_gpio_mode(typec->u_rd->c2_gpio), mt_get_gpio_out(typec->u_rd->c2_gpio));
...
int usb_redriver_config(struct usbtypc *typec, int ctrl_pin, int stat)
{
	...
	fusb_printk(K_DEBUG, "%s gpio=%d, mode=%d, out=%d\n", __func__, pin_num,
		    mt_get_gpio_mode(pin_num), mt_get_gpio_out(pin_num));
...
{
	int retval = 0;
	int fusb302_eint_pin;
	struct device_node *node;
	u32 ints[2] = { 0, 0 };
	u32 ints1[2] = { 0, 0 };
	unsigned int gpiopin, debounce;
	unsigned int usb_typec_eint_type;

	mt_set_gpio_mode(GPIO_USB_TYPEC_EINT_PIN, GPIO_USB_TYPEC_EINT_PIN_M_GPIO);
	mt_set_gpio_dir(GPIO_USB_TYPEC_EINT_PIN, GPIO_DIR_IN);
	mt_set_gpio_pull_enable(GPIO_USB_TYPEC_EINT_PIN, GPIO_PULL_ENABLE);
	mt_set_gpio_pull_select(GPIO_USB_TYPEC_EINT_PIN, GPIO_PULL_UP);
	...
	if (node) {
		...
		debounce = ints[1];
		usb_typec_eint_type = ints1[1];
#if defined(CONFIG_MTK_LEGACY)
		mt_gpio_set_debounce(gpiopin, debounce);
#else
		gpio_request(gpiopin, "USB3");
	...
	retval =
	    request_irq(typec->irqnum, fusb300_eint_isr, IRQF_TRIGGER_LOW, "fusb300_eint", typec);
	if (retval != 0) {
...
/*
	fusb_printk(K_DEBUG, "external eint register done, irqnum = %d, gpio_mode(%d), "
		    "gpio_pull_enable(%d), gpio_pull_select(%d)\n", typec->irqnum,
		    mt_get_gpio_mode(GPIO_USB_TYPEC_EINT_PIN),
		    mt_get_gpio_pull_enable(GPIO_USB_TYPEC_EINT_PIN),
		    mt_get_gpio_pull_select(GPIO_USB_TYPEC_EINT_PIN));
...
/*FIXME: need to move DTS*/
#define FUSB300_I2C_ADDR 0x22
#define FUSB300_I2C_BUS 1
#define FUSB300_NAME "FUSB300"

static const struct i2c_device_id usb_i2c_id[] = { {FUSB300_NAME, 0}, {} };
static struct i2c_board_info usb_i2c_dev __initdata = { I2C_BOARD_INFO(FUSB300_NAME, FUSB300_I2C_ADDR) };
...
static int usb_gpio_debugfs_show(struct seq_file *s, void *unused)
{
	...
	if (pin) {
		fusb_printk(K_DEBUG, "en=%d, mode=%d, out=%d, pullen=%d\n", pin,
			    mt_get_gpio_mode(pin), mt_get_gpio_out(pin),
			    mt_get_gpio_pull_enable(pin));

		seq_printf(s, "EN[%x] mode=%d out=%d pullen=%d\n", pin,
			   mt_get_gpio_mode(pin), mt_get_gpio_out(pin),
			   mt_get_gpio_pull_enable(pin));
	}
	...
	if (pin) {
		fusb_printk(K_DEBUG, "sel=%d, mode=%d, out=%d, pullen=%d\n", pin,
			    mt_get_gpio_mode(pin), mt_get_gpio_out(pin),
			    mt_get_gpio_pull_enable(pin));

		seq_printf(s, "SEL[%x] mode=%d out=%d pullen=%d\n", pin,
			   mt_get_gpio_mode(pin), mt_get_gpio_out(pin),
			   mt_get_gpio_pull_enable(pin));
	}
	...
	if (pin) {
		fusb_printk(K_DEBUG, "C1=%d, mode=%d, out=%d, pullen=%d dir=%d\n", pin,
			    mt_get_gpio_mode(pin), mt_get_gpio_out(pin),
			    mt_get_gpio_pull_enable(pin), mt_get_gpio_dir(pin));

		seq_printf(s, "EN[%x] mode=%d out=%d pullen=%d dir=%d\n", pin,
			   mt_get_gpio_mode(pin), mt_get_gpio_out(pin),
			   mt_get_gpio_pull_enable(pin), mt_get_gpio_dir(pin));
	}
	...
	if (pin) {
		fusb_printk(K_DEBUG, "C2=%d, mode=%d, out=%d, pullen=%d dir=%d\n", pin,
			    mt_get_gpio_mode(pin), mt_get_gpio_out(pin),
			    mt_get_gpio_pull_enable(pin), mt_get_gpio_dir(pin));

		seq_printf(s, "SEL[%x] mode=%d out=%d pullen=%d dir=%d\n", pin,
			   mt_get_gpio_mode(pin), mt_get_gpio_out(pin),
			   mt_get_gpio_pull_enable(pin), mt_get_gpio_dir(pin));
	}
...
{
	i2c_register_board_info(FUSB300_I2C_BUS, &usb_i2c_dev, 1);
