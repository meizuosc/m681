...
UINT32 u4VDEC_HAL_H264_GetStartCode_8530(UINT32 u4BSID, UINT32 u4VDecID)
{
    ...
    if (u4BSID == 0)
    {
        ...
        if (((u4Temp >> 8) != START_CODE) || fgVLDRem03)
        {
            ...
            {
				pr_debug("Can not find AVC start code\n");
                VDEC_ASSERT(0);
            }
        }
    }
...
// **************************************************************************
UINT32 u4VDEC_HAL_H264_ReadRdPtr(UINT32 u4BSID, UINT32 u4VDecID, ULONG u4VFIFOSa, UINT32 *pu4Bits)
{
    return u4VDecReadH264VldRPtr(u4BSID, u4VDecID, pu4Bits, PHYSICAL((ULONG)u4VFIFOSa));
}
...
void vVDEC_HAL_H264_WriteScalingList(UINT32 u4VDecID, UINT32 u4Idx, CHAR *pcSlicePtr)
{
    ...
    if (u4Idx < 6)
    {
        ...
        for (i = 0; i < 4; i++)
        {
			...
			/*pr_debug("vVDEC_HAL_H264_WriteScalingList %d %d %d %d\n", pcSlicePtr[i<<2],
						pcSlicePtr[(i<<2) + 1],pcSlicePtr[(i<<2) + 2],pcSlicePtr[(i<<2) + 3]);*/
...
void vVDEC_HAL_H264_InitPRefList(UINT32 u4VDecID, BOOL fgPicFrm, UINT32 u4MaxFrameNum, UINT32 u4CurrPicNum)
{
...
#if VDEC_UFO_SUPPORT
	pr_debug("MC_664 write == 0x00000010\n");
    vVDecWriteMC(u4VDecID, 664 * 4, 0x10);
    vVDecWriteMC(u4VDecID, 722 * 4, 0x01);
#endif
    ...
    if (i == 10000)
    {
		pr_debug("[VDEC][8550_AVC]: RW_AVLD_RESET_PIC_NUM: Pooling FAIL!!!\n");
    }
#endif
...
void vVDEC_HAL_H264_SetPRefPicListReg(UINT32 u4VDecID, VDEC_INFO_H264_P_REF_PRM_T *prPRefPicListInfo)
{
...
#if VDEC_UFO_SUPPORT
    if (prPRefPicListInfo->ucPicStruct == FRAME)
    {
		pr_debug("%d %d\n", ucRegIdx, prPRefPicListInfo->i4PicNum);
        UINT32 u4RegValue;
        u4RegValue = u4VDecReadMC(u4VDecID, 664 * 4);
		pr_debug("MC_664 read == 0x%08x\n", u4RegValue);
        vVDecWriteMC(u4VDecID, 664 * 4, u4RegValue | 0x010);
		pr_debug("MC_664 write == 0x%08x\n", u4RegValue | 0x010);
        vVDecWriteMC(u4VDecID, 722 * 4, 0x01);
        u4RegValue = u4VDecReadMC(u4VDecID, 718 * 4);
		pr_debug("MC_718 read == 0x%08x\n", u4RegValue);
        vVDecWriteMC(u4VDecID, 718 * 4, u4RegValue | (0x01 << ucRegIdx));
		pr_debug("MC_718 write == 0x%08x (%d)\n", u4RegValue | (0x01 << ucRegIdx), prPRefPicListInfo->i4PicNum);
    }
    else
    {
		pr_debug("%d %d\n", ucRegIdx, prPRefPicListInfo->i4PicNum);
    ...
    if (ucFld == FRAME)
    {
        ...
        vVDecWriteMC(u4VDecID, RW_MC_P_LIST0 + (ucRegIdx << 2), PHYSICAL((ULONG)  prPRefPicListInfo->u4FBufYStartAddr));
#else
        vVDecWriteMC(u4VDecID, RW_MC_P_LIST0 + (ucRegIdx << 2), u4AbsDramANc((ULONG)  prPRefPicListInfo->u4FBufYStartAddr));
#endif
#if VDEC_MVC_SUPPORT
...
#endif
        {
            ...
            vVDecWriteAVCVLD(u4VDecID, RW_AVLD_REORD_P0_RPL + (ucRegIdx << 2), u4Param);
			/*pr_debug("SetPRefPicList2: [0x%X] = 0x%X\n",RW_AVLD_REORD_P0_RPL + (ucRegIdx<<2),u4Param);*/
        }
    ...
    else
    {
        if (ucFld == TOP_FIELD)
        {
            ...
            if (fgIsMVCDecode(u4VDecID))
            {
                ...
                vVDecWriteAVCVLD(u4VDecID, RW_AVLD_REORD_P0_RPL + (ucRegIdx << 2), u4Param);
				/*pr_debug("SetPRefPicList3: [0x%X] = 0x%X\n",
				RW_AVLD_REORD_P0_RPL + (ucRegIdx<<2),u4Param);*/
            }
            else
#endif
            {
                ...
                vVDecWriteAVCVLD(u4VDecID, RW_AVLD_REORD_P0_RPL + (ucRegIdx << 2), u4Param);
				/*pr_debug("SetPRefPicList4: [0x%X] = 0x%X\n",
				RW_AVLD_REORD_P0_RPL + (ucRegIdx<<2),u4Param);*/
            }
...
#if VDEC_UFO_SUPPORT
BOOL bVDEC_HAL_H264_SetBRefRegforUFO(UINT32 u4VDecID, VDEC_INFO_H264_B_REF_PRM_T *prBRefPicListInfo, BOOL bForB0, BOOL bForB1)
{
    UCHAR ucRegIdx;
    UINT32 u4RegValue;
    UINT32 u4MC719, u4MC720;
    BOOL bB0B1Both = bForB0 && bForB1;
	pr_debug("bVDEC_HAL_H264_SetBRefRegforUFO in\n");
    ucRegIdx = ((prBRefPicListInfo->u4FBufInfo >> 16) & 0xff);
    if (bB0B1Both == TRUE)
    {
        if (prBRefPicListInfo->ucPicStruct == FRAME)
        {
			pr_debug("%d (%d %d)\n", ucRegIdx, prBRefPicListInfo->i4TFldPOC, prBRefPicListInfo->i4BFldPOC);
            u4RegValue = u4VDecReadMC(u4VDecID, 664 * 4);
			pr_debug("MC_664 read == 0x%08x\n", u4RegValue);
            vVDecWriteMC(u4VDecID, 664 * 4, u4RegValue | 0x010);
			pr_debug("MC_664 write == 0x%08x\n", u4RegValue | 0x010);
            vVDecWriteMC(u4VDecID, 722 * 4, 0x01);
            u4RegValue = u4VDecReadMC(u4VDecID, 719 * 4);
			pr_debug("MC_719 read == 0x%08x\n", u4RegValue);
            vVDecWriteMC(u4VDecID, 719 * 4, u4RegValue | (0x01 << ucRegIdx));
			pr_debug("MC_719 write == 0x%08x (%d)\n",
			u4RegValue|(0x01 << ucRegIdx),
			prBRefPicListInfo->i4PicNum);
            u4MC719 = u4RegValue | (0x01 << ucRegIdx);
        }
        else
        {
            u4RegValue = u4VDecReadMC(u4VDecID, 719 * 4);
            ...
            vVDecWriteMC(u4VDecID, 719 * 4, u4RegValue & ~(0x01 << ucRegIdx));
			pr_debug("MC_719 write == 0x%08x (%d)\n",
			u4RegValue & ~(0x01 << ucRegIdx), prBRefPicListInfo->i4PicNum);
        }
        if (prBRefPicListInfo->ucPicStruct1 == FRAME)
        {
			pr_debug("%d (%d %d)\n",
			ucRegIdx,
			prBRefPicListInfo->i4TFldPOC1,
			prBRefPicListInfo->i4BFldPOC1);
            u4RegValue = u4VDecReadMC(u4VDecID, 664 * 4);
			pr_debug("MC_664 read == 0x%08x\n", u4RegValue);
            vVDecWriteMC(u4VDecID, 664 * 4, u4RegValue | 0x010);
			pr_debug("MC_664 write == 0x%08x\n", u4RegValue | 0x010);
            vVDecWriteMC(u4VDecID, 722 * 4, 0x01);
            u4RegValue = u4VDecReadMC(u4VDecID, 720 * 4);
			pr_debug("MC_720 read == 0x%08x\n", u4RegValue);
            vVDecWriteMC(u4VDecID, 720 * 4, u4RegValue | (0x01 << ucRegIdx));
			pr_debug("MC_720 write == 0x%08x (%d)\n",
			u4RegValue | (0x01 << ucRegIdx),
			prBRefPicListInfo->i4PicNum1);
            u4MC720 = u4RegValue | (0x01 << ucRegIdx);
        }
        else
        {
            u4RegValue = u4VDecReadMC(u4VDecID, 720 * 4);
    ...
    else
    {
        if (prBRefPicListInfo->ucPicStruct == FRAME)
        {
            if (bForB0 == TRUE)
            {
				pr_debug("%d (%d %d)\n",
				ucRegIdx,
				prBRefPicListInfo->i4TFldPOC,
				prBRefPicListInfo->i4BFldPOC);
                u4RegValue = u4VDecReadMC(u4VDecID, 664 * 4);
				pr_debug("MC_664 read == 0x%08x\n", u4RegValue);
                vVDecWriteMC(u4VDecID, 664 * 4, u4RegValue | 0x010);
				pr_debug("MC_664 write == 0x%08x\n", u4RegValue | 0x010);
	...
	pr_debug("bVDEC_HAL_H264_SetBRefRegforUFO out\n");
#if 0
    /*
    else {
        /u4RegValue = u4VDecReadMC(u4VDecID, 719 * 4);
    ...
    */
    ...
    if (prBRefPicListInfo->ucPicStruct1 == FRAME && bForB1 == TRUE && bForB0 == FALSE)
    {
    ...
    if (u4MC719 == 0 && u4MC720 == 0) {
        //vVDecWriteMC(u4VDecID, 722 * 4, 0x0);
    }
    */
#endif
    return TRUE;
...
BOOL bVDEC_HAL_H264_SetBRefPicListReg(UINT32 u4VDecID, VDEC_INFO_H264_B_REF_PRM_T *prBRefPicListInfo)
{
    ...
    if (prH264DecPrm->fgIsReduceMVBuffer)
    {
		...
		/*pr_debug("AVC use reduced mv buffer\n");*/
    }
#endif
#endif
    ...
    if (ucFld == FRAME)
    {
        // B_0
#if CONFIG_DRV_VIRTUAL_ADDR
        vVDecWriteMC(u4VDecID, RW_MC_B_LIST0 + (ucRegIdx << 2), PHYSICAL((ULONG) prBRefPicListInfo->u4FBufYStartAddr));
#else
        vVDecWriteMC(u4VDecID, RW_MC_B_LIST0 + (ucRegIdx << 2), u4AbsDramANc((ULONG) prBRefPicListInfo->u4FBufYStartAddr));
#endif
		/*pr_debug("MC_%d -> %d\n", 279 + ucRegIdx, prBRefPicListInfo->ucPicStruct);*/
#if VDEC_MVC_SUPPORT
        if (fgIsMVCDecode(u4VDecID))
        {
...
#if CONFIG_DRV_VIRTUAL_ADDR
        vVDecWriteMC(u4VDecID, RW_MC_B_LIST1 + (ucRegIdx << 2), PHYSICAL((ULONG) prBRefPicListInfo->u4FBufYStartAddr1));
#else
        vVDecWriteMC(u4VDecID, RW_MC_B_LIST1 + (ucRegIdx << 2), u4AbsDramANc((ULONG) prBRefPicListInfo->u4FBufYStartAddr1));
#endif
		/*pr_debug("MC_%d -> %d\n", 311 + ucRegIdx, prBRefPicListInfo->ucPicStruct1);*/
#if VDEC_MVC_SUPPORT
        if (fgIsMVCDecode(u4VDecID))
        {
...
#if CONFIG_DRV_VIRTUAL_ADDR
        u4Temp = PHYSICAL((ULONG) prBRefPicListInfo->u4FBufMvStartAddr1);
#else
        u4Temp = u4AbsDramANc((ULONG) prBRefPicListInfo->u4FBufMvStartAddr1);
#endif
...
#if VDEC_UFO_SUPPORT
        bVDEC_HAL_H264_SetBRefRegforUFO(u4VDecID, prBRefPicListInfo, TRUE, TRUE);
#endif
    }
    else
    {
...
#if VDEC_UFO_SUPPORT
            bVDEC_HAL_H264_SetBRefRegforUFO(u4VDecID, prBRefPicListInfo, TRUE, FALSE);
#endif
        }
        // B_1
        if ((prBRefPicListInfo->u4ListIdx == 2) || (prBRefPicListInfo->u4ListIdx == 3) || (prBRefPicListInfo->u4ListIdx == 4) || (prBRefPicListInfo->u4ListIdx == 5))
        {
            ...
            {
#if CONFIG_DRV_VIRTUAL_ADDR
                vVDecWriteMC(u4VDecID, RW_MC_B_LIST1 + (ucRegIdx << 2), PHYSICAL((ULONG) prBRefPicListInfo->u4FBufYStartAddr));
#else
                vVDecWriteMC(u4VDecID, RW_MC_B_LIST1 + (ucRegIdx << 2), u4AbsDramANc((ULONG) prBRefPicListInfo->u4FBufYStartAddr));
#endif
				/*pr_debug("MC_%d -> %d\n", 311 + ucRegIdx, prBRefPicListInfo->ucPicStruct);*/
#if VDEC_MVC_SUPPORT
                if (fgIsMVCDecode(u4VDecID))
                {
...
#if CONFIG_DRV_VIRTUAL_ADDR
                u4Temp = PHYSICAL((ULONG) prBRefPicListInfo->u4FBufMvStartAddr);
#else
                u4Temp = u4AbsDramANc((ULONG) prBRefPicListInfo->u4FBufMvStartAddr);
#endif
...
#if VDEC_UFO_SUPPORT
            bVDEC_HAL_H264_SetBRefRegforUFO(u4VDecID, prBRefPicListInfo, FALSE, TRUE);
#endif
        }
    }
...
{
    UINT32 u4Temp;
	pr_debug("vVDEC_HAL_H264_B1ListSwap\n");
...
#if VDEC_UFO_SUPPORT
    UINT u4Bit0, u4Bit1;
    u4Temp = u4VDecReadMC(u4VDecID, 720 * 4);
    u4Bit0 = (u4Temp & 0x00000001) ? 0x01 : 0x00;
    u4Bit1 = (u4Temp & 0x00000002) ? 0x01 : 0x00;
    vVDecWriteMC(u4VDecID, 720 * 4, (u4Temp & 0xFFFFFFFC) | (u4Bit0 << 1) | (u4Bit1));
	pr_debug("MC_720 write == 0x%08x\n", (u4Temp & 0xFFFFFFFC) | (u4Bit0 << 1) | (u4Bit1));
#endif
}
...
#if CONFIG_DRV_VIRTUAL_ADDR
        vVDecWriteAVCFG(u4VDecID, RW_FGT_OUT_Y_ADDR, PHYSICAL((ULONG) prFGTPrm->pucFGTTrgYAddr) >> 4);
        vVDecWriteAVCFG(u4VDecID, RW_FGT_OUT_C_ADDR, PHYSICAL((ULONG) prFGTPrm->pucFGTTrgCAddr) >> 4);
        vVDecWriteAVCFG(u4VDecID, RW_FGT_IN_Y_ADDR, PHYSICAL((ULONG) prFGTPrm->pucFGTScrYAddr) >> 4);
        vVDecWriteAVCFG(u4VDecID, RW_FGT_IN_C_ADDR, PHYSICAL((ULONG) prFGTPrm->pucFGTScrCAddr) >> 4);
#else
        vVDecWriteAVCFG(u4VDecID, RW_FGT_OUT_Y_ADDR, u4AbsDramANc((ULONG) prFGTPrm->pucFGTTrgYAddr) >> 4);
        vVDecWriteAVCFG(u4VDecID, RW_FGT_OUT_C_ADDR, u4AbsDramANc((ULONG) prFGTPrm->pucFGTTrgCAddr) >> 4);
        vVDecWriteAVCFG(u4VDecID, RW_FGT_IN_Y_ADDR, u4AbsDramANc((ULONG) prFGTPrm->pucFGTScrYAddr) >> 4);
        vVDecWriteAVCFG(u4VDecID, RW_FGT_IN_C_ADDR, u4AbsDramANc((ULONG) prFGTPrm->pucFGTScrCAddr) >> 4);
#endif
...
INT32 i4VDEC_HAL_H264_DecStart(UINT32 u4VDecID, VDEC_INFO_DEC_PRM_T *prDecPrm)
{
    ...
    else
    {
        ...
        UINT32 u4RegValue;
        if (fgIsFrmPic(u4VDecID))
        {
            u4RegValue = u4VDecReadMC(u4VDecID, 664 * 4);
			pr_debug("MC_664 read == 0x%08x\n", u4RegValue);
            vVDecWriteMC(u4VDecID, 664 * 4, u4RegValue | 0x01);
			pr_debug("MC_664 write == 0x%08x\n", u4RegValue | 0x01);
            memset(prH264DecPrm->prCurrFBufInfo->u4YStartAddr, 0, prH264DecPrm->prCurrFBufInfo->u4DramPicArea);
        }
        // for pp out (UFO ENC)
        vVDecWriteMC(u4VDecID, 698 * 4, PHYSICAL((UINT32) prH264DecPrm->prCurrFBufInfo->u4YStartAddr) + prDecPrm->u4PIC_SIZE_BS);
        vVDecWriteMC(u4VDecID, 699 * 4, PHYSICAL((UINT32) prH264DecPrm->prCurrFBufInfo->u4YStartAddr) + prDecPrm->u4PIC_SIZE_BS + prDecPrm->u4UFO_LEN_SIZE_Y);
        // for MC (UFO DEC)
...
#else
        //vVDecWriteMC(u4VDecID, 664 * 4, 0);
        //vVDecWriteMC(u4VDecID, 722 * 4, 0);
#endif
#if CONFIG_DRV_VIRTUAL_ADDR
        vVDecWriteMC(u4VDecID, RW_MC_PP_Y_ADDR, PHYSICAL((ULONG) prH264DecPrm->prCurrFBufInfo->u4YStartAddr) >> 9);
        vVDecWriteMC(u4VDecID, RW_MC_PP_C_ADDR, PHYSICAL((ULONG)(prH264DecPrm->prCurrFBufInfo->u4YStartAddr + prH264DecPrm->prCurrFBufInfo->u4CAddrOffset)) >> 8);
#else
        vVDecWriteMC(u4VDecID, RW_MC_PP_Y_ADDR, u4AbsDramANc((ULONG) prH264DecPrm->prCurrFBufInfo->u4YStartAddr) >> 9);
        vVDecWriteMC(u4VDecID, RW_MC_PP_C_ADDR, u4AbsDramANc((ULONG)(prH264DecPrm->prCurrFBufInfo->u4YStartAddr + prH264DecPrm->prCurrFBufInfo->u4CAddrOffset)) >> 8);
#endif
...
#if CONFIG_DRV_VIRTUAL_ADDR
    vVDecWriteMC(u4VDecID, RW_AMC_Y_OUT_ADDR, PHYSICAL((ULONG) prH264DecPrm->prCurrFBufInfo->u4YStartAddr));
#else
    vVDecWriteMC(u4VDecID, RW_AMC_Y_OUT_ADDR, u4AbsDramANc((ULONG) prH264DecPrm->prCurrFBufInfo->u4YStartAddr));
#endif
    ...
    vVDecWriteAVCMV(u4VDecID, RW_AMV_WR_ADDR,
                    ((PHYSICAL((ULONG) prH264DecPrm->prCurrFBufInfo->u4MvStartAddr) >> 4)) + ((prDecPrm->ucPicStruct == BOTTOM_FIELD) ? u4FieldDistance : 0));
#else
    vVDecWriteAVCMV(u4VDecID, RW_AMV_WR_ADDR,
                    ((u4AbsDramANc((ULONG) prH264DecPrm->prCurrFBufInfo->u4MvStartAddr) >> 4)) + ((prDecPrm->ucPicStruct == BOTTOM_FIELD) ? u4FieldDistance : 0));
#endif
...
#if VDEC_FIELD_COMPACT
	pr_debug("[H264] Enable Field Compact Mode\n");
    ...
    vVDecWriteMISC(u4VDecID, VDEC_CRC_REG_EN, 1<<4);
    vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_PIC_PIX_SIZE, ((((prDecPrm->u4PicW) & 0xffff) << 0 ) | (((prDecPrm->u4PicH) & 0xffff) << 16 )));
#if VDEC_UFO_SUPPORT
    UINT32 u4RegValue1;
    //vVDecWritePP(u4VDecID, 800 * 4, 7);
    //vVDecWriteMC(u4VDecID, 664 * 4, 0);
    u4RegValue1 = u4VDecReadMC(u4VDecID, 664 * 4);
	pr_debug("MC_664 read == 0x%08x\n", u4RegValue1);
    u4RegValue1 = u4VDecReadMC(u4VDecID, 718 * 4);
	pr_debug("MC_718 read == 0x%08x\n", u4RegValue1);
    u4RegValue1 = u4VDecReadMC(u4VDecID, 719 * 4);
	pr_debug("MC_719 read == 0x%08x\n", u4RegValue1);
    u4RegValue1 = u4VDecReadMC(u4VDecID, 720 * 4);
	pr_debug("MC_720 read == 0x%08x\n", u4RegValue1);
    u4RegValue1 = u4VDecReadMC(u4VDecID, 722 * 4);
	pr_debug("MC_722 read == 0x%08x\n", u4RegValue1);
#endif

    u4Reg = u4VDecReadAVCVLD(u4VDecID, RO_AVLD_BARL);
	pr_debug("{H264 DEC} u4InstID = 0x%x, Input Window: 0x%08x\n", u4VDecID, u4Reg);
...
void vVDEC_HAL_H264_SetInterViewPRefPicListReg(UINT32 u4VDecID, VDEC_INFO_H264_P_REF_PRM_T *prPRefPicListInfo)
{
    ...
    if (ucFld == FRAME)
    {
        ...
        vVDecWriteAVCVLD(u4VDecID, RW_AVLD_REORD_P0_RPL + (ucRegIdx << 2), u4Param);
		/*pr_debug("SetInterPRefPicList1: [0x%X] = 0x%X\n", RW_AVLD_REORD_P0_RPL + (ucRegIdx<<2),  u4Param);*/
    ...
    else
    {
        if (ucFld == TOP_FIELD)
        {
            ...
            u4Param = (1 << 19) + (2 << 20) + (prPRefPicListInfo->u4ViewId & 0x7ffff);
            vVDecWriteAVCVLD(u4VDecID, RW_AVLD_REORD_P0_RPL + (ucRegIdx << 2), u4Param);
			/*pr_debug("SetInterPRefPicList2: [0x%X] = 0x%X\n", RW_AVLD_REORD_P0_RPL + (ucRegIdx<<2),  u4Param);*/
        ...
        else
        {
            ...
            u4Param = (1 << 19) + (2 << 20) + (prPRefPicListInfo->u4ViewId & 0x7ffff);
            vVDecWriteAVCVLD(u4VDecID, RW_AVLD_REORD_P0_RPL + (ucRegIdx << 2), u4Param);
			/*pr_debug("SetInterPRefPicList3: [0x%X] = 0x%X\n", RW_AVLD_REORD_P0_RPL + (ucRegIdx<<2),  u4Param);*/
...
void vVDEC_HAL_H264_SetInterViewB0RefPicListReg(UINT32 u4VDecID, VDEC_INFO_H264_P_REF_PRM_T *prPRefPicListInfo)
{
    ...
    if (ucFld == FRAME)
    {
        // B_0
#if CONFIG_DRV_VIRTUAL_ADDR
        vVDecWriteMC(u4VDecID, RW_MC_B_LIST0 + (ucRegIdx << 2), PHYSICAL((ULONG) prPRefPicListInfo->u4FBufYStartAddr));
#else
        vVDecWriteMC(u4VDecID, RW_MC_B_LIST0 + (ucRegIdx << 2), u4AbsDramANc((ULONG) prPRefPicListInfo->u4FBufYStartAddr));
#endif
...
void vVDEC_HAL_H264_SetInterViewB1RefPicListReg(UINT32 u4VDecID, VDEC_INFO_H264_P_REF_PRM_T *prPRefPicListInfo)
{
    ...
    if (ucFld == FRAME)
    {
        ...
        ucFBufIdx = prPRefPicListInfo->ucFBufIdx;
#if CONFIG_DRV_VIRTUAL_ADDR
        vVDecWriteMC(u4VDecID, RW_MC_B_LIST1 + (ucRegIdx << 2), PHYSICAL((ULONG) prPRefPicListInfo->u4FBufYStartAddr));
#else
        vVDecWriteMC(u4VDecID, RW_MC_B_LIST1 + (ucRegIdx << 2), u4AbsDramANc((ULONG) prPRefPicListInfo->u4FBufYStartAddr));
#endif
...
#if CONFIG_DRV_VIRTUAL_ADDR
        u4Temp = PHYSICAL((ULONG) prPRefPicListInfo->u4FBufMvStartAddr);
#else
        u4Temp = u4AbsDramANc((ULONG) prPRefPicListInfo->u4FBufMvStartAddr);
#endif
        vVDecWriteAVCMV(u4VDecID, RW_AMV_B1_REF_ADDR + (ucRegIdx << 3), u4Temp >> 4);
...
{
#if 0//VDEC_UFO_SUPPORT
    UINT32 u4Idx;
    UINT32 u4RegValue;
    for (u4Idx = 0; u4Idx <= 750; u4Idx++)
    {
        u4RegValue = u4VDecReadMC(u4VDecID, 4 * u4Idx);
		pr_debug("RISCRead_MC( %d); /* return 0x%x */\n", u4Idx, u4RegValue);
        msleep(1);
    }
    for (u4Idx = 0; u4Idx <= 50; u4Idx++)
    {
        u4RegValue = u4VDecReadVLDTOP(u4VDecID, 4 * u4Idx);
		pr_debug("RISCRead_VLDTOP( %d); /* return 0x%x */\n", u4Idx, u4RegValue);
        msleep(1);
    }
    for (u4Idx = 140; u4Idx <= 160; u4Idx++)
    {
        u4RegValue = u4VDecReadAVCMV(u4VDecID, 4 * u4Idx);
		pr_debug("RISCRead_AVCMV( %d); /* return 0x%x */\n", u4Idx, u4RegValue);
        msleep(1);
    }
    for (u4Idx = 815; u4Idx <= 824; u4Idx++)
    {
        u4RegValue = u4VDecReadPP(u4VDecID, 4 * u4Idx);
		pr_debug("RISCRead_PP( %d); /* return 0x%x */\n", u4Idx, u4RegValue);
        msleep(1);
    }
#endif
}
