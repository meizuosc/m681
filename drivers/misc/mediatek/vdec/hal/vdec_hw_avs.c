...
#ifdef VDEC_REG_DUMP
		pr_debug("[VDEC]RISCWrite('VDEC_FULL_ADDR_AVS_VLD + 4*%d, 32'h%x); \n", u4Addr >> 2, u4Val);
#endif
#ifdef VDEC_SIM_DUMP
        vVDecSimDump(u4VDecID, AVS_VLD_REG_OFFSET0, u4Addr, u4Val);
#endif
		/*pr_debug("[VDEC]0x	  %x (AVS vld Register %d) =  0x%x\n", u4Addr, u4Addr/4, u4Val );*/
    }
    else
    {
...
#ifdef VDEC_REG_DUMP
		pr_debug("[VDEC]RISCRead('VDEC_FULL_ADDR_AVS_VLD + 4*%d, 32'h%x); \n", u4Addr >> 2, u4RegVal);
#endif
        return u4RegVal;
    }
    else
...
void vVDecWriteAVSMV(UINT32 u4VDecID, UINT32 u4Addr, UINT32 u4Val)
{
    ...
    if (u4VDecID == 0)
    {
		...
		/*pr_debug("[VDEC]0x	  %x (AVS MV Register %d) =  0x%x\n", u4Addr, u4Addr/4, u4Val );*/
    }
    else
    {
...
{
	/*pr_debug("VDEC: INIT AVS Barrel Shift2\n");*/
    VDEC_ASSERT(0);
    return TRUE;
}
