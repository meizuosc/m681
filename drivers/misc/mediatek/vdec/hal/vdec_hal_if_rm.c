...
#endif
{
    ...
    if (prParsingPic->fgEnableMCOutput)
    {
...
#if VMMU_SUPPORT
        {
			...
			pr_debug("[RM] vVDEC_HAL_RM_TriggerDecode, 1, Page Addr = 0x%x\n", u4Page_addr);
...
#if VMMU_SUPPORT
        {
			...
			pr_debug("[RM] vVDEC_HAL_RM_TriggerDecode, 2, Page Addr = 0x%x\n", u4Page_addr);
    ...
    else
    {
...
#if VMMU_SUPPORT
        {
			...
			pr_debug("[RM] vVDEC_HAL_RM_TriggerDecode, 3, Page Addr = 0x%x\n", u4Page_addr);
...
#if VMMU_SUPPORT
        {
			...
			pr_debug("[RM] vVDEC_HAL_RM_TriggerDecode, 4, Page Addr = 0x%x\n", u4Page_addr);
...
#if VMMU_SUPPORT
    {
        ...
        u4Page_addr = (UINT32)_pucVMMUTable[u4VDecID] + ((((UINT32) prParsingPic->u4OutBufY) / (4 * 1024)) * 4);
		pr_debug("[RM] vVDEC_HAL_RM_TriggerDecode, 5, Page Addr = 0x%x\n", u4Page_addr);
        vPage_Table(u4VDecID, u4Page_addr, PHYSICAL((UINT32) prParsingPic->u4OutBufY), PHYSICAL((UINT32) prParsingPic->u4OutBufY) + PIC_Y_SZ);
        u4RegValue = (UINT32)(prParsingPic->u4OutBufY) >> 9;
    }
#else
...
#if VMMU_SUPPORT
    {
        ...
        u4Page_addr = (UINT32)_pucVMMUTable[u4VDecID] + ((((UINT32) prParsingPic->u4OutBufC) / (4 * 1024)) * 4);
		pr_debug("[RM] vVDEC_HAL_RM_TriggerDecode, 6, Page Addr = 0x%x\n", u4Page_addr);
        vPage_Table(u4VDecID, u4Page_addr, PHYSICAL((UINT32) prParsingPic->u4OutBufC), PHYSICAL((UINT32) prParsingPic->u4OutBufC) + PIC_C_SZ);
        u4RegValue = (UINT32)(prParsingPic->u4OutBufC) >> 8;
    }
#else
    ...
    if (prParsingPic->ePtype == RM_INTERPIC || prParsingPic->ePtype == RM_TRUEBPIC)
    {
...
#if VMMU_SUPPORT
        {
			...
			pr_debug("[RM] vVDEC_HAL_RM_TriggerDecode, 7, Page Addr = 0x%x\n", u4Page_addr);
...
#if VMMU_SUPPORT
        {
			...
			pr_debug("[RM] vVDEC_HAL_RM_TriggerDecode, 8, Page Addr = 0x%x\n", u4Page_addr);
    ...
    if (prParsingPic->ePtype == RM_TRUEBPIC)
    {
...
#if VMMU_SUPPORT
        {
			...
			pr_debug("[RM] vVDEC_HAL_RM_TriggerDecode, 9, Page Addr = 0x%x\n", u4Page_addr);
...
#if VMMU_SUPPORT
        {
			...
			pr_debug("[RM] vVDEC_HAL_RM_TriggerDecode, 10, Page Addr = 0x%x\n", u4Page_addr);
...
void vVDEC_HAL_RM_TriggerDecode(UINT32 u4VDecID, VDEC_INFO_RM_PICINFO_T *prParsingPic)
{
    ...
    if (prParsingPic->ePtype == RM_TRUEBPIC)
    {
        u4RegValue = prParsingPic->fgBwdIsI;
#ifdef VDEC_RM_HAL_ENABLELOG
		pr_debug("[RM] RW_RMMV_CPLOCPICTYPE = %x \n", u4RegValue);
#endif //VDEC_RM_HAL_ENABLELOG
        vVDecWriteRMMV(u4VDecID, RW_RMMV_CPLOCPICTYPE, u4RegValue);
    }
