...
void vVLDDec(UINT32 u4VDecID, VDEC_INFO_DEC_PRM_T *prDecPrm)
{
    ...
    if (prMp2->ucPicStruct != FRM_PIC)
    {
		...
		pr_debug("<vdec> skip vVDecWriteVLD(u4VDecID, 0x9c, 0)\n");
    }
...
void vVLDDoDec(UINT32 u4VDecID, VDEC_INFO_DEC_PRM_T *prDecPrm, BOOL fgBPic, BOOL fgB21FrmPic)
{
    ...
    if (prMpegDecPrm->rMpegPpInfo.fgPpEnable)
    {
        ...
        //u4Proc |= VLD_PERRCON;
		/*pr_debug("MPV keep error concealment on when PP enable\n");*/
    ...
    {
		/*pr_debug("MPV Dump register before decode start\n");*/
...
void vVLDM4vDecPrmProc(UINT32 u4VDecID, VDEC_INFO_DEC_PRM_T *prDecPrm)
{
    ...
    u4RegVal1 |= (0x1 << 16);//| (0x1 << 9); // bit 9 is DIVX mode flag
...
void vVLDMp4TriggerDec(UINT32 u4VDecID)
{
    ...
    //for error concealment(mt6582&mt6593), it needs to set VLD187[8]=0
#ifdef MPEG4_6589_ERROR_CONCEAL
    UINT32 vld187;
    vld187 = u4VDecReadVLD(u4VDecID, RW_VLD_RESET_COMMOM_SRAM) & (~(1 << 8));
    vVDecWriteVLD(u4VDecID, RW_VLD_RESET_COMMOM_SRAM, vld187);
#endif
    ...
    else
    {
...
#endif
        //default setting:
#if MPEG4_VLD_WRAPPER_PREFETCH_MODE == 0
        //vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_PRED_SRAM_CFG, (u4VDecReadVLDTOP(u4VDecID, RW_VLD_TOP_PRED_SRAM_CFG) & (~(0x1<<30))));//vld_top_11[30]=0
        vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_PRED_SRAM_CFG, (u4VDecReadVLDTOP(u4VDecID, RW_VLD_TOP_PRED_SRAM_CFG) & (~(0x1 << 30))));
        vVDecWriteMC(u4VDecID, RW_MC_PREFETCH, u4VDecReadMC(u4VDecID, RW_MC_PREFETCH) & (~(0x1 << 28))); //mc_651[28]=0
        vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_PRED_MODE, (u4VDecReadVLDTOP(u4VDecID, RW_VLD_TOP_PRED_MODE) & (~(0x3 << 4)))); //vld_top_16[5:4]=2d'0
        vVDecWriteVLD(u4VDecID, RW_VLD_MP4_DATA_PARTITION, (u4VDecReadVLD(u4VDecID, RW_VLD_MP4_DATA_PARTITION) & (~(0x1 << 2))) | (0x1 << 7)); //vld_180[2]=0

        //mode1: vld wrapper = bypass, vld prefetch = bypass, read burst = on.
#elif MPEG4_VLD_WRAPPER_PREFETCH_MODE == 1
		pr_debug("MPEG4_VLD_WRAPPER_PREFETCH_MODE == 1\n");
        vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_PRED_SRAM_CFG, (u4VDecReadVLDTOP(u4VDecID, RW_VLD_TOP_PRED_SRAM_CFG) | (0x1 << 30))); //vld_top_11[30]=1
        vVDecWriteMC(u4VDecID, RW_MC_PREFETCH, u4VDecReadMC(u4VDecID, RW_MC_PREFETCH) | (1 << 28)); //mc_651[28]=1
        vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_PRED_MODE, (u4VDecReadVLDTOP(u4VDecID, RW_VLD_TOP_PRED_MODE) & (~(0x1 << 4))) | (0x1 << 5)); //vld_top_16[5:4]=2d'2(0"10"000)
        vVDecWriteVLD(u4VDecID, RW_VLD_MP4_DATA_PARTITION, (u4VDecReadVLD(u4VDecID, RW_VLD_MP4_DATA_PARTITION) | (0x1 << 2))); //vld_180[2]=1
        vVDecWriteVLD(u4VDecID, RW_VLD_MP4_DATA_PARTITION, (u4VDecReadVLD(u4VDecID, RW_VLD_MP4_DATA_PARTITION) | (0x1 << 7)));
        //mode2: vld wrapper = no bypass, vld prefetch = bypass, read burst = off.
#elif MPEG4_VLD_WRAPPER_PREFETCH_MODE == 2
		pr_debug("MPEG4_VLD_WRAPPER_PREFETCH_MODE == 2\n");
        vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_PRED_SRAM_CFG, (u4VDecReadVLDTOP(u4VDecID, RW_VLD_TOP_PRED_SRAM_CFG) & (~(0x1 << 30)))); //vld_top_11[30]=0
        vVDecWriteMC(u4VDecID, RW_MC_PREFETCH, u4VDecReadMC(u4VDecID, RW_MC_PREFETCH) | (1 << 28)); //mc_651[28]=1
        vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_PRED_MODE, (u4VDecReadVLDTOP(u4VDecID, RW_VLD_TOP_PRED_MODE) & (~(0x3 << 4)))); //vld_top_16[5:4]=2d'0
        vVDecWriteVLD(u4VDecID, RW_VLD_MP4_DATA_PARTITION, (u4VDecReadVLD(u4VDecID, RW_VLD_MP4_DATA_PARTITION) & (~(0x1 << 2)))); //vld_180[2]=0
        vVDecWriteVLD(u4VDecID, RW_VLD_MP4_DATA_PARTITION, (u4VDecReadVLD(u4VDecID, RW_VLD_MP4_DATA_PARTITION) | (0x1 << 7)));
#endif
        vVDecWriteVLD(u4VDecID, RW_VLD_MP4_DCAC_NOT_CODED_SZ, (u4VDecReadVLD(u4VDecID, RW_VLD_MP4_DCAC_NOT_CODED_SZ) & (~(0xFFFF << 16))) | ((6 * ((_tVerMpvDecPrm[u4VDecID].u4PicW + 15) >> 4)) << 16));
        ...
        vVDecWriteMC(u4VDecID, RW_MC_PREFETCH, u4VDecReadMC(u4VDecID, RW_MC_PREFETCH) | (0x1 << 28));                             //mc_651[28]=1
        //vVDecWriteVLD(u4VDecID, RW_VLD_MP4_DATA_PARTITION, (u4VDecReadVLD(u4VDecID, RW_VLD_MP4_DATA_PARTITION) & (~(0x1<<7))));
        //vVDecWriteVLD(u4VDecID, RW_VLD_MP4_DCAC_NOT_CODED_LOOP_SZ, (_tVerMpvDecPrm[u4VDecID].SpecDecPrm.rVDecMPEGDecPrm.u4DecW >> 4) | (((_tVerMpvDecPrm[u4VDecID].SpecDecPrm.rVDecMPEGDecPrm.u4DecW >> 4) * 5) << 16));
...
#if MPEG4_6582_SUPPORT  // if sorenson263, remove this option
    // Verification only, not for driver [
    vVDecWriteVLD(u4VDecID, RW_VLD_MP4_ERR_MASK, u4VDecReadVLD(u4VDecID, RW_VLD_MP4_ERR_MASK) & (~(0x7 << 11))); //not use,VLD_184
    ...
    */
    ...
    // llatency buf bypass off
    vVDecWriteDV(u4VDecID, VDEC_DV_LAT_BUF_BYPASS, u4VDecReadDV(u4VDecID, VDEC_DV_LAT_BUF_BYPASS) & 0x0);
#endif
                       ...
                       UCHAR ucDataPartitioned)
{
    ...
    else if (_u4ResyncMarkerCnt[u4VDecID] == 2)
    {
        u4ResyncMarkMB |= ((_u4ResyncMarkMbx[u4VDecID][0]) << 16);
        u4ResyncMarkMB |= ((_u4ResyncMarkMby[u4VDecID][0]));
    ...
    //MC127 for sk_502b_gmc_qpel.m4v in mpeg4 mode
...
void vVDecMpegCommSetting(UINT32 u4VDecID, VDEC_INFO_DEC_PRM_T *prDecPrm)
{
...
#if VMMU_SUPPORT
    {
		...
		pr_debug("[MPEG] vVDecMpegCommSetting, 1, Page Addr = 0x%x\n", u4Page_addr);
		...
		pr_debug("[MPEG] vVDecMpegCommSetting, 2, Page Addr = 0x%x\n", u4Page_addr);
		...
		pr_debug("[MPEG] vVDecMpegCommSetting, 3, Page Addr = 0x%x\n", u4Page_addr);
		...
		pr_debug("[MPEG] vVDecMpegCommSetting, 4, Page Addr = 0x%x\n", u4Page_addr);
		...
		pr_debug("[MPEG] vVDecMpegCommSetting, 5, Page Addr = 0x%x\n", u4Page_addr);
		...
		pr_debug("[MPEG] vVDecMpegCommSetting, 6, Page Addr = 0x%x\n", u4Page_addr);
	...
	/*pr_debug( "[MPEG4] Enable Field Compact Mode\n");*/
    ...
    else
    {
		...
		pr_debug("//<vdec> here ucDecFBufIdx %d, w %d, h %d\n", prDecPrm->ucDecFBufIdx, prDecPrm->u4PicW, prDecPrm->u4PicH);
        //here to disbale always PP mode
#if ((VDEC_DDR3_SUPPORT && (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8560))||VDEC_UFO_ENABLE) // DDR3+PPOUT setting mtk40343
        //#if 1 //Bypass mode
		pr_debug("//<vdec> here ucDecFBufIdx %d, w %d, h %d\n", prDecPrm->ucDecFBufIdx, prDecPrm->u4PicW, prDecPrm->u4PicH);

        if ((prDecPrm->u4PicW > 16) && (prDecPrm->u4PicH > 16)) //when width or height is less than 16,should be set MC out @mtk40343
        {
			pr_debug("<vdec> PP enable %s, %d!\n", __FUNCTION__, __LINE__);
            vVDecWriteMC(u4VDecID, RW_MC_PP_ENABLE, 1);
...
#else
		pr_debug("[Louis] gg RW_MC_PP_UN-ENABLE %d\n", __LINE__);
        vVDecWriteMC(u4VDecID, RW_MC_PP_ENABLE, 0);
#endif
    }

#ifdef VDEC_SARNOFF_ON//Qing Li add here for testing sarnoff
	pr_debug("VDEC_SARNOFF_ON\n");
