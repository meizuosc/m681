...
void vVDecWriteVP8VLD(UINT32 u4VDecID, UINT32 u4Addr, UINT32 u4Val, UINT32 u4BSID)
{
    ...
    if (u4VDecID == 0)
    {
		...
		/*pr_debug("VP8Write(VP8VLD(0x%X) + 0x%X[%d])=0x%X\n",
		VP8_VLD_REG_OFFSET0,
		u4Addr,
		u4Addr>>2,
		u4Val);*/
        ...
        if (_fgVP8DumpReg)
        {
			pr_debug("RISCWrite(`VP8_VLD_BASE + %d*4, 32'h%x);\n", u4Addr >> 2, u4Val);
        }
#endif
#ifdef VDEC_SIM_DUMP
...
void vVDecWriteVP8MC(UINT32 u4VDecID, UINT32 u4Addr, UINT32 u4Val)
{
    ...
    if (u4VDecID == 0)
    {
        ...
        if (_fgVP8DumpReg)
        {
			pr_debug("RISCWrite('MC_REG_OFFSET0 + %d*4, 32'h%x);\n", u4Addr >> 2, u4Val);
        }
#endif
#ifdef VDEC_SIM_DUMP
...
void vVDecWriteVP8MV(UINT32 u4VDecID, UINT32 u4Addr, UINT32 u4Val)
{
    ...
    if (u4VDecID == 0)
    {
        ...
        if (_fgVP8DumpReg)
        {
			pr_debug("RISCWrite(VP8MV(0x%X) + 0x%X[%d])=0x%X\n",
			VP8_MV_REG_OFFSET0,
			u4Addr,
			u4Addr >> 2,
			u4Val);
...
void vVDecWriteVP8VLD2(UINT32 u4VDecID, UINT32 u4Addr, UINT32 u4Val)
{
    ...
    if (u4VDecID == 0)
    {
        ...
        if (_fgVP8DumpReg)
        {
			pr_debug("VP8Write(VLD2(0x%X) + 0x%X[%d])=0x%X\n",
			VP8_VLD2_REG_OFFSET0,
			u4Addr,
			u4Addr >> 2,
			u4Val);
        }
#endif
#ifdef VDEC_REG_DUMP
		pr_debug("[VDEC]RISCWrite('VP8_VLD2_REG_OFFSET0 + 4*%d, 32'h%x);\n", u4Addr >> 2, u4Val);
#endif
...
#ifdef VDEC_REG_DUMP
		pr_debug("[VDEC]RISCRead('VP8_VLD2_REG_OFFSET0 + 4*%d, 32'h%x);\n", u4Addr >> 2, u4RegVal);
#endif
        return u4RegVal;
    }
    else
...
#endif
		/*pr_debug("[VDEC]0x	  %x (AVS PP Register %d) =  0x%x\n", u4Addr, u4Addr/4, u4Val );*/
    }
    else
    {
    ...
    if (u1VP8VLD == VP8_VLD1)
    {
        if ((u4VDecReadVLD(u4VDecID, RO_VLD_FETCHOK) & VLD_FETCH_OK) == 0)
        {
            ...
            while ((u4VDecReadVLD(u4VDecID, RO_VLD_FETCHOK) & VLD_FETCH_OK) == 0)
            {
                ...
                if (u4Cnt >= 0x1000)
                {
					pr_debug("VP8VLD 1 WaitSramStable fail\n");
                    return (FALSE);
                }
            }
        }
...

    {
        if ((u4VDecReadVP8VLD2(u4VDecID, RO_VLD_FETCHOK) & VLD_FETCH_OK) == 0)
        {
            ...
            while ((u4VDecReadVP8VLD2(u4VDecID, RO_VLD_FETCHOK) & VLD_FETCH_OK) == 0)
            {
                ...
                if (u4Cnt >= 0x1000)
                {
					pr_debug("VP8VLD 2 WaitSramStable fail\n");
                    return (FALSE);
                }
            }
        }
...
void vVDec_VP8_HwAccCoefProbUpdate(UINT32 u4VDecID)
{
    ...
    {
		pr_debug("Get NewCoefProb counter=%d\n", u4WaitCount);
...
void vVDEC_VP8_HwAccMVProbUpdate(UINT32 u4VDecID)
{
    ...
    {
		pr_debug("Get NewMVProb counter=%d\n", u4WaitCount);
...
void vVDecWritevp8PP(UINT32 u4VDecID, UINT32 u4Addr, UINT32 u4Val, UINT32 u4BSID)
{
    ...
    if (u4VDecID == 0)
    {
		...
		pr_debug("VP8Write(PP(0x%X) + 0x%X[%d])=0x%X\n", RM_VDEC_PP_BASE, u4Addr, u4Addr >> 2, u4Val);
#ifdef VDEC_SIM_DUMP
