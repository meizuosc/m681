...
INT32 i4VDEC_HAL_Common_Gcon_Enable(void)
{
	...
	pr_debug("VDEC GCON REG 0 is 0x%x\n", u4ReadGconReg(0));
	pr_debug("VDEC GCON REG 2 is 0x%x\n", u4ReadGconReg(8));
...
BOOL fgBreakVDec(UINT32 u4InstID)
{
	...
	/*pr_debug("u4Cnt = 0x%x, cid[0] = 0x%x, cid[1] = 0x%x, cid[2] = 0x%x\n", u4Cnt, cid[0], cid[1], cid[2]);*/
	...
	/*pr_debug("u4Mbx = 0x%x, u4Mby = 0x%x\n", u4Mbx, u4Mby);*/

    if (u4Cnt == 0x5000)
    {
		pr_debug("[2] fgBreakVDec Fail\n");
        return FALSE;
    }
	/*pr_debug("Break done, Count = 0x%x\n", u4Cnt);*/
	pr_debug("fgBreakVDec Pass\n");
    return TRUE;
}
#endif
