...
BOOL fgH264VLDInitBarrelShifter(UINT32 u4BSID, UINT32 u4VDecID, UINT32 u4FIFOSa, UINT32 u4VLDRdPtr, UINT32 u4VLDWrPtr)
{
	...
	pr_debug("@@@@ 0 >>> u4TgtByteAddr %d, u4ByteAddr %x\n", u4TgtByteAddr, u4ByteAddr);
#if 1
    i = 0;
    while (u4TgtByteAddr)
    {
...
#endif
        {
            if ((i > 2) || (u4TgtByteAddr > 1))
            {
                u4TgtByteAddr --;
                i++;
				pr_debug("@@@@ 1 >>> u4TgtByteAddr %d, i %d\n", u4TgtByteAddr, i);
            }
        }
        u4ByteAddr ++;
        i++;
        u4VDecAVCVLDGetBitS(u4BSID, u4VDecID, 8);
		pr_debug("@@@@ 2 >>> u4TgtByteAddr %d, i %d\n", u4TgtByteAddr, i);
    }
#else  //for 6589
#if 1
    while (u4TgtByteAddr)
    {
        ...
        {
            u4TgtByteAddr --;
			/*pr_debug("@@@@ 1 >>> u4TgtByteAddr %d\n", u4TgtByteAddr);*/
        }
    }
    u4ByteAddr ++;
    u4VDecAVCVLDGetBitS(u4BSID, u4VDecID, 8);

	/*pr_debug("@@@@ 2 >>> u4TgtByteAddr %d\n", u4TgtByteAddr);*/
}
#endif
#endif
...
BOOL fgInitH264BarrelShift1(UINT32 u4VDecID, VDEC_INFO_H264_BS_INIT_PRM_T *prH264BSInitPrm)
{
    ...
    vVDecSetVLDVFIFO(0, u4VDecID, PHYSICAL((ULONG)prH264BSInitPrm->u4VFifoSa), PHYSICAL((ULONG) prH264BSInitPrm->u4VFifoEa));
#if CONFIG_DRV_VIRTUAL_ADDR
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8580)
    vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_PRED_ADDR, (prH264BSInitPrm->u4PredSa ? PHYSICAL((ULONG) prH264BSInitPrm->u4PredSa) : 0));
#else
    vVDecWriteAVCVLD(u4VDecID, RW_AVLD_PRED_ADDR, (prH264BSInitPrm->u4PredSa ? PHYSICAL((ULONG) prH264BSInitPrm->u4PredSa) : 0));
#endif
#else
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8580)
    vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_PRED_ADDR, u4AbsDramANc((ULONG) prH264BSInitPrm->u4PredSa));
#else
...
BOOL fgInitH264BarrelShift2(UINT32 u4VDecID, VDEC_INFO_H264_BS_INIT_PRM_T *prH264BSInitPrm)
{
    ...
    vVDecSetVLDVFIFO(1, u4VDecID, PHYSICAL(prH264BSInitPrm->u4VFifoSa), PHYSICAL((ULONG) prH264BSInitPrm->u4VFifoEa));
...
#ifdef VDEC_SIM_DUMP
	pr_debug("<vdec> ReadH264VldRPtr, dRptr:0x%08X, sra:0x%08X, swa:0x%08X, scnt:0x%08X, sum:0x%08X\n",
...
#ifdef VDEC_SIM_DUMP
	/*pr_debug("<vdec> ReadH264VldRPtr, RdPtr=0x%08X (%u) @(%s, %d)\n", u4ByteAddr, u4ByteAddr);*/
#endif
