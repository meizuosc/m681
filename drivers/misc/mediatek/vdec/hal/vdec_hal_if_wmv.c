...
#else
		pr_debug("[VDEC]WMV: Wrong VDecDecID\n");
        //return 0;
#endif
    }
}
...
INT32 i4VDEC_HAL_WMV_InitVDecHW(UINT32 u4VDecID, VDEC_INFO_WMV_VFIFO_PRM_T *prWmvVFifoInitPrm)
{
    ...
    vVDecSetVLDVFIFO(0, u4VDecID, (UINT32)PHYSICAL((ULONG) prWmvVFifoInitPrm->u4VFifoSa), (UINT32)PHYSICAL((ULONG) prWmvVFifoInitPrm->u4VFifoEa));
...
UINT32 u4VDEC_HAL_WMV_GetRealBitStream(UINT32 u4BSID, UINT32 u4VDecID, UINT32 u4ShiftBits)
{
...
#if WMV_LOG_TMP
	pr_debug("GetBS: sh:%d, shfit0 val:0x%x, return:0x%x\n", u4ShiftBits, dRegVal0, u4RetVal);
#endif

#if 0 //(WMV_8320_SUPPORT)
    if (_dRegValRight == dRegVal0)
    {
		pr_debug("WMV_GetRealBitStream: 1st right!!\n");
    }
    else if (_dRegValWrong == dRegVal0)
    {
		...
		pr_debug("WMV_GetRealBitStream: addr:0x%x, 1st wrong!!\n", u4Addr);
    }
#endif
...
INT32 i4VDEC_HAL_WMV_InitBarrelShifter(UINT32 u4BSID, UINT32 u4VDecID, VDEC_INFO_WMV_BS_INIT_PRM_T *prWmvBSInitPrm, BOOL fgIsVC1)
{
...
#ifdef VDEC_SIM_DUMP
	pr_debug("if(`VDEC_PROCESS_FLAG == 1) wait(`VDEC_BITS_PROC_NOP == 1);\n");
#endif
    ...
    vVDecSetVLDVFIFO(u4BSID, u4VDecID, (UINT32)PHYSICAL((ULONG) prWmvBSInitPrm->u4VFifoSa), (UINT32)PHYSICAL((ULONG) prWmvBSInitPrm->u4VFifoEa));

    u4VLDRemainByte = ((prWmvBSInitPrm->u4ReadPointer)) & 0xf;


    vVDecWriteVLD(u4VDecID, RW_VLD_RPTR + (u4BSID << 10), (UINT32)PHYSICAL((ULONG) prWmvBSInitPrm->u4ReadPointer));
    vVDecWriteVLD(u4VDecID, WO_VLD_WPTR + (u4BSID << 10), (UINT32)PHYSICAL((ULONG) prWmvBSInitPrm->u4WritePointer));
...
#if WMV_LOG_TMP
		pr_debug("InitBarrelShifter, Reg 0x1D0:0x%x\n", u4VldVal);
#endif
...
#else
        vVDecWriteVLD(u4VDecID, WO_VLD_WPTR + (u4BSID << 10), (((UINT32)PHYSICAL((ULONG) prWmvBSInitPrm->u4WritePointer) << 4) | 0x2));
#endif
#if (WMV_8320_SUPPORT)
        {
            UINT32 u4VldVal = u4VDecReadVLD(u4VDecID, RW_VLD_LDSH);
    ...
    if (fgIsVC1)
    {
        u4VLDRemainByte -= u4VDec03Number(((ULONG)(prWmvBSInitPrm->u4ReadPointer)) & (0xFFFFFFFFFFFFFFF0), u4VLDRemainByte);
    }
    ...
    for (j = 0; j < u4VLDRemainByte; j++)
    {
        ...
        u4VLDByte = u4VDecReadVldRPtr(u4BSID, u4VDecID, &u4VLDBit, (UINT32)PHYSICAL((ULONG) prWmvBSInitPrm->u4VFifoSa));
...
#if WMV_LOG_TMP
	pr_debug("i4VDEC_HAL_WMV_InitBarrelShifter, rd:0x%x, input window:0x%x, 0x%x\n",
...
// **************************************************************************
UINT32 u4VDEC_HAL_WMV_ReadRdPtr(UINT32 u4BSID, UINT32 u4VDecID, ULONG u4VFIFOSa, UINT32 *pu4Bits)
{
    return u4VDecReadVldRPtr(u4BSID, u4VDecID, pu4Bits, (UINT32)PHYSICAL((ULONG) u4VFIFOSa));
}
...
// Return      : None
// **************************************************************************
void vVDEC_HAL_WMV_AlignRdPtr(UINT32 u4BSID, UINT32 u4VDecID, ULONG u4VFIFOSa, UINT32 u4AlignType)
{
    INT32 i;
    UINT32 u4VLDByte, u4VLDbits;

    u4VLDByte = u4VDecReadVldRPtr(u4BSID, u4VDecID, &u4VLDbits, (UINT32)PHYSICAL(u4VFIFOSa));
    if (u4VLDbits != 0)
    {
...
{
	pr_debug("i4VDEC_HAL_WMV_HWDecBP start\n");
    INT32 i, a = 0;
    INT32 vop_type;
#if 0
    ...
    vVDecWriteVLD(u4VDecID, RW_VLD_DCACSA, u4AbsDramANc((ULONG) prWmvDecBpPrm->rWmvWorkBufSa.u4DcacSa) >> 2); //192 DCAC
    vVDecWriteVLD(u4VDecID, RW_VLD_MVSA, ((u4AbsDramANc((ULONG) prWmvDecBpPrm->rWmvWorkBufSa.u4Mv1Sa) >> 2) & 0x3FFFFF)); //194 Mv_1#1(frame/top field)
    vVDecWriteVLD(u4VDecID, RW_VLD_ADDREXTEND,
                  (u4VDecReadVLD(u4VDecID, RW_VLD_ADDREXTEND) & 0xFFFFFFC0) | (((u4AbsDramANc((ULONG) prWmvDecBpPrm->rWmvWorkBufSa.u4Mv1Sa) >> 2) >> 22) & 0x3F)); //194 Mv_1#1(frame/top field)
    //MPEG-4 use    WriteREG(RW_VLD_BMB1, 0x2d124f80); //196 set once
    //MPEG-4 use    WriteREG(RW_VLD_BMB2, 0x2d13d620); //197 set once
    a198 = ((u4AbsDramANc((ULONG) prWmvDecBpPrm->rWmvWorkBufSa.u4Bp1Sa) >> 2) & 0xFFFFFF) +
           (((u4AbsDramANc((ULONG) prWmvDecBpPrm->rWmvWorkBufSa.u4Bp1Sa) >> 2) & 0xF000000) << 4) +
    ...
    vVDecWriteVLD(u4VDecID, RW_VLD_MV3_ADDR, u4AbsDramANc((ULONG) prWmvDecBpPrm->rWmvWorkBufSa.u4Mv3Sa) >> 2); //214 Mv_3 for new RTL
    vVDecWriteVLD(u4VDecID, RW_VLD_BP2_ADDR, u4AbsDramANc((ULONG) prWmvDecBpPrm->rWmvWorkBufSa.u4Bp2Sa) >> 2); //203 Bp_2
    vVDecWriteVLD(u4VDecID, RW_VLD_BP3_ADDR, u4AbsDramANc((ULONG) prWmvDecBpPrm->rWmvWorkBufSa.u4Bp3Sa) >> 2); //204 Bp_3
    vVDecWriteVLD(u4VDecID, RW_VLD_BP4_ADDR, u4AbsDramANc((ULONG) prWmvDecBpPrm->rWmvWorkBufSa.u4Bp4Sa) >> 2); //205 Bp_4
    vVDecWriteVLD(u4VDecID, RW_VLD_MV2_ADDR, u4AbsDramANc((ULONG) prWmvDecBpPrm->rWmvWorkBufSa.u4Mv2Sa) >> 2); //211 Mv_2
    vVDecWriteVLD(u4VDecID, RW_VLD_DCAC2_ADDR, u4AbsDramANc((ULONG) prWmvDecBpPrm->rWmvWorkBufSa.u4Dcac2Sa) >> 2); //213 Dcac2
    //MC post process
    vVDecWriteMC(u4VDecID, RW_MC_PP_DBLK_Y_ADDR, u4AbsDramANc((ULONG) prWmvDecBpPrm->rWmvWorkBufSa.u4Pp1Sa) >> 4);
    vVDecWriteMC(u4VDecID, RW_MC_PP_DBLK_C_ADDR, u4AbsDramANc((ULONG) prWmvDecBpPrm->rWmvWorkBufSa.u4Pp2Sa) >> 4);
    // ~initialize prediction
...
#if WMV_LOG_TMP
		pr_debug("HWDecBP, Reg 0x1D0:0x%x\n", u4VldVal);
#endif
	...
	pr_debug("i4VDEC_HAL_WMV_HWDecBP end\n");
    return HAL_HANDLE_OK;
}
...
{
	pr_debug("i4VDEC_HAL_WMV_DecStart start\n");
...
#if VMMU_SUPPORT
    {
		...
		pr_debug("[WMV] i4VDEC_HAL_WMV_DecStart, 1, Page Addr = 0x%x\n", u4Page_addr);
        vPage_Table(u4VDecID, u4Page_addr, (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa), (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa) + PIC_Y_SZ);
		...
		pr_debug("[WMV] i4VDEC_HAL_WMV_DecStart, 2, Page Addr = 0x%x\n", u4Page_addr);
        vPage_Table(u4VDecID, u4Page_addr, (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa), (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa) + PIC_C_SZ);
		...
		pr_debug("[WMV] i4VDEC_HAL_WMV_DecStart, 3, Page Addr = 0x%x\n", u4Page_addr);
        vPage_Table(u4VDecID, u4Page_addr, (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa), (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa) + PIC_Y_SZ);
		...
		pr_debug("[WMV] i4VDEC_HAL_WMV_DecStart, 4, Page Addr = 0x%x\n", u4Page_addr);
        vPage_Table(u4VDecID, u4Page_addr, (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic1CSa), (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic1CSa) + PIC_C_SZ);
		...
		pr_debug("[WMV] i4VDEC_HAL_WMV_DecStart, 5, Page Addr = 0x%x\n", u4Page_addr);
        vPage_Table(u4VDecID, u4Page_addr, (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa), (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa) + PIC_Y_SZ);
		...
		pr_debug("[WMV] i4VDEC_HAL_WMV_DecStart, 6, Page Addr = 0x%x\n", u4Page_addr);
        vPage_Table(u4VDecID, u4Page_addr, (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic2CSa), (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic2CSa) + PIC_C_SZ);
...
#else
    vVDecWriteMC(u4VDecID, RW_MC_R1Y, (UINT32)(u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa) >> prWmvDecPrm->i4MemBase) >> 9); // div 512
    vVDecWriteMC(u4VDecID, RW_MC_R1C, (UINT32)(u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa) >> prWmvDecPrm->i4MemBase) >> 8); // div 256
    vVDecWriteMC(u4VDecID, RW_MC_R2Y, (UINT32)(u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa) >> prWmvDecPrm->i4MemBase) >> 9); // div 512
    vVDecWriteMC(u4VDecID, RW_MC_R2C, (UINT32)(u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic1CSa) >> prWmvDecPrm->i4MemBase) >> 8); // div 256
    vVDecWriteMC(u4VDecID, RW_MC_BY, (UINT32)(u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa) >> prWmvDecPrm->i4MemBase) >> 8);  // div 256
    vVDecWriteMC(u4VDecID, RW_MC_BC, (UINT32)(u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic2CSa) >> prWmvDecPrm->i4MemBase) >> 7);  // div 128
    vVDecWriteMC(u4VDecID, RW_MC_BY1, (UINT32)(u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa) >> prWmvDecPrm->i4MemBase) >> 9);  // div 256
    vVDecWriteMC(u4VDecID, RW_MC_BC1, (UINT32)(u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic2CSa) >> prWmvDecPrm->i4MemBase) >> 8);  // div 128
#endif
    ...
    // initialize prediction
    vVDecWriteVLD(u4VDecID, RW_VLD_DCACSA, (UINT32)(u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4DcacSa) >> 2)); //192 DCAC
    vVDecWriteVLD(u4VDecID, RW_VLD_MVSA, (UINT32)((u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Mv1Sa) >> 2) & 0x3FFFFF)); //194 Mv_1#1(frame/top field)
    vVDecWriteVLD(u4VDecID, RW_VLD_ADDREXTEND, (u4VDecReadVLD(u4VDecID, RW_VLD_ADDREXTEND) & 0xFFFFFFC0) |
                  (((u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Mv1Sa) >> 2) >> 22) & 0x3F)); //194 Mv_1#1(frame/top field)
    //MPEG-4 use    WriteREG(RW_VLD_BMB1, 0x2d124f80); //196 set once
    //MPEG-4 use    WriteREG(RW_VLD_BMB2, 0x2d13d620); //197 set once
    a198 = ((u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Bp1Sa) >> 2) & 0xFFFFFF) +
           (((u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Bp1Sa) >> 2) & 0xF000000) << 4) +
           (pred_use_wdle << 25) +  //wdle
           (1 << 26);
    ...
    vVDecWriteVLD(u4VDecID, RW_VLD_MBDRAM_SEL, a201); //201 set once
    vVDecWriteVLD(u4VDecID, RW_VLD_MV3_ADDR, u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Mv3Sa) >> 2); //214 Mv_3 for new RTL
    vVDecWriteVLD(u4VDecID, RW_VLD_BP2_ADDR, u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Bp2Sa) >> 2); //203 Bp_2
    vVDecWriteVLD(u4VDecID, RW_VLD_BP3_ADDR, u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Bp3Sa) >> 2); //204 Bp_3
    vVDecWriteVLD(u4VDecID, RW_VLD_BP4_ADDR, u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Bp4Sa) >> 2); //205 Bp_4
    vVDecWriteVLD(u4VDecID, RW_VLD_MV2_ADDR, u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Mv2Sa) >> 2); //211 Mv_2
    vVDecWriteVLD(u4VDecID, RW_VLD_DCAC2_ADDR, u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Dcac2Sa) >> 2); //213 Dcac2
    //WriteREG(RW_VLD_MV2_ADDR, 0x00011170); //211 Mv_2
    //WriteREG(RW_VLD_MVSA, 0x00013880); //194 Mv_1#2(bottom field)
    //MC post process
    vVDecWriteMC(u4VDecID, RW_MC_PP_DBLK_Y_ADDR, u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pp1Sa) >> 4);
    vVDecWriteMC(u4VDecID, RW_MC_PP_DBLK_C_ADDR, u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pp2Sa) >> 4);
    ...
    {
        UINT32 u4YStartAddr;
        UINT32 u4PIC_W_H = ((((prDecPrm->u4PicW + 15) >> 4) - 1) << 16) | (((prDecPrm->u4PicH + 15) >> 4) - 1);
        ...
        if ((prWmvDecPrm->prPPS->ucPicType == BVOP) || (prWmvDecPrm->prPPS->ucPicType == BIVOP)) //prog_B
        {
            ...
            u4YStartAddr = (UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa;
        }
        else if (prDecPrm->ucDecFBufIdx == 0) //pic1
        {
            ...
            u4YStartAddr = (UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa;
        }
        else //pic2
        {
            ...
            u4YStartAddr = (UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa;
        }

		pr_debug("WMV_DecStart u4PIC_W_H 0x%x\n", u4PIC_W_H);
		pr_debug("WMV_DecStart u4YStartAddr 0x%x\n", u4YStartAddr);
		pr_debug("WMV_DecStart prDecPrm->u4PIC_SIZE_BS	0x%x\n", prDecPrm->u4PIC_SIZE_BS);
		pr_debug("WMV_DecStart prDecPrm->u4UFO_LEN_SIZE_Y 0x%x\n", prDecPrm->u4UFO_LEN_SIZE_Y);
		pr_debug("WMV_DecStart prDecPrm->u4PIC_SIZE_Y_BS  0x%x\n", prDecPrm->u4PIC_SIZE_Y_BS);

		pr_debug("WMV_DecStart PHYSICAL((UINT32) u4YStartAddr) + prDecPrm->u4PIC_SIZE_BS  0x%x\n",
		PHYSICAL((UINT32) u4YStartAddr) + prDecPrm->u4PIC_SIZE_BS);


        vVDecWriteMC(u4VDecID, 700 * 4, u4PIC_W_H);
        //stephen wong test
        vVDecWriteMC(u4VDecID, 664 * 4, 0x11);
        //stephen wong test end
        // for pp out (UFO ENC)
        vVDecWriteMC(u4VDecID, 698 * 4, PHYSICAL((UINT32) u4YStartAddr) + prDecPrm->u4PIC_SIZE_BS); //set start of Y LEN data
        vVDecWriteMC(u4VDecID, 699 * 4, PHYSICAL((UINT32) u4YStartAddr) + prDecPrm->u4PIC_SIZE_BS + prDecPrm->u4UFO_LEN_SIZE_Y); //set start of C LEN data
        // for MC (UFO DEC)
        vVDecWriteMC(u4VDecID, 663 * 4, prDecPrm->u4PIC_SIZE_BS);
        vVDecWriteMC(u4VDecID, 701 * 4, prDecPrm->u4UFO_LEN_SIZE_Y);
        vVDecWriteMC(u4VDecID, 343 * 4, prDecPrm->u4PIC_SIZE_Y_BS);
        memset(u4YStartAddr, 0, u4DramPicArea);
    }
    vVDecWriteMC(u4VDecID, RW_MC_PP_WB_BY_POST, 0x00000001);
	pr_debug("WMV_DecStart PP ENABLE1\n");
	pr_debug("WMV_DecStart PP ENSABLE1\n");
	pr_debug("WMV_DecStart PP ENSABLE1\n");
#else
    ...
    if ((prWmvDecPrm->prPPS->ucPicType == BVOP) || (prWmvDecPrm->prPPS->ucPicType == BIVOP)) //prog_B
    {
#if VMMU_SUPPORT
        {
            ...
            u4Page_addr = (UINT32)_pucVMMUTable[u4VDecID] + (((prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa) / (4 * 1024)) * 4);
			pr_debug("[WMV] i4VDEC_HAL_WMV_DecStart, 7, Page Addr = 0x%x\n", u4Page_addr);
            vPage_Table(u4VDecID, u4Page_addr, (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa), (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa) + PIC_Y_SZ);
            vVDecWriteMC(u4VDecID, RW_MC_PP_Y_ADDR, ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa) >> prWmvDecPrm->i4MemBase >> 9);

            u4Page_addr = (UINT32)_pucVMMUTable[u4VDecID] + (((prWmvDecPrm->rWmvWorkBufSa.u4Pic2CSa) / (4 * 1024)) * 4);
			pr_debug("[WMV] i4VDEC_HAL_WMV_DecStart, 8, Page Addr = 0x%x\n", u4Page_addr);
            vPage_Table(u4VDecID, u4Page_addr, (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic2CSa), (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic2CSa) + PIC_C_SZ);
            vVDecWriteMC(u4VDecID, RW_MC_PP_C_ADDR, ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic2CSa) >> prWmvDecPrm->i4MemBase >> 8);
        }
#else
        vVDecWriteMC(u4VDecID, RW_MC_PP_Y_ADDR, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa) >> prWmvDecPrm->i4MemBase) >> 9);
        vVDecWriteMC(u4VDecID, RW_MC_PP_C_ADDR, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic2CSa) >> prWmvDecPrm->i4MemBase) >> 8);
#endif
    }
    else if (prDecPrm->ucDecFBufIdx == 0) //pic1
    {
#if VMMU_SUPPORT
        {
            ...
            u4Page_addr = (UINT32)_pucVMMUTable[u4VDecID] + (((prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa) / (4 * 1024)) * 4);
			pr_debug("[WMV] i4VDEC_HAL_WMV_DecStart, 9, Page Addr = 0x%x\n", u4Page_addr);
            vPage_Table(u4VDecID, u4Page_addr, (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa), (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa) + PIC_Y_SZ);
            vVDecWriteMC(u4VDecID, RW_MC_PP_Y_ADDR, ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa) >> prWmvDecPrm->i4MemBase >> 9);

            u4Page_addr = (UINT32)_pucVMMUTable[u4VDecID] + (((prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa) / (4 * 1024)) * 4);
			pr_debug("[WMV] i4VDEC_HAL_WMV_DecStart, 10, Page Addr = 0x%x\n", u4Page_addr);
            vPage_Table(u4VDecID, u4Page_addr, (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa), (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa) + PIC_C_SZ);
            vVDecWriteMC(u4VDecID, RW_MC_PP_C_ADDR, ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa) >> prWmvDecPrm->i4MemBase >> 8);
        }
#else
        vVDecWriteMC(u4VDecID, RW_MC_PP_Y_ADDR, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa) >> prWmvDecPrm->i4MemBase) >> 9);
        vVDecWriteMC(u4VDecID, RW_MC_PP_C_ADDR, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa) >> prWmvDecPrm->i4MemBase) >> 8);
#endif
    }
    else //pic2
    {
#if VMMU_SUPPORT
        {
            ...
            u4Page_addr = (UINT32)_pucVMMUTable[u4VDecID] + (((prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa) / (4 * 1024)) * 4);
			pr_debug("[WMV] i4VDEC_HAL_WMV_DecStart, 11, Page Addr = 0x%x\n", u4Page_addr);
            vPage_Table(u4VDecID, u4Page_addr, (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa), (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa) + PIC_Y_SZ);
            vVDecWriteMC(u4VDecID, RW_MC_PP_Y_ADDR, ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa) >> prWmvDecPrm->i4MemBase >> 9);

            u4Page_addr = (UINT32)_pucVMMUTable[u4VDecID] + (((prWmvDecPrm->rWmvWorkBufSa.u4Pic1CSa) / (4 * 1024)) * 4);
			pr_debug("[WMV] i4VDEC_HAL_WMV_DecStart, 12, Page Addr = 0x%x\n", u4Page_addr);
            vPage_Table(u4VDecID, u4Page_addr, (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic1CSa), (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic1CSa) + PIC_C_SZ);
            vVDecWriteMC(u4VDecID, RW_MC_PP_C_ADDR, ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic1CSa) >> prWmvDecPrm->i4MemBase >> 8);
        }
#else
        vVDecWriteMC(u4VDecID, RW_MC_PP_Y_ADDR, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa) >> prWmvDecPrm->i4MemBase) >> 9);
        vVDecWriteMC(u4VDecID, RW_MC_PP_C_ADDR, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic1CSa) >> prWmvDecPrm->i4MemBase) >> 8);
#endif
    }
    ...
    else // >= VDEC_WMV3
    {
		...
		/*pr_debug("use_rule_b = %d\n",use_rule_b);*/
		/*pr_debug("i4Overlap = %d,ucPicType = 0x%x,i4WMV3Profile = %d\n",*/
    ...
    //-----------------------------------------------------
    {
        ...
        else // >= VDEC_WMV3
        {
...
#if VMMU_SUPPORT
            {
                ...
                u4Page_addr = (UINT32)_pucVMMUTable[u4VDecID] + (((prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa) / (4 * 1024)) * 4);
				pr_debug("[WMV] i4VDEC_HAL_WMV_DecStart, 13, Page Addr = 0x%x\n", u4Page_addr);
                vPage_Table(u4VDecID, u4Page_addr, (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa), (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa) + PIC_Y_SZ);
                vVDecWriteMC(u4VDecID, RW_MC_R1Y, ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa) >> prWmvDecPrm->i4MemBase >> 9);

                u4Page_addr = (UINT32)_pucVMMUTable[u4VDecID] + (((prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa) / (4 * 1024)) * 4);
				pr_debug("[WMV] i4VDEC_HAL_WMV_DecStart, 14, Page Addr = 0x%x\n", u4Page_addr);
                vPage_Table(u4VDecID, u4Page_addr, (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa), (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa) + PIC_C_SZ);
                vVDecWriteMC(u4VDecID, RW_MC_R1C, ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa) >> prWmvDecPrm->i4MemBase >> 8);

                u4Page_addr = (UINT32)_pucVMMUTable[u4VDecID] + (((prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa) / (4 * 1024)) * 4);
				pr_debug("[WMV] i4VDEC_HAL_WMV_DecStart, 15, Page Addr = 0x%x\n", u4Page_addr);
                vPage_Table(u4VDecID, u4Page_addr, (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa), (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa) + PIC_Y_SZ);
                vVDecWriteMC(u4VDecID, RW_MC_R2Y, ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa) >> prWmvDecPrm->i4MemBase >> 9);

                u4Page_addr = (UINT32)_pucVMMUTable[u4VDecID] + (((prWmvDecPrm->rWmvWorkBufSa.u4Pic1CSa) / (4 * 1024)) * 4);
				pr_debug("[WMV] i4VDEC_HAL_WMV_DecStart, 16, Page Addr = 0x%x\n", u4Page_addr);
                vPage_Table(u4VDecID, u4Page_addr, (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic1CSa), (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic1CSa) + PIC_C_SZ);
                vVDecWriteMC(u4VDecID, RW_MC_R2C, ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic1CSa) >> prWmvDecPrm->i4MemBase >> 8);

                u4Page_addr = (UINT32)_pucVMMUTable[u4VDecID] + (((prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa) / (4 * 1024)) * 4);
				pr_debug("[WMV] i4VDEC_HAL_WMV_DecStart, 17, Page Addr = 0x%x\n", u4Page_addr);
                vPage_Table(u4VDecID, u4Page_addr, (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa), (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa) + PIC_Y_SZ);
                vVDecWriteMC(u4VDecID, RW_MC_BY, ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa) >> prWmvDecPrm->i4MemBase >> 8);
                vVDecWriteMC(u4VDecID, RW_MC_BY1, ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa) >> prWmvDecPrm->i4MemBase >> 9);

                u4Page_addr = (UINT32)_pucVMMUTable[u4VDecID] + (((prWmvDecPrm->rWmvWorkBufSa.u4Pic2CSa) / (4 * 1024)) * 4);
				pr_debug("[WMV] i4VDEC_HAL_WMV_DecStart, 18, Page Addr = 0x%x\n", u4Page_addr);
                vPage_Table(u4VDecID, u4Page_addr, (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic2CSa), (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic2CSa) + PIC_C_SZ);
                vVDecWriteMC(u4VDecID, RW_MC_BC, ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic2CSa) >> prWmvDecPrm->i4MemBase >> 7);
                vVDecWriteMC(u4VDecID, RW_MC_BC1, ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic2CSa) >> prWmvDecPrm->i4MemBase >> 8);
            }
#else
            vVDecWriteMC(u4VDecID, RW_MC_R1Y, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa) >> prWmvDecPrm->i4MemBase) >> 9); // div 512
            vVDecWriteMC(u4VDecID, RW_MC_R1C, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa) >> prWmvDecPrm->i4MemBase) >> 8); // div 256
            vVDecWriteMC(u4VDecID, RW_MC_R2Y, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa) >> prWmvDecPrm->i4MemBase) >> 9); // div 512
            vVDecWriteMC(u4VDecID, RW_MC_R2C, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic1CSa) >> prWmvDecPrm->i4MemBase) >> 8); // div 256
            vVDecWriteMC(u4VDecID, RW_MC_BY, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa) >> prWmvDecPrm->i4MemBase) >> 8);  // div 256
            vVDecWriteMC(u4VDecID, RW_MC_BC, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic2CSa) >> prWmvDecPrm->i4MemBase) >> 7);  // div 128
            vVDecWriteMC(u4VDecID, RW_MC_BY1, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa) >> prWmvDecPrm->i4MemBase) >> 9);  // div 256
            vVDecWriteMC(u4VDecID, RW_MC_BC1, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic2CSa) >> prWmvDecPrm->i4MemBase) >> 8);  // div 128
#endif
            ...
            if ((prWmvDecPrm->prICOMPS->ucFrameTypeLast == SKIPFRAME) && (prWmvDecPrm->prPPS->ucPicType == BVOP))
            {
                if (my_forward == 1)
                {
#if VMMU_SUPPORT
                    {
                        ...
                        u4Page_addr = (UINT32)_pucVMMUTable[u4VDecID] + (((prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa) / (4 * 1024)) * 4);
						pr_debug("[WMV] i4VDEC_HAL_WMV_DecStart, 19, Page Addr = 0x%x\n",
						u4Page_addr);
                        vPage_Table(u4VDecID, u4Page_addr, (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa), (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa) + PIC_Y_SZ);
                        vVDecWriteMC(u4VDecID, RW_MC_R1Y, ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa) >> prWmvDecPrm->i4MemBase >> 9);
                        ...
                        u4Page_addr = (UINT32)_pucVMMUTable[u4VDecID] + (((prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa) / (4 * 1024)) * 4);
						pr_debug("[WMV] i4VDEC_HAL_WMV_DecStart, 20, Page Addr = 0x%x\n",
						u4Page_addr);
                        vPage_Table(u4VDecID, u4Page_addr, (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa), (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa) + PIC_C_SZ);
                        vVDecWriteMC(u4VDecID, RW_MC_R1C, ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa) >> prWmvDecPrm->i4MemBase >> 8);
                    ...
                    vVDecWriteMC(u4VDecID, RW_MC_R1Y, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa) >> prWmvDecPrm->i4MemBase) >> 9); // div 512
                    vVDecWriteMC(u4VDecID, RW_MC_R1C, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa) >> prWmvDecPrm->i4MemBase) >> 8); // div 256
                    vVDecWriteMC(u4VDecID, RW_MC_R2Y, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa) >> prWmvDecPrm->i4MemBase) >> 9); // div 512
                    vVDecWriteMC(u4VDecID, RW_MC_R2C, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa) >> prWmvDecPrm->i4MemBase) >> 8); // div 256
#endif
                }
                else
                {
#if VMMU_SUPPORT
                    {
                        ...
                        u4Page_addr = (UINT32)_pucVMMUTable[u4VDecID] + (((prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa) / (4 * 1024)) * 4);
						pr_debug("[WMV] i4VDEC_HAL_WMV_DecStart, 21, Page Addr = 0x%x\n",
						u4Page_addr);
                        vPage_Table(u4VDecID, u4Page_addr, (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa), (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa) + PIC_Y_SZ);
                        ...
                        u4Page_addr = (UINT32)_pucVMMUTable[u4VDecID] + (((prWmvDecPrm->rWmvWorkBufSa.u4Pic1CSa) / (4 * 1024)) * 4);
						pr_debug("[WMV] i4VDEC_HAL_WMV_DecStart, 22, Page Addr = 0x%x\n",
						u4Page_addr);
                        vPage_Table(u4VDecID, u4Page_addr, (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic1CSa), (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic1CSa) + PIC_C_SZ);
...
#else
                    vVDecWriteMC(u4VDecID, RW_MC_R1Y, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa) >> prWmvDecPrm->i4MemBase) >> 9); // div 512
                    vVDecWriteMC(u4VDecID, RW_MC_R1C, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic1CSa) >> prWmvDecPrm->i4MemBase) >> 8); // div 256
                    vVDecWriteMC(u4VDecID, RW_MC_R2Y, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa) >> prWmvDecPrm->i4MemBase) >> 9); // div 512
                    vVDecWriteMC(u4VDecID, RW_MC_R2C, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic1CSa) >> prWmvDecPrm->i4MemBase) >> 8); // div 256
#endif
                }
...
#if VMMU_SUPPORT
		pr_debug("[WMV] i4VDEC_HAL_WMV_DecStart, VMMUTable:[0x%x, 0x%x]\n",
		((UINT32)_pucVMMUTable[u4VDecID]),
		(UINT32)PHYSICAL((ULONG)_pucVMMUTable[u4VDecID]));

        vVDecVMMUEnable((UINT32)PHYSICAL((ULONG)_pucVMMUTable[u4VDecID]));
#endif
    ...
    //-----------------------------------------------------
    {
        ...
        {
            a194 = ((u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Mv12Sa) >> 2) & 0x3FFFFF) + //dram_addr_base
                   ((prWmvDecPrm->prSPS->u4NumMBX * 4) << 22); //Mv_1_2
            a194_1 = (((u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Mv12Sa) >> 2) >> 22) & 0x3F);
        }
        else
        {
            a194 = ((u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Mv1Sa) >> 2) & 0x3FFFFF) + //dram_addr_base
                   ((prWmvDecPrm->prSPS->u4NumMBX * 4) << 22); //Mv_1
            a194_1 = (((u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Mv1Sa) >> 2) >> 22) & 0x3F);
        }
	...
	pr_debug("pic_width = %d  pic_height = %d\n",
	((prWmvDecPrm->prSPS->u4NumMBX) * 16),
	((prWmvDecPrm->prSPS->u4NumMBY) * 16)); /*sun for temp*/
#endif
#else
    a36 = (0 << 31) + //MPEG1_flag
          ((height + 15) << 16) + //vertical_size
    ...
    if (prWmvDecPrm->fgWmvMode)
    {
...
#if (CONFIG_DRV_FPGA_BOARD)
        aAvcMv131 = (UINT32)PHYSICAL(aAvcMAddr) >> 4;
#else
		...
		pr_debug("i4Ratio0 = %d, i4Ratio1 = %d\n", i4Ratio0, i4Ratio1);
		...
		/*pr_debug("AVCMV_131 = %d\n", aAvcMv131);*/
		/*pr_debug("AVCMV_134 = %d\n", aAvcMv134);*/
		/*pr_debug("AVCMV_135 = %d\n", aAvcMv135);*/
		/*pr_debug("AVCMV_137 = %d\n", aAvcMv137);*/
...
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8550)
#if VDEC_FIELD_COMPACT
	pr_debug("[WMV] Enable Field Compact Mode\n");
...
#if WMV_LOG_TMP
	pr_debug("WMV_DecStart, Input window :0x%x, 0x%x\n", u4VDecVLDGetBitS(0, 0, 0), u4VDecReadVLD(u4VDecID, 0xf0));
#endif
...
#if WMV_LOG_TMP
		pr_debug("i4VDEC_HAL_WMV_DecStart, Reg 0x1D0:0x%x\n",
               u4VldVal);
#endif
	...
	pr_debug("i4VDEC_HAL_WMV_DecStart end\n");
    return HAL_HANDLE_OK;
}
...
INT32 i4VDEC_HAL_WMV_VPStart(UINT32 u4VDecID, VDEC_INFO_DEC_PRM_T *prDecPrm)
{
...
#if VMMU_SUPPORT
    {
        ...
        u4Page_addr = (UINT32)_pucVMMUTable[u4VDecID] + (((prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa) / (4 * 1024)) * 4);
		pr_debug("[WMV] i4VDEC_HAL_WMV_VPStart, 1, Page Addr = 0x%x\n", u4Page_addr);
        vPage_Table(u4VDecID, u4Page_addr, (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa), (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa) + PIC_Y_SZ);
        vVDecWriteMC(u4VDecID, RW_MC_R1Y, ((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa) >> 9);
        vVDecWriteMC(u4VDecID, RW_MC_DIGY, ((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa) >> 9);

        u4Page_addr = (UINT32)_pucVMMUTable[u4VDecID] + (((prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa) / (4 * 1024)) * 4);
		pr_debug("[WMV] i4VDEC_HAL_WMV_VPStart, 2, Page Addr = 0x%x\n", u4Page_addr);
        vPage_Table(u4VDecID, u4Page_addr, (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa), (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa) + PIC_C_SZ);
        vVDecWriteMC(u4VDecID, RW_MC_R1C, (UINT32)((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa >> 8));
        vVDecWriteMC(u4VDecID, RW_MC_DIGC, (UINT32)((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa >> 8));

        u4Page_addr = (UINT32)_pucVMMUTable[u4VDecID] + (((prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa) / (4 * 1024)) * 4);
		pr_debug("[WMV] i4VDEC_HAL_WMV_VPStart, 3, Page Addr = 0x%x\n", u4Page_addr);
        vPage_Table(u4VDecID, u4Page_addr, (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa), (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa) + PIC_Y_SZ);
        vVDecWriteMC(u4VDecID, RW_MC_R2Y, ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa) >> 9);

        u4Page_addr = (UINT32)_pucVMMUTable[u4VDecID] + (((prWmvDecPrm->rWmvWorkBufSa.u4Pic1CSa) / (4 * 1024)) * 4);
		pr_debug("[WMV] i4VDEC_HAL_WMV_VPStart, 4, Page Addr = 0x%x\n", u4Page_addr);
        vPage_Table(u4VDecID, u4Page_addr, (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic1CSa), (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic1CSa) + PIC_C_SZ);
        vVDecWriteMC(u4VDecID, RW_MC_R2C, ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic1CSa) >> 8);

        u4Page_addr = (UINT32)_pucVMMUTable[u4VDecID] + (((prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa) / (4 * 1024)) * 4);
		pr_debug("[WMV] i4VDEC_HAL_WMV_VPStart, 5, Page Addr = 0x%x\n", u4Page_addr);
        vPage_Table(u4VDecID, u4Page_addr, (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa), (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa) + PIC_Y_SZ);
        ...
        u4Page_addr = (UINT32)_pucVMMUTable[u4VDecID] + (((prWmvDecPrm->rWmvWorkBufSa.u4Pic2CSa) / (4 * 1024)) * 4);
		pr_debug("[WMV] i4VDEC_HAL_WMV_VPStart, 6, Page Addr = 0x%x\n", u4Page_addr);
        vPage_Table(u4VDecID, u4Page_addr, (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic2CSa), (UINT32)PHYSICAL((ULONG)prWmvDecPrm->rWmvWorkBufSa.u4Pic2CSa) + PIC_C_SZ);
...
#else
    vVDecWriteMC(u4VDecID, RW_MC_R1Y, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa)) >> 9); // div 512
    vVDecWriteMC(u4VDecID, RW_MC_R1C, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa)) >> 8); // div 256
    vVDecWriteMC(u4VDecID, RW_MC_R2Y, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa)) >> 9); // div 512
    vVDecWriteMC(u4VDecID, RW_MC_R2C, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic1CSa)) >> 8); // div 256
    vVDecWriteMC(u4VDecID, RW_MC_BY, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa)) >> 8);  // div 256
    vVDecWriteMC(u4VDecID, RW_MC_BC, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic2CSa)) >> 7);  // div 128
    vVDecWriteMC(u4VDecID, RW_MC_BY1, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa)) >> 9);  // div 256
    vVDecWriteMC(u4VDecID, RW_MC_BC1, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic2CSa)) >> 8);  // div 128
    vVDecWriteMC(u4VDecID, RW_MC_DIGY, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa)) >> 9); // div 512
    vVDecWriteMC(u4VDecID, RW_MC_DIGC, (u4AbsDramANc((ULONG) prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa)) >> 8); // div 256
#endif
...
#if WMV_LOG_TMP
		pr_debug("WMV_DecEndProcess, Reg 0x1D0:0x%x\n",
               u4VldVal);
#endif
...
void vVDEC_HAL_WMV_VDec_DumpReg(UINT32 u4VDecID, BOOL fgBefore)
{
	...
	pr_debug("WMV Dump Register: ");
    if (fgBefore == TRUE)
    {
		pr_debug("Before Decode\n");
    }
    else
    {
		pr_debug("After Decode\n");
    }

	pr_debug("VLD Registers\n");
    ...
    for (i = i4VldStart; i < i4VldEnd; i++)
    {
		...
		pr_debug("%04d (0x%04X) = 0x%08X\n", i, (i << 2), u4Data);
        msleep(10);
    }

	pr_debug("MC Registers\n");
...
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8555)
	pr_debug("Y CRC Registers\n");
    for (i = 0; i < 4; i++)
    {
        u4Data = u4ReadReg(VDEC_CRC_REG_OFFSET0 + ((2 + i) * 4));
		pr_debug("%04d (0x%04X) = 0x%08X\n", (2 + i), ((2 + i) << 2), u4Data);
    }
	pr_debug("C CRC Registers\n");
    for (i = 0; i < 4; i++)
    {
