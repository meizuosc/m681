...
// Cheng-Jung 20131020 prevent kernel stack overflow - make local variable global
...
void vVDEC_HAL_VP6_InitCtx(UINT32 u4VDecID, UINT32 u4BSID, VDEC_INFO_VP6_FRM_HDR_T *prFrmHdr)
{
    ...
    if (prFrmHdr->fgUseHuffman)
    {
        ...
        // DC Coeff & Run Coeff
		pr_debug("init DC Huff Tree and Zero Run Huff Tree\n");
        for (j = 0; j < 4; j++)
        {
            for (i = 0; i < 4; i++)
            {
        ...
        // EOB Run
		pr_debug("init EOB Run Huff Tree\n");
...
#ifdef VDEC_SIM_DUMP
	pr_debug("<vdec> VP6_VLD_SHIFT_BIT(%d);\n", u4ShiftBits);
#endif
...
INT32 i4VDEC_HAL_VP6_InitBarrelShifter2(UINT32 u4BSID, UINT32 u4VDecID, VDEC_INFO_VP6_BS_INIT_PRM_T *prVp6BSInitPrm, VDEC_INFO_VP6_FRM_HDR_T *prVDecVp6FrmHdr)
{
    ...
    if (u4VDecVP6ReadVLD2(RO_VLD_SRAMCTRL) & PROCESS_FLAG)
    {
        while ((u4VDecVP6ReadVLD2(RO_VLD_SRAMCTRL) & AA_FIT_TARGET_SCLK) == 0)
        {
            ...
            if (u4Cnt >= WAIT_THRD)
            {
				pr_debug("<vdec> [VP6] BARRELSHIFTER 2 INITIL FAIL\n");
                break;
            }
        }
    }
    ...
    {
		pr_debug("<vdec> [VP6] BARRELSHIFTER 2 FETCH FAIL\n");
        return FALSE;
    }
...
INT32 i4VDEC_HAL_VP6_DecStart(UINT32 u4VDecID, VDEC_INFO_DEC_PRM_T *prDecPrm)
{
...
#if VMMU_SUPPORT
    {
		...
		pr_debug("[VP6] i4VDEC_HAL_VP6_DecStart, 1, Page Addr = 0x%x\n", u4Page_addr);
		...
		pr_debug("[VP6] i4VDEC_HAL_VP6_DecStart, 2, Page Addr = 0x%x\n", u4Page_addr);
		...
		pr_debug("[VP6] i4VDEC_HAL_VP6_DecStart, 3, Page Addr = 0x%x\n", u4Page_addr);
		...
		pr_debug("[VP6] i4VDEC_HAL_VP6_DecStart, 4, Page Addr = 0x%x\n", u4Page_addr);
		...
		pr_debug("[VP6] i4VDEC_HAL_VP6_DecStart, 5, Page Addr = 0x%x\n", u4Page_addr);
		...
		pr_debug("[VP6] i4VDEC_HAL_VP6_DecStart, 6, Page Addr = 0x%x\n", u4Page_addr);
...
UINT32 u4VDEC_HAL_VP6_GetBoolCoderShift(UINT32 u4BSID, UINT32 u4VDecID, UINT32 u4ShiftBits)
{
	...
	pr_debug("<vdec> VP6_VLD_READ_BOOL(%d)\n", u4ShiftBits);
