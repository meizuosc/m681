...
//Yi Feng modify @20141011
extern LONG  VA_VDEC_BASE;
extern LONG  VA_VDEC_GCON_BASE;
...
void vWriteReg(UINT32 dAddr, UINT32 dVal)
{
    mt_reg_sync_writel(dVal, (ULONG)(VA_VDEC_BASE + dAddr)); //*(volatile UINT32 *)(IO_BASE + dAddr) = dVal
}

UINT32 u4ReadReg(UINT32 dAddr)
{
    return (UINT32)(*(volatile UINT32 *)(VA_VDEC_BASE + dAddr));
}

void vWriteGconReg(UINT32 dAddr, UINT32 dVal)
{
    mt_reg_sync_writel(dVal, (ULONG)(VA_VDEC_GCON_BASE + dAddr)); //*(volatile UINT32 *)(IO_BASE + dAddr) = dVal
}

UINT32 u4ReadGconReg(UINT32 dAddr)
{
    return (UINT32)(*(volatile UINT32 *)(VA_VDEC_GCON_BASE + dAddr));
}
...
UINT32 u4VDecReadVLD(UINT32 u4VDecID, UINT32 u4Addr)
{
    ...
    else
    {
		...
		pr_debug("PIP_ONE_HW: Wrong HW ID!!!\n");
        VDEC_ASSERT(0);
#endif
...
#endif
{
    ...
    UINT32 u4VDecPDNCtrlModule2 = 0x1f;
    ...
    u4Cnt = 50000;
#ifdef VDEC_SIM_DUMP
	pr_debug("if(`VDEC_PROCESS_FLAG == 1) wait(`VDEC_BITS_PROC_NOP == 1);  //HWReset WaitSramStable\n");
    ...
    switch (u4VDecType)
    {
        ...
        case VDEC_MPEG2:
            ...
            //        w/   DBK  7F5A151D
#if VDEC_PP_ENABLE
            u4VDecPDNCtrlModule = 0x7F5A151D;
#else
            u4VDecPDNCtrlModule = 0x7F5A15FD;
#endif
    ...
    if (u4VDecType != VDEC_H265)
    {
        ...
        vVDecWriteDV(u4VDecID, 0x178, u4VDecPDNCtrlModule2);  // MISC(94)
        // Turn off auto-power-off after IRQ
        vVDecWriteDV(u4VDecID, 0xf4, 0);  // MISC(61)
        vWriteGconReg(6 * 4, 1);
...
#endif
	/*pr_debug("sysclk = 0x%x,pwd = 0x%x,HWID = %d,codec = 0x%x\n",
	u4VDecReadDV(u4VDecID,
	RW_SYS_CLK_SEL),
	u4VDecReadDV(u4VDecID,
	RW_PDN_CTRL)&0x3fffff,
	u4VDecID,u4VDecType);*/
    ...
    u4Val = u4VDecReadMISC(u4VDecID, 0x228);
    if (((u4Val >> 16) & 0x1))
    {
		pr_debug("[INFO] Fake engine test! [frame %d]\n", _u4PicCnt[u4VDecID]);
        ...
        u4Val = u4VDecReadMISC(u4VDecID, 0x210);
		pr_debug("[INFO] MISC_132 u4Val 0x%x\n", u4Val);
        u4Val = u4VDecReadMISC(u4VDecID, 0x214);
		pr_debug("[INFO] MISC_133 u4Val 0x%x\n", u4Val);
        u4Val = u4VDecReadMISC(u4VDecID, 0x218);
		pr_debug("[INFO] MISC_134 u4Val 0x%x!\n", u4Val);
    }
    else
    {
		pr_debug("[INFO] Fake engine still activated!! [frame %d]\n", _u4PicCnt[u4VDecID]);
    }
#endif
...
#ifdef VDEC_SIM_DUMP
	pr_debug("<vdec> ReadVldRPtr, dRptr:0x%08X, sra:0x%08X, swa:0x%08X, scnt:0x%08X, sum:0x%08X, u4BsBufLen=%u\n",
...
#ifdef VDEC_SIM_DUMP
	pr_debug("wait(`VDEC_INI_FETCH_RDY == 1);\n");
#endif
...
UINT32 u4VDec03Number(ULONG u4Addr, UINT32 u4Range)
{
    INT32 i;
    UINT32 u403Cnt = 0;
    UINT32 u4ZeroCnt = 0;
...
{
    UINT32 VDEC_PHY_BASE = 0x16020000;
    if (szFunction)
    {
        msleep(1);
        if (DUMP_FOR_SIM)
        {
...
#endif
            {
                ...
                {
                    case DV_REG_OFFSET0:
						pr_debug("		RISCWrite_MISC(%d, 32'h%x);\n",
						u4Addr >> 2,
						u4Val);
                        break;
                    case VLD_REG_OFFSET0:
						pr_debug("		RISCWrite_VLD(%d, 32'h%x);\n",
						u4Addr >> 2,
						u4Val);
                        break;
                    case VLD_TOP_REG_OFFSET0:
						pr_debug("		RISCWrite_VLD_TOP(%d, 32'h%x);\n",
						u4Addr >> 2,
						u4Val);
                        break;
                    case MC_REG_OFFSET0:
						pr_debug("		RISCWrite_MC(%d, 32'h%x);\n",
						u4Addr >> 2,
						u4Val);
                        break;
                    case AVC_MV_REG_OFFSET0:
						pr_debug("		RISCWrite_MV(%d, 32'h%x);\n",
						u4Addr >> 2,
						u4Val);
                        break;
                    case RM_VDEC_PP_BASE:
						pr_debug("		RISCWrite_PP(%d, 32'h%x);\n",
						u4Addr >> 2,
						u4Val);
                        break;
                    case HEVC_VLD_REG_OFFSET0:
						pr_debug("		RISCWrite_HEVC_VLD(%d, 32'h%x);\n",
						u4Addr >> 2,
						u4Val);
                        break;
                    default:
						pr_debug("		RISCWrite_Others( %d, 32'h%x ); /* base 0x%x */\n",
						u4Addr >> 2,
						u4Val,
						u4BaseReg);
                        break;
            ...
            else
            {
                ...
                {
                    case DV_REG_OFFSET0:
                        _u4RegisterLogLen[u4VDecID] += sprintf(_pucRegisterLog[u4VDecID] + _u4RegisterLogLen[u4VDecID], "        RISCWrite_MISC(%d, 32'h%x);\n", u4Addr >> 2, u4Val);
                        break;
                    case VLD_REG_OFFSET0:
                    ...
                    case RM_VDEC_PP_BASE:
                        ...
                        break;
                    case HEVC_VLD_REG_OFFSET0:
                        _u4RegisterLogLen[u4VDecID] += sprintf(_pucRegisterLog[u4VDecID] + _u4RegisterLogLen[u4VDecID], "        RISCWrite_HEVC_VLD(%d, 32'h%x);\n", u4Addr >> 2, u4Val);
                        break;
                    default:
        ...
        {
			/*pr_debug("%s(u4VDecID, 0x%08X, 0x%08X); /* 0x%08X */\n",
			szFunction,
			u4Addr,
			u4Val,
			u4Val,
			u4BaseReg);*/
			pr_debug("%s(u4VDecID, 0x%x, 0x%x);\n", szFunction, u4Addr, u4Val);
        }
    }
    else
    {
    ...
    if (szFunction)
    {
        if (DUMP_FOR_SIM)
        {
...
#endif
            {
                ...
                {
                    case DV_REG_OFFSET0:
						pr_debug("		RISCRead_MISC(%d); /* return 0x%x */\n",
						u4Addr >> 2,
						u4Val);
                        break;
                    case VLD_REG_OFFSET0:
						pr_debug("		RISCRead_VLD(%d); /* return 0x%x */\n",
						u4Addr >> 2,
						u4Val);
                        break;
                    case VLD_TOP_REG_OFFSET0:
						pr_debug("		RISCRead_VLD_TOP(%d); /* return 0x%x */\n",
						u4Addr >> 2,
						u4Val);
                        break;
                    case MC_REG_OFFSET0:
                        ...
                        break;
                    case AVC_MV_REG_OFFSET0:
						pr_debug("		RISCRead_MV(%d); /* return 0x%x */\n",
						u4Addr >> 2,
						u4Val);
                        break;
                    case RM_VDEC_PP_BASE:
                        ...
                        break;
                    case HEVC_VLD_REG_OFFSET0:
						pr_debug("		RISCRead_HEVC_VLD(%d); /* return 0x%x */\n",
						u4Addr >> 2,
						u4Val);
                        break;
                    default:
						pr_debug("		RISCRead_Others( %d);  /* return 0x%x */ /* base 0x%x */\n",
						u4Addr >> 2,
						u4Val,
						u4BaseReg);
                        break;
            ...
            else
            {
                ...
                {
                    case DV_REG_OFFSET0:
                        _u4RegisterLogLen[u4VDecID] += sprintf(_pucRegisterLog[u4VDecID] + _u4RegisterLogLen[u4VDecID], "        RISCRead_MISC(%d); /* base 0x%x, return 0x%x , 0x%x */\n", u4Addr >> 2, u4BaseReg + VDEC_PHY_BASE , u4Val, u4Addr);
                        break;
                    case VLD_REG_OFFSET0:
                        _u4RegisterLogLen[u4VDecID] += sprintf(_pucRegisterLog[u4VDecID] + _u4RegisterLogLen[u4VDecID], "        RISCRead_VLD(%d); /* base 0x%x, return 0x%x , 0x%x */\n", u4Addr >> 2, u4BaseReg + VDEC_PHY_BASE , u4Val, u4Addr);
                        break;
                    case VLD_TOP_REG_OFFSET0:
                        _u4RegisterLogLen[u4VDecID] += sprintf(_pucRegisterLog[u4VDecID] + _u4RegisterLogLen[u4VDecID], "        RISCRead_VLD_TOP(%d); /* base 0x%x, return 0x%x , 0x%x */\n", u4Addr >> 2, u4BaseReg + VDEC_PHY_BASE , u4Val, u4Addr);
                        break;
                    case MC_REG_OFFSET0:
                        _u4RegisterLogLen[u4VDecID] += sprintf(_pucRegisterLog[u4VDecID] + _u4RegisterLogLen[u4VDecID], "        RISCRead_MC(%d); /* base 0x%x, return 0x%x , 0x%x */\n", u4Addr >> 2, u4BaseReg + VDEC_PHY_BASE , u4Val, u4Addr);
                        break;
                    case AVC_MV_REG_OFFSET0:
                        _u4RegisterLogLen[u4VDecID] += sprintf(_pucRegisterLog[u4VDecID] + _u4RegisterLogLen[u4VDecID], "        RISCRead_MV(%d); /* base 0x%x, return 0x%x , 0x%08x */\n", u4Addr >> 2, u4BaseReg + VDEC_PHY_BASE , u4Val, u4Addr);
                        break;
                    case RM_VDEC_PP_BASE:
                        _u4RegisterLogLen[u4VDecID] += sprintf(_pucRegisterLog[u4VDecID] + _u4RegisterLogLen[u4VDecID], "        RISCRead_PP(%d); /* base 0x%x, return 0x%x , 0x%x */\n", u4Addr >> 2, u4BaseReg + VDEC_PHY_BASE , u4Val, u4Addr);
                        break;
                    case HEVC_VLD_REG_OFFSET0:
                        _u4RegisterLogLen[u4VDecID] += sprintf(_pucRegisterLog[u4VDecID] + _u4RegisterLogLen[u4VDecID], "        RISCRead_HEVC_VLD(%d); /* base 0x%x, return 0x%x , 0x%x */\n", u4Addr >> 2, u4BaseReg + VDEC_PHY_BASE , u4Val, u4Addr);
                        break;
                    default:
                        _u4RegisterLogLen[u4VDecID] += sprintf(_pucRegisterLog[u4VDecID] + _u4RegisterLogLen[u4VDecID], "        RISCRead_Others( %d);  /* return 0x%08X */ /* base 0x%08X */\n", u4Addr >> 2, u4Val, u4BaseReg + VDEC_PHY_BASE);
                        break;
                }
            }
        ...
        {
			/*pr_debug("%s(u4VDecID, 0x%08X); /* return 0x%08X */ /* 0x%08X */\n", szFunction, u4Addr, u4Val, u4BaseReg);*/
			pr_debug("%s(u4VDecID, 0x%x); /* return 0x%x */\n", szFunction, u4Addr, u4Val);
        }
    }
    else
    {
...
void vVDecSimDump(UINT32 u4VDecID, UINT32 u4OffsetAddr, UINT32 u4Addr, UINT32 u4Val)
{
    ...
    switch (u4OffsetAddr)
    {
        ...
        case VLD_REG_OFFSET1:
			pr_debug("[VDEC]RISCWrite('VDEC_FULL_ADDR_VLD + 4*%d, 32'h%x ); \n", u4Addr >> 2, u4Val);
            break;

#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8580)
        case VLD_TOP_REG_OFFSET0:
        case VLD_TOP_REG_OFFSET1:
			pr_debug("[VDEC]RISCWrite('VDEC_FULL_ADDR_VLD_TOP + 4*%d, 32'h%x );  \n", u4Addr >> 2, u4Val);
            break;
#endif

        case MC_REG_OFFSET0:
        case MC_REG_OFFSET1:
			pr_debug("[VDEC]RISCWrite('VDEC_FULL_ADDR_MC + 4*%d, 32'h%x ); \n", u4Addr >> 2, u4Val);
            break;

#if 0
        case VDEC_CRC_REG_OFFSET0:
			pr_debug("[VDEC]RISCWrite('VDEC_FULL_ADDR_CRC + 4*%d, 32'h%x ); \n", u4Addr >> 2, u4Val);
            break;
#endif

        case DV_REG_OFFSET0:
        case DV_REG_OFFSET1:
			pr_debug("[VDEC]RISCWrite('VDEC_FULL_ADDR_DV + 4*%d, 32'h%x );  \n", u4Addr >> 2, u4Val);
            break;
        ...
        case AVC_VLD_REG_OFFSET1:
            ...
            {
				pr_debug("[VDEC]RISCWrite('VDEC_FULL_ADDR_AVC_VLD + 4*%d, 32'h%x ); \n", u4Addr >> 2, u4Val);
            }
        ...
        case AVC_MV_REG_OFFSET0:
			...
			pr_debug("[VDEC]RISCWrite('VDEC_FULL_ADDR_AVC_MV + 4*%d, 32'h%x ); \n", u4Addr >> 2, u4Val);
            break;

            //       case AVC_FG_REG_OFFSET0:
            //       case AVC_FG_REG_OFFSET1:
			pr_debug("[VDEC]RISCWrite('VDEC_FULL_ADDR_FG + 4*%d, 32'h%x ); \n", u4Addr >> 2, u4Val);
            break;
    }
...
void vVDecSimDumpAvcVldR(UINT32 u4VDecID, UINT32 u4OffsetAddr, UINT32 u4Addr, UINT32 u4Val)
{
    ...
    */
    ...
    {
#if _LOG_FANTASIA_1_
		pr_debug("[H264] fantasia >> Current frame %d dump frame %d \n", _u4FileCnt[u4VDecID], _u4DumpRegPicNum[u4VDecID]);
#endif
        return;
    }
    //msleep(5);
    switch (u4OffsetAddr)
    {
        case VLD_REG_OFFSET0:
        case VLD_REG_OFFSET1:
			/*pr_debug("RISCRead('VDEC_FULL_ADDR_VLD + 4*%d, 32'h%x); \n", u4Addr>>2, u4Val);*/
        ...
        case VLD_TOP_REG_OFFSET0:
        case VLD_TOP_REG_OFFSET1:
			/*pr_debug("RISCRead('VDEC_FULL_ADDR_VLD_TOP + 4*%d, 32'h%x); \n", u4Addr>>2, u4Val);*/
        ...
        case MC_REG_OFFSET0:
        case MC_REG_OFFSET1:
			/*pr_debug("RISCRead('VDEC_FULL_ADDR_MC + 4*%d, 32'h%x); \n", u4Addr>>2, u4Val);*/
            DBG_H264_PRINTF("RISCRead('VDEC_FULL_ADDR_MC + 4*%d, 32'h%x); \n", u4Addr >> 2, u4Val);
            break;

#if 0
        case VDEC_CRC_REG_OFFSET0:
			pr_debug("[VDEC]RISCRead('VDEC_FULL_ADDR_CRC + 4*%d, 32'h%x); \n", u4Addr >> 2, u4Val);
            break;
#endif

        case DV_REG_OFFSET0:
        case DV_REG_OFFSET1:
			/*pr_debug("RISCRead('VDEC_FULL_ADDR_DV + 4*%d, 32'h%x); \n", u4Addr>>2, u4Val);*/
        ...
        case AVC_VLD_REG_OFFSET1:
            if (u4Addr >= RW_AVLD_CTRL)
            {
				/*pr_debug("[VDEC]RISCRead('VDEC_FULL_ADDR_AVC_VLD + 4*%d, 32'h%x); \n", u4Addr>>2, u4Val);*/
				/*pr_debug("RISCRead_AVC_VLD(%d, 32'h%x); \n", u4Addr>>2, u4Val);*/
        ...
        case AVC_MV_REG_OFFSET0:
            //       case AVC_MV_REG_OFFSET1:
			/*pr_debug("RISCRead('VDEC_FULL_ADDR_AVC_MV + 4*%d, 32'h%x); \n", u4Addr>>2, u4Val);*/
            DBG_H264_PRINTF("RISCRead('VDEC_FULL_ADDR_AVC_MV + 4*%d, 32'h%x); \n", u4Addr >> 2, u4Val);
            break;

            //       case AVC_FG_REG_OFFSET0:
            //       case AVC_FG_REG_OFFSET1:
			/*pr_debug("RISCRead('VDEC_FULL_ADDR_FG + 4*%d, 32'h%x); \n", u4Addr>>2, u4Val);*/
