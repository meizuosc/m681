...
INT32 i4VDEC_HAL_VP8_InitVDecHW(UINT32 u4VDecID, VDEC_INFO_VP8_VFIFO_PRM_T *prVp8VFifoInitPrm)
{
    ...
    vVDecSetVLDVFIFO(0, u4VDecID, PHYSICAL((ULONG)prVp8VFifoInitPrm->u4VFifoSa), PHYSICAL((ULONG)prVp8VFifoInitPrm->u4VFifoEa));
    ...
    if (u4VldId == VP8_VLD1)
    {
        if (u4VDecReadVLD(0, RO_VLD_SRAMCTRL) & PROCESS_FLAG)
        {
            while ((u4VDecReadVLD(0, RO_VLD_SRAMCTRL) & AA_FIT_TARGET_SCLK) == 0)
            {
                ...
                if (u4Cnt >= WAIT_THRD)
                {
					pr_debug("VP8 VLD1 HWReset WaitSramStable Fail\n");
                    break;
                }
            }
        }
...
BOOL fgVDEC_Vp8DecReset(UINT32 u4InstID, VDEC_INFO_VP8_FRM_HDR_T *pVp8DecInfo, BOOL fgInit)
{
    ...
    if (!fgInit)
    {
        ...
        {
			pr_debug("VP8 Init VLD1 BarrelShifter fail\n");
            vVDEC_VP8_VldReset(VP8_VLD1);
        }
    ...
    if (!fgInit)
    {
        ...
        vVDecWriteVP8VLD2(0, RW_VLD_VSTART, (UINT32)PHYSICAL((ULONG)(pVp8DecInfo->u4FifoStart)) >> 6);
        vVDecWriteVP8VLD2(0, RW_VLD_VEND, (UINT32)PHYSICAL((ULONG)(pVp8DecInfo->u4FifoEnd)) >> 6);
    }
...
INT32 i4VDEC_HAL_VP8_InitBarrelShifter(UINT32 u1VP8VLD, UINT32 u4VDecID, VDEC_INFO_VP8_FRM_HDR_T *pVp8DecInfo)
{
    ...
    if (u1VP8VLD == VP8_VLD1)
    {
        ...
        vVDecWriteVLD(u4VDecID, RW_VLD_VSTART , (UINT32)PHYSICAL((ULONG)pVp8DecInfo->u4FifoStart) >> 6);
        vVDecWriteVLD(u4VDecID, RW_VLD_VEND, (UINT32)PHYSICAL((ULONG)pVp8DecInfo->u4FifoEnd) >> 6);
    }
    else
    {
        vVDecWriteVP8VLD2(u4VDecID, RW_VLD_RDY_SWTICH, READY_TO_RISC_1);
        vVDecWriteVP8VLD2(u4VDecID, RW_VLD_VSTART , (UINT32)PHYSICAL((ULONG)pVp8DecInfo->u4FifoStart) >> 6);
        vVDecWriteVP8VLD2(u4VDecID, RW_VLD_VEND, (UINT32)PHYSICAL((ULONG)pVp8DecInfo->u4FifoEnd) >> 6);
    }
    ...
    for (i = 0; i < 5; i++)
    {
        if (u1VP8VLD == VP8_VLD1)
        {
            ...
            vVDecWriteVLD(u4VDecID, RW_VLD_RPTR, (UINT32)PHYSICAL((ULONG) pVp8DecInfo->u4VldStartPos));
            vVDecWriteVLD(u4VDecID, RW_VLD_RPTR, (UINT32)PHYSICAL((ULONG) pVp8DecInfo->u4VldStartPos));
            vVDecWriteVLD(u4VDecID, WO_VLD_WPTR, (UINT32)PHYSICAL((ULONG) pVp8DecInfo->u4WritePos));
        ...
        else
        {
            ...
            vVDecWriteVP8VLD2(u4VDecID, RW_VLD_RPTR, (UINT32)PHYSICAL((ULONG)(pVp8DecInfo->u4VldStartPos)));
            vVDecWriteVP8VLD2(u4VDecID, RW_VLD_RPTR, (UINT32)PHYSICAL((ULONG)(pVp8DecInfo->u4VldStartPos)));
            vVDecWriteVP8VLD2(u4VDecID, WO_VLD_WPTR, (UINT32)PHYSICAL((ULONG)(pVp8DecInfo->u4WritePos)));
...
// **************************************************************************
UINT32 u4VDEC_HAL_VP8_ReadRdPtr(UINT32 u4BSID, UINT32 u4VDecID, ULONG u4VFIFOSa, UINT32 *pu4Bits)
{
...
// Return      : None
// **************************************************************************
void vVDEC_HAL_VP8_AlignRdPtr(UINT32 u4BSID, UINT32 u4VDecID, ULONG u4VFIFOSa, UINT32 u4AlignType)
{
    INT32 i;
    UINT32 u4VLDByte, u4VLDbits;

    u4VLDByte = u4VDecReadVldRPtr(u4BSID, u4VDecID, &u4VLDbits, PHYSICAL((ULONG)u4VFIFOSa));
    if (u4VLDbits != 0)
    {
...
UINT32 i4VDEC_HAL_VP8_DecStart(UINT32 u4VDecID, VDEC_INFO_VP8_FRM_HDR_T *pVp8DecInfo)
{
...
#if VDEC_UFO_SUPPORT
	u4PIC_WIDTH[u4VDecID]  = ((pVp8DecInfo->u4Width + 63) >> 6) << 6;
	u4PIC_HEIGHT[u4VDecID] = ((pVp8DecInfo->u4Height + 63) >> 6) << 6;
	u4PIC_SIZE_Y[u4VDecID] = u4PIC_WIDTH[u4VDecID] * u4PIC_HEIGHT[u4VDecID];
	u4PIC_SIZE[u4VDecID]   = ((u4PIC_SIZE_Y[u4VDecID] + (u4PIC_SIZE_Y[u4VDecID] >> 1) + 511) >> 9) << 9;

	u4UFO_LEN_SIZE_Y[u4VDecID] = ((((u4PIC_SIZE_Y[u4VDecID] + 255) >> 8) + 63 + (16*8)) >> 6) << 6;
	u4UFO_LEN_SIZE_C[u4VDecID] = (((u4UFO_LEN_SIZE_Y[u4VDecID] >> 1) + 15 + (16*8)) >> 4) << 4;

	u4PIC_SIZE_Y_BS[u4VDecID] = (((u4PIC_SIZE_Y[u4VDecID] + 4095) >> 12) << 12);
	u4PIC_SIZE_BS[u4VDecID] = ((u4PIC_SIZE_Y_BS[u4VDecID] + (u4PIC_SIZE_Y[u4VDecID] >> 1) + 511) >> 9) << 9;
	u4PIC_SIZE_REF[u4VDecID] = (((u4PIC_SIZE_BS[u4VDecID] + (u4UFO_LEN_SIZE_Y[u4VDecID] << 1)) + 4095) >> 12) << 12;

	_tVerMpvDecPrm[u4VDecID].u4PIC_SIZE_BS = u4PIC_SIZE_BS[u4VDecID];
	_tVerMpvDecPrm[u4VDecID].u4UFO_LEN_SIZE_Y = u4UFO_LEN_SIZE_Y[u4VDecID];
	_tVerMpvDecPrm[u4VDecID].u4PIC_SIZE_Y_BS = u4PIC_SIZE_Y_BS[u4VDecID];

	_tVerMpvDecPrm[u4VDecID].u4PicW = pVp8DecInfo->u4Width;
	_tVerMpvDecPrm[u4VDecID].u4PicH = pVp8DecInfo->u4Height;

	u4DramPicSize = u4PIC_SIZE_Y_BS[u4VDecID];
	u4DramPicArea = (((u4PIC_SIZE_REF[u4VDecID] + (u4PIC_SIZE_Y[u4VDecID] >> 4)) + 4095) >> 12) << 12;

	pr_debug("TRC-i4VDEC_HAL_VP8_DecStart +0, u4PIC_SIZE_Y[%d]=%d\n", u4VDecID, u4PIC_SIZE_Y[u4VDecID]);
	pr_debug("TRC-i4VDEC_HAL_VP8_DecStart +0, u4PIC_SIZE[%d]=%d\n", u4VDecID, u4PIC_SIZE[u4VDecID]);
	pr_debug("TRC-i4VDEC_HAL_VP8_DecStart +0, u4UFO_LEN_SIZE_Y[%d]=%d\n", u4VDecID, u4UFO_LEN_SIZE_Y[u4VDecID]);
	pr_debug("TRC-i4VDEC_HAL_VP8_DecStart +0, u4UFO_LEN_SIZE_C[%d]=%d\n", u4VDecID, u4UFO_LEN_SIZE_C[u4VDecID]);
	pr_debug("TRC-i4VDEC_HAL_VP8_DecStart +0, u4PIC_SIZE_Y_BS[%d]=%d\n", u4VDecID, u4PIC_SIZE_Y_BS[u4VDecID]);
	pr_debug("TRC-i4VDEC_HAL_VP8_DecStart +0, u4PIC_SIZE_BS[%d]=%d\n", u4VDecID, u4PIC_SIZE_BS[u4VDecID]);
	pr_debug("TRC-i4VDEC_HAL_VP8_DecStart +0, u4PIC_SIZE_REF[%d]=%d\n", u4VDecID, u4PIC_SIZE_REF[u4VDecID]);

	pr_debug("TRC-i4VDEC_HAL_VP8_DecStart +0, u4DramPicSize=%d\n", u4DramPicSize);
	pr_debug("TRC-i4VDEC_HAL_VP8_DecStart +0, u4DramPicArea=%d\n", u4DramPicArea);

	UINT32 u4PIC_UFO_WIDTH = pVp8DecInfo->u4Width;
	UINT32 u4PIC_UFO_HEIGHT = pVp8DecInfo->u4Height;
	UINT32 u4PIC_W_H = ((((u4PIC_UFO_WIDTH)/16) - 1) << 16) | (((u4PIC_UFO_HEIGHT)/16) - 1);
	pr_debug("TRC-i4VDEC_HAL_VP8_DecStart +0, u4PIC_UFO_WIDTH=%d\n", u4PIC_UFO_WIDTH);
	pr_debug("TRC-i4VDEC_HAL_VP8_DecStart +0, u4PIC_UFO_HEIGHT=%d\n", u4PIC_UFO_HEIGHT);
	pr_debug("TRC-i4VDEC_HAL_VP8_DecStart +0, u4PIC_W_H=%d\n", u4PIC_W_H);

	vVDecWriteMC(u4VDecID, 700 * 4, u4PIC_W_H);
	vVDecWriteMC(u4VDecID, 664 * 4, 0x11);

	// for pp out (UFO ENC)
	vVDecWriteMC(u4VDecID, 698 * 4, PHYSICAL((UINT32) pVp8DecInfo->u4CurYAddr) + u4PIC_SIZE_BS[u4VDecID]);
	vVDecWriteMC(u4VDecID, 699 * 4, PHYSICAL((UINT32) pVp8DecInfo->u4CurYAddr) + u4PIC_SIZE_BS[u4VDecID] + u4UFO_LEN_SIZE_Y[u4VDecID]);
	// for MC (UFO DEC)
	vVDecWriteMC(u4VDecID, 663 * 4, u4PIC_SIZE_BS[u4VDecID]);
	vVDecWriteMC(u4VDecID, 701 * 4, u4UFO_LEN_SIZE_Y[u4VDecID]);
	vVDecWriteMC(u4VDecID, 343 * 4, u4PIC_SIZE_Y_BS[u4VDecID]);
	
	memset(pVp8DecInfo->u4CurYAddr, 0, u4DramPicArea);

	pVp8DecInfo->u4CurCAddr = pVp8DecInfo->u4CurYAddr + u4PIC_SIZE_Y_BS[u4VDecID];
#endif
	...
	/*pr_debug("Page Addr = 0x%x\n", u4Page_addr);*/
    vPage_Table(u4VDecID, u4Page_addr, PHYSICAL((ULONG)pVp8DecInfo->u4LstYAddr), PHYSICAL((ULONG)pVp8DecInfo->u4LstYAddr) + PIC_Y_SZ + PIC_C_SZ);
	...
	/*pr_debug("Page Addr = 0x%x\n", u4Page_addr);*/
    vPage_Table(u4VDecID, u4Page_addr, PHYSICAL((ULONG)pVp8DecInfo->u4GldYAddr), PHYSICAL((ULONG)pVp8DecInfo->u4GldYAddr) + PIC_Y_SZ + PIC_C_SZ);
	...
	/*pr_debug("Page Addr = 0x%x\n", u4Page_addr);*/
    vPage_Table(u4VDecID, u4Page_addr, PHYSICAL((ULONG)pVp8DecInfo->u4AlfYAddr), PHYSICAL((ULONG)pVp8DecInfo->u4AlfYAddr) + PIC_Y_SZ + PIC_C_SZ);
    vVDecWriteVP8MC(u4VDecID, RW_MC_PIC3Y_ADD, (pVp8DecInfo->u4AlfYAddr));

#else
    vVDecWriteVP8MC(u4VDecID, RW_MC_PIC1Y_ADD, (UINT32)PHYSICAL((ULONG)pVp8DecInfo->u4LstYAddr));
    vVDecWriteVP8MC(u4VDecID, RW_MC_PIC2Y_ADD, (UINT32)PHYSICAL((ULONG)pVp8DecInfo->u4GldYAddr));
    vVDecWriteVP8MC(u4VDecID, RW_MC_PIC3Y_ADD, (UINT32)PHYSICAL((ULONG)pVp8DecInfo->u4AlfYAddr));
#endif
    vVDecWriteVP8MC(u4VDecID, RW_VLD_MBDRAM_SEL, u4VDecReadVP8MC(u4VDecID, RW_VLD_MBDRAM_SEL) | (1 << 16));
	/*pr_debug("LoopFilter = %d\n", prVp8DecParam->iLoopFilterLvl);*/
    ...
    //#endif
    {
		...
		/*pr_debug("Page Addr = 0x%x\n", u4Page_addr);*/
        vPage_Table(u4VDecID, u4Page_addr, PHYSICAL((ULONG)pVp8DecInfo->u4CurYAddr), PHYSICAL((ULONG)pVp8DecInfo->u4CurYAddr) + PIC_Y_SZ);
		...
		/*pr_debug("Page Addr = 0x%x\n", u4Page_addr);*/
        vPage_Table(u4VDecID, u4Page_addr, PHYSICAL((ULONG)pVp8DecInfo->u4CurCAddr), PHYSICAL((ULONG)pVp8DecInfo->u4CurCAddr) + PIC_C_SZ);
        vVDecWriteVP8MC(u4VDecID, RW_MC_PP_C_ADDR, (pVp8DecInfo->u4CurCAddr) >> 8);

        vVDecVMMUEnable(PHYSICAL((ULONG)_pucVMMUTable[u4VDecID]));
#else
        vVDecWriteVP8MC(u4VDecID, RW_MC_PP_Y_ADDR, ((UINT32)PHYSICAL((ULONG)pVp8DecInfo->u4CurYAddr)) >> 9);
        vVDecWriteVP8MC(u4VDecID, RW_MC_PP_C_ADDR, ((UINT32)PHYSICAL((ULONG)pVp8DecInfo->u4CurCAddr)) >> 8);
#endif
#if VDEC_DDR3_SUPPORT
    ...
    u4HalValue = u4VDecReadVLDTOP(u4VDecID, RW_VLD_TOP_WEBP_CTRL);
	pr_debug("RW_VLD_TOP_WEBP_CTRL read = 0x%x\n", u4HalValue);
...
#endif
	pr_debug("RW_VLD_TOP_WEBP_CTRL write = 0x%x\n", u4HalValue);
    ...
    if (_fgVP8DumpReg)
    {
		pr_debug("******* INPUT Window *******\n");
		pr_debug("VP8_VLD1 VBSR1: 0x%08x\n", u4VDecReadVP8VLD(0, RO_VP8_VBSR1));
		pr_debug("VP8_VLD2 VBSR2: 0x%08x\n", u4VDecReadVP8VLD(0, RO_VP8_VBSR2));
    }
#ifdef VP8_REG_DUMP
    if (_fgVP8DumpReg)
    {
...
#if VDEC_UFO_SUPPORT
		// Debug only
		pr_debug("TRC-i4VDEC_HAL_VP8_DecStart +9, u4VDecReadMC(u4VDecID, 700 * 4)=0x%x\n",
		u4VDecReadMC(u4VDecID, 700 * 4));
		pr_debug("TRC-i4VDEC_HAL_VP8_DecStart +9, u4VDecReadMC(u4VDecID, 664 * 4)=0x%x\n",
		u4VDecReadMC(u4VDecID, 664 * 4));
		pr_debug("TRC-i4VDEC_HAL_VP8_DecStart +9, u4VDecReadMC(u4VDecID, 698 * 4)=0x%x\n",
		u4VDecReadMC(u4VDecID, 698 * 4));
		pr_debug("TRC-i4VDEC_HAL_VP8_DecStart +9, u4VDecReadMC(u4VDecID, 699 * 4)=0x%x\n",
		u4VDecReadMC(u4VDecID, 699 * 4));
	
		pr_debug("TRC-i4VDEC_HAL_VP8_DecStart +9, u4VDecReadMC(u4VDecID, 663 * 4)=0x%x\n",
		u4VDecReadMC(u4VDecID, 663 * 4));
		pr_debug("TRC-i4VDEC_HAL_VP8_DecStart +9, u4VDecReadMC(u4VDecID, 701 * 4)=0x%x\n",
		u4VDecReadMC(u4VDecID, 701 * 4));
		pr_debug("TRC-i4VDEC_HAL_VP8_DecStart +9, u4VDecReadMC(u4VDecID, 343 * 4)=0x%x\n",
		u4VDecReadMC(u4VDecID, 343 * 4));
#endif
