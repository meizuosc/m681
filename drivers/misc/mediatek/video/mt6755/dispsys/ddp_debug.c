...
//static unsigned int dbg_log_level = 1;  //for debug
...
/* --------------------------------------------------------------------------- */
/*******yuv test*****/
int yuvlayer_layerout[PRIMARY_SESSION_INPUT_LAYER_COUNT+1];
YUV_STAT_STRUCT yuv_stat[MAX_YUV_RECORD];
int yuv_stat_index;
/******end*********/
...
static void process_dbg_opt(const char *opt)
{
	...
	} else if (0 == strncmp(opt, "yuv_layer_account", 17)) {
		int i = 0;
		int j = 0;

		for (i = 0; i <= PRIMARY_SESSION_INPUT_LAYER_COUNT; i++) {
			pr_warn("[DDP]%d layer_account=%d\n", i, yuvlayer_layerout[i]);
		    sprintf(buf, "%d Layer CNT =%d\n", i, yuvlayer_layerout[i]);
			buf = buf + strlen(buf);
		}
		pr_warn("[DDP]yuv layer num > 1 cnt=%d\n", yuv_stat_index);
		sprintf(buf, "yuv layer num > 1 cnt=%d\n", yuv_stat_index);
		buf = buf + strlen(buf);

		for (i = 0; i < MAX_YUV_RECORD; i++)
			for (j = 0; j < MAX_YUV_NUM; j++) {
				pr_warn("YUV F%d I%d layer=%d x%d y%d w%d h%d name:%s\n",
					i, i, yuv_stat[i].yuv_layer[j].layer_id,
					yuv_stat[i].yuv_layer[j].dst_x, yuv_stat[i].yuv_layer[j].dst_y,
					yuv_stat[i].yuv_layer[j].dst_w, yuv_stat[i].yuv_layer[j].dst_h,
					yuv_stat[i].sname);
			    sprintf(buf, "YUV F%d I%d layer=%d x%d y%d w%d h%d name:%s\n",
					i, j, yuv_stat[i].yuv_layer[j].layer_id,
					yuv_stat[i].yuv_layer[j].dst_x, yuv_stat[i].yuv_layer[j].dst_y,
					yuv_stat[i].yuv_layer[j].dst_w, yuv_stat[i].yuv_layer[j].dst_h,
					yuv_stat[i].sname);
				buf = buf + strlen(buf);
