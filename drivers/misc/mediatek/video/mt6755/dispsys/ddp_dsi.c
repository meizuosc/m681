				    ...
				    unsigned int bypass)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		if (bypass == 0) {
			DSI_OUTREGBIT(cmdq, DSI_RACK_REG, DSI_REG[i]->DSI_RACK,
				DSI_RACK_BYPASS, 0);
		} else
...
bool is_ili9885_txd_lcd = false;  // add by liuyang,20160627, for TXD1 ESD check requirement.
int ddp_dsi_build_cmdq(DISP_MODULE_ENUM module, void *cmdq_trigger_handle, CMDQ_STATE state)
{
	...
	DSI_T0_INS t0;
	DSI_T0_INS t1;
//	unsigned char para[] = {0xc8,0x60,0x10,0xF1};
#if 1     // for txd1 esd check.
	DSI_T0_INS t2;
	DSI_T0_INS t3;
//	DSI_T0_INS t4;
#endif
	...
	if (state == CMDQ_WAIT_LCM_TE) {
		...
		for (i = 0; i < ESD_CHECK_NUM; i++) {
			DISPDBG("[DSI]enter read check_table cmd=0x%x\n",dsi_params->lcm_esd_check_table[i].cmd);
...
//Start:Reqxxx,liuyang3.wt,ADD,20160627,ILI9885_TXD1 ESD check requirement.
		DISPCHECK("[DSI]is_ili9885_txd_lcd == %d\n", is_ili9885_txd_lcd);   
		//DSI_set_cmdq_V2_DSI0(cmdq_trigger_handle, 0xC1, 4, para, 1); //debug write long package
#if 1		
		if ( is_ili9885_txd_lcd ){
/*	
			t4.CONFG = 0x00;
			t4.Data0 = 0xD6;
			t4.Data1 = 0x80;
			...
			DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[0],
				AS_UINT32(&t4));
			DSI_OUTREG32(cmdq_trigger_handle, &DSI_REG[dsi_i]->DSI_CMDQ_SIZE, 1);
			// start DSI 
			DSI_OUTREG32(cmdq_trigger_handle, &DSI_REG[dsi_i]->DSI_START, 0);
			DSI_OUTREG32(cmdq_trigger_handle, &DSI_REG[dsi_i]->DSI_START, 1);
			if (dsi_i == 0)
...
//End:Reqxxx,liuyang3.wt,ADD,20160627,ILI9885_TXD1 ESD check requirement.
		...
		for (i = 0; i < ESD_CHECK_NUM; i++) {
			DISPCHECK("[DSI]enter cmp check_table cmd=0x%x\n",dsi_params->lcm_esd_check_table[i].cmd);
