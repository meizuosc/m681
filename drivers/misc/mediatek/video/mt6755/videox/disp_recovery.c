...
//	primary_display_manual_unlock();  //mtk delete
	...
	primary_display_manual_unlock();  //mtk add
...
int primary_display_esd_check(void)
{
	...
	primary_display_manual_lock();  //mtk add
...
//	primary_display_manual_lock();  //mtk delete
...
bool is_ili9885_txd1 = false;  // add by liuyang,20160627, for TXD1 ESD check requirement.
static int primary_display_check_recovery_worker_kthread(void *data)
{
	...
	while (1) {
//Start:for TXD ESD check requirement.		
		if (is_ili9885_txd1)
			msleep(500);/*0.5s*/
		else
			msleep(2000);
//End:for TXD1 ESD check requirement.
		...
		if (esd_check_enable) {
			...
			i = 0;/*repeat*/
			do {
				//Start:bug268382,liuyang3.wt,ADD,flash screen when light screen.
				//mdelay(100); //add by MTK  //Delete,20160204,Playing songs while lock screen would cause noise. why???
				//End:bug268382,liuyang3.wt,ADD,flash screen when light screen.
