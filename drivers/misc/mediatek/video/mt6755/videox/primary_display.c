...
/*begin:add by chenhaibing for lcm_info 20151201*/

extern bool is_ili9885_txd_lcd;  // add by liuyang,20160627, for TXD1 ESD check requirement.
extern bool is_ili9885_txd1;

extern char lcd_info[40];
unsigned int info_count=6; //modified by chenhaibing for diff tcl glass info 20151022
void process_lcd_info(unsigned int param,char *lcm_name)
{ 
	char tmp_info[64];   
	char* const delim = "_"; 
	char *tok;  
	char *cur; 
	int i=0;      
	pr_info("lcd_info %s ",lcm_name);   
//Start:Reqxxx,liuyang3.wt,ADD,20160627,ILI9885_TXD1 ESD check requirement.
	if (strncmp(lcm_name, "ili9885_fhd_dsi_vdo_txd1", 24) == 0){
		is_ili9885_txd_lcd = true;
		is_ili9885_txd1 = true;
	}
	else if (strncmp(lcm_name, "ili9885_fhd_dsi_vdo_txd", 23) == 0 || strncmp(lcm_name, "ili9885_fhd_dsi_vdo_djn", 23) == 0){
		is_ili9885_txd_lcd = true;
		is_ili9885_txd1 = false;
	}
	else{
		is_ili9885_txd_lcd = false;
		is_ili9885_txd1 = false;
	}
//End:Reqxxx,liuyang3.wt,ADD,20160627,ILI9885_TXD1 ESD check requirement.	
	memset(lcd_info, 0,sizeof(lcd_info));
	strncpy(tmp_info,lcm_name,strlen(lcm_name));	
	cur=tmp_info; 
	...
	while((tok = strsep(&cur,delim))!=NULL) 
	{
		...
		if(i==0){	      
			strncpy(lcd_info,tok,strlen(tok));    
		}	    
		else{	      
			strcat(lcd_info,",");
			strncat(lcd_info,tok,strlen(tok));
			}	         
		i++;	        	
	}   

	pr_info("lcd_info %s, is_ili9885_txd_lcd = %d\n",lcd_info, is_ili9885_txd_lcd);  
	
	return;
	
}/*end:add by chenhaibing for lcm_info 20151201*/
int primary_display_init(char *lcm_name, unsigned int lcm_fps, int is_lcm_inited)
{
    ...
    process_lcd_info(info_count,lcm_name);//chenhaibing 20151201 add for lcm_info
	dprec_init();
	dpmgr_init();
