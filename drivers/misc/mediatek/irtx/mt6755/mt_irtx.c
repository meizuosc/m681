...
static void set_irtx_duty(int duty_cycle)
{
	...
	if (duty_cycle == 33)
		...
		pr_err("[IRTX] non-dafault duty cycle\n");
...
static int dev_char_open(struct inode *inode, struct file *file)
{
	...
	pr_debug("[IRTX] open GPIO_IRTX_OUT_PIN defined\n");
#else
#ifdef GPIO_ANT_SEL0
	pr_debug("[IRTX] open GPIO_ANT_SEL0 defined\n");
#else
	pr_debug("[IRTX] open GPIO19 defined\n");
#endif
#endif
#endif				/* !defined(CONFIG_MTK_LEGACY) */
...
/*	mt_set_gpio_out(gpio_id, GPIO_OUT_ZERO);  */
	...
	if (ret < 0) {
		pr_err("[IRTX] register/alloc _chrdev_region fail ret=%d\n", ret);
		goto exit;
	}
