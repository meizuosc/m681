...
void SetSinWavePara(unsigned char UcTableVal, unsigned char UcMethodVal)
{
	...
	if (UsFreqDat == 0xFFFF) {	/* Sine波?止 */
...
#ifdef	USE_SINLPF
		if ((UcMethodVal == CIRCWAVE) || (UcMethodVal == SINEWAVE)
		    || (UcMethodVal == XACTTEST) || (UcMethodVal == YACTTEST)) {
		...
		if ((UcMethodVal == CIRCWAVE) || (UcMethodVal == SINEWAVE)) {
#ifdef	USE_SINLPF
			RegWriteA(WC_DPI1ADD0, (unsigned char)MES1BZ2);
			/* 0x01B0       input Meas-Fil */
			RegWriteA(WC_DPI1ADD1, (unsigned char)((MES1BZ2 >> 8) & 0x0001));
			/* 0x01B1       input Meas-Fil */
			RegWriteA(WC_DPI2ADD0, (unsigned char)MES2BZ2);
			/* 0x01B2       input Meas-Fil */
			RegWriteA(WC_DPI2ADD1, (unsigned char)((MES2BZ2 >> 8) & 0x0001));
			/* 0x01B3       input Meas-Fil */
			RegWriteA(WC_DPO1ADD0, (unsigned char)SXOFFZ1);
			/* 0x01B8       output SXOFFZ1 */
			RegWriteA(WC_DPO1ADD1, (unsigned char)((SXOFFZ1 >> 8) & 0x0001));
			/* 0x01B9       output SXOFFZ1 */
			RegWriteA(WC_DPO2ADD0, (unsigned char)SYOFFZ1);
			/* 0x01BA       output SYOFFZ1 */
			RegWriteA(WC_DPO2ADD1, (unsigned char)((SYOFFZ1 >> 8) & 0x0001));
			/* 0x01BA       output SYOFFZ1 */
			...
			if (UcMethodVal == XACTTEST) {
				RegWriteA(WC_DPO2ADD0, (unsigned char)SXOFFZ1);
				/* 0x01BA       output SXOFFZ1 */
				RegWriteA(WC_DPO2ADD1, (unsigned char)((SXOFFZ1 >> 8) & 0x0001));
				/* 0x01BB       output SXOFFZ1 */
				RegWriteA(WC_MES2ADD0, (unsigned char)SINXZ);
				/* 0x0196       */
				RegWriteA(WC_MES2ADD1, (unsigned char)((SINXZ >> 8) & 0x0001));
				/* 0x0197       */
			} else {
				RegWriteA(WC_DPO2ADD0, (unsigned char)SYOFFZ1);
				/* 0x01BA       output SYOFFZ1 */
				...
				RegWriteA(WC_MES2ADD0, (unsigned char)SINYZ);
				/* 0x0196       */
				RegWriteA(WC_MES2ADD1, (unsigned char)((SINYZ >> 8) & 0x0001));
				/* 0x0197       */
			}
...
void StbOnn(void)
{
	...
	if (((UcRegValx & 0x01) != 0x01) && ((UcRegValy & 0x01) != 0x01)) {
		...
		while ((UcRegIni & 0x77) != 0x66) {
			...
			if (CmdRdChk() != 0)
				break;	/* Dead Lock check (responce check) */
			if ((UcRegIni & 0x77) == 0)
				UcRegIniCnt++;
			if (UcRegIniCnt > 10)
				break;	/* Status Error */
...
unsigned char RunGea(void)
{
	...
	for (UcCnt = 0; UcCnt < 10; UcCnt++) {
		...
		UsGxoVal[UcCnt] = (unsigned short)GenMes(AD2Z, 0);
		...
		RegWriteA(WC_MES1ADD1, 0x00);	/* 0x0195 */
		ClrGyr(0x1000, CLR_FRAM1);	/* Measure Filter RAM Clear */
		UsGyoVal[UcCnt] = (unsigned short)GenMes(AD3Z, 0);
		...
		if (UcCnt > 0) {
			if ((short)UsGxoVal[0] > (short)UsGxoVal[UcCnt]) {
				UsDif =
				    (unsigned short)((short)UsGxoVal[0] - (short)UsGxoVal[UcCnt]);
			} else {
				UsDif =
				    (unsigned short)((short)UsGxoVal[UcCnt] - (short)UsGxoVal[0]);
			}
			...
			if ((short)UsGyoVal[0] > (short)UsGyoVal[UcCnt]) {
				UsDif =
				    (unsigned short)((short)UsGyoVal[0] - (short)UsGyoVal[UcCnt]);
			} else {
				UsDif =
				    (unsigned short)((short)UsGyoVal[UcCnt] - (short)UsGyoVal[0]);
			}
...
/* RamWrite32A( msmean     , 0x3D800000 );                 // 0x1230       1/CmMesLoop[15:0] */
	...
	if (UcMeasMode == 0x00) {
		StMeasGInfo.StGyrAgl.UlGyrXagl =
		    (unsigned long)StMeasGInfo.StGyrAmp.UsGyrXamp * 1000L / ((unsigned long)UcFreq *
									     FIXCOEFF);
		StMeasGInfo.StGyrAgl.UlGyrYagl =
		    (unsigned long)StMeasGInfo.StGyrAmp.UsGyrYamp * 1000L / ((unsigned long)UcFreq *
									     FIXCOEFF);
