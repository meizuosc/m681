/*
 * GT9762 voice coil motor driver
 */
...
#define LENS_I2C_BUSNUM 2	//wangjie, M91

#define AF_DRVNAME "GT9763AF_SUNNY2"
#define I2C_SLAVE_ADDRESS        0x18
#define I2C_REGISTER_ID            0x11
#define PLATFORM_DRIVER_NAME "lens_actuator_gt9763af_sunny2"
#define AF_DRIVER_CLASS_NAME "actuatordrv_gt9763af_sunny2"
...
static inline int getAFInfo(__user stGT9762AF_MotorInfo * pstMotorInfo)
{
	stGT9762AF_MotorInfo stMotorInfo;
	...
	if (copy_to_user(pstMotorInfo, &stMotorInfo, sizeof(stGT9762AF_MotorInfo)))
...
#ifdef LensdrvCM3
static inline int getAFMETA(__user stGT9762AF_MotorMETAInfo * pstMotorMETAInfo)
{
	stGT9762AF_MotorMETAInfo stMotorMETAInfo;
	...
	if (copy_to_user(pstMotorMETAInfo, &stMotorMETAInfo, sizeof(stGT9762AF_MotorMETAInfo)))
		LOG_INF("copy to user failed when getting motor information\n");
...
//wangjie, init setup
static int GT9762AF_mode_setup(void)
{
	int i4RetValue = 0;
	char puSendCmd[2] = { 0xEC, 0xA3 };	//Selection mode on

	LOG_INF("GT9762AF_mode_setup enter!!\n");
	...
	puSendCmd[0] = 0xA1;
	puSendCmd[1] = 0x0D;	//DVO[3:1] : 010/64, MCLK[1:0] : 01, DLC mode
	i4RetValue = i2c_master_send(g_pstAF_I2Cclient, puSendCmd, 2);
	if (i4RetValue < 0) {
		LOG_INF("I2C send failed!!\n");
		return -1;
	...
	//T_SRC[4:0] setting
	puSendCmd[0] = 0xF2;
	puSendCmd[1] = 0x18;	//T_SRC[4:0] , Tvib = 11.8 F13V02K-202
	i4RetValue = i2c_master_send(g_pstAF_I2Cclient, puSendCmd, 2);
	if (i4RetValue < 0) {
		LOG_INF("I2C send failed!!\n");
		return -1;
	...
	LOG_INF("GT9762AF_mode_setup exit!!\n");
...
static inline int moveAF(unsigned long a_u4Position)
{
	...
	if (g_s4AF_Opened == 1) {
		...
		if(GT9762AF_mode_setup())
			LOG_INF("GT9762AF mode setup failed!!\n");
...
static long AF_Ioctl(struct file *a_pstFile, unsigned int a_u4Command, unsigned long a_u4Param)
{
	...
	switch (a_u4Command) {
	case GT9762AFIOC_G_MOTORINFO:
		i4RetValue = getAFInfo((__user stGT9762AF_MotorInfo *) (a_u4Param));
		break;
#ifdef LensdrvCM3
	case GT9762AFIOC_G_MOTORMETAINFO:
		i4RetValue = getAFMETA((__user stGT9762AF_MotorMETAInfo *) (a_u4Param));
		break;
#endif
	case GT9762AFIOC_T_MOVETO:
		i4RetValue = moveAF(a_u4Param);
		break;

	case GT9762AFIOC_T_SETINFPOS:
		i4RetValue = setAFInf(a_u4Param);
		break;

	case GT9762AFIOC_T_SETMACROPOS:
...
static int __init GT9762AF_i2C_init(void)
{
...
static void __exit GT9762AF_i2C_exit(void)
{
	platform_driver_unregister(&g_stAF_Driver);
}
module_init(GT9762AF_i2C_init);
module_exit(GT9762AF_i2C_exit);

MODULE_DESCRIPTION("GT9762AF lens module driver");
