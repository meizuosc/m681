...
#define AF_DRVNAME "GT9762AF"
#define I2C_SLAVE_ADDRESS        0x18
#define I2C_REGISTER_ID            0x18
#define PLATFORM_DRIVER_NAME "lens_actuator_gt9762af"
#define AF_DRIVER_CLASS_NAME "actuatordrv_gt9762af"
...
static int GT9762AF_mode_setup(void)
{
	...
	puSendCmd[0] = 0xA1;
	puSendCmd[1] = 0x4D;	//DVO[3:1] : 010/64, MCLK[1:0] : 01, DLC mode
	i4RetValue = i2c_master_send(g_pstAF_I2Cclient, puSendCmd, 2);
	if (i4RetValue < 0) {
		LOG_INF("I2C send failed!!\n");
		return -1;
	...
	//T_SRC[4:0] setting
	puSendCmd[0] = 0xF2;
	puSendCmd[1] = 0x60;	//T_SRC[4:0] : 0x60, Tvib = 10.27
	i4RetValue = i2c_master_send(g_pstAF_I2Cclient, puSendCmd, 2);
	if (i4RetValue < 0) {
		LOG_INF("I2C send failed!!\n");
		return -1;
