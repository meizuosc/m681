...
void IniSrv(void)
{
	...
	RegWriteA(WH_SMTTMR, 0x00);	/* 0x017E               1.3ms=(0+1)*16/11.718kHz */
#else
...
#ifdef	USE_VH_SYNC
	RegWriteA(STROBEFC, 0x80);	/* 0x001C       外?入力Strobe信?の有効 */
...
void ClrGyr(unsigned short UsClrFil, unsigned char UcClrMod)
{
	...
	/*Enable Clear */
	RegWriteA(WC_RAMINITON, UcClrMod);	/* 0x0102       [ - | - | - | - ][ - | - | 遅?Clr | 係?Clr ] */
