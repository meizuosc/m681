...
static unsigned int simp_msdc_init(struct simp_mmc_host *mmc_host)
{
	...
	if (host->id == 0)
		msdc_set_driving_by_id(host->id, &msdc0_hw, 0);

	else  if (host->id == 1)
		msdc_set_driving_by_id(host->id, &msdc1_hw, 0);
...
static unsigned int simple_mmc_attach_mmc(struct simp_mmc_host *host)
{
	...
	if (!host2 || !host2->mmc || !host2->mmc->card) {
		...
		MTK_DUMP_PR_DBG("[%s]: cache is not enabled,"
			" no need to disable\n",
			__func__);
	}
#endif
	 ...
	 */
	if (ocr & 0x7F) {
		MTK_DUMP_PR_DBG("msdc0: card claims to support voltages below "
