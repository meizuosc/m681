...
#define GFX1XM_INPUT_HOME_KEY   KEY_HOME
...
int gfx1xm_power_on(bool onoff)
{
	if(onoff)
	{
	 	//hwPowerOn(MT6328_POWER_LDO_VCAMD , VOL_2800, "FP28");
	}
	else
	{
		//hwPowerDown(MT6328_POWER_LDO_VCAMD, "FP28");
	}
}
...
static struct config_buf config316_buf_list[] = {
	...
	.date = 0x7df051c,
	.buffer = {
0x41, 0x96, 0x96, 0xE4, 0x0C, 0x30, 0x4F, 0x02, 0x00, 0x80, 0x50, 0x96, 0x96, 0xE4, 0x0C, 0x30, 
0x4F, 0x02, 0x00, 0x03, 0x00, 0x80, 0x08, 0xFA, 0x14, 0x00, 0x00, 0x01, 0x50, 0xB3, 0x33, 0xB3, 
0x33, 0x03, 0x80, 0x50, 0x00, 0x01, 0x50, 0x09, 0x0A, 0x0B, 0x00, 0x83, 0x08, 0x10, 0x12, 0x12, 
...
0x50, 0x00, 0x00, 0x00, 0x70, 0x00, 0x03, 0x00, 0x32, 0x01, 0xA0, 0x00, 0x00, 0x79, 0xC8, 0x00, 
0x00, 0x00, 0x28, 0x00, 0x05, 0x04, 0x48, 0x00, 0x08, 0x00, 0x07, 0x00, 0x20, 0x00, 0x15, 0x00, 
0x3D, 0x00, 0x72, 0x00, 0x22, 0x00, 0x00, 0x00, 0x03, 0x07, 0x80, 0x00, 0x20, 0x00, 0x70, 0x04, 
0x30, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x7C, 0x01
	},
    },

};

static struct config_buf config318_buf_list[] = {
    {
	.date = 0x7df051c,
	.buffer = {
...
#ifdef SPI_ASYNC
static void gfx1xm_spi_complete(void *arg)
{
    complete(arg);
}
#endif //SPI_ASYNC
...
void gfx1xm_spi_setup(struct gfx1xm_dev *gfx1xm_dev, int max_speed_hz)
{
    ...
    //gfx1xm_dev->spi->irq = GFX1XM_IRQ_NUM;//change by sonia
...
static ssize_t gfx1xm_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
    ...
    mutex_lock(&gfx1xm_dev->buf_lock);
    gfx1xm_dev->spi->max_speed_hz=4*1000*1000;
...
static long gfx1xm_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GFX1XM_IOC_CMD:
		...
		if(copy_to_user(ioc->buf, gf_dev->buffer + GFX1XM_RDATA_OFFSET, ioc->len)) {
		    pr_err("Failed to copy data from kernel to user.\n");
		    retval = -EFAULT;
		...
		if(copy_from_user(gf_dev->buffer + GFX1XM_WDATA_OFFSET, ioc->buf, ioc->len)){
...
static bool hw_config(struct gfx1xm_dev *gfx1xm_dev)
{
    mutex_lock(&gfx1xm_dev->buf_lock);
		
	if(chip_version_config==0x38){
	    memcpy(gfx1xm_dev->buffer + GFX1XM_WDATA_OFFSET, config318_list[0].config[0].buffer, GFX1XM_CFG_LEN);

	}
	else if(chip_version_config==0x36){
	    memcpy(gfx1xm_dev->buffer + GFX1XM_WDATA_OFFSET, config316_list[0].config[0].buffer, GFX1XM_CFG_LEN);
	}
	
	gfx1xm_spi_write_bytes(gfx1xm_dev, GFX1XM_CFG_ADDR, GFX1XM_CFG_LEN, gfx1xm_dev->buffer);
    mutex_unlock(&gfx1xm_dev->buf_lock);
...
{  
	...
	//gfx1xm_spi_write_byte(gfx1xm_dev, GFX1XM_MODE_STATUS, GFX1XM_FF_MODE);
	//gfx1xm_dev->mode = GFX1XM_FF_MODE;		
	gfx1xm_debug(DEFAULT_DEBUG,"gfx1xm  suspend.\n");

//add for remove FF mode, xunanbin 151104
       mt_set_gpio_dir(GFX1XM_IRQ_PIN, GPIO_DIR_OUT);
       mt_set_gpio_out(GFX1XM_IRQ_PIN, GPIO_OUT_ZERO);

       gfx1xm_spi_write_byte(gfx1xm_dev, 0x8043, 0x02);
       gfx1xm_debug(DEFAULT_DEBUG,"gfx1xm: sleep chip \n");
//add end
...
//add for remove FF mode, xunanbin 151104
       ...
       //reset hardware
       gfx1xm_hw_reset();
       gfx1xm_spi_write_byte(gfx1xm_dev, 0x8043, 0x01);
...
static void gfx1xm_timer_work(struct work_struct *work)
{
	...
	if (i<3){
	    ...
	    if(!ret)
		pr_warn("[info] %s write 0x8040 fail\n", __func__);

	    if(!hw_config(gfx1xm_dev))
		pr_warn("[info] %s write config fail\n", __func__);
#endif
	    enable_irq(gfx1xm_dev->spi->irq);
...
static void gfx1xm_timer_func(unsigned long arg)
{
    struct gfx1xm_dev* gfx1xm_dev = (struct gfx1xm_dev*)arg;
    schedule_work(&gfx1xm_dev->spi_work);	
}
static flag=0;//linan
static irqreturn_t gfx1xm_irq(int irq, void* handle)
{
    struct gfx1xm_dev *gfx1xm_dev = (struct gfx1xm_dev *)handle;
    u8 mode = 0x80;
    u8	status;
    ...
    gfx1xm_spi_read_byte(gfx1xm_dev, GFX1XM_MODE_STATUS, &mode);
    gfx1xm_debug(DEFAULT_DEBUG, "status = 0x%x, mode = %d\n", status, mode);
	...
	case GFX1XM_IMAGE_MODE:
		/*if(status ==0xb0 && flag==0)
		{
                    flag=1;
		    printk("____________==============\n");
		    input_report_key(gfx1xm_dev->input, GFX1XM_INPUT_BACK_KEY, 1);
		    input_report_key(gfx1xm_dev->input, GFX1XM_INPUT_BACK_KEY, 0);
		    input_sync(gfx1xm_dev->input);
		}
		if(status ==0xa0 || status ==0xc0)
			flag=0;
		*/
#ifdef GFX1XM_FASYNC
	    if(gfx1xm_dev->async) {
	...
	case GFX1XM_KEY_MODE:
		...
		if (status & GFX1XM_HOME_KEY_MASK) {
		    input_report_key(gfx1xm_dev->input, GFX1XM_INPUT_BACK_KEY, (status & GFX1XM_HOME_KEY_STA)>>4);
		    input_sync(gfx1xm_dev->input);
		  printk("gfx1xm:====key input_sync=======\n");  
		}
	    ...
	    gfx1xm_spi_write_byte(gfx1xm_dev, GFX1XM_BUFFER_STATUS, (status & 0x7F));
	    break;
...
static int gfx1xm_open(struct inode *inode, struct file *filp)
{
    ...
    FUNC_ENTRY();
    printk("gfx1xm: gfx1xm_open entry\n");
	...
	if(status == 0) {
	    ...
	    enable_irq(gfx1xm_dev->spi->irq);
	    //add begin by Emily 2015.10.28 for  MP test
	     mutex_lock(&gfx1xm_dev->buf_lock);
	    gfx1xm_dev->spi->max_speed_hz=1*1000*1000;
	    spi_setup(gfx1xm_dev->spi);
	    mutex_unlock(&gfx1xm_dev->buf_lock);
	    gfx1xm_spi_write_byte(gfx1xm_dev, GFX1XM_MODE_STATUS, 0x00);	
	    printk("gfx1xm: gfx1xm_open set mode 0\n");    
	    //add end by Emily 2015.10.28 for  MP test
	}
    } else {
	pr_err("No device for minor %d\n", iminor(inode));
    }
    mutex_unlock(&device_list_lock);
    printk("gfx1xm: gfx1xm_open exit\n");
    FUNC_EXIT();
    return status;
}
...
static int gfx1xm_release(struct inode *inode, struct file *filp)
{
    ...
    int    status = 0;
    FUNC_ENTRY();
     printk("gfx1xm: gfx1xm_release entry\n");
    mutex_lock(&device_list_lock);
    gfx1xm_dev = filp->private_data;
	...
	gfx1xm_dev->users --;
     //add begin by Emily 2015.10.28 for  MP test	   
	    ...
	    spi_setup(gfx1xm_dev->spi);
	    mutex_unlock(&gfx1xm_dev->buf_lock);
	    gfx1xm_spi_write_byte(gfx1xm_dev, GFX1XM_MODE_STATUS, 0x01);	    
      printk("gfx1xm: gfx1xm_release set mode 1\n");	
	    enable_irq(gfx1xm_dev->spi->irq);    
	 //add end by Emily 2015.10.28 for  MP test
 ...
 /*    if(!gfx1xm_dev->users) {
    ...
    mutex_unlock(&device_list_lock);
    FUNC_EXIT();
    printk("gfx1xm: gfx1xm_release exit\n");
    return status;
}
...
static int  gfx1xm_probe(struct spi_device *spi)
{
	...
	FUNC_ENTRY();
	gfx1xm_debug("driver version is %s.\n",GFX1XM_DRIVER_VERSION);
...
#endif
		err = request_threaded_irq(spi->irq, NULL, gfx1xm_irq, 
		IRQF_TRIGGER_RISING | IRQF_ONESHOT,
		dev_name(&gfx1xm_dev->spi->dev), gfx1xm_dev);
		if(!err) {
		disable_irq(gfx1xm_dev->spi->irq);
		}
#if ESD_PROTECT
		INIT_WORK(&gfx1xm_dev->spi_work, gfx1xm_timer_work);
		init_timer(&gfx1xm_dev->gfx1xm_timer); 
		gfx1xm_dev->gfx1xm_timer.function = gfx1xm_timer_func;
		gfx1xm_dev->gfx1xm_timer.expires = jiffies + 3*HZ;
		gfx1xm_dev->gfx1xm_timer.data = gfx1xm_dev;
		add_timer(&gfx1xm_dev->gfx1xm_timer);  
#endif // ESD_PROTECT 
//enable_irq(gfx1xm_dev->spi->irq);//linan
#if CONFIG_HAS_EARLYSUSPEND		
...
register_early_suspend(&gfx1xm_dev->early_fp);
	...
	FUNC_EXIT();
	  //add begin by Emily 2015.10.28 for  MP test	   
	    ...
	    mutex_unlock(&gfx1xm_dev->buf_lock);
	    gfx1xm_spi_write_byte(gfx1xm_dev, GFX1XM_MODE_STATUS, 0x01);	    
      printk("gfx1xm¡êogfx1xm_release set mode 1\n");	
	    enable_irq(gfx1xm_dev->spi->irq);    
	 //add end by Emily 2015.10.28 for  MP test
	return status;
	}
...
{
    int status;
gfx1xm_debug(DEFAULT_DEBUG,"driver init is gfx1xm_init.\n");
