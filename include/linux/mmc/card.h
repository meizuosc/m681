...
#define MMC_HS400_MAX_DTR   400000000
#endif
...
#ifdef CONFIG_MTK_EMMC_CACHE
#define mmc_card_doing_flush(c)	((c)->state & MMC_STATE_DOING_flush)
#define mmc_card_need_flush(c)	((c)->state & MMC_STATE_NEED_flush)
#endif
...
#ifdef CONFIG_MTK_EMMC_CACHE
#define mmc_card_set_need_flush(c)	((c)->state |= MMC_STATE_NEED_flush)
#define mmc_card_set_doing_flush(c)	((c)->state |= MMC_STATE_DOING_flush)
#define mmc_card_clr_need_flush(c)	((c)->state &= ~MMC_STATE_NEED_flush)
#define mmc_card_clr_doing_flush(c)	((c)->state &= ~MMC_STATE_DOING_flush)
#endif
...
static inline void __maybe_unused add_quirk_mmc_ksi_v03_skip_trim(struct mmc_card *card, int data)
{
	if (mmc_card_mmc(card)) {
		if ((unsigned int)card->cid.prv == 0x3)
			card->quirks |= data;
	}
}
