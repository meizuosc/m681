...
bool check_consistency_of_irq_settings(struct irq_desc *desc)
{
	...
	if (cpumask_equal(d->affinity, cpu_possible_mask)) {
		/*
	...
	} else if (!cpumask_equal(&per_cpu_list_affinity, d->affinity)) {
	...
	if (!ret) {
		pr_err("[IRQ MIGRATION] IRQ %d: smp affinity is not consistent with per-cpu irq-need-migrate list\n",
		       d->irq);
		...
		for_each_cpu(cpu, &tmp_affinity)
			if (cpumask_test_cpu(cpu, d->affinity))
				pr_err("[IRQ MIGRATION] @CPU%u: smp affinity is set, but per-cpu list is not set\n",
				       cpu);
			else
				pr_err("[IRQ MIGRATION] @CPU%u: smp affinity is not set, but per-cpu list is set\n",
				       cpu);
	}
...
check_gic:
	...
	if (!mt_get_irq_gic_targets(d, &gic_target_affinity)) {
		...
		pr_err("[IRQ MIGRATION] unable to get GICD_ITARGETSR setting of IRQ %d\n", d->irq);
		ret = false;
	} else if (!cpumask_equal(&gic_target_affinity, d->affinity)) {
		pr_err("[IRQ MIGRATION] IRQ %d: smp affinity is not consistent with GICD_ITARGETSR\n", d->irq);
		cpumask_xor(&tmp_affinity, &gic_target_affinity, d->affinity);

		/* iterates on cpus with inconsitent setting */
		for_each_cpu(cpu, &tmp_affinity)
			if (cpumask_test_cpu(cpu, d->affinity))
				pr_err("[IRQ MIGRATION] @CPU%u: smp affinity is set, but GICD_ITARGETSR is not set\n",
				       cpu);
			else
				pr_err("[IRQ MIGRATION] @CPU%u: smp affinity is not set, but GICD_ITARGETSR is set\n",
				       cpu);
		ret = false;
	}
...
out:
	...
	if (!ret)
		pr_err("[IRQ MIGRATION] IRQ %d: the affintiy setting is INCONSITENT\n", d->irq);
...
void dump_irq_need_migrate_list(const struct cpumask *mask)
{
	...
	for_each_cpu(cpu, mask) {
		pr_debug("[IRQ MIGRATION] dump per-cpu irq-need-migrate list of CPU%u\n", cpu);
		list_for_each_safe(pos, temp, &(irq_need_migrate_list[cpu].list)) {
			node = list_entry_rcu(pos, struct per_cpu_irq_desc, list);
			pr_debug("[IRQ MIGRATION] IRQ %d\n", (node->desc->irq_data).irq);
		}
	}
	rcu_read_unlock();
}
...
static void del_from_irq_need_migrate_list(struct irq_desc *desc, const struct cpumask *cpumask_to_del)
{
	...
	for_each_cpu(cpu, cpumask_to_del) {
		...
		list_for_each_entry_safe(node, next, &(irq_need_migrate_list[cpu].list), list) {
			if (node->desc != desc)
				continue;
			pr_debug("[IRQ MIGRATION] list_del to cpu %d\n", cpu);
...
static bool add_to_irq_need_migrate_list(struct irq_desc *desc, const struct cpumask *cpumask_to_add)
{
	...
	for_each_cpu(cpu, cpumask_to_add) {
		...
		if (node == NULL) {
			pr_err("[IRQ MIGRATION] kmalloc failed: cannot add node into CPU%d per-cpu IRQ list\n",
				cpu);
			spin_unlock(&(irq_need_migrate_list[cpu].lock));
			ret = false;
			break;
		...
		node->desc = desc;
		pr_debug("[IRQ MIGRATION] list_add to cpu %d\n", cpu);
...
bool update_irq_need_migrate_list(struct irq_desc *desc, const struct cpumask *new_affinity)
{
	...
	pr_debug("[IRQ MIGRATION] update per-cpu list (IRQ %d)\n", d->irq);
	...
	/* special cases */
	if (cpumask_equal(new_affinity, cpu_possible_mask)) {
		/*
		...
		return true;
	} else if (cpumask_equal(d->affinity, cpu_possible_mask)) {
		/*
...
#ifdef CONFIG_CPU_ISOLATION
	cpumask_andnot(&tmp_cpumask, cpu_online_mask, cpu_isolate_mask);
	if (cpumask_any_and(affinity, &tmp_cpumask) >= nr_cpu_ids) {
		affinity = &tmp_cpumask;
		ret = true;
	} else {
		cpumask_and(affinity, affinity, &tmp_cpumask);
	}
#else
...
void migrate_irqs(void)
{
	...
	list_for_each_safe(pos, temp, &(irq_need_migrate_list[smp_processor_id()].list)) {
		...
		pr_debug("[IRQ MIGRATION] CPU%u is going down, IRQ%u needs to be migrated\n",
