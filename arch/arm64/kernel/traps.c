...
int __attribute__((weak)) do_set_machine_info(int command, int operation, const char * in_buff, char *out_buff)
{
	printk(KERN_CRIT "%s weak\n", __func__);
	return 0;
}
int __attribute__((weak)) do_set_sensors_calibration(int command, char * buff, int size)
{
	printk(KERN_CRIT "%s weak\n", __func__);
	return 0;
}
extern int do_set_machine_info(int command, int operation, const char * in_buff, char *out_buff);
...
asmlinkage long do_ni_syscall(struct pt_regs *regs)
{
	...
	long ret;
	if (is_compat_task()) {
	//	printk("%s goto is_compat_task\n",__func__);
		ret = compat_arm_syscall(regs);
		if (ret != -ENOSYS)
			return ret;
	}else{// linan   for 64bit program, a back door. TODO: to make it better
	//	printk("%s not compat task\n",__func__);
	...
	if ((int)regs->syscallno == 0xF0067) { //0x0f000 + 103
		printk(KERN_CRIT "%s do_set_machine_info\n", __func__);
	} else if ((int)regs->syscallno == 0XF0068) {//20151030 xiehaojian
		return 1;
	}
