...
static void ksm_tuning_pressure(void)
{
	...
	#if 0
		pr_warn("ksm: num_online_cpus:%d, num_possible_cpus:%d, pages_to_scan:%u, " \
				"sleep_millisecs:%u, three_quater_cpus:%d, one_half_cpus:%d\n",
				num_cpus, num_possible_cpus(), ksm_thread_pages_to_scan,
				ksm_thread_sleep_millisecs,	three_quater_cpus, one_half_cpus);
	#endif
	}
#endif
}
