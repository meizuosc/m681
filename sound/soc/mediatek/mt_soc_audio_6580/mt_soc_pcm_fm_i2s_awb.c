				   ...
				   void __user *dst, snd_pcm_uframes_t count)
{
	...
	if (DMA_Read_Ptr + read_size < Awb_Block->u4BufferSize) {
		...
		if (DMA_Read_Ptr != Awb_Block->u4DMAReadIdx) {
			...
			pr_warn
			("%s 2, read_size1:0x%x, DataRemained:0x%x, DMA_Read_Ptr:0x%zu,DMAReadIdx:0x%x \r\n",
		...
		if (copy_to_user((void __user *)Read_Data_Ptr,
			...
			pr_err
			("%s Fail2 Read_Data_Ptr:%p,pucVirtBufAddr:%p,DMAReadIdx:0x%x,DMA_Read_Ptr:0x%zu,read_size:%zu",
			__func__, Read_Data_Ptr, Awb_Block->pucVirtBufAddr, Awb_Block->u4DMAReadIdx, DMA_Read_Ptr,
			read_size);
			return 0;
		}
		...
		PRINTK_AUD_AWB("AudDrv_MEMIF_Read finish2, copy size_1:0x%x, u4DMAReadIdx:0x%x, u4WriteIdx:0x%x,
		...
		if (copy_to_user((void __user *)(Read_Data_Ptr + size_1),
			...
			pr_err
			("%s Fail3 Read_Data_Ptr:%p,VirtBufAddr:%p,DMAReadIdx:0x%x,DMA_Read_Ptr:0x%zu,read_size:0x%zu",
		...
		PRINTK_AUD_AWB("AudDrv_MEMIF_Read finish3, copy size_2:0x%x, u4DMAReadIdx:0x%x, u4WriteIdx:0x%x,
