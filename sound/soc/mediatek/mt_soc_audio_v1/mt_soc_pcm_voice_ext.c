...
static int mtk_voice_ext_pcm_open(struct snd_pcm_substream *substream)
{
	...
	pr_debug("mtk_voice_ext_pcm_open\n");
	...
	pr_debug("mtk_voice_ext_pcm_open runtime rate = %d channels = %d\n", runtime->rate,
		 runtime->channels);
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		pr_debug("SNDRV_PCM_STREAM_PLAYBACK mtkalsa_voice_ext_constraints\n");
		runtime->rate = 16000;
	} else {
		pr_debug("SNDRV_PCM_STREAM_CAPTURE mtkalsa_voice_ext_constraints\n");
		runtime->rate = 16000;
	}

	if (err < 0) {
		pr_debug("mtk_voice_ext_close\n");
		mtk_voice_ext_close(substream);
		return err;
	}
	pr_debug("mtk_voice_ext_pcm_open return\n");
	return 0;
}
...
{
	pr_debug("mtk_voice_ext_trigger cmd = %d\n", cmd);
	switch (cmd) {
...
{
	pr_debug("mtk_voice_ext_pcm_silence\n");
	return 0;		/* do nothing */
}
...
static int mtk_voice1_ext_prepare(struct snd_pcm_substream *substream)
{
	...
	EnableAfe(true);
	VoiceExtPcm.mPcmModeWidebandSel =
	    (runtimeStream->rate ==
...
static int mtk_voice_ext_probe(struct platform_device *pdev)
{
	...
	return snd_soc_register_platform(&pdev->dev, &mtk_soc_voice_ext_platform);
}
...
static int mtk_voice_ext_platform_probe(struct snd_soc_platform *platform)
{
	pr_debug("mtk_voice_ext_platform_probe\n");
	return 0;
}
...
const struct dev_pm_ops mtk_voice_ext_pm_ops = {
...
#ifdef CONFIG_OF
static const struct of_device_id mt_soc_pcm_voice_ext_of_ids[] = {
...
static struct platform_driver mtk_voice_ext_driver = {
	.driver = {
		   ...
		   .owner = THIS_MODULE,
#ifdef CONFIG_OF
		   .of_match_table = mt_soc_pcm_voice_ext_of_ids,
#endif
#ifdef CONFIG_PM
