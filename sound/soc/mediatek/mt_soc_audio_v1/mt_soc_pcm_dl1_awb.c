				...
				void __user *dst, snd_pcm_uframes_t count)
{
	...
	if (Awb_Block->u4DataRemained > Awb_Block->u4BufferSize) {
		pr_debug("mtk_dl1_awb_pcm_copy u4DataRemained=%x > u4BufferSize=%x",
	...
	DMA_Read_Ptr =
	    Awb_Block->u4DMAReadIdx + Get_Mem_CopySizeByStream(Soc_Aud_Digital_Block_MEM_AWB,
							       substream);
	pr_debug("%s, u4DMAReadIdx= 0x%x  Get_Mem_CopySizeByStream = 0x%x\n",
	...
	pr_debug("%s Awb_Block = %p finish0, count:0x%lx, read_size:0x%lx, Remained:0x%x, ReadIdx:0x%x, u4WriteIdx:0x%x\n",
	     __func__, Awb_Block, count, read_size, Awb_Block->u4DataRemained, Awb_Block->u4DMAReadIdx,
	     Awb_Block->u4WriteIdx);
	...
	if (DMA_Read_Ptr + read_size <= Awb_Block->u4BufferSize) {
		...
		count -= read_size;
		pr_debug("%s finish1, copy size:0x%lx, ReadIdx:0x%x, WriteIdx:0x%x, DataRemained:0x%x CopySize:0x%x\n",
		...
		if (copy_to_user
		    ((void __user *)Read_Data_Ptr, (Awb_Block->pucVirtBufAddr + DMA_Read_Ptr),
		     size_1)) {
			pr_debug("%s Fail 2 copy to user Read_Data_Ptr:%p, ReadIdx:0x%x, DMA_Read_Ptr:%zu,read_size:%zu",
			     __func__, Read_Data_Ptr, Awb_Block->u4DMAReadIdx,
			     DMA_Read_Ptr, read_size);
			return 0;
		}
		...
		if (DMA_Read_Ptr >= Awb_Block->u4BufferSize) {
			pr_debug("mtk_dl1_awb_pcm_copy 2, DMA_Read_Ptr out of bound.\n");
			DMA_Read_Ptr %= Awb_Block->u4BufferSize;
		}
		spin_unlock(&auddrv_Dl1AWBInCtl_lock);

		pr_debug("%s finish2, copy size_1:0x%x, ReadIdx:0x%x, WriteIdx:0x%x, u4DataRemained:0x%x\n",
		     __func__, size_1, Awb_Block->u4DMAReadIdx, Awb_Block->u4WriteIdx,
		     Awb_Block->u4DataRemained);

		if (DMA_Read_Ptr != Awb_Block->u4DMAReadIdx) {
			pr_debug("%s 3, read_size2:%x, DataRemained:%x, DMA_Read_Ptr:0x%lx,DMAReadIdx:%x\n",
		...
		if (copy_to_user
			...
			pr_debug("%s Fail 3 copy to user Read_Data_Ptr:%p, ReadIdx:0x%x , DMA_Read_Ptr:%zu, read_size:%zu",
			     __func__, Read_Data_Ptr, Awb_Block->u4DMAReadIdx,
			     DMA_Read_Ptr, read_size);
			return read_count << 2;
		}
		...
		pr_debug("%s finish3, copy size_2:0x%x, DMAReadIdx:0x%x,WriteIdx:0x%x u4DataRemained:0x%x\n",
