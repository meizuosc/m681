...
static snd_pcm_uframes_t mtk_capture2_pcm_pointer(struct snd_pcm_substream *substream)
{
	...
	if (VUL2_Control_context->interruptTrigger == 1) {
		...
		pr_debug("[Auddrv] mtk_capture2_pcm_pointer =0x%x HW_memory_index = 0x%x\n",
				      ...
				      struct snd_pcm_hw_params *hw_params)
{
	...
	if (mUL2UseSram == true) {
		...
		pr_debug("mtk_capture2_pcm_hw_params mUL2UseSram dma_bytes = %zu\n",
			 runtime->dma_bytes);
				 ...
				 void __user *dst, snd_pcm_uframes_t count)
{
	...
	if (Vul_Block->u4DataRemained > Vul_Block->u4BufferSize) {
		pr_debug("mtk_capture2_pcm_copy u4DataRemained=%x > u4BufferSize=%x",
	...
	pr_debug("%s finish0, read_count:%zu, read_size:%zu,u4DataRemained:%x, u4DMAReadIdx:0x%x, u4WriteIdx:%x\n",
	...
	if (DMA_Read_Ptr + read_size < Vul_Block->u4BufferSize) {
		if (DMA_Read_Ptr != Vul_Block->u4DMAReadIdx) {
			pr_debug("%s 1, read_size:%zu, DataRemained:0x%x,DMA_Read_Ptr:%zu, DMAReadIdx:%x\n"
		...
		if (copy_to_user
				((void __user *)Read_Data_Ptr, (Vul_Block->pucVirtBufAddr + DMA_Read_Ptr),
				 read_size)) {
			pr_debug("%s Fail 1 copy to user Read_Data_Ptr%p,u4DMAReadIdx:0x%x, DMA_Read_Ptr:0x%zd,read_size:%zd",
		...
		pr_debug("%s finish1, copy size:%zu, u4DMAReadIdx:0x%x,u4WriteIdx:%x, u4DataRemained:%x \r\n",
		...
		if (DMA_Read_Ptr != Vul_Block->u4DMAReadIdx) {
			pr_debug("%s 2, read_size1:%x, DataRemained:%x,DMA_Read_Ptr:0x%zu, DMAReadIdx:%x \r\n",
		...
		if (copy_to_user
				((void __user *)Read_Data_Ptr, (Vul_Block->pucVirtBufAddr + DMA_Read_Ptr),
				 size_1)) {
			pr_debug("%s Fail 2 copy to user Read_Data_Ptr:%p, u4DMAReadIdx:0x%x, DMA_Read_Ptr:0x%zu,read_size:%zu",
			 __func__, Read_Data_Ptr, Vul_Block->u4DMAReadIdx,
			 DMA_Read_Ptr, read_size);
			return 0;
		}
...
static struct page *mtk_capture2_pcm_page(struct snd_pcm_substream *substream, unsigned long offset)
{
	pr_debug("%s\n", __func__);
