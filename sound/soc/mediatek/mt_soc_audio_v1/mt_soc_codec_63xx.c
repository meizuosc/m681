...
static void LowPowerAdcClockEnable(bool enable)
{
	...
	if (enable == true) {
		...
		if (LowPowerAdcClockCount < 0) {
			pr_debug("LowPowerAdcClockCount <0 =%d\n ", LowPowerAdcClockCount);
...
void SetHplTrimOffset(int Offset)
{
	...
	if ((Offset > 2098) || (Offset < 1998)) {
		mHplTrimOffset = 2048;
		pr_debug("SetHplTrimOffset offset may be invalid offset = %d\n", Offset);
	}
}
...
{
	/* pr_debug("mt63xx_codec_prepare set up  rate = %d\n", substream->runtime->rate); */
...
static int Voice_Amp_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	if ((ucontrol->value.integer.value[0] == true)
		...
		mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_VOLUME_HSOUTL] =
		   ...
		   && (mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_VOLUME_HSOUTL] == true)) {
		mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_VOLUME_HSOUTL] =
		    ucontrol->value.integer.value[0];
		Voice_Amp_Change(false);
	}
...
static int Speaker_Amp_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	if ((ucontrol->value.integer.value[0] == true)
		...
		mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_VOLUME_SPKL] =
		   ...
		   && (mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_VOLUME_SPKL] == true)) {
		mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_VOLUME_SPKL] =
		    ucontrol->value.integer.value[0];
		Speaker_Amp_Change(false);
	}
...
static void Headset_Speaker_Amp_Change(bool enable)
{
	if (enable) {
		...
		Ana_Set_Reg(AUDBUF_CFG1, 0x0100, 0xffffffff);       /* Disable pre-charge buffer*/
...
static int Audio_Ext_Speaker_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	if (Ext_Speaker_Mode == true) {
		/* on */
		pr_debug("%s Ext_Speaker_Mode = %d", __func__, Ext_Speaker_Mode);
	} else {
		/* off */
		pr_debug("%s Ext_Speaker_Mode = %d", __func__, Ext_Speaker_Mode);
	}
...
static int Audio_Ext_Speaker_Get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
...
static const struct snd_kcontrol_new mt_ext_dev_controls[] = {
	SOC_ENUM_EXT("Ext_Speaker_Amp_Switch", Audio_Ext_dev_Enum[0], Audio_Ext_Speaker_Get,
		     Audio_Ext_Speaker_Set),
};

#ifdef CONFIG_MTK_SPEAKER
static const char * const speaker_amp_function[] = { "CALSSD", "CLASSAB", "RECEIVER" };

static const char * const speaker_PGA_function[] = { "4Db", "5Db", "6Db", "7Db", "8Db", "9Db", "10Db",
...
static int Audio_Speaker_Current_Sensing_Set(struct snd_kcontrol *kcontrol,
					     struct snd_ctl_elem_value *ucontrol)
{
	static int dauxadc1_tempval;

	if (ucontrol->value.integer.value[0]) {
		pr_debug("enable Audio_Speaker_Current_Sensing_Set\n");
		...
		pr_debug("disable Audio_Speaker_Current_Sensing_Set\n");
...
int Audio_AuxAdcData_Get_ext(void)
{
	...
	do {
		...
		/* pr_debug("hochi o [%d] v [%d] i [%d] t [%d]\n",ov_flag,v_cnt,i_cnt,dRecReadIndex); */
		/*
		   if (ov_flag != 0)
		   {
		   pr_debug("hochi %s overflow\n", __func__);
		   pr_debug("hochi ov MT6332_AUXADC_CON33 [%x]\n", iv_queue);
		   break;
		   }
		   else
		 */
		...
		/* pr_debug("o [%d] v [%d] i [%d]",ov_flag,v_cnt,i_cnt); */
		...
		if (/*(v_cnt > 0) || */ (i_cnt > 0)) {
			...
			for (i = 0; i < cnt1; i++) {
				...
				/* must_print("v_tmp=0x%x i_tmp= 0x%x, hw_read_idx %d, V_Addr 0x%x,
				I_Addr 0x%x\n\n", v_tmp, i_tmp, hw_read_idx,
				I_Buffer_Table[hw_read_idx], V_Buffer_Table[hw_read_idx]); */
				/* } */
				if (dCurValue > dMax)
					dMax = dCurValue;
			...
			for (i = 0; i < cnt2; i++) {
				...
				/*
				   if ((v_tmp & 0x8000) == 0 || (i_tmp & 0x8000) == 0)
				   {
				   pr_debug("hochi AUXADC_CON33=0x%x at %d\n\n", iv_queue, hw_read_idx);
				   pr_debug("hochi v_tmp=0x%x i_tmp= 0x%x, hw_read_idx %d, V_Addr 0x%x,
				   I_Addr 0x%x\n\n", v_tmp, i_tmp, hw_read_idx,
				   I_Buffer_Table[hw_read_idx], V_Buffer_Table[hw_read_idx]);
				   }
				 */
			...
			if (iov_flag) {
				Ana_Set_Reg(MT6332_AUXADC_CON12, 0x0080, 0x0080);
				pr_debug("vibspk auxadc skip io [%d] i [%d]\n", iov_flag, iov_cnt);
				Ana_Set_Reg(MT6332_AUXADC_CON12, 0, 0x0080);
			}
		}
	...
	/* if (ov_flag) */
	/* pr_debug("hochi : overflow dRecReadIndex [%d] dValidCount [%d]
	dCheckCount [%d]\n",dRecReadIndex,dValidCount,dCheckCount); */
	/* else */
	pr_debug
	    ("vibspk auxadc- : dMax = %d dRecReadIndex [%d] dValidCount [%d] dCheckCount [%d]\n",
...
static int Headset_PGAL_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	/* pr_debug("%s(), index = %d arraysize = %lu\n", __func__,
	ucontrol->value.enumerated.item[0], ARRAY_SIZE(DAC_DL_PGA_Headset_GAIN)); */
	...
	if (offset > 0)
		...
		pr_debug("return for gain is the same");
		return 0;
	} else
		step = 1;
	...
	while (offset != 0) {
		...
		Ana_Set_Reg(ZCD_CON2, (mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_HPOUTL]),
			    0x001f);
		msleep(1);
		offset += step;
	}
...
static int Headset_PGAR_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	offset = mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_HPOUTR] -
	    ucontrol->value.integer.value[0];
	if (offset > 0)
		step = -1;
	 else if (offset == 0) {
...
/* remove while due to headset gain update too late for user experience, trade off: headset pop hw limitation */
	...
	while (offset != 0) {
		...
		Ana_Set_Reg(ZCD_CON2,
			    (mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_HPOUTR]) << 7,
			    0x0f80);
		msleep(1);
		offset += step;
	}
...
static bool TurnOnADcPowerACC(int ADCType, bool enable)
{
	...
	if (enable) {
		...
		if (GetAdcStatus() == false) {
			...
			/* OpenMicbias2(true); */
			if (mAdc_Power_Mode == false) {
				/* Enable LCLDO18_ENC (1.8V), Remote-Sense */
				Ana_Set_Reg(AUDLDO_NVREG_CFG1, 0x0007, 0xffff);
				/* Enable  LCLDO19_ADCCH0_1, Remote-Sense */
			...
			(GetULNewIFFrequency(mBlockSampleRate[AUDIO_ANALOG_DEVICE_IN_ADC]) << 10), 0xffff);
			Ana_Set_Reg(AFE_UL_SRC0_CON0_H, (ULSampleRateTransform(SampleRate_VUL1) << 3
			| ULSampleRateTransform(SampleRate_VUL1) << 1), 0x001f);	/* ULsampling rate */
			Ana_Set_Reg(AFE_ADDA2_UL_SRC_CON0_H, (ULSampleRateTransform(SampleRate_VUL2) << 3
		...
		if (ADCType == AUDIO_ANALOG_DEVICE_IN_ADC1) {
			if (mAdc_Power_Mode == false) {
				/* Enable LCLDO19_ADCCH0_1, Remote-SENSEINFOBYTES */
				Ana_Set_Reg(AUDLDO_NVREG_CFG2, 0x0007, 0x000f);
				/* Enable LCLDO19_ADCCH2, Remote-Sensen) */
			...
			if (mAdc_Power_Mode == false) {
				...
				/* Enable LCLDO19_ADCCH3, Remote-Sense */
			}
			 else {
		...
		if ((GetAdcStatus() == false)) {
			...
			/* Disable ADC CH0_1 (PGA in) Disable ADC CH_2 (PGA in) */
			Ana_Set_Reg(AUDPREAMP_CFG0, 0x0000, 0xffff);
			/* Disable PGA CH0_1 (CH0 in) Disable PGA CH_2 */
			Ana_Set_Reg(AUDPREAMP_CFG1, 0x0000, 0xffff);
			/* Set PGA CH0_1 gain = 0dB  Set PGA CH_2 gain = 0dB */
			Ana_Set_Reg(AUDPREAMPGAIN_CFG0, 0x0000, 0xffff);
			/* disable LCLDO19_ADCCH0_1, Remote-Sense */

			Ana_Set_Reg(AUDLDO_NVREG_CFG2, 0x2222, 0xffff);
			/* disable LCLDO18_ENC (1.8V), Remote-Sense */
...
static bool TurnOnADcPowerDCC(int ADCType, bool enable)
{
	...
	if (enable) {
		...
		if (GetAdcStatus() == false) {
			...
			/* OpenMicbias2(true); */
			if (mAdc_Power_Mode == false) {
				/* Enable LCLDO18_ENC (1.8V), Remote-SENSEINFOBYTES */
				Ana_Set_Reg(AUDLDO_NVREG_CFG1, 0x0007, 0xffff);
				/* Enable  LCLDO19_ADCCH0_1, Remote-Sense */
				Ana_Set_Reg(AUDLDO_NVREG_CFG2, 0x0000, 0x8888);
			} else {
...
static bool TurnOnVOWADcPowerACC(int ADCType, bool enable)
{
	...
	if (enable) {
		...
		EnableSideGenHw(Soc_Aud_InterConnectionOutput_O03,
				Soc_Aud_InterConnectionOutput_Num_Output, false);
		AudDrv_Clk_Off();
#endif
#if defined(MTK_VOW_SUPPORT)
		...
		/* if (GetAdcStatus() == false) //[Todo]Implement concurrncy test */
		{
			...
			/* Set MICBIAS0 lowpower mode - Enable MICBIAS0 (2.7v) */
			...
			/* Set CLK from MADPLL and Enable ADCCLK (12.84MHz/4) */
			...
			/* Set UP GLB lowpower mode, set ADC low CLK rate mode */
		...
		/* if (GetAdcStatus() == false) */
		{
			...
			ClsqAuxEnable(false);
			/* turn off analog */
			/* "Disable UP GLB lowpower mode Disable ADC low CLK rate mode" */
			...
			/* "Set CLK from CLKSQ Disable ADC CLK (12.84MHz/4)" */
			...
			/* Disable LCLDO19_ADCCH0_1, Remote-Sense */
			Ana_Set_Reg(AUDLDO_NVREG_CFG2, 0x2222, 0xffff);
			/* "Disable LCLDO18_ENC (1.8V), Remote-Sense  Set LCLDO19_ADC voltage 1.9V" */
			Ana_Set_Reg(AUDLDO_NVREG_CFG1, 0x0002, 0xffff);
			/* "Disable MICBIAS0 lowpower modeDisable MICBIAS0 (2.7V)" */
...
static const char * const Audio_AnalogMic_Mode[] = {
	"XXXXXXX", "XXXXXXX", "XXXX" , "XXXXXXXXXXXXXX", "XXXXXXXXXXXXXXXX"
	};
...
static int Audio_ADC3_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.integer.value[0]) {
		...
		} else if (mAudio_Analog_Mic3_mode == AUDIO_ANALOGUL_MODE_DCCECMDIFF
			   || mAudio_Analog_Mic3_mode == AUDIO_ANALOGUL_MODE_DCCECMSINGLE) {
...
static int Audio_ADC4_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.integer.value[0]) {
		...
		} else if (mAudio_Analog_Mic4_mode == AUDIO_ANALOGUL_MODE_DCCECMDIFF
			   || mAudio_Analog_Mic4_mode == AUDIO_ANALOGUL_MODE_DCCECMSINGLE) {
				     ...
				     struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.integer.value[0]) {
		...
		/* Set PGA CH0_1 gain = 18dB ; Set PGA CH_2 gain = 18dB */
		...
		/* time slot1= 47, time slot2=24 @ 384K interval. */
		...
		/* L/R-ch @ sample rate = 8*8K for tone = 0dB of 1K Hz example. */
...
static const struct snd_kcontrol_new mt6331_UL_Codec_controls[] = {
	...
	SOC_ENUM_EXT("Audio_Vow_Digital_Func_Switch", Audio_UL_Enum[23],
...
#endif
	snd_soc_add_codec_controls(codec, mt_ext_dev_controls, ARRAY_SIZE(mt_ext_dev_controls));
