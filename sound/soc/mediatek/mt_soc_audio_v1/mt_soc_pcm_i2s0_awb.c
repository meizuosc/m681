				 ...
				 void __user *dst, snd_pcm_uframes_t count)
{
	...
	if (Awb_Block->u4DataRemained > Awb_Block->u4BufferSize) {
		pr_debug("mtk_i2s0_awb_pcm_copy u4DataRemained=%x > u4BufferSize=%x",
	...
	pr_debug("%s finish0, read_count:0x%zd, read_size:0x%zd, DataRemained:0x%x, ReadIdx:0x%x, WriteIdx:0x%x\n",
	...
	if (DMA_Read_Ptr + read_size < Awb_Block->u4BufferSize) {
		...
		pr_debug("%s finish2, copy size_1:0x%x, ReadIdx:0x%x,WriteIdx:0x%x, u4DataRemained:0x%x\n",
		      __func__, size_1, Awb_Block->u4DMAReadIdx, Awb_Block->u4WriteIdx, Awb_Block->u4DataRemained);
		...
		count -= read_size;
		Read_Data_Ptr += read_size;
		pr_debug("%s finish3, copy size_2:0x%x, ReadIdx:0x%x,WriteIdx:0x%x u4DataRemained:0x%x\n",
