...
static uint16 Voice_ExtInt_Status;
...
static int mtk_voice_extint_pcm_open(struct snd_pcm_substream *substream)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		pr_debug("SNDRV_PCM_STREAM_CAPTURE mtkalsa_voice_extint_constraints\n");
		runtime->rate = 16000;
	}
...
static int mtk_voice1_extint_prepare(struct snd_pcm_substream *substream)
{
	...
	if (Voice_ExtInt_Status == 0) {
		...
		SetConnection(Soc_Aud_InterCon_Connection, Soc_Aud_InterConnectionInput_I09,
			      Soc_Aud_InterConnectionOutput_O25);
		VoiceExtIntPcm_Ext.mPcmModeWidebandSel =
		    (runtimeStream->rate ==
		     8000) ? Soc_Aud_PCM_MODE_PCM_MODE_8K : Soc_Aud_PCM_MODE_PCM_MODE_16K;
		VoiceExtIntPcm_Int.mPcmModeWidebandSel =
		    (runtimeStream->rate ==
		     8000) ? Soc_Aud_PCM_MODE_PCM_MODE_8K : Soc_Aud_PCM_MODE_PCM_MODE_16K;
...
static int mtk_voice_extint_probe(struct platform_device *pdev)
{
	...
	return snd_soc_register_platform(&pdev->dev, &mtk_soc_voice_extint_platform);
}
...
const struct dev_pm_ops mtk_voice_extint_pm_ops = {
...
#ifdef CONFIG_OF
static const struct of_device_id mt_soc_pcm_voice_extint_of_ids[] = {
...
static struct platform_driver mtk_voice_extint_driver = {
	.driver = {
		   ...
		   .owner = THIS_MODULE,
#ifdef CONFIG_OF
		   .of_match_table = mt_soc_pcm_voice_extint_of_ids,
#endif
#ifdef CONFIG_PM
