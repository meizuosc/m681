...
static snd_pcm_uframes_t mtk_bt_dai_pcm_pointer(struct snd_pcm_substream *substream)
{
	...
	pr_debug("mtk_bt_dai_pcm_pointer Dai_Block->u4DMAReadIdx;= 0x%x\n",
			       ...
			       void __user *dst, snd_pcm_uframes_t count)
{
	...
	if (Dai_Block->u4DataRemained > Dai_Block->u4BufferSize) {
		pr_debug
		("!!!!!!!!!!!!mtk_bt_dai_pcm_copy u4DataRemained=%x > u4BufferSize=%x",
	...
	pr_debug("%s finish0, read_count:0x%zd, read_size:0x%zd,u4DataRemained:0x%x, u4DMAReadIdx:0x%x, u4WriteIdx:0x%x\n",
	...
	if (DMA_Read_Ptr + read_size < Dai_Block->u4BufferSize) {
		if (DMA_Read_Ptr != Dai_Block->u4DMAReadIdx) {
			pr_debug("%s 1, read_size:%zu, DataRemained:0x%x,DMA_Read_Ptr:%zu, DMAReadIdx:0x%x\n",
				 __func__, read_size, Dai_Block->u4DataRemained, DMA_Read_Ptr,
				 Dai_Block->u4DMAReadIdx);
		}

		if (copy_to_user
				((void __user *)Read_Data_Ptr, (Dai_Block->pucVirtBufAddr + DMA_Read_Ptr),
				 read_size)) {
			pr_debug("%s Fail 1 copy to user Read_Data_Ptr:%p, u4DMAReadIdx:0x%x, DMA_Read_Ptr:%zu,read_size:%zu",
				  __func__, Read_Data_Ptr, Dai_Block->u4DMAReadIdx,
		...
		count -= read_size;
		pr_debug("%s finish1, copy size:0x%zd, u4DMAReadIdx:0x%x,u4WriteIdx:0x%x, u4DataRemained:0x%x\n",
				__func__, read_size, Dai_Block->u4DMAReadIdx, Dai_Block->u4WriteIdx,
		...
		if (DMA_Read_Ptr != Dai_Block->u4DMAReadIdx) {
			...
			__func__, size_1, Dai_Block->u4DataRemained, DMA_Read_Ptr,
			Dai_Block->u4DMAReadIdx);
		}
		if (copy_to_user
				((void __user *)Read_Data_Ptr, (Dai_Block->pucVirtBufAddr + DMA_Read_Ptr),
		...
		pr_debug("%s finish2, copy size_1:0x%x, u4DMAReadIdx:0x%x,u4WriteIdx:0x%x, u4DataRemained:0x%x \r\n",
		...
		pr_debug("%s finish3, copy size_2:0x%x, u4DMAReadIdx:0x%x u4WriteIdx:0x%x u4DataRemained:0x%x\n",
