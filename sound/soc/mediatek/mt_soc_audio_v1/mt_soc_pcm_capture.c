...
static snd_pcm_uframes_t mtk_capture_pcm_pointer(struct snd_pcm_substream *substream)
{
	...
	pr_debug("mtk_capture_pcm_pointer UL1_Block->u4WriteIdx= 0x%x, u4DataRemained=0x%x\n",
	...
	if (GetMemoryPathEnable(Soc_Aud_Digital_Block_MEM_VUL) == true) {
		...
		if (HW_Cur_ReadIdx == 0) {
			pr_debug("[Auddrv] mtk_capture_pcm_pointer  HW_Cur_ReadIdx ==0\n");
		...
		pr_debug("%s u4DMAReadIdx=0x%x u4WriteIdx =0x%x u4DataRemained =0x%x u4BufferSize= 0x%x,Hw_Get_bytes= 0x%x\n",
		...
		if (UL1_Block->u4DataRemained > UL1_Block->u4BufferSize) {
			bIsOverflow = true;
			pr_debug("overflow %s u4DMAReadIdx=0x%x u4WriteIdx = 0x%x u4DataRemained = 0x%x,Hw_Get_bytes= 0x%x\n",
		...
		pr_debug("[Auddrv] mtk_capture_pcm_pointer =0x%x HW_memory_index = 0x%x\n",
...
static int mtk_capture_pcm_open(struct snd_pcm_substream *substream)
{
	...
	if (ret < 0) {
		pr_debug("mtk_capture_pcm_close\n");
				...
				void __user *dst, snd_pcm_uframes_t count)
{
	...
	pr_debug("mtk_capture_pcm_copy pos = %lu, count = %lu\n ", pos, count);
	...
	pr_debug("%s finish0, read_count:%zd, read_size:%zd,u4DataRemained:%x,u4DMAReadIdx:0x%x, u4WriteIdx:%x\n",
	...
	if (DMA_Read_Ptr + read_size < Vul_Block->u4BufferSize) {
		...
		if (copy_to_user
				((void __user *)Read_Data_Ptr, (Vul_Block->pucVirtBufAddr + DMA_Read_Ptr),
				 read_size)) {
			pr_debug("%s Fail 1 copy to user Read_Data_Ptr:%p,u4DMAReadIdx:0x%x, DMA_Read_Ptr:%zu,read_size:%zu",
		...
		count -= read_size;
		pr_debug("%s finish1, read_size:%zd, u4DMAReadIdx:0x%x,u4WriteIdx:%x, u4DataRemained:%x\n",
		...
		if (DMA_Read_Ptr != Vul_Block->u4DMAReadIdx) {
			pr_debug("%s 2, read_size1:%x, DataRemained:%x, DMA_Read_Ptr:%zu, DMAReadIdx:%x\n",
		...
		if (copy_to_user
				((void __user *)Read_Data_Ptr, (Vul_Block->pucVirtBufAddr + DMA_Read_Ptr),
				 size_1)) {
			pr_debug("%s Fail 2 copy to user Read_Data_Ptr:%p, u4DMAReadIdx:0x%x, DMA_Read_Ptr:%zu,read_size:%zu",
			 __func__, Read_Data_Ptr, Vul_Block->u4DMAReadIdx,
			 DMA_Read_Ptr, read_size);
			return 0;
		}
		...
		if (DMA_Read_Ptr != Vul_Block->u4DMAReadIdx) {
			pr_debug("%s 3, read_size2:%x, DataRemained:%x,DMA_Read_Ptr:%zu, DMAReadIdx:%x \r\n",
		...
		if (copy_to_user
			...
			pr_debug("%s Fail 3 copy to user Read_Data_Ptr:%p,	u4DMAReadIdx:0x%x , DMA_Read_Ptr:%zu, read_size:%zu",
			 __func__, Read_Data_Ptr, Vul_Block->u4DMAReadIdx,
			 DMA_Read_Ptr, read_size);
			return read_count << 2;
		}
