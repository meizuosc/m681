				       ...
				       struct snd_pcm_hw_params *hw_params)
{
	...
	/* pr_debug("runtime->hw.buffer_bytes_max = 0x%lx\n", runtime->hw.buffer_bytes_max); */
				  ...
				  void __user *dst, snd_pcm_uframes_t count)
{
	...
	pr_debug("Awb_Block->u4DMAReadIdx= 0x%x Get_Mem_CopySizeByStream = 0x%x \r\n",
	...
	if (DMA_Read_Ptr >= Awb_Block->u4BufferSize) {
		pr_debug("%s 1, DMA_Read_Ptr out of bound.CopySizeByStream = %d, u4BufferSize = %d, DMA_Read_Ptr = %zu\n",
		     __func__,  stream_copy_size, Awb_Block->u4BufferSize, DMA_Read_Ptr);
		DMA_Read_Ptr %= Awb_Block->u4BufferSize;
	}
	spin_unlock_irqrestore(&auddrv_AWBInCtl_lock, flags);

	pr_debug("%s finish0, read_count:%zd, read_size:0x%zd,4DataRemained:0x%x, DMAReadIdx:0x%x, WriteIdx:0x%x\n",
	...
	if (DMA_Read_Ptr + read_size <= Awb_Block->u4BufferSize) {
		...
		if (copy_to_user
			...
			pr_debug
			    ("%s Fail 1 copy to user Read_Data_Ptr:%p,ReadIdx:0x%x, DMA_Read_Ptr:%zu, read_size:%zu",
			     __func__, Read_Data_Ptr, Awb_Block->u4DMAReadIdx, DMA_Read_Ptr, read_size);
			return 0;
		}
		...
		pr_debug
		    ("%s finish1, copy size:0x%zd, ReadIdx:0x%x,WriteIdx:0x%x, DataRemained:0x%x MaxCopySize:0x%x\n",
		...
		if (copy_to_user
			...
			pr_debug("%s Fail 2 copy to user Read_Data_Ptr:%p,ReadIdx:0x%x, DMA_Read_Ptr:%zd,read_size:%zd",
			     __func__, Read_Data_Ptr, Awb_Block->u4DMAReadIdx,
		...
		if (DMA_Read_Ptr >= Awb_Block->u4BufferSize) {
			pr_debug
			    ("DEBUG: %s 2, DMA_Read_Ptr out of bound. u4BufferSize = %d, DMA_Read_Ptr = %zu\n",
			     __func__, Awb_Block->u4BufferSize, DMA_Read_Ptr);
			DMA_Read_Ptr %= Awb_Block->u4BufferSize;
		}
		spin_unlock(&auddrv_AWBInCtl_lock);

		pr_debug
		    ("%s finish2, copy size_1:0x%x, ReadIdx:0x%x u4WriteIdx:0x%x, DataRemained:0x%x\n",
		...
		if (DMA_Read_Ptr != Awb_Block->u4DMAReadIdx) {
			...
			pr_debug
			    ("%s 3, read_size2:%x, DataRemained:%x, DMA_Read_Ptr:0x%zd, DMAReadIdx:%x\n",
		...
		if (copy_to_user
			...
			pr_debug
			    ("%s Fail 3 Read_Data_Ptr:%p,ReadIdx:0x%x, DMA_Read_Ptr:0x%zd, read_size:%zd",
			     __func__, Read_Data_Ptr, Awb_Block->u4DMAReadIdx,
			     DMA_Read_Ptr, read_size);
			return read_count << 2;
		}
		...
		pr_debug
		    ("%s finish3, copy size_2:0x%x, ReadIdx:0x%x WriteIdx:0x%x u4DataRemained:0x%x \r\n",
	...
	if (Mrgrx_Awb_Capture_dma_buf == NULL) {
		...
		if (Mrgrx_Awb_Capture_dma_buf != NULL) {
			Mrgrx_Awb_Capture_dma_buf->area =
			    dma_alloc_coherent(platform->dev, MRGRX_MAX_BUFFER_SIZE,
