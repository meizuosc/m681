...
static snd_pcm_uframes_t mtk_pcm_fmtx_pointer(struct snd_pcm_substream *substream)
{
	...
	pr_debug("[mtk_pcm_fmtx_pointer] Afe_Block->u4DMAReadIdx = 0x%x\n",
				  ...
				  struct snd_pcm_hw_params *hw_params)
{
	...
	pr_debug("mtk_pcm_fmtx_hw_params\n");
	if (fake_buffer) {
		...
		/* ------------------------------------------------------- */
		pr_debug("1 dma_bytes = %d dma_area = %p dma_addr = 0x%x\n",
				substream->runtime->dma_bytes, substream->runtime->dma_area,
	...
	pr_debug("2 dma_bytes = %d dma_area = %p dma_addr = 0x%x\n",
			substream->runtime->dma_bytes, substream->runtime->dma_area,
			(unsigned int)substream->runtime->dma_addr);
	return ret;
}
...
static int mtk_pcm_fmtx_open(struct snd_pcm_substream *substream)
{
	...
	if (ret < 0)
		pr_debug("[mtk_pcm_fmtx_open]snd_pcm_hw_constraint_integer failed\n");
	...
	if (ret < 0) {
		pr_debug("[mtk_pcm_fmtx_open]mtk_pcm_fmtx_close\n");
...
static int mtk_pcm_fmtx_open(struct snd_pcm_substream *substream)
{
	...
	if (ret < 0) {
		pr_debug("ret < 0 mtkalsa_fmtx_playback close\n");
		mtk_pcm_fmtx_close(substream);
		return ret;
	}
	...
	if (copy_size != 0) {
		...
		if (Afe_WriteIdx_tmp + copy_size < Afe_Block->u4BufferSize) {
			if (!access_ok(VERIFY_READ, data_w_ptr, copy_size)) {
				pr_debug
				    ("[mtk_pcm_fmtx_copy] 0ptr invalid data_w_ptr=%p, size=%d",
				      data_w_ptr, copy_size);
				pr_debug
				    ("[mtk_pcm_fmtx_copy] u4BufferSize=%d, u4DataRemained=%d",
				     Afe_Block->u4BufferSize, Afe_Block->u4DataRemained);
			} else {
				pr_debug("memcpy Afe_Block->pucVirtBufAddr+Afe_WriteIdx= %p data_w_ptr = %p copy_size = 0x%x\n",
			...
			pr_debug
			    ("%s finish1, copy_size:%d, WriteIdx:%x, ReadIdx=%x, DataRemained:%x, count=%zu\n",
			...
			if (!access_ok(VERIFY_READ, data_w_ptr, size_1)) {
				...
				pr_debug("%s mcmcpy pucVirtBufAddr+Afe_WriteIdx= %p data_w_ptr = %p size_1 = %x\n",
				     __func__, Afe_Block->pucVirtBufAddr + Afe_WriteIdx_tmp, data_w_ptr,
				     size_1);
				if ((copy_from_user
				     ((Afe_Block->pucVirtBufAddr + Afe_WriteIdx_tmp), data_w_ptr,
				      size_1))) {
					pr_debug("[mtk_pcm_fmtx_copy] Fail 1 copy from user");
					return -1;
				}
			}
			spin_lock_irqsave(&auddrv_FMTxCtl_lock, flags);
			...
			if (!access_ok(VERIFY_READ, data_w_ptr + size_1, size_2)) {
				pr_debug
				    ("[mtk_pcm_fmtx_copy] 2ptr invalid data_w_ptr=%p, size_1=%d, size_2=%d",
				...
				pr_debug("%s mcmcpy pucVirtBufAddr+Afe_WriteIdx= %p data_w_ptr+size_1 = %p size_2 = %x\n",
			...
			pr_debug("%s finish2, copy size:%x, WriteIdx:%x, ReadIdx=%x DataRemained:%x\n",
			     __func__, copy_size, Afe_Block->u4WriteIdx, Afe_Block->u4DMAReadIdx,
