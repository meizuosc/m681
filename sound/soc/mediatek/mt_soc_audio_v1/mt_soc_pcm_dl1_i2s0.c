				  ...
				  struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_debug
	("%s() samplerate = %d,mi2s0_hdoutput_control = %d,extcodec_echoref_control = %d, sidegen_control = %d\n",
	...
	if (mi2s0_sidegen_control != 0) {
		...
		if (GetMemoryPathEnable(Soc_Aud_Digital_Block_I2S_IN_2) == false ||
			...
			Enable4pinI2S(true, mi2s0_hdoutput_control, samplerate);
		}
	} else {
		if (GetMemoryPathEnable(Soc_Aud_Digital_Block_I2S_4PIN_IN_OUT) == true)
			Enable4pinI2S(false, mi2s0_hdoutput_control, samplerate);
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	if ((ucontrol->value.integer.value[0] == true) && (mi2s0_hdoutput_control == false)) {
		pr_debug("%s(), mi2s0_hdoutput_control=%d, enable APLL!!!!\n", __func__,
			 mi2s0_hdoutput_control);
		...
		pr_debug("%s(), mi2s0_hdoutput_control=%d, disable APLL!!!!\n", __func__,
			 mi2s0_hdoutput_control);
		/* set APLL clock setting */
		EnableApll1(false);
		EnableApll2(false);
	...
	pr_debug("%s(), mi2s0_hdoutput_control=%d\n", __func__, mi2s0_hdoutput_control);
	...
	if (copy_size != 0) {
		...
		if (Afe_WriteIdx_tmp + copy_size < Afe_Block->u4BufferSize) {
			if (!access_ok(VERIFY_READ, data_w_ptr, copy_size)) {
				...
				PRINTK_AUD_DL1("memcpy Afe_Block->pucVirtBufAddr+Afe_WriteIdx= 0x%x
			...
			PRINTK_AUD_DL1(" finish1, copy_size:%x, WriteIdx:%x, ReadIdx=%x,
			    DataRemained:%x, count=%x \r\n",
			...
			if (!access_ok(VERIFY_READ, data_w_ptr, size_1)) {
				pr_debug(" 1ptr invalid data_w_ptr=%p, size_1=%d", data_w_ptr,
					 size_1);
				pr_debug(" u4BufferSize=%d, u4DataRemained=%d",
					 Afe_Block->u4BufferSize, Afe_Block->u4DataRemained);
			} else {

				PRINTK_AUD_DL1("mcmcpy Afe_Block->pucVirtBufAddr+Afe_WriteIdx= %x
