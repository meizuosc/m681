...
/* defaults */
#define ULTDM_MAX_BUFFER_SIZE     (64*1024)
#define ULTDM_MIN_PERIOD_SIZE     1
#define ULTDM_MAX_PERIOD_SIZE     (ULTDM_MAX_BUFFER_SIZE/(2*2))	/* 2 channels , 16bits */
...
static struct snd_pcm_hardware mtk_capture_hardware = {
	...
	.buffer_bytes_max = ULTDM_MAX_BUFFER_SIZE,
	.period_bytes_max = ULTDM_MAX_BUFFER_SIZE,
	.periods_min = ULTDM_MIN_PERIOD_SIZE,
	.periods_max = ULTDM_MAX_PERIOD_SIZE,
	.fifo_size = 0,
};
...
/* Afe_Set_Reg(AFE_DAC_CON1, 4 << 8 ,  0x00000f00); // I2S sample rate //geo  temp force set 16khz */
	...
	Afe_Set_Reg(AFE_CONN4, 1 << 30, 0x40000000);	/* By Pass ASRC */

#endif

/* Afe_Set_Reg(AFE_SGEN_CON0, 0x049e19e1 ,  0xffffffff); // SGen to I0/I1 */
...
{
	pr_debug("mtk_capture_tdm_prepare substream->rate = %d  substream->channels = %d\n",
	...
	SetCLkMclk(Soc_Aud_I2S0, substream->runtime->rate);
	return 0;
}
...
static snd_pcm_uframes_t mtk_capture_pcm_pointer(struct snd_pcm_substream *substream)
{
	...
	pr_debug("mtk_capture_tdm_pointer Awb_Block->u4WriteIdx;= 0x%x\n",
	...
	if (TDM_VUL_Control_context->interruptTrigger == 1) {
		...
		if (HW_Cur_ReadIdx == 0) {
			pr_debug("[Auddrv] mtk_awb_tdm_pointer  HW_Cur_ReadIdx ==0\n");
		...
		pr_debug("[Auddrv] mtk_awb_tdm_pointer =0x%x HW_memory_index = 0x%x\n",
				     ...
				     struct snd_pcm_hw_params *hw_params)
{
	...
	pr_debug("mtk_capture_tdm_hw_params\n");
	...
	if (Capture_dma_buf->area) {
		pr_debug("mtk_capture_tdm_hw_params Capture_dma_buf->area\n");
/* runtime->dma_bytes = params_buffer_bytes(hw_params); */
		...
		pr_debug("mtk_capture_tdm_hw_params snd_pcm_lib_malloc_pages\n");
...
{
	if (Capture_dma_buf->area) {
		pr_debug("mtk_capture_tdm_hw_free dma_buf->area\n");

		return 0;
	} else {
		pr_debug("mtk_capture_tdm_hw_free snd_pcm_lib_free_pages\n");
...
static int mtk_capture_pcm_open(struct snd_pcm_substream *substream)
{
	...
	pr_debug("mtk_capture_tdm_open runtime rate = %d channels = %d\n", runtime->rate,
		 runtime->channels);

	if (ret < 0) {
		pr_debug("mtk_capture_tdm_close\n");
		mtk_capture_pcm_close(substream);
		return ret;
	}
	pr_debug("mtk_capture_tdm_open return\n");
	return 0;
}
...
{
	pr_debug("mtk_capture_tdm_trigger cmd = %d\n", cmd);
				...
				void __user *dst, snd_pcm_uframes_t count)
{
	...
	pr_debug("mtk_capture_tdm_copy pos = %lucount = %lu\n ", pos, count);
	...
	pr_debug
	    ("%s finish0, read_count:%zd, read_size:%zd, DataRemained:%x,ReadIdx:0x%x, u4WriteIdx:%x\n",
	...
	if (DMA_Read_Ptr + read_size < Vul_Block->u4BufferSize) {
		...
		if (copy_to_user
			...
			pr_debug
			    ("%s Fail 1 copy to user Read_Data_Ptr:%p, ReadIdx:0x%x, DMA_Read_Ptr:%zu,read_size:%zu",
		...
		pr_debug
		    ("%s finish1, copy size:%zd, ReadIdx:0x%x,WriteIdx:%x, u4DataRemained:%x\n",
		...
		if (copy_to_user
			...
			pr_debug
			    ("%s Fail 2 copy to user Read_Data_Ptr:%p,ReadIdx:0x%x, DMA_Read_Ptr:%zu,read_size:%zu",
			     __func__, Read_Data_Ptr, Vul_Block->u4DMAReadIdx,
			     DMA_Read_Ptr, read_size);
			return 0;
		}
		...
		pr_debug
		    ("%s finish2, copy size_1:%x, ReadIdx:0x%x,WriteIdx:0x%x, u4DataRemained:%x\n",
		...
		if (copy_to_user
			...
			pr_debug
			    ("%s Fail 3 copy to user Data_Ptr:%p, ReadIdx:0x%x , DMA_Read_Ptr:%zu, read_size:%zu",
			     __func__, Read_Data_Ptr, Vul_Block->u4DMAReadIdx,
			     DMA_Read_Ptr, read_size);
			return read_count << 2;
		}
...
static int mtk_asoc_capture_pcm_new(struct snd_soc_pcm_runtime *rtd)
{
	pr_debug("mtk_asoc_capture_tdm_new\n");
	return 0;
}
...
static int mtk_afe_capture_probe(struct snd_soc_platform *platform)
{
/* AudDrv_Allocate_mem_Buffer(platform->dev, Soc_Aud_Digital_Block_MEM_VUL, ULTDM_MAX_BUFFER_SIZE); */
/* Capture_dma_buf =  Get_Mem_Buffer(Soc_Aud_Digital_Block_MEM_VUL); */
	...
	pr_debug("mtk_afe_capture_probe TDM UL dma_alloc_coherent\n");

	Capture_dma_buf->area = dma_alloc_coherent(platform->dev,
						   ULTDM_MAX_BUFFER_SIZE,
						   &Capture_dma_buf->addr, GFP_KERNEL);

	if (Capture_dma_buf->area)
		Capture_dma_buf->bytes = ULTDM_MAX_BUFFER_SIZE;
...
MODULE_DESCRIPTION("AFE tdm module platform driver");
MODULE_LICENSE("GPL");
