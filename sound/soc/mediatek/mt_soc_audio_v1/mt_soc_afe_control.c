...
static bool AudioSpeakerProtectUsingSram;
...
void DumpMemifSubStream(void)
{
	...
	for (i = 0; i <= Soc_Aud_Digital_Block_MEM_HDMI; i++) {
		...
		while (head != NULL) {
			/* pr_debug("%s AFE_Mem_Control_context[%d] head = %p
...
static void DecreaseI2SDivCounter(uint32 Diveder_name)
{
	...
	if (GetI2SGroup(Diveder_name) == AUDIO_APLL1_GROUP1) {
		...
		if (mAPLL1I2SDividercounter[index] < 0) {
			pr_debug("mAPLL1I2SDividercounter[%d] = %d", index,
				 mAPLL1I2SDividercounter[index]);
		...
		if (mAPLL2I2SDividercounter[index] < 0) {
			pr_debug("mAPLL2I2SDividercounter[%d] = %d", index,
				 mAPLL2I2SDividercounter[index]);
...
static long GetI2SDivCounter(uint32 Diveder_name)
{
	...
	if (GetI2SGroup(Diveder_name) == AUDIO_APLL1_GROUP1) {
		index = Diveder_name - AUDIO_APLL1_DIV0;
		pr_debug("%s Diveder_name = %d index = %d mAPLL1I2SDividercounter[index] = %d\n",
			 __func__, Diveder_name, index, mAPLL1I2SDividercounter[index]);
		return mAPLL1I2SDividercounter[index];
	} else if (GetI2SGroup(Diveder_name) == AUDIO_APLL2_GROUP2) {
		index = Diveder_name - AUDIO_APLL2_DIV0;
		pr_debug("%s Diveder_name = %d index = %d mAPLL2I2SDividercounter[index] = %d\n",
...
void EnableApll1(bool bEnable)
{
	...
	if (bEnable == true) {
		...
		if (APLL1State < 0) {
			pr_debug("APLL1Stat = %d", APLL1State);
			APLL1State = 0;
		}
	}
	spin_unlock(&I2s_Div_lock);
...
void EnableApll2(bool bEnable)
{
	...
	if (bEnable == true) {
		...
		/* errro handle for counter */
		if (APLL2State < 0) {
			pr_debug("APLL2State = %d", APLL2State);
			APLL2State = 0;
		}
	}
	pr_debug("-%s bEnable = %d\n", __func__, bEnable);
...
bool Audio_ModemPcm2_ASRC_Set(bool Enable)
{
	...
	pr_debug("-%s(), mPcm2AsyncFifoSel=%d\n", __func__, mPcm2AsyncFifoSel);
	return 0;
}
...
bool SetModemPcmEnable(int modem_index, bool modem_pcm_on)
{
	...
	if (modem_index == MODEM_1) {
		if (modem_pcm_on == true) {
			pr_debug("%s(), modem_index(%d), modem_pcm_on = %d, mPcm2AsyncFifoSel = %d\n",
			     __func__, modem_index, modem_pcm_on, mPcm2AsyncFifoSel);
			if (mPcm2AsyncFifoSel == false) {
				/* turn on ASRC before Modem PCM on */
			...
			pr_debug("%s(), modem_index(%d), modem_pcm_on = %d, dNeedDisableASM=%d\n",
				 __func__, modem_index, modem_pcm_on, dNeedDisableASM);
			if (dNeedDisableASM) {
				/* turn off ASRC after Modem PCM off */
...
bool SetIrqMcuCounter(uint32 Irqmode, uint32 Counter)
{
	...
	case Soc_Aud_IRQ_MCU_MODE_IRQ5_MCU_MODE:
		{
			pr_debug(" %s Soc_Aud_IRQ_MCU_MODE_IRQ5_MCU_MODE  Counter = %d ", __func__,
				 Counter);
...
void Enable4pinI2S(bool enable, bool low_jitter_on, uint32 samplerate)
{
	...
	if (enable == true) {
		if (GetMemoryPathEnable(Soc_Aud_Digital_Block_I2S_4PIN_IN_OUT) == false) {
			...
			Afe_Set_Reg(AFE_I2S_CON3, 0x0, 0x1);
			udelay(20);
			/* Sets to 1 to gated I2S0/I2S3 engine clock */
			Afe_Set_Reg(AUDIO_TOP_CON1, (0x1 << 4) | (0x1 << 7), (0x1 << 4) | (0x1 << 7));

			Afe_Set_Reg(AUDIO_TOP_CON1, 0x2, 0x2);	/* I2S0/I2S3 SOFT_Reset Hi */
			/* Sets to 1 to gated I2S0 engine clock */
			Afe_Set_Reg(AUDIO_TOP_CON1, (0x1 << 4) | (0x1 << 7), (0x1 << 4) | (0x1 << 7));
			/* Afe_Set_Reg(AUDIO_TOP_CON1, 0x1 << 7,  0x1 << 7); // Sets to 1 to gated I2S3 engine clock */
			...
			Afe_Set_Reg(AFE_I2S_CON, Audio_I2S_Dac, 0xFFFFFFFE);
	    ...
	    /** 2nd I2S Out **/
			...
			Afe_Set_Reg(AFE_I2S_CON3, u32AudioI2S, 0xFFFFFFFE);

			Afe_Set_Reg(AUDIO_TOP_CON1, (0x0 << 4) | (0x0 << 7),
				    (0x1 << 4) | (0x1 << 7));
			/* Afe_Set_Reg(AUDIO_TOP_CON1, 0x0 << 7,  0x1 << 7); */
	    ...
	    /** 4pin control sequence **/
			...
			Afe_Set_Reg(AUDIO_TOP_CON1, (0x1 << 4) | (0x1 << 7), (0x1 << 4) | (0x1 << 7));
			/* Afe_Set_Reg(AUDIO_TOP_CON1, 0x1 << 7,  0x1 << 7); // Sets to 1 to gated I2S3 engine clock */
			Afe_Set_Reg(AFE_DAC_CON0, 0x0, 0x1);	/* Force disable AFE_ON!! */
			udelay(200);
			...
			Afe_Set_Reg(AUDIO_TOP_CON1, 0x0, 0x2);	/* // I2S0/I2S3 SOFT_Reset Lo */
			Afe_Set_Reg(AFE_I2S_CON, 0x1, 0x1);
			Afe_Set_Reg(AFE_I2S_CON3, 0x1, 0x1);
			Afe_Set_Reg(AUDIO_TOP_CON1, (0x0 << 4) | (0x0 << 7),
				    (0x1 << 4) | (0x1 << 7));
			...
			Afe_Set_Reg(AFE_DAC_CON0, 0x1, 0x1);	/* Force recover AFE_ON!! */
		}
...
bool Set2ndI2SIn(AudioDigtalI2S *mDigitalI2S)
{
	...
	if (!m2ndI2S->mI2S_SLAVE)	{
		/* Master setting SampleRate only */
...
void Auddrv_HDMI_Interrupt_Handler(void)	/* irq5 ISR handler */
{
	...
	if (GetMemoryPathEnable(Soc_Aud_Digital_Block_MEM_HDMI) == false) {
		pr_debug("%s() GetMemoryPathEnable(Soc_Aud_Digital_Block_MEM_HDMI) == false, return\n", __func__);
	...
	PRINTK_AUD_HDMI("+%s ReadIdx:%x WriteIdx:%x, DataRemained:%x,Afe_consumed_bytes:%x\n",
	...
	if (Afe_Block->u4DataRemained < Afe_consumed_bytes || Afe_Block->u4DataRemained <= 0
		...
		PRINTK_AUD_HDMI("%s2 underflow ReadIdx:%x WriteIdx:%x, DataRemained:%x, Afe_consumed_bytes:%x\n",
		     __func__, Afe_Block->u4DMAReadIdx, Afe_Block->u4WriteIdx, Afe_Block->u4DataRemained,
		     Afe_consumed_bytes);
		...
		PRINTK_AUD_HDMI
		    ("-%s2 underflow ReadIdx:%x WriteIdx:%x,DataRemained:%x, Afe_consumed_bytes %x\n",
		     __func__, Afe_Block->u4DMAReadIdx, Afe_Block->u4WriteIdx, Afe_Block->u4DataRemained,
		     Afe_consumed_bytes);
	} else {

		PRINTK_AUD_HDMI
		    ("+%s3 normal ReadIdx:%x ,DataRemained:%x, WriteIdx:%x\n",
		     __func__, Afe_Block->u4DMAReadIdx, Afe_Block->u4DataRemained, Afe_Block->u4WriteIdx);
...
void Auddrv_AWB_Interrupt_Handler(void)
{
	...
	if (GetMemoryPathEnable(Soc_Aud_Digital_Block_MEM_AWB) == false) {
		spin_unlock_irqrestore(&Mem_Block->substream_lock, flags);
		pr_debug("-%s(), GetMemoryPathEnable(Soc_Aud_Digital_Block_MEM_AWB) = %d\n ",
	...
	if (MaxCopySize) {
		...
		pr_debug("%s update read pointer u4DMAReadIdx:0x%x,u4WriteIdx:0x%x, pucPhysBufAddr:0x%x\n",
		     __func__, mBlock->u4DMAReadIdx, mBlock->u4WriteIdx, mBlock->pucPhysBufAddr);
	}
	/* HW already fill in */
	...
	pr_debug("%s Hw_bytes:0x%x, u4DMAReadIdx:0x%x, u4WriteIdx:0x%x, u4DataRemained = 0x%x\n",
	__func__, Hw_Get_bytes, mBlock->u4DMAReadIdx, mBlock->u4WriteIdx	, mBlock->u4DataRemained);
...
void Auddrv_DAI_Interrupt_Handler(void)
{
	...
	pr_debug("%s  Hw_Get_bytes:0x%x, HW_Cur_ReadIdx:0x%x,u4DMAReadIdx:0x%x, u4WriteIdx:0x%x\n",
...
void Auddrv_MOD_DAI_Interrupt_Handler(void)
{
	...
	pr_debug("%s Hw_Get_bytes:0x%x, HW_Cur_ReadIdx:0x%x,u4DMAReadIdx:0x%x, u4WriteIdx:0x%x, pucPhysBufAddr:0x%x\n",
	     __func__, Hw_Get_bytes, HW_Cur_ReadIdx, mBlock->u4DMAReadIdx, mBlock->u4WriteIdx,
	...
	/* buffer overflow */
	if (mBlock->u4DataRemained > mBlock->u4BufferSize) {
		pr_debug("%s buffer overflow u4DMAReadIdx:%x, WriteIdx:%x, u4DataRemained:%x, u4BufferSize:%x\n"
		, __func__, mBlock->u4DMAReadIdx, mBlock->u4WriteIdx, mBlock->u4DataRemained, mBlock->u4BufferSize);
		/*
		   mBlock->u4DataRemained = mBlock->u4BufferSize / 2;
		   mBlock->u4DMAReadIdx = mBlock->u4WriteIdx - mBlock->u4BufferSize / 2;
...
#define MAGIC_NUMBER 0xFFFFFFC0
	...
	pr_debug("+%s  ReadIdx:%x WriteIdx:%x, DataRemained:%x,Afe_consumed_bytes:%x HW_memory_index = %x\n",
	__func__, Afe_Block->u4DMAReadIdx, Afe_Block->u4WriteIdx, Afe_Block->u4DataRemained,
	Afe_consumed_bytes, HW_memory_index);

	if (Afe_Block->u4DataRemained < Afe_consumed_bytes || Afe_Block->u4DataRemained <= 0
	    || Afe_Block->u4DataRemained > Afe_Block->u4BufferSize) {
		pr_debug("DL_Handling underflow\n");
...
void Auddrv_UL1_Interrupt_Handler(void)
{
	...
	pr_debug("+Auddrv_UL1_Interrupt_Handler HW_Cur_ReadIdx = 0x%x\n "
	, HW_Cur_ReadIdx);
	if (CheckSize(HW_Cur_ReadIdx)) {
		spin_unlock_irqrestore(&Mem_Block->substream_lock, flags);
		return;
	...
	pr_debug("%s Hw_Get_bytes:%x, HW_Cur_ReadIdx:%x,u4DMAReadIdx:%x, u4WriteIdx:0x%x, MemIfNum = %d\n",
	__func__, Hw_Get_bytes, HW_Cur_ReadIdx, mBlock->u4DMAReadIdx, mBlock->u4WriteIdx
	, Mem_Block->MemIfNum);
...
void Auddrv_UL2_Interrupt_Handler(void)
{
	...
	PRINTK_AUD_UL2("Auddrv_UL2_Interrupt_Handler Hw_Get_bytes:%x,
	HW_Cur_ReadIdx:%x, u4DMAReadIdx:%x, u4WriteIdx:0x%x, pucPhysBufAddr:%x Mem_Block->MemIfNum = %d\n",
	...
	/* buffer overflow */
	if (mBlock->u4DataRemained > mBlock->u4BufferSize) {
		pr_debug("%s buffer overflow ReadIdx:%x, u4WriteIdx:%x, DataRemained:%x, u4BufferSize:%x\n",
		__func__, mBlock->u4DMAReadIdx, mBlock->u4WriteIdx, mBlock->u4DataRemained,
		mBlock->u4BufferSize);
