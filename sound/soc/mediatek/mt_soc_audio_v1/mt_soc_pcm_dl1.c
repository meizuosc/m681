...
static int mtk_pcm_dl1_open(struct snd_pcm_substream *substream)
{
	...
	if (ret < 0) {
		pr_debug("ret < 0 mtk_soc_pcm_dl1_close\n");
	...
	if (copy_size != 0) {
		...
		if (Afe_WriteIdx_tmp + copy_size < Afe_Block->u4BufferSize) {
			...
			if (!access_ok(VERIFY_READ, data_w_ptr, copy_size)) {
				...
				PRINTK_AUD_DL1("memcpy Afe_Block->pucVirtBufAddr+Afe_WriteIdx= %p
			...
			if (!access_ok(VERIFY_READ, data_w_ptr, size_1)) {
				pr_debug("AudDrv_write 1ptr invalid data_w_ptr=%p, size_1=%d",
					 data_w_ptr, size_1);
...
static int Auddrv_Reg_map_new(void)
{
	...
	if (node) {
		...
		pr_debug("[mt_soc_pcm_dl1] node NULL, can't iomap AFE_BASE!!!\n");
	}
	...
	if (!afe_irq_number) {
		pr_debug("[mt_soc_pcm_dl1] get afe_irq_number failed!!!\n");
		return -1;
	}
	return 0;
}
