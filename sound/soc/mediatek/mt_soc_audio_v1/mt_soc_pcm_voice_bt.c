...
{
	pr_debug("mtk_voice_bt_trigger cmd = %d\n", cmd);
	switch (cmd) {
...
{
	pr_debug("mtk_voice_bt_pcm_silence\n");
	return 0;		/* do nothing */
}
...
static int mtk_voice_bt_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node)
		dev_set_name(&pdev->dev, "%s", MT_SOC_VOICE_BT);
...
#ifdef CONFIG_OF
static const struct of_device_id mt_soc_pcm_voice_bt_of_ids[] = {
...
static struct platform_driver mtk_voice_bt_driver = {
	.driver = {
		   .name = MT_SOC_VOICE_BT,
		   .owner = THIS_MODULE,
#ifdef CONFIG_OF
		   .of_match_table = mt_soc_pcm_voice_bt_of_ids,
#endif
#ifdef CONFIG_PM
...
static int __init mtk_soc_voice_bt_platform_init(void)
{
	...
	pr_debug("%s\n", __func__);
#ifndef CONFIG_OF
	soc_mtk_voice_bt_dev = platform_device_alloc(MT_SOC_VOICE_BT, -1);
