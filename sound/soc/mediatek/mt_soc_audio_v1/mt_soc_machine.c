...
static const struct snd_kcontrol_new mt_soc_controls[] = {
	SOC_ENUM_EXT("I2S low Jitter fucntion", mt_soc_machine_enum[0], mt6595_get_lowjitter,
...
static int __init mt_soc_snd_init(void)
{
	...
	if (!mt_snd_device) {
		pr_debug("mt6589_probe  platform_device_alloc fail\n");
		return -ENOMEM;
	}
	...
	if (ret != 0) {
		pr_debug("mt_soc_snd_init goto put_device fail\n");
		goto put_device;
	}
