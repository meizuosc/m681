 ...
 * Description:
 * ------------
 *   Audio Ul1 data1 uplink I14 to O9
 *
 * Author:
 * -------
 * Tina Tsai
...
static struct snd_dma_buffer *mod_adci2s_dma_buf;
 ...
 */
static void StartAudiomod_adci2sHardware(struct snd_pcm_substream *substream);
static void StopAudiomod_adci2sHardware(struct snd_pcm_substream *substream);
static int mtk_mod_adci2s_probe(struct platform_device *pdev);
static int mtk_mod_adci2s_pcm_close(struct snd_pcm_substream *substream);
static int mtk_asoc_mod_adci2s_pcm_new(struct snd_soc_pcm_runtime *rtd);
static int mtk_afe_mod_adci2s_probe(struct snd_soc_platform *platform);

static struct snd_pcm_hardware mtk_mod_adci2s_hardware = {
...
static void StopAudiomod_adci2sHardware(struct snd_pcm_substream *substream)
{
	pr_debug("StopAudiomod_adci2sHardware\n");
...
static void StartAudiomod_adci2sHardware(struct snd_pcm_substream *substream)
{
	pr_debug("StartAudiomod_adci2sHardware\n");
...
static int mtk_mod_adci2s_pcm_prepare(struct snd_pcm_substream *substream)
{
	return 0;
}

static int mtk_mod_adci2s_alsa_stop(struct snd_pcm_substream *substream)
{
	/* AFE_BLOCK_T *Vul_Block = &(VUL_Control_context->rBlock); */
	pr_debug("mtk_mod_adci2s_alsa_stop\n");
	StopAudiomod_adci2sHardware(substream);
	RemoveMemifSubStream(Soc_Aud_Digital_Block_MEM_VUL, substream);
	return 0;
}

static kal_int32 Previous_Hw_cur;
static snd_pcm_uframes_t mtk_mod_adci2s_pcm_pointer(struct snd_pcm_substream *substream)
{
	kal_int32 HW_memory_index = 0;
	kal_int32 HW_Cur_ReadIdx = 0;
	kal_uint32 Frameidx = 0;
	AFE_BLOCK_T *Awb_Block = &(VUL_Control_context->rBlock);

	pr_debug("mtk_mod_adci2s_pcm_pointer Awb_Block->u4WriteIdx;= 0x%x\n",
		       Awb_Block->u4WriteIdx);
	if (VUL_Control_context->interruptTrigger == 1) {
		...
		pr_debug("[Auddrv] mtk_awb_pcm_pointer =0x%x HW_memory_index = 0x%x\n",
			       HW_Cur_ReadIdx, HW_memory_index);
		VUL_Control_context->interruptTrigger = 0;
...
static int mtk_mod_adci2s_pcm_hw_params(struct snd_pcm_substream *substream,
					struct snd_pcm_hw_params *hw_params)
{
	...
	pr_debug("mtk_mod_adci2s_pcm_hw_params\n");

	dma_buf->dev.type = SNDRV_DMA_TYPE_DEV;
	dma_buf->dev.dev = substream->pcm->card->dev;
	dma_buf->private_data = NULL;
	pr_debug("mod_adci2s_dma_buf = %p mod_adci2s_dma_buf->area = %p\n", mod_adci2s_dma_buf,
		 mod_adci2s_dma_buf->area);

	if (mod_adci2s_dma_buf->area) {
		pr_debug("mtk_mod_adci2s_pcm_hw_params mod_adci2s_dma_buf->area\n");
		runtime->dma_bytes = mod_adci2s_dma_buf->bytes;
		runtime->dma_area = mod_adci2s_dma_buf->area;
		runtime->dma_addr = mod_adci2s_dma_buf->addr;
		runtime->buffer_size = mod_adci2s_dma_buf->bytes;
	} else {
		pr_debug("mtk_mod_adci2s_pcm_hw_params snd_pcm_lib_malloc_pages\n");
		ret = snd_pcm_lib_malloc_pages(substream, params_buffer_bytes(hw_params));
	}
	pr_debug("mtk_mod_adci2s_pcm_hw_params dma_bytes = %zu dma_area = %p dma_addr = 0x%lx\n",
	...
	pr_debug("dma_bytes = %zu dma_area = %p dma_addr = 0x%x\n",
		 substream->runtime->dma_bytes, substream->runtime->dma_area,
		 (unsigned int)substream->runtime->dma_addr);
	return ret;
}

static int mtk_mod_adci2s_pcm_hw_free(struct snd_pcm_substream *substream)
{
	pr_debug("mtk_mod_adci2s_pcm_hw_free\n");
	if (mod_adci2s_dma_buf->area)
		return 0;
	 else
...
static int mtk_mod_adci2s_pcm_open(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	int ret = 0;
	...
	runtime->hw = mtk_mod_adci2s_hardware;
	memcpy((void *)(&(runtime->hw)), (void *)&mtk_mod_adci2s_hardware,
	...
	pr_debug("mtk_mod_adci2s_pcm_open runtime rate = %d channels = %d\n", runtime->rate,
		 runtime->channels);

	if (ret < 0) {
		pr_debug("mtk_mod_adci2s_pcm_close\n");
		mtk_mod_adci2s_pcm_close(substream);
		return ret;
	}
	pr_debug("mtk_mod_adci2s_pcm_open return\n");
	return 0;
}
...
static int mtk_mod_adci2s_alsa_start(struct snd_pcm_substream *substream)
{
	pr_debug("mtk_mod_adci2s_alsa_start\n");
	SetMemifSubStream(Soc_Aud_Digital_Block_MEM_VUL, substream);
	StartAudiomod_adci2sHardware(substream);
	return 0;
}

static int mtk_mod_adci2s_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
	pr_debug("mtk_mod_adci2s_pcm_trigger cmd = %d\n", cmd);

	switch (cmd) {
	case SNDRV_PCM_TRIGGER_START:
	case SNDRV_PCM_TRIGGER_RESUME:
		return mtk_mod_adci2s_alsa_start(substream);
	case SNDRV_PCM_TRIGGER_STOP:
	case SNDRV_PCM_TRIGGER_SUSPEND:
		return mtk_mod_adci2s_alsa_stop(substream);
	}
	return -EINVAL;
}
...
static int mtk_mod_adci2s_pcm_copy(struct snd_pcm_substream *substream,
				   ...
				   void __user *dst, snd_pcm_uframes_t count)
{
	...
	pr_debug("mtk_mod_adci2s_pcm_copy pos = %lucount = %lu\n ", pos, count);
	...
	pr_debug
	    ("%s finish0, read_count:%zd, read_size:%zd, u4DataRemained:%x u4DMAReadIdx:0x%x, u4WriteIdx:%x\n",
	...
	if (DMA_Read_Ptr + read_size < Vul_Block->u4BufferSize) {
		...
		if (copy_to_user
			...
			pr_debug
			    ("%s Fail 1 copy to user Read_Data_Ptr:%p,ReadIdx:0x%x, DMA_Read_Ptr:0x%zd,read_size:%zd",
		...
		pr_debug
		    ("%s finish1, copy size:%zd, u4DMAReadIdx:0x%x,u4WriteIdx:%x, u4DataRemained:%x\n",
		...
		if (DMA_Read_Ptr != Vul_Block->u4DMAReadIdx) {
			...
			pr_debug
			    ("%s 2, read_size1:%x, DataRemained:%x,Read_Ptr:0x%zd, DMAReadIdx:%x\n",
		...
		if (copy_to_user
			...
			pr_debug
			    ("%s Fail 2 copy to user Read_Data_Ptr:%p, ReadIdx:0x%x, DMA_Read_Ptr:0x%zd,read_size:%zd",
		...
		if (DMA_Read_Ptr != Vul_Block->u4DMAReadIdx) {
			...
			pr_debug
			    ("%s 3, read_size2:%x, DataRemained:%x DMA_Read_Ptr:0x%zu, DMAReadIdx:%x\n",
		...
		pr_debug
		    ("%s finish3, copy size_2:%x, ReadIdx:0x%x WriteIdx:0x%x u4DataRemained:%x \r\n",
...
static int mtk_mod_adci2s_pcm_silence(struct snd_pcm_substream *substream,
...
static struct page *mtk_mod_adci2s_pcm_page(struct snd_pcm_substream *substream,
...
static struct snd_pcm_ops mtk_afe_mod_adci2s_ops = {
	.open = mtk_mod_adci2s_pcm_open,
	.close = mtk_mod_adci2s_pcm_close,
	.ioctl = snd_pcm_lib_ioctl,
	.hw_params = mtk_mod_adci2s_pcm_hw_params,
	.hw_free = mtk_mod_adci2s_pcm_hw_free,
	.prepare = mtk_mod_adci2s_pcm_prepare,
	.trigger = mtk_mod_adci2s_pcm_trigger,
	.pointer = mtk_mod_adci2s_pcm_pointer,
	.copy = mtk_mod_adci2s_pcm_copy,
	.silence = mtk_mod_adci2s_pcm_silence,
	.page = mtk_mod_adci2s_pcm_page,
};

static struct snd_soc_platform_driver mtk_soc_platform = {
	.ops = &mtk_afe_mod_adci2s_ops,
	.pcm_new = mtk_asoc_mod_adci2s_pcm_new,
	.probe = mtk_afe_mod_adci2s_probe,
};

static int mtk_mod_adci2s_probe(struct platform_device *pdev)
{
	pr_debug("mtk_mod_adci2s_probe\n");
	...
	if (pdev->dev.of_node)
		dev_set_name(&pdev->dev, "%s", MT_SOC_MOD_ADCI2S_PCM);
...
static int mtk_asoc_mod_adci2s_pcm_new(struct snd_soc_pcm_runtime *rtd)
{
	pr_debug("mtk_asoc_mod_adci2s_pcm_new\n");
	return 0;
}
...
static int mtk_afe_mod_adci2s_probe(struct snd_soc_platform *platform)
{
	pr_debug("mtk_afe_mod_adci2s_probe\n");
	AudDrv_Allocate_mem_Buffer(platform->dev, Soc_Aud_Digital_Block_MEM_VUL,
				   UL1_MAX_BUFFER_SIZE);
	mod_adci2s_dma_buf = Get_Mem_Buffer(Soc_Aud_Digital_Block_MEM_VUL);
	mAudioDigitalI2S = kzalloc(sizeof(AudioDigtalI2S), GFP_KERNEL);
	return 0;
}


static int mtk_mod_adci2s_remove(struct platform_device *pdev)
{
	pr_debug("%s\n", __func__);
...
#ifdef CONFIG_OF
static const struct of_device_id mt_soc_pcm_mod_adci2s_of_ids[] = {
	{.compatible = "mediatek,mt_soc_pcm_mod_adci2s",},
	{}
};
#endif

static struct platform_driver mtk_afe_mod_adci2s_driver = {
	.driver = {
		   .name = MT_SOC_MOD_ADCI2S_PCM,
		   .owner = THIS_MODULE,
#ifdef CONFIG_OF
		   .of_match_table = mt_soc_pcm_mod_adci2s_of_ids,
#endif
		   },
	.probe = mtk_mod_adci2s_probe,
	.remove = mtk_mod_adci2s_remove,
};

#ifndef CONFIG_OF
static struct platform_device *soc_mtkafe_mod_adci2s_dev;
#endif

static int __init mtk_soc_mod_adci2s_platform_init(void)
{
	int ret = 0;

	pr_debug("%s\n", __func__);
#ifndef CONFIG_OF
	soc_mtkafe_mod_adci2s_dev = platform_device_alloc(MT_SOC_MOD_ADCI2S_PCM, -1);
	if (!soc_mtkafe_mod_adci2s_dev)
		return -ENOMEM;


	ret = platform_device_add(soc_mtkafe_mod_adci2s_dev);
	if (ret != 0) {
		platform_device_put(soc_mtkafe_mod_adci2s_dev);
		return ret;
	}
#endif
	ret = platform_driver_register(&mtk_afe_mod_adci2s_driver);
	return ret;
}
module_init(mtk_soc_mod_adci2s_platform_init);

static void __exit mtk_soc_platform_exit(void)
{

	pr_debug("%s\n", __func__);
	platform_driver_unregister(&mtk_afe_mod_adci2s_driver);
}
