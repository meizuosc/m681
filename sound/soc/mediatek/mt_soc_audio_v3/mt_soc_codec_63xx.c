...
/*extern kal_uint32 PMIC_IMM_GetOneChannelValue(mt6328_adc_ch_list_enum dwChannel, int deCount,
					      int trimd); */
...
static bool TurnOnADcPowerACC(int ADCType, bool enable)
{
	...
	pr_warn("%s ADCType = %d enable = %d, ver=%d, refmic_using_ADC_L=%d\n", __func__, ADCType,
	       enable, ver, refmic_using_ADC_L);
...
static int32 Pmic_Loopback_Type = 0;
...
static int Pmic_Loopback_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.integer.value[0] == 0) { /* disable pmic lpbk */
		...
		/* Set UL Part */
		Ana_Set_Reg(AFE_PMIC_NEWIF_CFG2, 0x0c00, 0x0c00); /* config UL up8x_rxif adc voice mode, 48k sample rate */
		Ana_Set_Reg(AFE_UL_SRC0_CON0_H, (0x3 << 3 | 0x3 << 1) , 0x001E);/* ULsampling rate, 48k sample rate */
		Ana_Set_Reg(AFE_UL_SRC0_CON0_L, 0x0005, 0xffff);   /* power on uplink, and loopback from DL */
