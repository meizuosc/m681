...
static snd_pcm_uframes_t mtk_capture_pcm_pointer(struct snd_pcm_substream *substream)
{
	...
	if (GetMemoryPathEnable(Soc_Aud_Digital_Block_MEM_VUL) == true) {
		...
		PRINTK_AUD_UL1
		    ("%s u4DMAReadIdx=0x%x u4WriteIdx = 0x%x u4DataRemained = 0x%x u4BufferSize= 0x%x,
		     Hw_Get_bytes= 0x%x\n", __func__, UL1_Block->u4DMAReadIdx,
		     UL1_Block->u4WriteIdx, UL1_Block->u4DataRemained, UL1_Block->u4BufferSize,
		     Hw_Get_bytes);
		...
		if (UL1_Block->u4DataRemained > UL1_Block->u4BufferSize) {
			bIsOverflow = true;
			pr_warn
			    ("%s buffer overflow u4DMAReadIdx:%x, u4WriteIdx:%x, DataRemained:%x, BufferSize:%x\n",
				...
				void __user *dst, snd_pcm_uframes_t count)
{
	...
	if (DMA_Read_Ptr + read_size < Vul_Block->u4BufferSize) {
		...
		if (copy_to_user((void __user *)Read_Data_Ptr,
			...
			pr_err
			    ("%s Fail 1 copy to user Ptr:%p, Addr:%p, ReadIdx:0x%x, Read_Ptr:%zu,size:%zu",
		...
		PRINTK_AUD_UL1
		    ("%s finish1, copy size:%x, ReadIdx:0x%x, WriteIdx:%x, Remained:%x \r\n",
		     __func__, (unsigned int)read_size, Vul_Block->u4DMAReadIdx,
		...
		if (copy_to_user((void __user *)Read_Data_Ptr,
			...
			pr_err
			    ("%s Fail 2 copy to user Ptr:%p, Addr:%p, ReadIdx:0x%x, Read_Ptr:%zu,read_size:%zu",
			     __func__, Read_Data_Ptr, Vul_Block->pucVirtBufAddr,
			     Vul_Block->u4DMAReadIdx, DMA_Read_Ptr, read_size);
			return 0;
		}
		...
		if (DMA_Read_Ptr != Vul_Block->u4DMAReadIdx) {
			...
			pr_warn
			    ("%s 3, read_size2:%x, Remained:%x, DMA_Read_Ptr:%zu, DMAReadIdx:%x \r\n",
		...
		if (copy_to_user((void __user *)(Read_Data_Ptr + size_1),
			...
			pr_err
			    ("%s Fail 3 copy to user Ptr:%p, Addr:%p, ReadIdx:0x%x , Read_Ptr:%zu, read_size:%zu",
			     __func__, Read_Data_Ptr, Vul_Block->pucVirtBufAddr,
			     Vul_Block->u4DMAReadIdx, DMA_Read_Ptr, read_size);
			return read_count << 2;
		}
