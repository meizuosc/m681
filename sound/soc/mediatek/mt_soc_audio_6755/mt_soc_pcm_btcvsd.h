...
static DEFINE_SPINLOCK(auddrv_btcvsd_tx_lock);
static DEFINE_SPINLOCK(auddrv_btcvsd_rx_lock);
...
static int mtk_btcvsd_probe(struct platform_device *pdev);
static int mtk_pcm_btcvsd_close(struct snd_pcm_substream *substream);
static int mtk_asoc_pcm_btcvsd_new(struct snd_soc_pcm_runtime *rtd);
...
static volatile kal_uint32 *bt_hw_REG_Timcon, *bt_hw_REG_Modem, *bt_hw_REG_PKV;
...
static void AudDrv_BTCVSD_DataTransfer(BT_SCO_DIRECT uDir, kal_uint8 *pSrc,
					...
					CVSD_STATE uState);
static void AudDrv_BTCVSD_ReadFromBT(BT_SCO_PACKET_LEN uLen,
