...
/* static struct work_struct *mWorkq_audio_irq; */
...
/* static void AudDrv_IRQ_error_handler(struct work_struct *); */
...
#if 0
	mWorkq_audio_irq = kzalloc(sizeof(typeof(*mWorkq_audio_irq)), GFP_KERNEL);
	INIT_WORK(mWorkq_audio_irq, AudDrv_IRQ_error_handler);
#endif
...
void Auddrv_AWB_Interrupt_Handler(void)
{
	...
	if (GetMemoryPathEnable(Soc_Aud_Digital_Block_MEM_AWB) == false) {
		/* pr_debug("%s(), GetMemoryPathEnable(Soc_Aud_Digital_Block_MEM_AWB) == false, return\n ", __func__); */
...
void Auddrv_DAI_Interrupt_Handler(void)
{
	...
	if (GetMemoryPathEnable(Soc_Aud_Digital_Block_MEM_DAI) == false) {
		/* pr_debug("%s(), GetMemoryPathEnable(Soc_Aud_Digital_Block_MEM_DAI) == false, return\n ", __func__); */
		spin_unlock_irqrestore(&Mem_Block->substream_lock, flags);
		return;
	}
	...
	PRINTK_AUD_DAI("Auddrv_DAI_Interrupt_Handler Hw_Get_bytes:0x%x, HW_Cur_ReadIdx:0x%x,
	u4DMAReadIdx:0x%x, u4WriteIdx:0x%x, pucPhysBufAddr:0x%x Mem_Block->MemIfNum = %d\n",
	...
	/* buffer overflow */
	if (mBlock->u4DataRemained > mBlock->u4BufferSize) {
		PRINTK_AUD_DAI("Auddrv_DAI_Interrupt_Handler buffer overflow u4DMAReadIdx:%x,
		u4WriteIdx:%x, u4DataRemained:%x, u4BufferSize:%x\n",
...
#define MAGIC_NUMBER 0xFFFFFFC0
	...
	if (GetMemoryPathEnable(Soc_Aud_Digital_Block_MEM_DL1) == false) {
		/* pr_debug("%s(), GetMemoryPathEnable(Soc_Aud_Digital_Block_MEM_DL1) == false, return\n ", __func__); */
		spin_unlock_irqrestore(&Mem_Block->substream_lock, flags);
		return;
	}
...
#define MAGIC_NUMBER 0xFFFFFFC0
	...
	if (Afe_Block->u4DataRemained < Afe_consumed_bytes
	    || Afe_Block->u4DataRemained <= 0 || Afe_Block->u4DataRemained >
	    Afe_Block->u4BufferSize) {
		pr_debug("DL2_Handling underflow\n");
	} else {
...
void Auddrv_UL1_Interrupt_Handler(void)
{
	...
	if (GetMemoryPathEnable(Soc_Aud_Digital_Block_MEM_VUL) == false) {
		/* pr_debug("%s(), GetMemoryPathEnable(Soc_Aud_Digital_Block_MEM_VUL) == false, return\n ", __func__); */
		spin_unlock_irqrestore(&Mem_Block->substream_lock, flags);
		return;
