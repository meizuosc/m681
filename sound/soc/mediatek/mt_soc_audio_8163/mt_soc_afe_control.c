...
static bool mFMEnable = false;
static bool mOffloadEnable = false;
static bool mOffloadSWMode = false;
...
static int Aud_APLL_DIV_APLL1_cntr = 0;
static int Aud_APLL_DIV_APLL2_cntr = 0;
static int aud_hdmi_clk_cntr = 0;
...
bool Register_Aud_Irq(void *dev, uint32 afe_irq_number)
{
    ...
    ret = request_irq(MT8163_AFE_MCU_IRQ_LINE, AudDrv_IRQ_handler, IRQF_TRIGGER_LOW/*IRQF_TRIGGER_FALLING*/, "Afe_ISR_Handle", dev);
#endif
    return ret;
}
...
irqreturn_t AudDrv_IRQ_handler(int irq, void *dev_id)
{
    ...
    {
        if ( (mAudioMEMIF[Soc_Aud_Digital_Block_MEM_DL2]->mState == true) && (mOffloadSWMode == true) )
        {
...
void EnableALLbySampleRate(uint32 SampleRate)
{
    ...
    else if((GetApllbySampleRate(SampleRate) == Soc_Aud_APLL2) && (APLLUsage == Soc_Aud_APLL_NOUSE))
    {
        //enable APLL2
...
void EnableApll1(bool bEnable)
{
    ...
    if (bEnable)
    {
        ...
        {
            SetClkCfg(AUDIO_CLK_CFG_6, 0x1<<16, 0x1<<16); //set hf_faud_1_ck from apll1_ck

            SetClkCfg(AUDIO_CLK_AUDDIV_0, 0x1, 0x1);  // apll1_div0_pdn power down
            //SetClkCfg(AUDIO_CLK_AUDDIV_0, 0x2, 0x2); //apll2_div0_pdn power down.
            SetClkCfg(AUDIO_CLK_AUDDIV_0, 0x03000000, 0x07000000); // set apll1_ck_div0, 90.3168/4 = 22.5792M, used for I2S0~I2S3
            //SetClkCfg(AUDIO_CLK_AUDDIV_0, 0x0<<6, 1 << 6); //hf_faud_apll1_ck from hf_faud_1_ck
            //SetClkCfg(AUDIO_CLK_AUDDIV_0, 0x1<<7, 1 << 7);
            AudDrv_APLL22M_Clk_On();
            SetClkCfg(AUDIO_CLK_AUDDIV_0, 0x0, 0x1); // apll1_div0_pdn power up
            //SetClkCfg(AUDIO_CLK_AUDDIV_0, 0x0, 0x2); //apll2_div0_pdn power down.
            SetClkCfg(AUDIO_CLK_CFG_6, 0x0<<23, 0x1<<23); //turn on  hf_faud_2_ck
        }
    ...
    else
    {
        ...
        if (Aud_APLL_DIV_APLL1_cntr == 0)
        {
            ...
            SetClkCfg(AUDIO_CLK_CFG_6, 0x1<<23, 0x1<<23); //turn off  hf_faud_2_ck
        }
    }
}
...
void EnableApll2(bool bEnable)
{
    ...
    if (bEnable)
    {
        ...
        {
            SetClkCfg(AUDIO_CLK_CFG_6, 0x1<<24, 0x1<<24); //set hf_faud_2_ck from apll2_ck
            //SetClkCfg(AUDIO_CLK_AUDDIV_0, 0x1, 0x1); // apll1_div0_pdn power down
            SetClkCfg(AUDIO_CLK_AUDDIV_0, 0x2, 0x2); //apll2_div0_pdn power down.
            //SetClkCfg(AUDIO_CLK_AUDDIV_0, 0x30000000, 0x70000000); //  apll2_ck_div0, 98.3030/4 = 24.576M
            SetClkCfg(AUDIO_CLK_AUDDIV_0, 0x3<<28, 0x7<<28); //  apll2_ck_div0, 98.3030/4 = 24.576M
            //SetClkCfg(AUDIO_CLK_AUDDIV_0, 0x0<<6, 1 << 6); //hf_faud_apll1_ck from hf_faud_1_ck
            //SetClkCfg(AUDIO_CLK_AUDDIV_0, 0x1<<7, 1 << 7);
            AudDrv_APLL24M_Clk_On();
            //SetClkCfg(AUDIO_CLK_AUDDIV_0, 0x0, 0x1);  //apll2_div0_pdn power up.
            SetClkCfg(AUDIO_CLK_AUDDIV_0, 0x0, 0x2);  //apll2_div0_pdn power up.
            SetClkCfg(AUDIO_CLK_CFG_6, 0x0<<31, 0x1<<31); //turn on  hf_faud_2_ck
        }
    ...
    else
    {
        ...
        if (Aud_APLL_DIV_APLL2_cntr == 0)
        {
            ...
            SetClkCfg(AUDIO_CLK_AUDDIV_0, 0x2, 0x2); //apll2_div0_pdn power down.
            SetClkCfg(AUDIO_CLK_CFG_6, 0x1<<31, 0x1<<31); //turn off  hf_faud_2_ck
        }
    }
}
...
bool SetModemPcmEnable(int modem_index, bool modem_pcm_on)
{
    ...
    else if (modem_index == MODEM_2 || modem_index == MODEM_EXTERNAL) // MODEM_2 use PCM_INTF_CON1 (0x530) !!!
    {
        ...
        {
            Afe_Set_Reg(PCM_INTF_CON2, (modem_index - 1)<<8, 0x100); // selects internal MD2/MD3 PCM interface (0x538[8])
...
void SetHdmiPcmInterConnection(unsigned int connection_state, unsigned int channels)
{
	...
	switch (channels) {
	case 8:
		SetHDMIConnection(connection_state, Soc_Aud_InterConnectionInput_I36, Soc_Aud_InterConnectionOutput_O36);
		SetHDMIConnection(connection_state, Soc_Aud_InterConnectionInput_I37, Soc_Aud_InterConnectionOutput_O37);
	case 6:
		SetHDMIConnection(connection_state, Soc_Aud_InterConnectionInput_I34, Soc_Aud_InterConnectionOutput_O32);
		SetHDMIConnection(connection_state, Soc_Aud_InterConnectionInput_I35, Soc_Aud_InterConnectionOutput_O33);
	case 4:
		SetHDMIConnection(connection_state, Soc_Aud_InterConnectionInput_I32, Soc_Aud_InterConnectionOutput_O34);
		SetHDMIConnection(connection_state, Soc_Aud_InterConnectionInput_I33, Soc_Aud_InterConnectionOutput_O35);
	case 2:
		SetHDMIConnection(connection_state, Soc_Aud_InterConnectionInput_I30, Soc_Aud_InterConnectionOutput_O30);
		SetHDMIConnection(connection_state, Soc_Aud_InterConnectionInput_I31, Soc_Aud_InterConnectionOutput_O31);
		break;
	case 1:
		SetHDMIConnection(connection_state, Soc_Aud_InterConnectionInput_I30, Soc_Aud_InterConnectionOutput_O30);
		break;
...
bool SetIrqMcuSampleRate(uint32  Irqmode, uint32 SampleRate)
{
    switch (Irqmode)
    {
        ...
        case Soc_Aud_IRQ_MCU_MODE_IRQ7_MCU_MODE:
        {
            Afe_Set_Reg(AFE_IRQ_MCU_CON, (SampleRateTransform(SampleRate) << 24), 0x0f000000);
            break;
        }
...
bool SetIrqMcuCounter(uint32 Irqmode, uint32 Counter)
{
    ...
    switch (Irqmode)
    {
		...
		case Soc_Aud_IRQ_MCU_MODE_IRQ7_MCU_MODE:
        {
            Afe_Set_Reg(AFE_IRQ_MCU_CNT7, Counter , 0xffffffff);  // ox3BC [0~17] , ex 24bit , stereo, 48BCKs @CNT
...
bool  SetMemIfFetchFormatPerSample(uint32 InterfaceType, uint32 eFetchFormat)
{
    ...
    switch (InterfaceType)
    {
        ...
        case Soc_Aud_Digital_Block_MEM_DL2:
        {
            Afe_Set_Reg(AFE_MEMIF_PBUF_SIZE, mAudioMEMIF[InterfaceType]->mFetchFormatPerSample << 18 , 0x000c0000);
            break;
        }
        case Soc_Aud_Digital_Block_MEM_I2S:
        {
        ...
        case Soc_Aud_Digital_Block_MEM_VUL_DATA2:
        {
            Afe_Set_Reg(AFE_MEMIF_PBUF_SIZE, mAudioMEMIF[InterfaceType]->mFetchFormatPerSample << 14, 0x00004000);
            break;
        }
        case Soc_Aud_Digital_Block_MEM_DAI:
        {
...
//    substreamList *Temp = NULL;
    ...
    {
       AFE_Mem_Control_context[Soc_Aud_Digital_Block_MEM_DL2]->offloadCbk(AFE_Mem_Control_context[Soc_Aud_Digital_Block_MEM_DL2]->offloadstream);
    }
