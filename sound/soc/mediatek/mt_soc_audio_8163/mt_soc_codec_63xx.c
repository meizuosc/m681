...
static void Audio_Amp_Change(int channels , bool enable)
{
    if (enable)
    {
            ...
            mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_HEADSETR] == false)
        {
            ...
            Ana_Set_Reg(ABB_AFE_CON3, 581, 0xffff); // LCH cpmpensation value
            Ana_Set_Reg(ABB_AFE_CON4, 559, 0xffff); // RCH cpmpensation value
...
static void Speaker_Amp_Change(bool enable)
{
    ...
    if (enable)
    {
        ...
        {
            Ana_Set_Reg(AUDTOP_CON7, 0x35B0, 0xffff); // enable voice buffer and -11dB gain. Inter-connect voice buffer to SPK AMP
        }
        #else
...
static void Headset_Speaker_Amp_Change(bool enable)
{
    ...
    if (enable)
    {
        ...
        printk("Headset_Speaker_Amp_Change on \n");
