...
static bool voice_md2_Status = false;
...
static int mtk_voice_md2_pcm_open(struct snd_pcm_substream *substream)
{
    ...
    {
        printk("SNDRV_PCM_STREAM_CAPTURE mtkalsa_voice_md2_constraints\n");
        runtime->rate = 16000;
    }
...
static int mtk_voice1_ext_prepare(struct snd_pcm_substream *substream)
{
    ...
    EnableAfe(true);
    Voice2IntPcm.mPcmModeWidebandSel = (runtimeStream->rate == 8000) ? Soc_Aud_PCM_MODE_PCM_MODE_8K : Soc_Aud_PCM_MODE_PCM_MODE_16K;
...
struct dev_pm_ops mtk_voice_md2_pm_ops =
{
